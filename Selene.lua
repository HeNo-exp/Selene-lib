local a=game local b=a:GetService'ReplicatedStorage'local c=require local d={}
local e={}local f={}local g={}local function createVirtualFolder(h)if g[h]then
return g[h]end local i={__path=h,__isFolder=true}setmetatable(i,{__index=
function(j,k)if k=='Parent'then if h==''or h=='src'then return
createVirtualFolder''end local l={}for m in string.gmatch(h,'[^/]+')do table.
insert(l,m)end table.remove(l)return createVirtualFolder(table.concat(l,'/'))
elseif k=='FindFirstChild'or k=='WaitForChild'then return function(l,m,n)return
j[m]end elseif k=='IsA'then return function(l,m)return m=='Folder'or m==
'ModuleScript'or m=='Instance'end end local l=h==''and k or(h..'/'..k)if d[l]
then return createVirtualModule(l)else return createVirtualFolder(l)end end})g[h
]=i return i end local function createVirtualModule(h)if f[h]then return f[h]end
local i={__path=h,__isModule=true}setmetatable(i,{__index=function(j,k)if k==
'Parent'then local l={}for m in string.gmatch(h,'[^/]+')do table.insert(l,m)end
table.remove(l)return createVirtualFolder(table.concat(l,'/'))elseif k==
'FindFirstChild'or k=='WaitForChild'then return function(l,m,n)local o=j.Parent
if o then return o[m]end return nil end elseif k=='IsA'then return function(l,m)
return m=='ModuleScript'or m=='Instance'end end return nil end})f[h]=i return i
end local function getReactPackage(h)local i=b:FindFirstChild'Packages'if i and
i:FindFirstChild(h)then local j,k=pcall(c,i[h])if j then return k end end local
j=(typeof(getgenv)=='function'and getgenv())or shared if j then if j[h]then
return j[h]end if h=='React'and j.React then return j.React end if h==
'ReactRoblox'and j.ReactRoblox then return j.ReactRoblox end end local k=a:
GetService'CoreGui'local l=k:FindFirstChild'Packages'if l and l:FindFirstChild(h
)then local m,n=pcall(c,l[h])if m then return n end end error(
"[Selene Library] Package '"..tostring(h)..
"' not found in ReplicatedStorage.Packages or getgenv()."..tostring(h)..
[[. Please ensure React is loaded in getgenv() or ReplicatedStorage.Packages.]])
end local function createPkgProxy(h)return{__isPkgProxy=true,__pkgName=h}end
local function __require(h)if type(h)=='table'and h.__isModule then local i=h.
__path if e[i]then return e[i]end local j=d[i]if not j then error(
'[Selene Library] Module not found: '..tostring(i))end local k=j()e[i]=k return
k elseif type(h)=='table'and h.__isPkgProxy then return getReactPackage(h.
__pkgName)end return c(h)end local h=setmetatable({},{__index=function(h,i)local
j=b:FindFirstChild'Packages'if j and j:FindFirstChild(i)then return j[i]end
return createPkgProxy(i)end,__call=function(h,i)local j=b:FindFirstChild
'Packages'if j and j:FindFirstChild(i)then return j[i]end return createPkgProxy(
i)end})setmetatable({},{__index=function(i,j)if j=='Packages'then return h
elseif j=='FindFirstChild'or j=='WaitForChild'then return function(k,l,m)if l==
'Packages'then return h end return b:FindFirstChild(l)end end return b[j]end})d.
AppRecorder=function()local i=createVirtualModule'AppRecorder'
createVirtualFolder''local j=__require local k=game:GetService'HttpService'local
l={}l.__index=l function l.new(m)local n=setmetatable({_app=m,_active=false},l)
return n end function l.Start(m)m._active=true end function l.Stop(m)m._active=
false end function l.Dump(m)local n=j(i.Parent.Store)local o=n._state local p={}
local function traverse(q,r)for s,t in ipairs(q)do if t.value~=nil and t.name
then p[r]=p[r]or{}local u=t.value if typeof(u)=='Color3'then u={R=u.R,G=u.G,B=u.
B,_type='Color3'}elseif typeof(u)=='EnumItem'then u={Name=u.Name,EnumType=
tostring(u.EnumType),_type='EnumItem'}end p[r][t.name]=u end if t.elements then
traverse(t.elements,r)end if t.leftElements then traverse(t.leftElements,r)end
if t.rightElements then traverse(t.rightElements,r)end end end for q,r in
ipairs(o.sections or{})do for s,t in ipairs(r.tabs or{})do if t.pageSections
then traverse(t.pageSections,t.title or'Tab')end end end return k:JSONEncode(p)
end function l.Load(m,n)local o,p=pcall(function()return k:JSONDecode(n)end)if
not o or type(p)~='table'then return end local q=j(i.Parent.Store)local function 
deserializeValue(r)if type(r)=='table'and r._type=='Color3'then return Color3.
new(r.R,r.G,r.B)elseif type(r)=='table'and r._type=='EnumItem'then local s=r.
EnumType local t=r.Name local u=Enum[s]if u and u[t]then return u[t]end end
return r end q:Update(function(r)local function traverseAndLoad(s,t)for u,v in
ipairs(s)do local w=p[t]if w and v.name and w[v.name]~=nil then v.value=
deserializeValue(w[v.name])end if v.elements then traverseAndLoad(v.elements,t)
end if v.leftElements then traverseAndLoad(v.leftElements,t)end if v.
rightElements then traverseAndLoad(v.rightElements,t)end end end for s,t in
ipairs(r.sections or{})do for u,v in ipairs(t.tabs or{})do if v.pageSections
then traverseAndLoad(v.pageSections,v.title or'Tab')end end end end)end return l
end d['Components/BarChart']=function()local i=createVirtualModule
'Components/BarChart'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=game:GetService'TweenService'local p=TweenInfo.new(
0.35,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)local q=l.Component:extend
'AnimatedBar'function q.init(r)r.ref=l.createRef()end function q.didMount(r)
local s=r.ref.current if s then s.Size=r.props.targetSize end end function q.
didUpdate(r,s)local t=r.ref.current if t and s.targetSize~=r.props.targetSize
then t.Size=s.targetSize o:Create(t,p,{Size=r.props.targetSize}):Play()end end
function q.render(r)local s=r.props return l.createElement('Frame',{ref=r.ref,
Position=UDim2.new(0.5,0,1,0),AnchorPoint=Vector2.new(0.5,1),BackgroundColor3=s.
color,BorderSizePixel=0},s.children)end local r=l.Component:extend'BarChart'
function r.render(s)local t=s.props.element local u=t.title or'Bar Chart'local v
=t.subtitle local w=t.height or 180 local x=t.data or{}local y=#x local z=0 for
A,B in ipairs(x)do local C=0 if type(B)=='table'then C=B.value or B.Value or 0
elseif type(B)=='number'then C=B end if C>z then z=C end end if z==0 then z=1
end local A=0 if u or v then if u then A=A+16 end if v then A=A+12 end if u and
v then A=A+2 end A=A+10 end local B=12 local C={Layout=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,Padding=UDim.new(0,B
),SortOrder=Enum.SortOrder.LayoutOrder})}for D,E in ipairs(x)do local F=0 local
G=n.Colors.Accent if type(E)=='table'then F=E.value or E.Value or 0 G=E.color or
E.Color or n.Colors.Accent elseif type(E)=='number'then F=E end local H=math.
clamp(F/z,0.05,1)local I=1/y local J=-B*(1-I)C['BarCol_'..tostring(D)]=l.
createElement('Frame',{Size=UDim2.new(I,J,1,0),BackgroundTransparency=1,
LayoutOrder=D},{Bar=l.createElement(q,{targetSize=UDim2.new(0.6,0,H*0.85,0),
color=G},{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.Element
}),ValLabel=l.createElement('TextLabel',{Size=UDim2.new(1.5,0,0,14),Position=
UDim2.new(0.5,0,0,-16),AnchorPoint=Vector2.new(0.5,0),BackgroundTransparency=1,
Text=tostring(F),TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=9})})})end
local D={Layout=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection
.Horizontal,Padding=UDim.new(0,B),SortOrder=Enum.SortOrder.LayoutOrder})}for E,F
in ipairs(x)do local G=''if type(F)=='table'then G=F.label or F.Label or''else G
=tostring(E)end local H=1/y local I=-B*(1-H)D['Label_'..tostring(E)]=l.
createElement('TextLabel',{Size=UDim2.new(H,I,1,0),BackgroundTransparency=1,Text
=G,TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Medium,TextSize=9,LayoutOrder=
E})end return l.createElement('Frame',{Size=UDim2.new(1,0,0,w),BackgroundColor3=
n.Colors.Element,BorderSizePixel=0},{Corner=l.createElement('UICorner',{
CornerRadius=n.CornerRadius.Card}),Stroke=l.createElement('UIStroke',{Color=n.
Colors.Border,Thickness=1}),Padding=l.createElement('UIPadding',{PaddingTop=UDim
.new(0,10),PaddingBottom=UDim.new(0,10),PaddingLeft=UDim.new(0,12),PaddingRight=
UDim.new(0,12)}),List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,10),SortOrder=Enum.SortOrder.
LayoutOrder}),Header=(u or v)and l.createElement('Frame',{Size=UDim2.new(1,0,0,0
),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=1},{
List=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
Padding=UDim.new(0,2),SortOrder=Enum.SortOrder.LayoutOrder}),TitleLabel=u and l.
createElement('TextLabel',{Size=UDim2.new(1,0,0,16),BackgroundTransparency=1,
Text=u,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=13,TextXAlignment=
Enum.TextXAlignment.Left,LayoutOrder=1}),SubLabel=v and l.createElement(
'TextLabel',{Size=UDim2.new(1,0,0,12),BackgroundTransparency=1,Text=v,TextColor3
=n.Colors.TextSecondary,Font=n.Fonts.Regular,TextSize=10,TextXAlignment=Enum.
TextXAlignment.Left,LayoutOrder=2})}),Canvas=l.createElement('Frame',{Size=UDim2
.new(1,0,1,-A),BackgroundTransparency=1,LayoutOrder=2},{YAxis=l.createElement(
'Frame',{Name='YAxis',Size=UDim2.new(0,24,1,-16),BackgroundTransparency=1},{Max=
l.createElement('TextLabel',{Size=UDim2.new(1,0,0,10),Position=UDim2.fromScale(0
,0.15),AnchorPoint=Vector2.new(0,0.5),BackgroundTransparency=1,Text=tostring(z),
TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Medium,TextSize=8,TextXAlignment=
Enum.TextXAlignment.Left}),Mid=l.createElement('TextLabel',{Size=UDim2.new(1,0,0
,10),Position=UDim2.fromScale(0,0.575),AnchorPoint=Vector2.new(0,0.5),
BackgroundTransparency=1,Text=tostring(math.round(z/2)),TextColor3=n.Colors.
TextSecondary,Font=n.Fonts.Medium,TextSize=8,TextXAlignment=Enum.TextXAlignment.
Left}),Min=l.createElement('TextLabel',{Size=UDim2.new(1,0,0,10),Position=UDim2.
fromScale(0,1),AnchorPoint=Vector2.new(0,0.5),BackgroundTransparency=1,Text='0',
TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Medium,TextSize=8,TextXAlignment=
Enum.TextXAlignment.Left})}),PlotContainer=l.createElement('Frame',{Name=
'PlotContainer',Size=UDim2.new(1,-28,1,0),Position=UDim2.new(0,28,0,0),
BackgroundTransparency=1},{GridLine_Top=l.createElement('Frame',{Size=UDim2.new(
1,0,0,1),Position=UDim2.fromScale(0,0.15),BackgroundColor3=n.Colors.Border,
BackgroundTransparency=0.6,BorderSizePixel=0}),GridLine_Mid=l.createElement(
'Frame',{Size=UDim2.new(1,0,0,1),Position=UDim2.fromScale(0,0.575),
BackgroundColor3=n.Colors.Border,BackgroundTransparency=0.6,BorderSizePixel=0}),
GridLine_Bottom=l.createElement('Frame',{Size=UDim2.new(1,0,0,1),Position=UDim2.
fromScale(0,1),BackgroundColor3=n.Colors.Border,BackgroundTransparency=0.6,
BorderSizePixel=0}),YAxisLine=l.createElement('Frame',{Size=UDim2.new(0,1,1,-16)
,Position=UDim2.fromScale(0,0),BackgroundColor3=n.Colors.Border,BorderSizePixel=
0}),BarArea=l.createElement('Frame',{Size=UDim2.new(1,0,1,-16),
BackgroundTransparency=1},C),LabelsArea=l.createElement('Frame',{Size=UDim2.new(
1,0,0,16),Position=UDim2.new(0,0,1,-16),BackgroundTransparency=1},D)})})})end
return r end d['Components/Button']=function()local i=createVirtualModule
'Components/Button'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=game:GetService'TweenService'local m=j(k.
Packages.React)local n=i.Parent.Parent local o=j(n.Theme)local p=m.Component:
extend'Button'function p.init(q)q.state={hovered=false,pressed=false}q.btnRef=m.
createRef()q.strokeRef=m.createRef()end function p.didMount(q)local r=q.btnRef.
current local s=q.strokeRef.current local t=q.state.hovered local u=q.state.
pressed if r then r.BackgroundColor3=u and o.Colors.Accent or(t and o.Colors.
ElementHover or o.Colors.Element)end if s then s.Color=t and o.Colors.
BorderHover or o.Colors.Border end end function p.didUpdate(q,r,s)local t=q.
btnRef.current local u=q.strokeRef.current local v=q.state.hovered local w=q.
state.pressed local x=(v~=s.hovered)or(w~=s.pressed)if x then local y=w and o.
Colors.Accent or(v and o.Colors.ElementHover or o.Colors.Element)local z=v and o
.Colors.BorderHover or o.Colors.Border local A=TweenInfo.new(0.2,Enum.
EasingStyle.Quad,Enum.EasingDirection.Out)if t then l:Create(t,A,{
BackgroundColor3=y}):Play()end if u then l:Create(u,A,{Color=z}):Play()end end
end function p.render(q)local r=q.props.element local s=q.state.hovered local t=
q.state.pressed return m.createElement('TextButton',{ref=q.btnRef,Size=UDim2.
new(1,0,0,36),BackgroundColor3=t and o.Colors.Accent or(s and o.Colors.
ElementHover or o.Colors.Element),BackgroundTransparency=0,BorderSizePixel=0,
Text='',AutoButtonColor=false,[m.Event.MouseEnter]=function()q:setState{hovered=
true}end,[m.Event.MouseLeave]=function()q:setState{hovered=false,pressed=false}
end,[m.Event.MouseButton1Down]=function()q:setState{pressed=true}end,[m.Event.
MouseButton1Up]=function()if t then q:setState{pressed=false}if r.callback then
task.spawn(r.callback)end end end},{Corner=m.createElement('UICorner',{
CornerRadius=o.CornerRadius.Element}),Stroke=m.createElement('UIStroke',{ref=q.
strokeRef,Color=s and o.Colors.BorderHover or o.Colors.Border,Thickness=1}),
Content=m.createElement('Frame',{Size=UDim2.new(1,0,1,0),BackgroundTransparency=
1},{List=m.createElement('UIListLayout',{FillDirection=Enum.FillDirection.
Horizontal,HorizontalAlignment=Enum.HorizontalAlignment.Center,VerticalAlignment
=Enum.VerticalAlignment.Center,Padding=UDim.new(0,8)}),Icon=r.icon and m.
createElement('ImageLabel',{Size=UDim2.new(0,16,0,16),BackgroundTransparency=1,
Image=r.icon,ImageColor3=o.Colors.Text})or nil,Label=m.createElement('TextLabel'
,{Size=UDim2.new(0,0,1,0),AutomaticSize=Enum.AutomaticSize.X,
BackgroundTransparency=1,Text=r.name or'Button',TextColor3=o.Colors.Text,Font=o.
Fonts.Bold,TextSize=13})})})end return p end d['Components/ColorPicker']=
function()local i=createVirtualModule'Components/ColorPicker'createVirtualFolder
''local j=__require local k=game:GetService'ReplicatedStorage'local l=j(k.
Packages.React)local m=i.Parent.Parent local n=j(m.Theme)local o=l.Component:
extend'ColorPicker'function o.init(p)p.state={hovered=false}end function o.
render(p)local q=p.props.element local r=p.props.Store local s=q.value or Color3
.fromRGB(255,255,255)local t=q.transparency or 0 local u=p.state.hovered local v
=math.round(s.R*255)local w=math.round(s.G*255)local x=math.round(s.B*255)local
y=math.round((1-t)*100)local z=string.format('#%02X%02X%02X (%d%%)',v,w,x,y)
return l.createElement('Frame',{Size=UDim2.new(1,0,0,36),BackgroundTransparency=
1},{List=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.
Horizontal,HorizontalAlignment=Enum.HorizontalAlignment.Right,VerticalAlignment=
Enum.VerticalAlignment.Center,Padding=UDim.new(0,8),SortOrder=Enum.SortOrder.
LayoutOrder}),HexLabel=l.createElement('TextLabel',{Size=UDim2.new(0,100,1,0),
BackgroundTransparency=1,Text=z,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,
TextSize=11,TextXAlignment=Enum.TextXAlignment.Right,LayoutOrder=1}),ColorButton
=l.createElement('TextButton',{Size=UDim2.fromOffset(24,24),BackgroundColor3=s,
BackgroundTransparency=t,BorderSizePixel=0,Text='',AutoButtonColor=false,
LayoutOrder=2,[l.Event.MouseEnter]=function()p:setState{hovered=true}end,[l.
Event.MouseLeave]=function()p:setState{hovered=false}end,[l.Event.
MouseButton1Click]=function()r:SetColorPicker{id=q.id,name=q.name or
'Select Color',value=s,transparency=t,callback=q.callback}end},{Corner=l.
createElement('UICorner',{CornerRadius=UDim.new(0,5)}),Stroke=l.createElement(
'UIStroke',{Color=u and n.Colors.BorderHover or n.Colors.Border,Thickness=1.5})}
)})end return o end d['Components/ColorPickerDialog']=function()local i=
createVirtualModule'Components/ColorPickerDialog'createVirtualFolder''local j=
__require local k=game:GetService'ReplicatedStorage'local l=j(k.Packages.React)
local m=game:GetService'UserInputService'local n=i.Parent.Parent local o=j(n.
Theme)local p=l.Component:extend'ColorPickerDialog'function p.init(q)local r=q.
props.colorPickerActive local s=r.value or Color3.fromRGB(255,255,255)local t=r.
transparency or 0 local u,v,w=s:ToHSV()q.state={h=u,s=v,v=w,a=1-t}q.wheelRef=l.
createRef()q.brightnessRef=l.createRef()q.opacityRef=l.createRef()end function p
.didMount(q)q.inputEndedConnection=m.InputEnded:Connect(function(r)if r.
UserInputType==Enum.UserInputType.MouseButton1 or r.UserInputType==Enum.
UserInputType.Touch then q.isDraggingWheel=false q.isDraggingBrightness=false q.
isDraggingOpacity=false end end)end function p.willUnmount(q)if q.
inputEndedConnection then q.inputEndedConnection:Disconnect()q.
inputEndedConnection=nil end end function p.render(q)local r=q.props local s=r.
colorPickerActive local t=r.Store local u=q.state.h local v=q.state.s local w=q.
state.v local x=q.state.a local y=s.value or Color3.fromRGB(255,255,255)local z=
s.transparency or 0 local A=Color3.fromHSV(u,v,w)local B=1-x local C=math.round(
A.R*255)local D=math.round(A.G*255)local E=math.round(A.B*255)local F=math.
round(x*100)local G=string.format('#%02X%02X%02X',C,D,E)local function 
updateWheel(H)local I=q.wheelRef.current if not I then return end local J=I.
AbsolutePosition local K=I.AbsoluteSize local L=J+K/2 local M=Vector2.new(H.
Position.X,H.Position.Y)local N=M-L local O=N.Magnitude local P=K.X/2 local Q=
math.clamp(O/P,0,1)local R=math.atan2(-N.Y,N.X)local S=(R+math.pi)/(2*math.pi)q:
setState{h=S,s=Q}end local function updateBrightness(H)local I=q.brightnessRef.
current if not I then return end local J=I.AbsolutePosition local K=I.
AbsoluteSize local L=math.clamp((H.Position.X-J.X)/K.X,0,1)q:setState{v=L}end
local function updateOpacity(H)local I=q.opacityRef.current if not I then return
end local J=I.AbsolutePosition local K=I.AbsoluteSize local L=math.clamp((H.
Position.X-J.X)/K.X,0,1)q:setState{a=L}end local function onWheelInput(H,I)if I.
UserInputType==Enum.UserInputType.MouseButton1 or I.UserInputType==Enum.
UserInputType.Touch then q.isDraggingWheel=true updateWheel(I)end end local 
function onWheelChange(H,I)if q.isDraggingWheel and(I.UserInputType==Enum.
UserInputType.MouseMovement or I.UserInputType==Enum.UserInputType.Touch)then
updateWheel(I)end end local function onBrightnessInput(H,I)if I.UserInputType==
Enum.UserInputType.MouseButton1 or I.UserInputType==Enum.UserInputType.Touch
then q.isDraggingBrightness=true updateBrightness(I)end end local function 
onBrightnessChange(H,I)if q.isDraggingBrightness and(I.UserInputType==Enum.
UserInputType.MouseMovement or I.UserInputType==Enum.UserInputType.Touch)then
updateBrightness(I)end end local function onOpacityInput(H,I)if I.UserInputType
==Enum.UserInputType.MouseButton1 or I.UserInputType==Enum.UserInputType.Touch
then q.isDraggingOpacity=true updateOpacity(I)end end local function 
onOpacityChange(H,I)if q.isDraggingOpacity and(I.UserInputType==Enum.
UserInputType.MouseMovement or I.UserInputType==Enum.UserInputType.Touch)then
updateOpacity(I)end end local function setHex(H)H=H:gsub('#','')if#H==6 then
local I=tonumber(H:sub(1,2),16)local J=tonumber(H:sub(3,4),16)local K=tonumber(H
:sub(5,6),16)if I and J and K then local L=Color3.fromRGB(I,J,K)local M,N,O=L:
ToHSV()q:setState{h=M,s=N,v=O}end end end local function setRGB(H,I)local J=
tonumber(I)if J then J=math.clamp(math.round(J),0,255)local K=H=='R'and J or C
local L=H=='G'and J or D local M=H=='B'and J or E local N=Color3.fromRGB(K,L,M)
local O,P,Q=N:ToHSV()q:setState{h=O,s=P,v=Q}end end local function setAlpha(H)
local I=tonumber(H)if I then I=math.clamp(I,0,100)q:setState{a=I/100}end end
local H=(u*2*math.pi)-math.pi local I=v*50*math.cos(H)local J=-v*50*math.sin(H)
local K=function(K,L,M,N)return l.createElement('Frame',{Size=UDim2.new(1,0,0,18
),BackgroundTransparency=1,LayoutOrder=M},{Box=l.createElement('TextBox',{Size=
UDim2.new(0,60,1,0),BackgroundColor3=o.Colors.Element,BorderSizePixel=0,Text=L,
TextColor3=o.Colors.Text,Font=o.Fonts.Medium,TextSize=10,ClearTextOnFocus=false,
[l.Event.FocusLost]=function(O)N(O.Text)end},{Corner=l.createElement('UICorner',
{CornerRadius=UDim.new(0,3)}),Stroke=l.createElement('UIStroke',{Color=o.Colors.
Border,Thickness=1})}),Label=l.createElement('TextLabel',{Size=UDim2.new(1,-66,1
,0),Position=UDim2.new(0,66,0,0),BackgroundTransparency=1,Text=K,TextColor3=o.
Colors.TextSecondary,Font=o.Fonts.Medium,TextSize=10,TextXAlignment=Enum.
TextXAlignment.Left})})end return l.createElement('Frame',{Size=UDim2.new(1,0,1,
0),BackgroundTransparency=1,ZIndex=1000},{Backdrop=l.createElement('TextButton',
{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.fromRGB(0,0,0),
BackgroundTransparency=0.5,BorderSizePixel=0,Text='',AutoButtonColor=false,
ZIndex=1,[l.Event.MouseButton1Down]=function()t:SetColorPicker(nil)end}),Card=l.
createElement('TextButton',{Size=UDim2.new(0,250,0,340),Position=UDim2.new(0.5,0
,0.5,0),AnchorPoint=Vector2.new(0.5,0.5),BackgroundColor3=o.Colors.Card,
BorderSizePixel=0,Text='',AutoButtonColor=false,Active=true,ZIndex=2},{Corner=l.
createElement('UICorner',{CornerRadius=o.CornerRadius.Card}),Stroke=l.
createElement('UIStroke',{Color=o.Colors.Border,Thickness=1}),Padding=l.
createElement('UIPadding',{PaddingTop=UDim.new(0,12),PaddingBottom=UDim.new(0,12
),PaddingLeft=UDim.new(0,12),PaddingRight=UDim.new(0,12)}),List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,10)
,HorizontalAlignment=Enum.HorizontalAlignment.Center,SortOrder=Enum.SortOrder.
LayoutOrder}),Title=l.createElement('TextLabel',{Size=UDim2.new(1,0,0,18),
BackgroundTransparency=1,Text=s.name or'Accent Tint Color',TextColor3=o.Colors.
Text,Font=o.Fonts.Bold,TextSize=13,TextXAlignment=Enum.TextXAlignment.Center,
LayoutOrder=1}),MainRow=l.createElement('Frame',{Size=UDim2.new(1,0,0,100),
BackgroundTransparency=1,LayoutOrder=2},{Wheel=l.createElement('ImageLabel',{ref
=q.wheelRef,Size=UDim2.new(0,100,0,100),BackgroundTransparency=1,Image=
'rbxassetid://6020299385',Active=true,[l.Event.InputBegan]=onWheelInput,[l.Event
.InputChanged]=onWheelChange},{Corner=l.createElement('UICorner',{CornerRadius=
UDim.new(0.5,0)}),Knob=l.createElement('Frame',{Size=UDim2.new(0,8,0,8),Position
=UDim2.new(0.5,I,0.5,J),AnchorPoint=Vector2.new(0.5,0.5),BackgroundColor3=Color3
.fromRGB(255,255,255),BorderSizePixel=0},{Corner=l.createElement('UICorner',{
CornerRadius=UDim.new(0.5,0)}),Stroke=l.createElement('UIStroke',{Color=Color3.
fromRGB(0,0,0),Thickness=1.5})})}),Fields=l.createElement('Frame',{Size=UDim2.
new(1,-110,1,0),Position=UDim2.new(1,0,0,0),AnchorPoint=Vector2.new(1,0),
BackgroundTransparency=1},{List=l.createElement('UIListLayout',{FillDirection=
Enum.FillDirection.Vertical,Padding=UDim.new(0,2),SortOrder=Enum.SortOrder.
LayoutOrder}),HexRow=K('Hex',G,1,setHex),RedRow=K('Red',tostring(C),2,function(L
)setRGB('R',L)end),GreenRow=K('Green',tostring(D),3,function(L)setRGB('G',L)end)
,BlueRow=K('Blue',tostring(E),4,function(L)setRGB('B',L)end),AlphaRow=K('Alpha',
tostring(F),5,setAlpha)})}),Sliders=l.createElement('Frame',{Size=UDim2.new(1,0,
0,48),BackgroundTransparency=1,LayoutOrder=3},{List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,6),
SortOrder=Enum.SortOrder.LayoutOrder}),Brightness=l.createElement('Frame',{Size=
UDim2.new(1,0,0,20),BackgroundTransparency=1,LayoutOrder=1},{Label=l.
createElement('TextLabel',{Size=UDim2.new(0,60,1,0),BackgroundTransparency=1,
Text='Brightness',TextColor3=o.Colors.TextSecondary,Font=o.Fonts.Medium,TextSize
=10,TextXAlignment=Enum.TextXAlignment.Left}),Bar=l.createElement('Frame',{ref=q
.brightnessRef,Size=UDim2.new(1,-65,0,8),Position=UDim2.new(0,65,0.5,-4),
BorderSizePixel=0,Active=true,[l.Event.InputBegan]=onBrightnessInput,[l.Event.
InputChanged]=onBrightnessChange},{Corner=l.createElement('UICorner',{
CornerRadius=UDim.new(0.5,0)}),Gradient=l.createElement('UIGradient',{Color=
ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromRGB(0,0,0)),
ColorSequenceKeypoint.new(1,Color3.fromHSV(u,v,1))}}),Knob=l.createElement(
'Frame',{Size=UDim2.new(0,6,0,14),Position=UDim2.new(w,0,0.5,0),AnchorPoint=
Vector2.new(0.5,0.5),BackgroundColor3=Color3.fromRGB(255,255,255),
BorderSizePixel=0},{Corner=l.createElement('UICorner',{CornerRadius=UDim.new(0,2
)}),Stroke=l.createElement('UIStroke',{Color=o.Colors.Border,Thickness=1})})})})
,Opacity=l.createElement('Frame',{Size=UDim2.new(1,0,0,20),
BackgroundTransparency=1,LayoutOrder=2},{Label=l.createElement('TextLabel',{Size
=UDim2.new(0,60,1,0),BackgroundTransparency=1,Text='Opacity',TextColor3=o.Colors
.TextSecondary,Font=o.Fonts.Medium,TextSize=10,TextXAlignment=Enum.
TextXAlignment.Left}),Bar=l.createElement('Frame',{ref=q.opacityRef,Size=UDim2.
new(1,-65,0,8),Position=UDim2.new(0,65,0.5,-4),BorderSizePixel=0,Active=true,[l.
Event.InputBegan]=onOpacityInput,[l.Event.InputChanged]=onOpacityChange},{Corner
=l.createElement('UICorner',{CornerRadius=UDim.new(0.5,0)}),Gradient=l.
createElement('UIGradient',{Color=ColorSequence.new{ColorSequenceKeypoint.new(0,
Color3.fromHSV(u,v,w)),ColorSequenceKeypoint.new(1,Color3.fromHSV(u,v,w))},
Transparency=NumberSequence.new{NumberSequenceKeypoint.new(0,1),
NumberSequenceKeypoint.new(1,0)}}),Knob=l.createElement('Frame',{Size=UDim2.new(
0,6,0,14),Position=UDim2.new(x,0,0.5,0),AnchorPoint=Vector2.new(0.5,0.5),
BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0},{Corner=l.
createElement('UICorner',{CornerRadius=UDim.new(0,2)}),Stroke=l.createElement(
'UIStroke',{Color=o.Colors.Border,Thickness=1})})})})}),PreviewRow=l.
createElement('Frame',{Size=UDim2.new(1,0,0,24),BackgroundTransparency=1,
LayoutOrder=4},{Stroke=l.createElement('UIStroke',{Color=o.Colors.Border,
Thickness=1}),NewColor=l.createElement('Frame',{Size=UDim2.new(0.5,0,1,0),
Position=UDim2.new(0,0,0,0),BackgroundColor3=A,BackgroundTransparency=B,
BorderSizePixel=0}),Initial=l.createElement('Frame',{Size=UDim2.new(0.5,0,1,0),
Position=UDim2.new(0.5,0,0,0),BackgroundColor3=y,BackgroundTransparency=z,
BorderSizePixel=0})}),ActionRow=l.createElement('Frame',{Size=UDim2.new(1,0,0,56
),BackgroundTransparency=1,LayoutOrder=5},{List=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,4),SortOrder=Enum.
SortOrder.LayoutOrder}),Confirm=l.createElement('TextButton',{Size=UDim2.new(1,0
,0,26),BackgroundColor3=o.Colors.Accent,BorderSizePixel=0,Text='Confirm',
TextColor3=Color3.fromRGB(255,255,255),Font=o.Fonts.Bold,TextSize=11,LayoutOrder
=1,[l.Event.MouseButton1Click]=function()t:UpdateElement(s.id,function(L)L.value
=A L.transparency=B end)if s.callback then task.spawn(s.callback,A,B)end t:
SetColorPicker(nil)end},{Corner=l.createElement('UICorner',{CornerRadius=o.
CornerRadius.Element})}),Cancel=l.createElement('TextButton',{Size=UDim2.new(1,0
,0,26),BackgroundColor3=o.Colors.Element,BorderSizePixel=0,Text='Cancel',
TextColor3=o.Colors.Text,Font=o.Fonts.Medium,TextSize=11,LayoutOrder=2,[l.Event.
MouseButton1Click]=function()t:SetColorPicker(nil)end},{Corner=l.createElement(
'UICorner',{CornerRadius=o.CornerRadius.Element}),Stroke=l.createElement(
'UIStroke',{Color=o.Colors.Border,Thickness=1})})})})})end return p end d[
'Components/ColorPickerWidget']=function()local i=createVirtualModule
'Components/ColorPickerWidget'createVirtualFolder''local j=__require local k=
game:GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.
Parent local n=j(m.Theme)local o=l.Component:extend'ColorPickerWidget'function o
.init(p)end function o.render(p)local q=p.props.element local r=p.props.Store
local s=q.value or Color3.fromRGB(255,255,255)local t=q.transparency or 0 local 
function triggerCallback(u,v)if q.callback then task.spawn(q.callback,u,v)end
end local function updateColor(u,v,w,x)local y=Color3.fromRGB(u,v,w)r:
UpdateElement(q.id,function(z)z.value=y z.transparency=x end)triggerCallback(y,x
)end local u=math.round(s.R*255)local v=math.round(s.G*255)local w=math.round(s.
B*255)local x=math.round(t*100)return l.createElement('Frame',{Size=UDim2.new(1,
0,0,0),BackgroundColor3=n.Colors.Element,BorderSizePixel=0,AutomaticSize=Enum.
AutomaticSize.Y},{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius
.Element}),Stroke=l.createElement('UIStroke',{Color=n.Colors.Border,Thickness=1}
),Padding=l.createElement('UIPadding',{PaddingTop=UDim.new(0,10),PaddingBottom=
UDim.new(0,10),PaddingLeft=UDim.new(0,10),PaddingRight=UDim.new(0,10)}),List=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,8),SortOrder=Enum.SortOrder.LayoutOrder}),Header=l.createElement(
'Frame',{Size=UDim2.new(1,0,0,20),BackgroundTransparency=1,LayoutOrder=1},{Title
=l.createElement('TextLabel',{Size=UDim2.new(0.5,0,1,0),BackgroundTransparency=1
,Text=q.name or'Color Picker',TextColor3=n.Colors.TextSecondary,Font=n.Fonts.
Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left}),Info=l.
createElement('TextLabel',{Size=UDim2.new(0.5,-24,1,0),Position=UDim2.new(1,-24,
0,0),AnchorPoint=Vector2.new(1,0),BackgroundTransparency=1,Text=string.format(
'#%02X%02X%02X (%d%%)',u,v,w,x),TextColor3=n.Colors.Text,Font=n.Fonts.Bold,
TextSize=11,TextXAlignment=Enum.TextXAlignment.Right}),ColorPreview=l.
createElement('Frame',{Size=UDim2.new(0,16,0,16),Position=UDim2.new(1,-16,0.5,-8
),BackgroundColor3=s,BackgroundTransparency=t,BorderSizePixel=0},{Corner=l.
createElement('UICorner',{CornerRadius=n.CornerRadius.Element}),Stroke=l.
createElement('UIStroke',{Color=n.Colors.Border,Thickness=1})})}),Red=l.
createElement(j(m.Components.Slider),{Store=r,element={name='Red',min=0,max=255,
value=u,callback=function(y)updateColor(y,v,w,t)end},LayoutOrder=2}),Green=l.
createElement(j(m.Components.Slider),{Store=r,element={name='Green',min=0,max=
255,value=v,callback=function(y)updateColor(u,y,w,t)end},LayoutOrder=3}),Blue=l.
createElement(j(m.Components.Slider),{Store=r,element={name='Blue',min=0,max=255
,value=w,callback=function(y)updateColor(u,v,y,t)end},LayoutOrder=4}),
Transparency=l.createElement(j(m.Components.Slider),{Store=r,element={name=
'Transparency',min=0,max=100,value=x,callback=function(y)updateColor(u,v,w,y/100
)end},LayoutOrder=5})})end return o end d['Components/Dropdown']=function()local
i=createVirtualModule'Components/Dropdown'createVirtualFolder''local j=__require
local k=game:GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.
Parent.Parent local n=j(m.Theme)local o=game:GetService'TweenService'local p=
table.find or function(p,q)for r,s in ipairs(p)do if s==q then return r end end
return nil end local q=table.clone or function(q)local r={}for s,t in pairs(q)do
r[s]=t end return r end local r=l.Component:extend'DropdownOption'function r.
init(s)s.state={hovered=false}end function r.render(s)local t=s.props local u=t.
opt local v=t.isSelected local w=t.isMulti local x=t.LayoutOrder local y=s.state
.hovered local z=(not w and v)and n.Colors.Accent or(y and n.Colors.ElementHover
or n.Colors.Element)return l.createElement('TextButton',{Size=UDim2.new(1,0,0,28
),BackgroundColor3=z,BorderSizePixel=0,Text='',AutoButtonColor=false,LayoutOrder
=x,[l.Event.MouseEnter]=function()s:setState{hovered=true}end,[l.Event.
MouseLeave]=function()s:setState{hovered=false}end,[l.Event.MouseButton1Click]=t
.OnClick},{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.
Element}),Content=l.createElement('Frame',{Size=UDim2.new(1,0,1,0),
BackgroundTransparency=1},{List=l.createElement('UIListLayout',{FillDirection=
Enum.FillDirection.Horizontal,VerticalAlignment=Enum.VerticalAlignment.Center,
Padding=UDim.new(0,8),SortOrder=Enum.SortOrder.LayoutOrder}),Padding=l.
createElement('UIPadding',{PaddingLeft=UDim.new(0,8)}),Checkbox=w and l.
createElement('Frame',{Size=UDim2.fromOffset(14,14),BackgroundColor3=v and n.
Colors.Accent or n.Colors.Element,BorderSizePixel=0,LayoutOrder=1},{Corner=l.
createElement('UICorner',{CornerRadius=UDim.new(0,3)}),Stroke=l.createElement(
'UIStroke',{Color=(v or y)and n.Colors.BorderHover or n.Colors.Border,Thickness=
1}),Inner=v and l.createElement('Frame',{Size=UDim2.fromOffset(6,6),Position=
UDim2.fromScale(0.5,0.5),AnchorPoint=Vector2.new(0.5,0.5),BackgroundColor3=
Color3.fromRGB(255,255,255),BorderSizePixel=0},{Corner=l.createElement(
'UICorner',{CornerRadius=UDim.new(0,1)})})or nil})or nil,Label=l.createElement(
'TextLabel',{Size=UDim2.new(1,w and-22 or 0,1,0),BackgroundTransparency=1,Text=
tostring(u),TextColor3=n.Colors.Text,Font=n.Fonts.Medium,TextSize=12,
TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=2})})})end local s=l.
Component:extend'Dropdown'function s.init(t)t.state={hovered=false,isOpen=false}
t.strokeRef=l.createRef()t.arrowRef=l.createRef()t.optionsRef=l.createRef()end
function s.didMount(t)local u=t.strokeRef.current local v=t.arrowRef.current
local w=t.optionsRef.current local x=t.state.hovered local y=t.state.isOpen if u
then u.Color=(x or y)and n.Colors.BorderHover or n.Colors.Border end if v then v
.Rotation=y and 180 or 0 end if w then local z=t.props.element.options or{}local
A=#z local B=A>0 and(A*28+(A-1)*4+8)or 0 w.Size=y and UDim2.new(1,0,0,B)or UDim2
.new(1,0,0,0)w.GroupTransparency=y and 0 or 1 w.Visible=y end end function s.
didUpdate(t,u,v)local w=t.strokeRef.current local x=t.arrowRef.current local y=t
.optionsRef.current local z=t.state.hovered local A=t.state.isOpen local B=
TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)if z~=v.hovered
or A~=v.isOpen then if w then local C=(z or A)and n.Colors.BorderHover or n.
Colors.Border o:Create(w,B,{Color=C}):Play()end end if A~=v.isOpen then if x
then o:Create(x,B,{Rotation=A and 180 or 0}):Play()end if y then local C=t.props
.element.options or{}local D=#C local E=D>0 and(D*28+(D-1)*4+8)or 0 if A then y.
Visible=true y.GroupTransparency=1 y.Size=UDim2.new(1,0,0,0)o:Create(y,B,{Size=
UDim2.new(1,0,0,E),GroupTransparency=0}):Play()else local F=o:Create(y,B,{Size=
UDim2.new(1,0,0,0),GroupTransparency=1})F:Play()F.Completed:Connect(function()if
not t.state.isOpen then y.Visible=false end end)end end end end function s.
render(t)local u=t.props.element local v=t.props.Store local w=u.options or{}
local x=u.maximum~=nil and u.maximum>1 local y=t.state.hovered local z=t.state.
isOpen local A=u.value if x then if type(A)~='table'then A=type(A)=='number'and{
A}or{}end else A=A or 1 end local B='Select...'if x then if#A>0 then local C={}
for D,E in ipairs(A)do if w[E]then table.insert(C,tostring(w[E]))end end B=table
.concat(C,', ')end else B=tostring(w[A]or'Select...')end local C={Layout=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder}),Padding=l.createElement(
'UIPadding',{PaddingTop=UDim.new(0,4),PaddingBottom=UDim.new(0,4)})}for D,E in
ipairs(w)do local F=false if x then F=p(A,D)~=nil else F=D==A end C['Opt_'..
tostring(D)]=l.createElement(r,{opt=E,isSelected=F,isMulti=x,LayoutOrder=D,
OnClick=function()if x then local G=q(A)local H=p(G,D)if H then table.remove(G,H
)else if#G<u.maximum then table.insert(G,D)end end v:UpdateElement(u.id,function
(I)I.value=G end)if u.callback then task.spawn(u.callback,G)end else v:
UpdateElement(u.id,function(G)G.value=D end)t:setState{isOpen=false}if u.
callback then task.spawn(u.callback,D)end end end})end local D=u.hideLabel
return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1
,AutomaticSize=Enum.AutomaticSize.Y},{List=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder})
,Header=l.createElement('TextButton',{Size=UDim2.new(1,0,0,36),BackgroundColor3=
n.Colors.Element,BorderSizePixel=0,Text='',AutoButtonColor=false,LayoutOrder=1,[
l.Event.MouseEnter]=function()t:setState{hovered=true}end,[l.Event.MouseLeave]=
function()t:setState{hovered=false}end,[l.Event.MouseButton1Click]=function()t:
setState{isOpen=not z}end},{Corner=l.createElement('UICorner',{CornerRadius=n.
CornerRadius.Element}),Stroke=l.createElement('UIStroke',{ref=t.strokeRef,Color=
(y or z)and n.Colors.BorderHover or n.Colors.Border,Thickness=1}),Title=not D
and l.createElement('TextLabel',{Size=UDim2.new(0.5,0,1,0),Position=UDim2.new(0,
10,0,0),BackgroundTransparency=1,Text=u.name or'Dropdown',TextColor3=n.Colors.
TextSecondary,Font=n.Fonts.Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment
.Left})or nil,Selected=l.createElement('TextLabel',{Size=D and UDim2.new(1,-40,1
,0)or UDim2.new(0.5,-30,1,0),Position=D and UDim2.new(0,10,0,0)or UDim2.new(1,-
30,0,0),AnchorPoint=D and Vector2.new(0,0)or Vector2.new(1,0),
BackgroundTransparency=1,Text=B,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,
TextSize=12,TextXAlignment=D and Enum.TextXAlignment.Left or Enum.TextXAlignment
.Right,TextTruncate=Enum.TextTruncate.AtEnd}),Arrow=l.createElement('ImageLabel'
,{ref=t.arrowRef,Size=UDim2.new(0,14,0,14),Position=UDim2.new(1,-22,0.5,-7),
BackgroundTransparency=1,Image='rbxassetid://10747383844',ImageColor3=n.Colors.
TextSecondary})}),Options=l.createElement('CanvasGroup',{ref=t.optionsRef,
BackgroundTransparency=1,LayoutOrder=2,ClipsDescendants=true},C)})end return s
end d['Components/ElementRenderer']=function()local i=createVirtualModule
'Components/ElementRenderer'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=l.Component:
extend'CustomComponentPlaceholder'function m.init(n)n.frameRef=l.createRef()end
function m.didMount(n)local o=n.frameRef.current local p=n.props.element if o
and p.instance then p.instance.Parent=o p.instance.Size=UDim2.new(1,0,1,0)p.
instance.Position=UDim2.new(0,0,0,0)end end function m.didUpdate(n,o)local p=n.
frameRef.current local q=n.props.element local r=o.element if r and r.instance
and r.instance~=q.instance then r.instance.Parent=nil end if p and q.instance
then q.instance.Parent=p q.instance.Size=UDim2.new(1,0,1,0)q.instance.Position=
UDim2.new(0,0,0,0)end end function m.willUnmount(n)local o=n.props.element if o.
instance then o.instance.Parent=nil end end function m.render(n)local o=n.props.
element local p=o.instance and o.instance.Size or UDim2.new(1,0,0,70)return l.
createElement('Frame',{ref=n.frameRef,Size=p,BackgroundTransparency=1})end local
n={}function n.render(o,p)local q=i.Parent.Parent local r=o.type if r=='Button'
then return l.createElement(j(q.Components.Button),{element=o,Store=p})elseif r
=='Toggle'then return l.createElement(j(q.Components.Toggle),{element=o,Store=p}
)elseif r=='Slider'then return l.createElement(j(q.Components.Slider),{element=o
,Store=p})elseif r=='Dropdown'then return l.createElement(j(q.Components.
Dropdown),{element=o,Store=p})elseif r=='TextBox'then return l.createElement(j(q
.Components.TextBox),{element=o,Store=p})elseif r=='Label'then return l.
createElement(j(q.Components.Label),{element=o,Store=p})elseif r=='Paragraph'
then return l.createElement(j(q.Components.Paragraph),{element=o,Store=p})elseif
r=='InfoGrid'then return l.createElement(j(q.Components.InfoGrid),{element=o,
Store=p})elseif r=='VStack'then return l.createElement(j(q.Components.VStack),{
element=o,Store=p})elseif r=='HStack'then return l.createElement(j(q.Components.
HStack),{element=o,Store=p})elseif r=='ColorPicker'then return l.createElement(
j(q.Components.ColorPicker),{element=o,Store=p})elseif r=='ColorPickerWidget'
then return l.createElement(j(q.Components.ColorPickerWidget),{element=o,Store=p
})elseif r=='KeybindField'then return l.createElement(j(q.Components.
KeybindField),{element=o,Store=p})elseif r=='Stepper'then return l.
createElement(j(q.Components.Stepper),{element=o,Store=p})elseif r==
'RadioButtonGroup'then return l.createElement(j(q.Components.RadioButtonGroup),{
element=o,Store=p})elseif r=='BarChart'then return l.createElement(j(q.
Components.BarChart),{element=o,Store=p})elseif r=='LineChart'then return l.
createElement(j(q.Components.LineChart),{element=o,Store=p})elseif r=='PieChart'
then return l.createElement(j(q.Components.PieChart),{element=o,Store=p})elseif
r=='PullDownButton'then return l.createElement(j(q.Components.PullDownButton),{
element=o,Store=p})elseif r=='TitleStack'then return l.createElement(j(q.
Components.TitleStack),{element=o,Store=p})elseif r=='Row'then return l.
createElement(j(q.Components.Row),{element=o,Store=p})elseif r=='Form'then
return l.createElement(j(q.Components.Form),{element=o,Store=p})elseif r==
'Section'then return l.createElement(j(q.Components.Section),{section=o,Store=p}
)elseif r=='CustomComponentPlaceholder'then return l.createElement(m,{element=o}
)end return nil end return n end d['Components/Form']=function()local i=
createVirtualModule'Components/Form'createVirtualFolder''local j=__require local
k=game:GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.
Parent local n=j(m.Components.ElementRenderer)local o=l.Component:extend'Form'
function o.render(p)local q=p.props.element local r=p.props.Store local s={
Layout=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical
,Padding=UDim.new(0,10),SortOrder=Enum.SortOrder.LayoutOrder})}for t,u in
ipairs(q.elements or{})do local v=n.render(u,r)if v then s['Row_'..u.id]=l.
createElement('Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,
AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=t},{Content=v})end end return l.
createElement('Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,
AutomaticSize=Enum.AutomaticSize.Y},s)end return o end d['Components/HStack']=
function()local i=createVirtualModule'Components/HStack'createVirtualFolder''
local j=__require local k=game:GetService'ReplicatedStorage'local l=j(k.Packages
.React)local m=i.Parent.Parent local n=j(m.Components.ElementRenderer)local o=l.
Component:extend'HStack'function o.render(p)local q=p.props.element local r=p.
props.Store local s=#q.elements local t=q.padding and q.padding.Offset or 6
local u={Layout=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection
.Horizontal,Padding=UDim.new(0,t),HorizontalAlignment=q.horizontalAlignment or
Enum.HorizontalAlignment.Left,VerticalAlignment=q.verticalAlignment or Enum.
VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder})}for v,w in
ipairs(q.elements or{})do local x=n.render(w,r)if x then local y=1/s local z=-t*
(1-y)u['El_'..w.id]=l.createElement('Frame',{Size=UDim2.new(y,z,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=v},{
Content=x})end end return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y},u)end return o end
d['Components/InfoGrid']=function()local i=createVirtualModule
'Components/InfoGrid'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=l.Component:extend'InfoGridBlock'function o.init(p)p.
state={hovered=false}end function o.render(p)local q=p.props local r=q.block
local s=q.scale local t=q.widthOffset local u=q.LayoutOrder local v=p.state.
hovered local w=r.callback~=nil local x=(w and v)and n.Colors.ElementHover or n.
Colors.Element local y=(w and v)and n.Colors.BorderHover or n.Colors.Border
local z={Size=UDim2.new(s,t,1,0),BackgroundColor3=x,BorderSizePixel=0,
LayoutOrder=u}if w then z.Text=''z.AutoButtonColor=false z[l.Event.MouseEnter]=
function()p:setState{hovered=true}end z[l.Event.MouseLeave]=function()p:setState
{hovered=false}end z[l.Event.MouseButton1Click]=function()if r.callback then
task.spawn(r.callback)end end end return l.createElement(w and'TextButton'or
'Frame',z,{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.
Element}),Stroke=l.createElement('UIStroke',{Color=y,Thickness=1}),Padding=l.
createElement('UIPadding',{PaddingTop=UDim.new(0,8),PaddingBottom=UDim.new(0,8),
PaddingLeft=UDim.new(0,10),PaddingRight=UDim.new(0,10)}),Content=l.
createElement('Frame',{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1},{List=l
.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,2)}),Title=l.
createElement('TextLabel',{Size=UDim2.new(1,0,0,12),BackgroundTransparency=1,
Text=r.title or'',TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Medium,TextSize
=10,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd
}),Val=l.createElement('TextLabel',{Size=UDim2.new(1,0,1,-14),
BackgroundTransparency=1,Text=r.content or'',TextColor3=n.Colors.Text,Font=n.
Fonts.Bold,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true}
)})})end local p=l.Component:extend'InfoGrid'function p.render(q)local r=q.props
.element local s=r.rows or{}local t={Layout=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,8),SortOrder=Enum.
SortOrder.LayoutOrder})}local u=8 for v,w in ipairs(s)do local x=0 for y,z in
ipairs(w)do x=x+(z.weight or 1)end local y={Layout=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,Padding=UDim.new(0,u
),SortOrder=Enum.SortOrder.LayoutOrder})}for z,A in ipairs(w)do local B=A.weight
or 1 local C=B/x local D=-u*(1-C)y['Block_'..tostring(z)]=l.createElement(o,{
block=A,scale=C,widthOffset=D,LayoutOrder=z})end t['Row_'..tostring(v)]=l.
createElement('Frame',{Size=UDim2.new(1,0,0,50),BackgroundTransparency=1,
LayoutOrder=v},y)end return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y},t)end return p end
d['Components/KeybindField']=function()local i=createVirtualModule
'Components/KeybindField'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=game:GetService'UserInputService'local m=j(
k.Packages.React)local n=i.Parent.Parent local o=j(n.Theme)local p=m.Component:
extend'KeybindField'function p.init(q)q.connection=nil q.state={hovered=false,
listening=false}end function p.willUnmount(q)if q.connection then q.connection:
Disconnect()end end function p.render(q)local r=q.props.element local s=q.props.
Store local t=r.value or Enum.KeyCode.Unknown local u=t==Enum.KeyCode.Unknown
local v=u and'None'or t.Name local w=q.state.hovered local x=q.state.listening
local y=x and'...'or v return m.createElement('Frame',{Size=UDim2.new(1,0,0,36),
BackgroundTransparency=1},{Label=m.createElement('TextLabel',{Size=UDim2.new(1,-
120,1,0),BackgroundTransparency=1,Text=r.name or'Keybind',TextColor3=o.Colors.
Text,Font=o.Fonts.Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left}),
BindButton=m.createElement('TextButton',{Size=UDim2.new(0,100,0,26),Position=
UDim2.new(1,-100,0.5,-13),BackgroundColor3=x and o.Colors.Accent or o.Colors.
Element,BorderSizePixel=0,Text=y,TextColor3=o.Colors.Text,Font=o.Fonts.Bold,
TextSize=11,AutoButtonColor=false,[m.Event.MouseEnter]=function()q:setState{
hovered=true}end,[m.Event.MouseLeave]=function()q:setState{hovered=false}end,[m.
Event.MouseButton1Click]=function()if x then return end q:setState{listening=
true}if q.connection then q.connection:Disconnect()end q.connection=l.InputBegan
:Connect(function(z,A)if z.UserInputType==Enum.UserInputType.Keyboard then local
B=z.KeyCode if B==Enum.KeyCode.Escape then B=Enum.KeyCode.Unknown end s:
UpdateElement(r.id,function(C)C.value=B end)q:setState{listening=false}if q.
connection then q.connection:Disconnect()q.connection=nil end if r.callback then
task.spawn(r.callback,B)end end end)end},{Corner=m.createElement('UICorner',{
CornerRadius=o.CornerRadius.Element}),Stroke=m.createElement('UIStroke',{Color=(
w or x)and o.Colors.BorderHover or o.Colors.Border,Thickness=1})})})end return p
end d['Components/Label']=function()local i=createVirtualModule
'Components/Label'createVirtualFolder''local j=__require local k=game:GetService
'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent local n=j(
m.Theme)local o=l.Component:extend'Label'function o.render(p)local q=p.props.
element return l.createElement('TextLabel',{Size=UDim2.new(1,0,0,18),
BackgroundTransparency=1,Text=q.text or'Label',TextColor3=q.color or n.Colors.
Text,Font=q.font or n.Fonts.Medium,TextSize=q.textSize or 13,TextXAlignment=q.
alignment or Enum.TextXAlignment.Left,RichText=true})end return o end d[
'Components/LineChart']=function()local i=createVirtualModule
'Components/LineChart'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=game:GetService'TweenService'local p=TweenInfo.new(
0.35,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)local q=l.Component:extend
'AnimatedLine'function q.init(r)r.ref=l.createRef()end function q.didMount(r)
local s=r.ref.current if s then local t=r.props s.Size=t.targetSize s.Position=t
.targetPosition s.Rotation=t.targetRotation end end function q.didUpdate(r,s)
local t=r.ref.current if t then local u=r.props if s.targetSize~=u.targetSize or
s.targetPosition~=u.targetPosition or s.targetRotation~=u.targetRotation then t.
Size=s.targetSize t.Position=s.targetPosition t.Rotation=s.targetRotation o:
Create(t,p,{Size=u.targetSize,Position=u.targetPosition,Rotation=u.
targetRotation}):Play()end end end function q.render(r)local s=r.props return l.
createElement('Frame',{ref=r.ref,AnchorPoint=Vector2.new(0.5,0.5),
BackgroundColor3=s.color,BorderSizePixel=0,ZIndex=1})end local r=l.Component:
extend'AnimatedDot'function r.init(s)s.ref=l.createRef()end function r.didMount(
s)local t=s.ref.current if t then t.Position=s.props.targetPosition end end
function r.didUpdate(s,t)local u=s.ref.current if u and t.targetPosition~=s.
props.targetPosition then u.Position=t.targetPosition o:Create(u,p,{Position=s.
props.targetPosition}):Play()end end function r.render(s)local t=s.props return
l.createElement('Frame',{ref=s.ref,Size=UDim2.new(0,6,0,6),AnchorPoint=Vector2.
new(0.5,0.5),BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0,
ZIndex=2},{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.Circle
}),Stroke=l.createElement('UIStroke',{Color=t.color,Thickness=1.5})})end local s
=l.Component:extend'LineChart'function s.init(t)t.canvasRef=l.createRef()t.state
={canvasSize=Vector2.new(300,100)}end function s.didMount(t)local u=t.canvasRef.
current if u then t.sizeConnection=u:GetPropertyChangedSignal'AbsoluteSize':
Connect(function()t:setState{canvasSize=u.AbsoluteSize}end)t:setState{canvasSize
=u.AbsoluteSize}end end function s.willUnmount(t)if t.sizeConnection then t.
sizeConnection:Disconnect()end end function s.render(t)local u=t.props.element
local v=u.title or'Line Chart'local w=u.subtitle local x=u.height or 180 local y
=u.data or{}local z=u.labels or{}local A=u.color or n.Colors.Accent local B=#y
local C=t.state.canvasSize local D=0 for E,F in ipairs(y)do local G=type(F)==
'table'and(F.value or F.Value or 0)or(tonumber(F)or 0)if G>D then D=G end end if
D==0 then D=1 end local E={}for F=1,B do local G=y[F]local H=type(G)=='table'and
(G.value or G.Value or 0)or(tonumber(G)or 0)local I=H/D local J=C.X*(F-1)/math.
max(B-1,1)local K=C.Y*(1-(I*0.7+0.15))table.insert(E,Vector2.new(J,K))end local
F={}if B>1 then for G=1,B-1 do local H=E[G]local I=E[G+1]local J=I-H local K=J.
Magnitude local L=math.deg(math.atan2(J.Y,J.X))F['Line_'..tostring(G)]=l.
createElement(q,{targetSize=UDim2.new(0,K,0,2),targetPosition=UDim2.new(0,H.X+J.
X/2,0,H.Y+J.Y/2),targetRotation=L,color=A})end end for G,H in ipairs(E)do F[
'Dot_'..tostring(G)]=l.createElement(r,{targetPosition=UDim2.new(0,H.X,0,H.Y),
color=A})end local G={Layout=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Horizontal,SortOrder=Enum.SortOrder.LayoutOrder})}for H=1,B do
local I=z[H]or''local J=1/B G['Label_'..tostring(H)]=l.createElement('TextLabel'
,{Size=UDim2.new(J,0,1,0),BackgroundTransparency=1,Text=I,TextColor3=n.Colors.
TextSecondary,Font=n.Fonts.Medium,TextSize=9,LayoutOrder=H})end local H=0 if v
or w then if v then H=H+16 end if w then H=H+12 end if v and w then H=H+2 end H=
H+10 end return l.createElement('Frame',{Size=UDim2.new(1,0,0,x),
BackgroundColor3=n.Colors.Element,BorderSizePixel=0},{Corner=l.createElement(
'UICorner',{CornerRadius=n.CornerRadius.Card}),Stroke=l.createElement('UIStroke'
,{Color=n.Colors.Border,Thickness=1}),Padding=l.createElement('UIPadding',{
PaddingTop=UDim.new(0,10),PaddingBottom=UDim.new(0,10),PaddingLeft=UDim.new(0,12
),PaddingRight=UDim.new(0,12)}),List=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,10),SortOrder=Enum.
SortOrder.LayoutOrder}),Header=(v or w)and l.createElement('Frame',{Size=UDim2.
new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,
LayoutOrder=1},{List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,2),SortOrder=Enum.SortOrder.
LayoutOrder}),TitleLabel=v and l.createElement('TextLabel',{Size=UDim2.new(1,0,0
,16),BackgroundTransparency=1,Text=v,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,
TextSize=13,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=1}),SubLabel=w
and l.createElement('TextLabel',{Size=UDim2.new(1,0,0,12),BackgroundTransparency
=1,Text=w,TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Regular,TextSize=10,
TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=2})}),ChartContainer=l.
createElement('Frame',{Size=UDim2.new(1,0,1,-H),BackgroundTransparency=1,
LayoutOrder=2},{YAxis=l.createElement('Frame',{Name='YAxis',Size=UDim2.new(0,24,
1,-16),BackgroundTransparency=1},{Max=l.createElement('TextLabel',{Size=UDim2.
new(1,0,0,10),Position=UDim2.fromScale(0,0.15),AnchorPoint=Vector2.new(0,0.5),
BackgroundTransparency=1,Text=tostring(D),TextColor3=n.Colors.TextSecondary,Font
=n.Fonts.Medium,TextSize=8,TextXAlignment=Enum.TextXAlignment.Left}),Mid=l.
createElement('TextLabel',{Size=UDim2.new(1,0,0,10),Position=UDim2.fromScale(0,
0.5),AnchorPoint=Vector2.new(0,0.5),BackgroundTransparency=1,Text=tostring(math.
round(D/2)),TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Medium,TextSize=8,
TextXAlignment=Enum.TextXAlignment.Left}),Min=l.createElement('TextLabel',{Size=
UDim2.new(1,0,0,10),Position=UDim2.fromScale(0,0.85),AnchorPoint=Vector2.new(0,
0.5),BackgroundTransparency=1,Text='0',TextColor3=n.Colors.TextSecondary,Font=n.
Fonts.Medium,TextSize=8,TextXAlignment=Enum.TextXAlignment.Left})}),
PlotContainer=l.createElement('Frame',{Name='PlotContainer',Size=UDim2.new(1,-28
,1,0),Position=UDim2.new(0,28,0,0),BackgroundTransparency=1},{GridLine_Top=l.
createElement('Frame',{Size=UDim2.new(1,0,0,1),Position=UDim2.fromScale(0,0.15),
BackgroundColor3=n.Colors.Border,BackgroundTransparency=0.6,BorderSizePixel=0}),
GridLine_Mid=l.createElement('Frame',{Size=UDim2.new(1,0,0,1),Position=UDim2.
fromScale(0,0.5),BackgroundColor3=n.Colors.Border,BackgroundTransparency=0.6,
BorderSizePixel=0}),GridLine_Bottom=l.createElement('Frame',{Size=UDim2.new(1,0,
0,1),Position=UDim2.fromScale(0,0.85),BackgroundColor3=n.Colors.Border,
BackgroundTransparency=0.6,BorderSizePixel=0}),YAxisLine=l.createElement('Frame'
,{Size=UDim2.new(0,1,1,-16),Position=UDim2.fromScale(0,0),BackgroundColor3=n.
Colors.Border,BorderSizePixel=0}),Canvas=l.createElement('Frame',{ref=t.
canvasRef,Size=UDim2.new(1,0,1,-16),BackgroundTransparency=1},F),LabelsRow=l.
createElement('Frame',{Size=UDim2.new(1,0,0,16),Position=UDim2.new(0,0,1,-16),
BackgroundTransparency=1},G)})})})end return s end d['Components/Notification']=
function()local i=createVirtualModule'Components/Notification'
createVirtualFolder''local j=__require local k=game:GetService
'ReplicatedStorage'local l=game:GetService'TweenService'local m=j(k.Packages.
React)local n=i.Parent.Parent local o=j(n.Theme)local p=m.Component:extend
'NotificationItem'function p.init(q)q.groupRef=m.createRef()q.scaleRef=m.
createRef()end function p.didMount(q)local r=q.groupRef.current if r then r.
Position=UDim2.new(1,50,0,0)r.GroupTransparency=1 local s=TweenInfo.new(0.4,Enum
.EasingStyle.Back,Enum.EasingDirection.Out)l:Create(r,s,{Position=UDim2.new(0,0,
0,0),GroupTransparency=0}):Play()end end function p.didUpdate(q,r)if q.props.
item.count~=r.item.count then local s=q.scaleRef.current if s then s.Scale=1.08
local t=TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out)l:
Create(s,t,{Scale=1}):Play()end end end function p.render(q)local r=q.props.item
local s=r.title or'Notification'local t=r.subtitle or''local u=r.icon or
'rbxassetid://10747373111'local v=r.count or 1 local w={}if v>=3 then w.
StackCard2=m.createElement('Frame',{Size=UDim2.new(0.92,0,1,0),Position=UDim2.
new(0.04,0,0,-8),BackgroundColor3=o.Colors.Card,BorderSizePixel=0,ZIndex=1},{
Corner=m.createElement('UICorner',{CornerRadius=o.CornerRadius.Card}),Stroke=m.
createElement('UIStroke',{Color=o.Colors.Border,Thickness=1,Transparency=0.6})})
end if v>=2 then w.StackCard1=m.createElement('Frame',{Size=UDim2.new(0.96,0,1,0
),Position=UDim2.new(0.02,0,0,-4),BackgroundColor3=o.Colors.Card,BorderSizePixel
=0,ZIndex=2},{Corner=m.createElement('UICorner',{CornerRadius=o.CornerRadius.
Card}),Stroke=m.createElement('UIStroke',{Color=o.Colors.Border,Thickness=1,
Transparency=0.3})})end w.MainCard=m.createElement('Frame',{Size=UDim2.new(1,0,1
,0),Position=UDim2.new(0,0,0,0),BackgroundColor3=o.Colors.Card,BorderSizePixel=0
,ZIndex=3},{Corner=m.createElement('UICorner',{CornerRadius=o.CornerRadius.Card}
),Stroke=m.createElement('UIStroke',{Color=o.Colors.Border,Thickness=1}),Padding
=m.createElement('UIPadding',{PaddingTop=UDim.new(0,10),PaddingBottom=UDim.new(0
,10),PaddingLeft=UDim.new(0,12),PaddingRight=UDim.new(0,12)}),List=m.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,10)}),AppIcon
=m.createElement('ImageLabel',{Size=UDim2.new(0,24,0,24),BackgroundTransparency=
1,Image=u,ImageColor3=o.Colors.Accent}),TextFrame=m.createElement('Frame',{Size=
UDim2.new(1,-34,1,0),BackgroundTransparency=1},{List=m.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,2),
SortOrder=Enum.SortOrder.LayoutOrder}),TitleLabel=m.createElement('TextLabel',{
Size=UDim2.new(1,0,0,16),BackgroundTransparency=1,Text=(v>1)and(s..
' <font color="'..string.format('#%02x%02x%02x',math.floor(o.Colors.Accent.R*255
),math.floor(o.Colors.Accent.G*255),math.floor(o.Colors.Accent.B*255))..'">x'..
tostring(v)..'</font>')or s,TextColor3=o.Colors.Text,Font=o.Fonts.Bold,TextSize=
12,TextXAlignment=Enum.TextXAlignment.Left,RichText=true,LayoutOrder=1}),
SubLabel=m.createElement('TextLabel',{Size=UDim2.new(1,0,0,14),
BackgroundTransparency=1,Text=t,TextColor3=o.Colors.TextSecondary,Font=o.Fonts.
Regular,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.
TextTruncate.AtEnd,LayoutOrder=2})})})w.Scale=m.createElement('UIScale',{ref=q.
scaleRef,Scale=1})return m.createElement('CanvasGroup',{ref=q.groupRef,Size=
UDim2.new(1,0,0,60),BackgroundTransparency=1},w)end local q=m.Component:extend
'NotificationList'function q.render(r)local s=r.props.notifications or{}local t=
{Layout=m.createElement('UIListLayout',{FillDirection=Enum.FillDirection.
Vertical,Padding=UDim.new(0,10),HorizontalAlignment=Enum.HorizontalAlignment.
Right,SortOrder=Enum.SortOrder.LayoutOrder})}for u,v in ipairs(s)do t['Item_'..
tostring(v.id)]=m.createElement(p,{item=v,LayoutOrder=u})end return m.
createElement('Frame',{Size=UDim2.new(0,280,0,0),Position=UDim2.new(1,-20,1,-20)
,AnchorPoint=Vector2.new(1,1),BackgroundTransparency=1,AutomaticSize=Enum.
AutomaticSize.Y,ZIndex=9999},t)end return{List=q,Item=p}end d[
'Components/Paragraph']=function()local i=createVirtualModule
'Components/Paragraph'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=l.Component:extend'Paragraph'function o.render(p)local
q=p.props.element return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y},{List=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder}),Title=q.title and l.
createElement('TextLabel',{Size=UDim2.new(1,0,0,16),BackgroundTransparency=1,
Text=q.title,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=12,
TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=1}),Body=l.createElement(
'TextLabel',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,Text=q.content or
'',TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Regular,TextSize=11,
TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,AutomaticSize=Enum.
AutomaticSize.Y,LayoutOrder=2})})end return o end d['Components/PieChart']=
function()local i=createVirtualModule'Components/PieChart'createVirtualFolder''
local j=__require local k=game:GetService'ReplicatedStorage'local l=game:
GetService'TweenService'local m=j(k.Packages.React)local n=i.Parent.Parent local
o=j(n.Theme)local p=TweenInfo.new(0.4,Enum.EasingStyle.Quad,Enum.EasingDirection
.Out)local q=m.Component:extend'AnimatedWedge'function q.init(r)r.containerRef=m
.createRef()r.gradientRef=m.createRef()end function q.didMount(r)local s=r.
containerRef.current local t=r.gradientRef.current if s and t then s.Rotation=r.
props.startAngle t.Rotation=math.clamp(r.props.sweepAngle,0.1,180)end end
function q.didUpdate(r,s)local t=r.containerRef.current local u=r.gradientRef.
current if t and u then local v=r.props local w=math.clamp(v.sweepAngle,0.1,180)
if s.startAngle~=v.startAngle then t.Rotation=s.startAngle l:Create(t,p,{
Rotation=v.startAngle}):Play()end if s.sweepAngle~=v.sweepAngle then local x=
math.clamp(s.sweepAngle or 0,0.1,180)u.Rotation=x l:Create(u,p,{Rotation=w}):
Play()end end end function q.render(r)local s=r.props return m.createElement(
'Frame',{ref=r.containerRef,Size=UDim2.fromScale(1,1),Position=UDim2.fromScale(
0.5,0.5),AnchorPoint=Vector2.new(0.5,0.5),BackgroundTransparency=1,ZIndex=s.
zIndex or 1},{ClipBox=m.createElement('Frame',{Size=UDim2.new(0.5,0,1,0),
Position=UDim2.new(0.5,0,0,0),AnchorPoint=Vector2.new(0,0),ClipsDescendants=true
,BackgroundTransparency=1},{CircleSegment=m.createElement('Frame',{Size=UDim2.
new(2,0,1,0),Position=UDim2.new(-1,0,0,0),AnchorPoint=Vector2.new(0,0),
BackgroundColor3=s.color,BorderSizePixel=0},{Corner=m.createElement('UICorner',{
CornerRadius=UDim.new(0.5,0)}),Gradient=m.createElement('UIGradient',{ref=r.
gradientRef,Rotation=math.clamp(s.sweepAngle,0.1,180),Transparency=
NumberSequence.new{NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(
0.5,0),NumberSequenceKeypoint.new(0.5001,1),NumberSequenceKeypoint.new(1,1)}})})
})})end local r=m.Component:extend'PieChart'function r.render(s)local t=s.props.
element local u=t.title local v=t.subtitle local w=t.size or 140 local x=t.donut
==true local y=t.data or{}local z=t.align or'Right'local A=0 for B,C in ipairs(y
)do local D=type(C)=='table'and(C.value or C.Value or 0)or(tonumber(C)or 0)A=A+D
end local B={}if A<=0 then B.Placeholder=m.createElement('Frame',{Size=UDim2.
new(1,0,1,0),Position=UDim2.fromScale(0.5,0.5),AnchorPoint=Vector2.new(0.5,0.5),
BackgroundColor3=Color3.fromRGB(80,80,80),BorderSizePixel=0},{Corner=m.
createElement('UICorner',{CornerRadius=UDim.new(0.5,0)})})else local C=0 for D,E
in ipairs(y)do local F=type(E)=='table'and(E.value or E.Value or 0)or(tonumber(E
)or 0)if F>0 then local G=(F/A)*360 local H=type(E)=='table'and(E.color or E.
Color or o.Colors.Accent)or o.Colors.Accent if G<=180 then B['Slice_'..tostring(
D)..'_1']=m.createElement(q,{startAngle=C,sweepAngle=G,color=H,zIndex=D})else B[
'Slice_'..tostring(D)..'_1']=m.createElement(q,{startAngle=C,sweepAngle=180,
color=H,zIndex=D})B['Slice_'..tostring(D)..'_2']=m.createElement(q,{startAngle=C
+180,sweepAngle=G-180,color=H,zIndex=D})end C=C+G end end end if x then B.
DonutHollow=m.createElement('Frame',{Name='DonutCenter',Size=UDim2.new(0.6,0,0.6
,0),Position=UDim2.fromScale(0.5,0.5),AnchorPoint=Vector2.new(0.5,0.5),
BackgroundColor3=o.Colors.Element,BorderSizePixel=0,ZIndex=100},{Corner=m.
createElement('UICorner',{CornerRadius=UDim.new(0.5,0)})})end local C={Layout=m.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,6),VerticalAlignment=Enum.VerticalAlignment.Center,SortOrder=Enum.
SortOrder.LayoutOrder})}for D,E in ipairs(y)do local F=type(E)=='table'and(E.
value or E.Value or 0)or(tonumber(E)or 0)local G=type(E)=='table'and(E.label or
E.Label or'Category')or('Category '..tostring(D))local H=type(E)=='table'and(E.
color or E.Color or o.Colors.Accent)or o.Colors.Accent local I=A>0 and math.
round((F/A)*100)or 0 C['Legend_'..tostring(D)]=m.createElement('Frame',{Size=
UDim2.new(1,0,0,18),BackgroundTransparency=1,LayoutOrder=D},{List=m.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,8),SortOrder=
Enum.SortOrder.LayoutOrder}),Indicator=m.createElement('Frame',{Size=UDim2.
fromOffset(8,8),BackgroundColor3=H,BorderSizePixel=0,LayoutOrder=1},{Corner=m.
createElement('UICorner',{CornerRadius=UDim.new(0,2)})}),Label=m.createElement(
'TextLabel',{Size=UDim2.new(0.5,0,1,0),BackgroundTransparency=1,Text=G,
TextColor3=o.Colors.Text,Font=o.Fonts.Medium,TextSize=11,TextXAlignment=Enum.
TextXAlignment.Left,LayoutOrder=2}),Percent=m.createElement('TextLabel',{Size=
UDim2.new(0.5,-16,1,0),BackgroundTransparency=1,Text=tostring(I)..'%',TextColor3
=o.Colors.TextSecondary,Font=o.Fonts.Bold,TextSize=10,TextXAlignment=Enum.
TextXAlignment.Right,LayoutOrder=3})})end local D=z=='Right'and 1 or 3 local E=z
=='Right'and 3 or 1 local F={Layout=m.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Horizontal,VerticalAlignment=Enum.
VerticalAlignment.Center,Padding=UDim.new(0,12),SortOrder=Enum.SortOrder.
LayoutOrder}),ChartArea=m.createElement('Frame',{Name='ChartArea',Size=UDim2.
fromOffset(w,w),BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=E},{
OuterCircleClip=m.createElement('CanvasGroup',{Size=UDim2.fromScale(1,1),
Position=UDim2.fromScale(0.5,0.5),AnchorPoint=Vector2.new(0.5,0.5),
BackgroundTransparency=1,GroupTransparency=0,BorderSizePixel=0},{Corner=m.
createElement('UICorner',{CornerRadius=UDim.new(0.5,0)}),WedgeContainer=m.
createElement('Frame',{Size=UDim2.fromScale(1,1),BackgroundTransparency=1},B)})}
),LegendStack=m.createElement('Frame',{Size=UDim2.new(1,-w-12,1,0),
BackgroundTransparency=1,LayoutOrder=2},C)}local G if u or v then G=m.
createElement('Frame',{Size=UDim2.new(0.35,-12,1,0),BackgroundTransparency=1,
LayoutOrder=D},{List=m.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,VerticalAlignment=Enum.VerticalAlignment.Center,Padding=
UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder}),TitleLabel=u and m.
createElement('TextLabel',{Size=UDim2.new(1,0,0,18),BackgroundTransparency=1,
Text=u,TextColor3=o.Colors.Text,Font=o.Fonts.Bold,TextSize=13,TextXAlignment=
Enum.TextXAlignment.Left,LayoutOrder=1}),SubLabel=v and m.createElement(
'TextLabel',{Size=UDim2.new(1,0,0,36),BackgroundTransparency=1,Text=v,TextColor3
=o.Colors.TextSecondary,Font=o.Fonts.Regular,TextSize=10,TextXAlignment=Enum.
TextXAlignment.Left,TextWrapped=true,LayoutOrder=2})})end local H={Layout=m.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,12),SortOrder
=Enum.SortOrder.LayoutOrder})}if G then H.HeaderArea=G H.Divider=m.
createElement('Frame',{Size=UDim2.new(0,1,1,-10),BackgroundColor3=o.Colors.
Border,BorderSizePixel=0,LayoutOrder=2})end H.ChartAndLegend=m.createElement(
'Frame',{Size=UDim2.new(G and 0.65 or 1,-12,1,0),BackgroundTransparency=1,
LayoutOrder=3},F)return m.createElement('Frame',{Size=UDim2.new(1,0,0,w+20),
BackgroundColor3=o.Colors.Element,BorderSizePixel=0},{Corner=m.createElement(
'UICorner',{CornerRadius=o.CornerRadius.Card}),Stroke=m.createElement('UIStroke'
,{Color=o.Colors.Border,Thickness=1}),Padding=m.createElement('UIPadding',{
PaddingTop=UDim.new(0,10),PaddingBottom=UDim.new(0,10),PaddingLeft=UDim.new(0,12
),PaddingRight=UDim.new(0,12)}),Body=m.createElement('Frame',{Size=UDim2.new(1,0
,1,0),BackgroundTransparency=1},H)})end return r end d['Components/Profile']=
function()local i=createVirtualModule'Components/Profile'createVirtualFolder''
local j=__require local k=game:GetService'ReplicatedStorage'local l=j(k.Packages
.React)local m=i.Parent.Parent local n=j(m.Theme)local o=l.Component:extend
'Profile'function o.render(p)local q=p.props.username or'Guest'local r=p.props.
subtext or'Welcome to Selene'local s=p.props.userId or 1 return l.createElement(
'Frame',{Size=UDim2.new(1,0,1,0),BackgroundColor3=n.Colors.Card,BorderSizePixel=
0},{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.Card}),Stroke
=l.createElement('UIStroke',{Color=n.Colors.Border,Thickness=1}),Padding=l.
createElement('UIPadding',{PaddingTop=UDim.new(0,12),PaddingBottom=UDim.new(0,12
),PaddingLeft=UDim.new(0,15),PaddingRight=UDim.new(0,15)}),Content=l.
createElement('Frame',{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1},{List=l
.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,15)}),Avatar=
l.createElement('ImageLabel',{Size=UDim2.new(0,48,0,48),BackgroundColor3=n.
Colors.Element,BorderSizePixel=0,Image=
'https://www.roblox.com/headshot-thumbnail/image?userId='..tostring(s)..
'&width=150&height=150&format=png'},{Corner=l.createElement('UICorner',{
CornerRadius=n.CornerRadius.Circle}),Stroke=l.createElement('UIStroke',{Color=n.
Colors.Border,Thickness=1})}),Info=l.createElement('Frame',{Size=UDim2.new(1,-63
,0,40),BackgroundTransparency=1},{List=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,VerticalAlignment=Enum.
VerticalAlignment.Center,Padding=UDim.new(0,2)}),Greeting=l.createElement(
'TextLabel',{Size=UDim2.new(1,0,0,20),BackgroundTransparency=1,Text='Hello, '..q
,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=16,TextXAlignment=Enum.
TextXAlignment.Left}),Sub=l.createElement('TextLabel',{Size=UDim2.new(1,0,0,16),
BackgroundTransparency=1,Text=r,TextColor3=n.Colors.TextSecondary,Font=n.Fonts.
Regular,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left})})})})end return o
end d['Components/PullDownButton']=function()local i=createVirtualModule
'Components/PullDownButton'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=game:GetService'TweenService'local m=j(k.
Packages.React)local n=i.Parent.Parent local o=j(n.Theme)local p=m.Component:
extend'PullDownOption'function p.init(q)q.state={hovered=false}end function p.
render(q)local r=q.props local s=r.opt local t=r.LayoutOrder local u=q.state.
hovered return m.createElement('TextButton',{Size=UDim2.new(1,0,0,28),
BackgroundColor3=u and o.Colors.ElementHover or o.Colors.Element,BorderSizePixel
=0,Text='',AutoButtonColor=false,LayoutOrder=t,[m.Event.MouseEnter]=function()q:
setState{hovered=true}end,[m.Event.MouseLeave]=function()q:setState{hovered=
false}end,[m.Event.MouseButton1Click]=r.OnClick},{Corner=m.createElement(
'UICorner',{CornerRadius=o.CornerRadius.Element}),Label=m.createElement(
'TextLabel',{Size=UDim2.new(1,-16,1,0),Position=UDim2.new(0,8,0,0),
BackgroundTransparency=1,Text=tostring(s),TextColor3=o.Colors.Text,Font=o.Fonts.
Medium,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left})})end local q=m.
Component:extend'PullDownButton'function q.init(r)r.state={hovered=false,isOpen=
false}r.strokeRef=m.createRef()r.arrowRef=m.createRef()r.optionsRef=m.createRef(
)end function q.didMount(r)local s=r.strokeRef.current local t=r.arrowRef.
current local u=r.optionsRef.current local v=r.state.hovered local w=r.state.
isOpen if s then s.Color=(v or w)and o.Colors.BorderHover or o.Colors.Border end
if t then t.Rotation=w and 180 or 0 end if u then local x=r.props.element.
options or{}local y=#x local z=y>0 and(y*28+(y-1)*4+8)or 0 u.Size=w and UDim2.
new(1,0,0,z)or UDim2.new(1,0,0,0)u.GroupTransparency=w and 0 or 1 u.Visible=w
end end function q.didUpdate(r,s,t)local u=r.strokeRef.current local v=r.
arrowRef.current local w=r.optionsRef.current local x=r.state.hovered local y=r.
state.isOpen local z=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)if x~=t.hovered or y~=t.isOpen then if u then local A=(x or
y)and o.Colors.BorderHover or o.Colors.Border l:Create(u,z,{Color=A}):Play()end
end if y~=t.isOpen then if v then l:Create(v,z,{Rotation=y and 180 or 0}):Play()
end if w then local A=r.props.element.options or{}local B=#A local C=B>0 and(B*
28+(B-1)*4+8)or 0 if y then w.Visible=true w.GroupTransparency=1 w.Size=UDim2.
new(1,0,0,0)l:Create(w,z,{Size=UDim2.new(1,0,0,C),GroupTransparency=0}):Play()
else local D=l:Create(w,z,{Size=UDim2.new(1,0,0,0),GroupTransparency=1})D:Play()
D.Completed:Connect(function()if not r.state.isOpen then w.Visible=false end end
)end end end end function q.render(r)local s=r.props.element local t=s.name or s
.label or'Menu'local u=s.options or{}local v=r.state.hovered local w=r.state.
isOpen local x={Layout=m.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,4),SortOrder=Enum.SortOrder.
LayoutOrder}),Padding=m.createElement('UIPadding',{PaddingTop=UDim.new(0,4),
PaddingBottom=UDim.new(0,4)})}for y,z in ipairs(u)do x['Opt_'..tostring(y)]=m.
createElement(p,{opt=z,LayoutOrder=y,OnClick=function()r:setState{isOpen=false}
if s.callback then task.spawn(s.callback,y)end end})end return m.createElement(
'Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.
AutomaticSize.Y},{List=m.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder}),Header=m.
createElement('TextButton',{Size=UDim2.new(1,0,0,36),BackgroundColor3=o.Colors.
Element,BorderSizePixel=0,Text='',AutoButtonColor=false,LayoutOrder=1,[m.Event.
MouseEnter]=function()r:setState{hovered=true}end,[m.Event.MouseLeave]=function(
)r:setState{hovered=false}end,[m.Event.MouseButton1Click]=function()r:setState{
isOpen=not w}end},{Corner=m.createElement('UICorner',{CornerRadius=o.
CornerRadius.Element}),Stroke=m.createElement('UIStroke',{ref=r.strokeRef,Color=
(v or w)and o.Colors.BorderHover or o.Colors.Border,Thickness=1}),Title=m.
createElement('TextLabel',{Size=UDim2.new(1,-30,1,0),Position=UDim2.new(0,10,0,0
),BackgroundTransparency=1,Text=t,TextColor3=o.Colors.Text,Font=o.Fonts.Bold,
TextSize=13,TextXAlignment=Enum.TextXAlignment.Left}),Arrow=m.createElement(
'ImageLabel',{ref=r.arrowRef,Size=UDim2.new(0,14,0,14),Position=UDim2.new(1,-22,
0.5,-7),BackgroundTransparency=1,Image='rbxassetid://10747383844',ImageColor3=o.
Colors.TextSecondary})}),Options=m.createElement('CanvasGroup',{ref=r.optionsRef
,BackgroundTransparency=1,LayoutOrder=2,ClipsDescendants=true},x)})end return q
end d['Components/RadioButtonGroup']=function()local i=createVirtualModule
'Components/RadioButtonGroup'createVirtualFolder''local j=__require local k=game
:GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=l.Component:extend'RadioOption'function o.init(p)p.
state={hovered=false}end function o.render(p)local q=p.props local r=q.opt local
s=q.isSelected local t=q.LayoutOrder local u=p.state.hovered return l.
createElement('TextButton',{Size=UDim2.new(1,0,0,24),BackgroundTransparency=1,
BorderSizePixel=0,Text='',AutoButtonColor=false,LayoutOrder=t,[l.Event.
MouseEnter]=function()p:setState{hovered=true}end,[l.Event.MouseLeave]=function(
)p:setState{hovered=false}end,[l.Event.MouseButton1Click]=q.OnClick},{List=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,8),SortOrder=
Enum.SortOrder.LayoutOrder}),RadioCircle=l.createElement('Frame',{Size=UDim2.
new(0,16,0,16),BackgroundColor3=s and n.Colors.Accent or n.Colors.Element,
BorderSizePixel=0,LayoutOrder=1},{Corner=l.createElement('UICorner',{
CornerRadius=n.CornerRadius.Circle}),Stroke=l.createElement('UIStroke',{Color=(u
or s)and n.Colors.BorderHover or n.Colors.Border,Thickness=1}),Fill=s and l.
createElement('Frame',{Size=UDim2.new(0,6,0,6),Position=UDim2.new(0.5,-3,0.5,-3)
,BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0},{Corner=l.
createElement('UICorner',{CornerRadius=n.CornerRadius.Circle})})or nil}),
OptionLabel=l.createElement('TextLabel',{Size=UDim2.new(1,-24,1,0),
BackgroundTransparency=1,Text=r,TextColor3=s and n.Colors.Text or n.Colors.
TextSecondary,Font=s and n.Fonts.Bold or n.Fonts.Medium,TextSize=12,
TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=2})})end local p=l.Component
:extend'RadioButtonGroup'function p.render(q)local r=q.props.element local s=q.
props.Store local t=r.options or{}local u=r.value or 1 local v={Layout=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder})}for w,x in ipairs(t)do
local y=w==u v['Opt_'..tostring(w)]=l.createElement(o,{opt=x,isSelected=y,
LayoutOrder=w,OnClick=function()s:UpdateElement(r.id,function(z)z.value=w end)if
r.callback then task.spawn(r.callback,w)end end})end return l.createElement(
'Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.
AutomaticSize.Y},{List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.
LayoutOrder}),Title=r.name and l.createElement('TextLabel',{Size=UDim2.new(1,0,0
,16),BackgroundTransparency=1,Text=r.name,TextColor3=n.Colors.Text,Font=n.Fonts.
Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=1}),
OptionsFrame=l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=2},v)})
end return p end d['Components/Row']=function()local i=createVirtualModule
'Components/Row'createVirtualFolder''local j=__require local k=game:GetService
'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent j(m.Theme)
local n=j(m.Components.ElementRenderer)local o=l.Component:extend'Row'function o
.render(p)local q=p.props.element local r=p.props.Store local s={}for t,u in
ipairs(q.leftElements or{})do s['Left_'..u.id]=n.render(u,r)end local t={}local
u=false for v,w in ipairs(q.rightElements or{})do t['Right_'..w.id]=n.render(w,r
)local x=w.type if x=='Slider'or x=='Stepper'or x=='BarChart'or x=='LineChart'or
x=='PieChart'or x=='VStack'or x=='HStack'or x=='ColorPickerWidget'then u=true
end end if#q.leftElements==0 then u=true end if u then return l.createElement(
'Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.
AutomaticSize.Y},{List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.
LayoutOrder}),LeftArea=#q.leftElements>0 and l.createElement('Frame',{Size=UDim2
.new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,
LayoutOrder=1},s)or nil,RightArea=l.createElement('Frame',{Size=UDim2.new(1,0,0,
0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=2},t)
})else local v={List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,VerticalAlignment=Enum.VerticalAlignment.Center})}for w,x
in pairs(s)do v[w]=x end local w={List=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,HorizontalAlignment=Enum.
HorizontalAlignment.Right,VerticalAlignment=Enum.VerticalAlignment.Center,
Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder})}for x,y in pairs(t)
do w[x]=y end return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1},{LeftArea=l.
createElement('Frame',{Size=UDim2.new(0.5,-6,0,0),AutomaticSize=Enum.
AutomaticSize.Y,BackgroundTransparency=1},v),RightArea=l.createElement('Frame',{
Size=UDim2.new(0.5,-6,0,0),Position=UDim2.new(0.5,6,0,0),AutomaticSize=Enum.
AutomaticSize.Y,BackgroundTransparency=1},w)})end end return o end d[
'Components/Section']=function()local i=createVirtualModule'Components/Section'
createVirtualFolder''local j=__require local k=game:GetService
'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent local n=j(
m.Theme)local o=j(m.Components.ElementRenderer)local p=l.Component:extend
'Section'local q={Discord=ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.
fromRGB(27,35,96)),ColorSequenceKeypoint.new(1,Color3.fromRGB(15,17,21))},Wave=
ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromRGB(90,20,20)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(15,17,21))},Friends=ColorSequence.new
{ColorSequenceKeypoint.new(0,Color3.fromRGB(120,90,15)),ColorSequenceKeypoint.
new(1,Color3.fromRGB(15,17,21))},Server=ColorSequence.new{ColorSequenceKeypoint.
new(0,Color3.fromRGB(15,75,45)),ColorSequenceKeypoint.new(1,Color3.fromRGB(15,17
,21))}}function p.render(r)local s=r.props.section local t=r.props.Store local u
if s.gradientPreset and q[s.gradientPreset]then u=q[s.gradientPreset]elseif s.
gradient then u=s.gradient end local v={Layout=l.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder,
Padding=UDim.new(0,8)})}for w,x in ipairs(s.elements)do local y=o.render(x,t)if
y then v['El_'..x.id]=l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=w},{
Content=y})end end return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundColor3=n.Colors.Card,BorderSizePixel=0,AutomaticSize=Enum.
AutomaticSize.Y,LayoutOrder=r.props.LayoutOrder},{Corner=l.createElement(
'UICorner',{CornerRadius=n.CornerRadius.Card}),Stroke=l.createElement('UIStroke'
,{Color=n.Colors.Border,Thickness=1}),Gradient=u and l.createElement(
'UIGradient',{Color=u,Rotation=135}),Padding=l.createElement('UIPadding',{
PaddingTop=UDim.new(0,14),PaddingBottom=UDim.new(0,14),PaddingLeft=UDim.new(0,14
),PaddingRight=UDim.new(0,14)}),Content=l.createElement('Frame',{Size=UDim2.new(
1,0,1,0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y},{List=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,10)}),Header=(s.name or
s.description)and l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=1},{List
=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
Padding=UDim.new(0,2),SortOrder=Enum.SortOrder.LayoutOrder}),Title=s.name and l.
createElement('TextLabel',{Size=UDim2.new(1,0,0,18),BackgroundTransparency=1,
Text=s.name,TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=15,
TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=1}),Desc=s.description and l
.createElement('TextLabel',{Size=UDim2.new(1,0,0,14),BackgroundTransparency=1,
Text=s.description,TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Regular,
TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=2})}),Divider=(#
s.elements>0 and(s.name or s.description))and l.createElement('Frame',{Size=
UDim2.new(1,0,0,1),BackgroundColor3=n.Colors.Border,BorderSizePixel=0,
LayoutOrder=2}),ElementsArea=l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=3},v)})}
)end return p end d['Components/Sidebar']=function()local i=createVirtualModule
'Components/Sidebar'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=game:GetService'TweenService'local p=l.Component:
extend'TabButton'function p.init(q)q.state={hovered=false}q.btnRef=l.createRef()
q.activeBarRef=l.createRef()q.labelRef=l.createRef()q.iconRef=l.createRef()end
function p.didMount(q)local r=q.btnRef.current local s=q.activeBarRef.current
local t=q.labelRef.current local u=q.iconRef.current local v=q.props local w=v.
isActive local x=q.state.hovered if r then r.BackgroundColor3=w and n.Colors.
Element or(x and n.Colors.ElementHover or Color3.fromRGB(0,0,0))r.
BackgroundTransparency=(w or x)and 0 or 1 end if s then s.Size=w and UDim2.new(0
,3,0,14)or UDim2.new(0,3,0,0)s.BackgroundTransparency=w and 0 or 1 end local y=(
w or x)and n.Colors.Text or n.Colors.TextSecondary if t then t.TextColor3=y end
if u then u.ImageColor3=y end end function p.didUpdate(q,r,s)local t=q.btnRef.
current local u=q.activeBarRef.current local v=q.labelRef.current local w=q.
iconRef.current local x=q.props local y=x.isActive local z=q.state.hovered local
A=(y~=r.isActive)or(z~=s.hovered)if A then local B=y and n.Colors.Element or(z
and n.Colors.ElementHover or Color3.fromRGB(0,0,0))local C=(y or z)and 0 or 1
local D=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)if t
then o:Create(t,D,{BackgroundColor3=B,BackgroundTransparency=C}):Play()end local
E=(y or z)and n.Colors.Text or n.Colors.TextSecondary if v then o:Create(v,D,{
TextColor3=E}):Play()end if w then o:Create(w,D,{ImageColor3=E}):Play()end end
if y~=r.isActive then if u then local B=y and UDim2.new(0,3,0,14)or UDim2.new(0,
3,0,0)local C=y and 0 or 1 local D=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)o:Create(u,D,{Size=B,BackgroundTransparency=C}):Play()end
end end function p.render(q)local r=q.props local s=r.tab local t=r.isActive
local u=r.Store local v=r.LayoutOrder local w=q.state.hovered local x=s.
parentTabId~=nil local y=x and 16 or 0 local z=x and UDim2.new(1,-16,0,32)or
UDim2.new(1,0,0,32)return l.createElement('TextButton',{ref=q.btnRef,Size=z,
Position=UDim2.new(0,y,0,0),BackgroundColor3=t and n.Colors.Element or Color3.
fromRGB(0,0,0),BackgroundTransparency=t and 0 or 1,BorderSizePixel=0,Text='',
AutoButtonColor=false,LayoutOrder=v,[l.Event.MouseEnter]=function()q:setState{
hovered=true}end,[l.Event.MouseLeave]=function()q:setState{hovered=false}end,[l.
Event.MouseButton1Click]=function()u:Update(function(A)A.activeTab=s.id end)end}
,{Corner=l.createElement('UICorner',{CornerRadius=n.CornerRadius.Element}),
ActiveBar=l.createElement('Frame',{ref=q.activeBarRef,Size=t and UDim2.new(0,3,0
,14)or UDim2.new(0,3,0,0),Position=UDim2.new(0,4,0.5,-7),BackgroundColor3=n.
Colors.Accent,BackgroundTransparency=t and 0 or 1,BorderSizePixel=0},{Corner=l.
createElement('UICorner',{CornerRadius=n.CornerRadius.Circle})}),Content=l.
createElement('Frame',{Size=UDim2.new(1,t and-15 or-10,1,0),Position=UDim2.new(0
,t and 12 or 8,0,0),BackgroundTransparency=1},{List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,VerticalAlignment=
Enum.VerticalAlignment.Center,Padding=UDim.new(0,8)}),Icon=s.icon and l.
createElement('ImageLabel',{ref=q.iconRef,Size=UDim2.new(0,14,0,14),
BackgroundTransparency=1,Image=s.icon,ImageColor3=(t or w)and n.Colors.Text or n
.Colors.TextSecondary})or nil,Label=l.createElement('TextLabel',{ref=q.labelRef,
Size=UDim2.new(1,s.icon and-22 or 0,1,0),BackgroundTransparency=1,Text=s.title,
TextColor3=(t or w)and n.Colors.Text or n.Colors.TextSecondary,Font=t and n.
Fonts.Bold or n.Fonts.Medium,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left
})})})end local q=l.Component:extend'Sidebar'function q.render(r)local s=r.props
.state local t=r.props.Store local u=r.props.LayoutOrder local v={Layout=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,8)}),Padding=l.
createElement('UIPadding',{PaddingTop=UDim.new(0,12),PaddingBottom=UDim.new(0,12
),PaddingLeft=UDim.new(0,12),PaddingRight=UDim.new(0,12)})}local w=1 for x,y in
ipairs(s.sections or{})do if y.title and y.title~=''then v['SecHeader_'..y.id]=l
.createElement('TextLabel',{Size=UDim2.new(1,0,0,18),BackgroundTransparency=1,
Text=string.upper(y.title),TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Bold,
TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=w})w=w+1 end for
z,A in ipairs(y.tabs or{})do local B=s.activeTab==A.id v['Tab_'..A.id]=l.
createElement(p,{tab=A,isActive=B,Store=t,LayoutOrder=w})w=w+1 end end return l.
createElement('Frame',{Size=UDim2.new(0,180,1,0),BackgroundColor3=n.Colors.
Sidebar,BorderSizePixel=0,LayoutOrder=u},{Border=l.createElement('Frame',{Size=
UDim2.new(0,1,1,0),Position=UDim2.new(1,-1,0,0),BackgroundColor3=n.Colors.Border
,BorderSizePixel=0}),NavList=l.createElement('ScrollingFrame',{Size=UDim2.new(1,
0,1,-65),BackgroundTransparency=1,BorderSizePixel=0,CanvasSize=UDim2.new(0,0,0,0
),AutomaticCanvasSize=Enum.AutomaticSize.Y,ScrollBarThickness=1,
ScrollBarImageColor3=n.Colors.Border},v),ProfileArea=s.profile and l.
createElement('Frame',{Size=UDim2.new(1,0,0,65),Position=UDim2.new(0,0,1,-65),
BackgroundTransparency=1},{Divider=l.createElement('Frame',{Size=UDim2.new(1,-20
,0,1),Position=UDim2.new(0,10,0,0),BackgroundColor3=n.Colors.Border,
BorderSizePixel=0}),Container=l.createElement('Frame',{Size=UDim2.new(1,-20,1,-1
),Position=UDim2.new(0,10,0,1),BackgroundTransparency=1},{List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,VerticalAlignment=
Enum.VerticalAlignment.Center,Padding=UDim.new(0,10)}),Avatar=l.createElement(
'ImageLabel',{Size=UDim2.new(0,36,0,36),BackgroundColor3=n.Colors.Element,
BorderSizePixel=0,Image=
'https://www.roblox.com/headshot-thumbnail/image?userId='..tostring(s.profile.
userId or 1)..'&width=150&height=150&format=png'},{Corner=l.createElement(
'UICorner',{CornerRadius=n.CornerRadius.Circle}),Stroke=l.createElement(
'UIStroke',{Color=n.Colors.Border,Thickness=1})}),Info=l.createElement('Frame',{
Size=UDim2.new(1,-46,0,36),BackgroundTransparency=1},{List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Vertical,VerticalAlignment=Enum
.VerticalAlignment.Center}),Username=l.createElement('TextLabel',{Size=UDim2.
new(1,0,0,16),BackgroundTransparency=1,Text=s.profile.username,TextColor3=n.
Colors.Text,Font=n.Fonts.Bold,TextSize=12,TextXAlignment=Enum.TextXAlignment.
Left,TextTruncate=Enum.TextTruncate.AtEnd}),Role=l.createElement('TextLabel',{
Size=UDim2.new(1,0,0,14),BackgroundTransparency=1,Text=s.profile.subtext or
'User',TextColor3=n.Colors.TextSecondary,Font=n.Fonts.Regular,TextSize=10,
TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd})})
})})})end return q end d['Components/Slider']=function()local i=
createVirtualModule'Components/Slider'createVirtualFolder''local j=__require
local k=game:GetService'ReplicatedStorage'local l=game:GetService
'UserInputService'local m=game:GetService'TweenService'local n=j(k.Packages.
React)local o=i.Parent.Parent local p=j(o.Theme)local q=n.Component:extend
'Slider'function q.init(r,s)r.trackRef=n.createRef()r.fillRef=n.createRef()r.
knobRef=n.createRef()r.strokeRef=n.createRef()r.dragConnection=nil r.
releaseConnection=nil r.state={hovered=false,value=s.element.value or(s.element.
min or 0)}end function q.didMount(r)local s=r.fillRef.current local t=r.knobRef.
current local u=r.strokeRef.current local v=r.props.element local w=v.min or 0
local x=v.max or 100 local y=r.state.value local z=math.clamp((y-w)/(x-w),0,1)
local A=r.state.hovered if s then s.Size=UDim2.new(z,0,1,0)end if t then t.
Position=UDim2.new(z,-6,0.5,-6)end if u then u.Color=A and p.Colors.BorderHover
or p.Colors.Border end end function q.didUpdate(r,s,t)local u=r.fillRef.current
local v=r.knobRef.current local w=r.strokeRef.current local x=r.props.element
local y=x.min or 0 local z=x.max or 100 local A=r.state.value if x.value~=A and
x.value~=s.element.value then r:setState{value=x.value}return end local B=math.
clamp((A-y)/(z-y),0,1)local C=math.clamp((t.value-y)/(z-y),0,1)local D=r.state.
hovered if B~=C then local E=TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)if u then m:Create(u,E,{Size=UDim2.new(B,0,1,0)}):Play()end
if v then m:Create(v,E,{Position=UDim2.new(B,-6,0.5,-6)}):Play()end end if D~=t.
hovered then local E=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)if w then m:Create(w,E,{Color=D and p.Colors.BorderHover or
p.Colors.Border}):Play()end end end function q.willUnmount(r)if r.dragConnection
then r.dragConnection:Disconnect()end if r.releaseConnection then r.
releaseConnection:Disconnect()end end function q.render(r)local s=r.props.
element local t=r.props.Store local u=s.min or 0 local v=s.max or 100 local w=r.
state.value math.clamp((w-u)/(v-u),0,1)local x=r.state.hovered local function 
updateValue(y)local z=r.trackRef.current if not z then return end local A=z.
AbsoluteSize.X local B=z.AbsolutePosition.X local C=y.Position.X local D=math.
clamp((C-B)/A,0,1)local E=math.round(u+(D*(v-u)))r:setState{value=E}t:
UpdateElement(s.id,function(F)F.value=E end)if s.callback then task.spawn(s.
callback,E)end end return n.createElement('Frame',{Size=UDim2.new(1,0,0,48),
BackgroundTransparency=1},{List=n.createElement('UIListLayout',{FillDirection=
Enum.FillDirection.Vertical,Padding=UDim.new(0,6)}),Header=n.createElement(
'Frame',{Size=UDim2.new(1,0,0,16),BackgroundTransparency=1},{Title=n.
createElement('TextLabel',{Size=UDim2.new(1,-60,1,0),BackgroundTransparency=1,
Text=s.name or'Slider',TextColor3=p.Colors.Text,Font=p.Fonts.Medium,TextSize=13,
TextXAlignment=Enum.TextXAlignment.Left}),Value=n.createElement('TextLabel',{
Size=UDim2.new(0,50,1,0),Position=UDim2.new(1,-50,0,0),BackgroundTransparency=1,
Text=tostring(w),TextColor3=p.Colors.TextSecondary,Font=p.Fonts.Bold,TextSize=12
,TextXAlignment=Enum.TextXAlignment.Right})}),BarRow=n.createElement('Frame',{
Size=UDim2.new(1,0,0,24),BackgroundTransparency=1},{LeftIcon=s.leftIcon and n.
createElement('ImageLabel',{Size=UDim2.new(0,16,0,16),Position=UDim2.new(0,0,0.5
,-8),BackgroundTransparency=1,Image=s.leftIcon,ImageColor3=p.Colors.
TextSecondary})or nil,RightIcon=s.rightIcon and n.createElement('ImageLabel',{
Size=UDim2.new(0,16,0,16),Position=UDim2.new(1,-16,0.5,-8),
BackgroundTransparency=1,Image=s.rightIcon,ImageColor3=p.Colors.TextSecondary})
or nil,Track=n.createElement('TextButton',{ref=r.trackRef,Size=UDim2.new(1,(s.
leftIcon and-24 or 0)+(s.rightIcon and-24 or 0),0,8),Position=UDim2.new(0,s.
leftIcon and 20 or 0,0.5,-4),BackgroundColor3=p.Colors.Element,BorderSizePixel=0
,Text='',AutoButtonColor=false,[n.Event.MouseEnter]=function()r:setState{hovered
=true}end,[n.Event.MouseLeave]=function()r:setState{hovered=false}end,[n.Event.
InputBegan]=function(y,z)if z.UserInputType==Enum.UserInputType.MouseButton1 or
z.UserInputType==Enum.UserInputType.Touch then updateValue(z)if r.dragConnection
then r.dragConnection:Disconnect()end if r.releaseConnection then r.
releaseConnection:Disconnect()end r.dragConnection=l.InputChanged:Connect(
function(A)if A.UserInputType==Enum.UserInputType.MouseMovement or A.
UserInputType==Enum.UserInputType.Touch then updateValue(A)end end)r.
releaseConnection=l.InputEnded:Connect(function(A)if A.UserInputType==Enum.
UserInputType.MouseButton1 or A.UserInputType==Enum.UserInputType.Touch then if
r.dragConnection then r.dragConnection:Disconnect()r.dragConnection=nil end if r
.releaseConnection then r.releaseConnection:Disconnect()r.releaseConnection=nil
end end end)end end},{Corner=n.createElement('UICorner',{CornerRadius=p.
CornerRadius.Circle}),Stroke=n.createElement('UIStroke',{ref=r.strokeRef,Color=x
and p.Colors.BorderHover or p.Colors.Border,Thickness=1}),Fill=n.createElement(
'Frame',{ref=r.fillRef,BackgroundColor3=p.Colors.Accent,BorderSizePixel=0,ZIndex
=1},{Corner=n.createElement('UICorner',{CornerRadius=p.CornerRadius.Circle})}),
Knob=n.createElement('Frame',{ref=r.knobRef,Size=UDim2.new(0,12,0,12),
BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0,ZIndex=2},{Corner
=n.createElement('UICorner',{CornerRadius=p.CornerRadius.Circle}),Stroke=n.
createElement('UIStroke',{Color=p.Colors.Border,Thickness=1})})})})})end return
q end d['Components/Stepper']=function()local i=createVirtualModule
'Components/Stepper'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=l.Component:extend'StepperDecButton'function o.init(p)
p.state={hovered=false}end function o.render(p)local q=p.props local r=p.state.
hovered return l.createElement('TextButton',{Size=UDim2.new(0,26,0,26),
BackgroundColor3=r and n.Colors.ElementHover or n.Colors.Element,BorderSizePixel
=0,Text='-',TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=14,LayoutOrder=1
,AutoButtonColor=false,[l.Event.MouseEnter]=function()p:setState{hovered=true}
end,[l.Event.MouseLeave]=function()p:setState{hovered=false}end,[l.Event.
MouseButton1Click]=q.OnClick},{Corner=l.createElement('UICorner',{CornerRadius=n
.CornerRadius.Element}),Stroke=l.createElement('UIStroke',{Color=r and n.Colors.
BorderHover or n.Colors.Border,Thickness=1})})end local p=l.Component:extend
'StepperIncButton'function p.init(q)q.state={hovered=false}end function p.render
(q)local r=q.props local s=q.state.hovered return l.createElement('TextButton',{
Size=UDim2.new(0,26,0,26),BackgroundColor3=s and n.Colors.ElementHover or n.
Colors.Element,BorderSizePixel=0,Text='+',TextColor3=n.Colors.Text,Font=n.Fonts.
Bold,TextSize=14,LayoutOrder=3,AutoButtonColor=false,[l.Event.MouseEnter]=
function()q:setState{hovered=true}end,[l.Event.MouseLeave]=function()q:setState{
hovered=false}end,[l.Event.MouseButton1Click]=r.OnClick},{Corner=l.
createElement('UICorner',{CornerRadius=n.CornerRadius.Element}),Stroke=l.
createElement('UIStroke',{Color=s and n.Colors.BorderHover or n.Colors.Border,
Thickness=1})})end local q=l.Component:extend'Stepper'function q.render(r)local
s=r.props.element local t=r.props.Store local u=s.min or 0 local v=s.max or 100
local w=s.step or 1 local x=s.value or u local function updateValue(y)y=math.
clamp(y,u,v)local z=1/w if z==math.floor(z)then y=math.round(y*z)/z end t:
UpdateElement(s.id,function(A)A.value=y end)if s.callback then task.spawn(s.
callback,y)end end return l.createElement('Frame',{Size=UDim2.new(1,0,0,36),
BackgroundTransparency=1},{Label=l.createElement('TextLabel',{Size=UDim2.new(1,-
130,1,0),BackgroundTransparency=1,Text=s.name or'Stepper',TextColor3=n.Colors.
Text,Font=n.Fonts.Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left}),
Container=l.createElement('Frame',{Size=UDim2.new(0,120,0,26),Position=UDim2.
new(1,-120,0.5,-13),BackgroundTransparency=1},{List=l.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,VerticalAlignment=
Enum.VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder}),DecButton=l
.createElement(o,{OnClick=function()updateValue(x-w)end}),Display=l.
createElement('Frame',{Size=UDim2.new(0,68,0,26),BackgroundTransparency=1,
LayoutOrder=2},{Input=s.fielded and l.createElement('TextBox',{Size=UDim2.new(1,
0,1,0),BackgroundTransparency=1,Text=tostring(x),TextColor3=n.Colors.Text,Font=n
.Fonts.Bold,TextSize=12,TextXAlignment=Enum.TextXAlignment.Center,
ClearTextOnFocus=false,[l.Event.FocusLost]=function(y)local z=tonumber(y.Text)if
z then updateValue(z)else y.Text=tostring(x)end end})or l.createElement(
'TextLabel',{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text=tostring(x),
TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=12})}),IncButton=l.
createElement(p,{OnClick=function()updateValue(x+w)end})})})end return q end d[
'Components/TabContainer']=function()local i=createVirtualModule
'Components/TabContainer'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=game:GetService'TweenService'local m=j(k.
Packages.React)local n=i.Parent.Parent local o=j(n.Theme)local p=table.clone or
function(p)local q={}for r,s in pairs(p)do q[r]=s end return q end local q=j(n.
Components.Section)local r=j(n.Components.Profile)local s=m.Component:extend
'TabContainer'function s.init(t)t.groupRef=m.createRef()end function s.didMount(
t)local u=t.groupRef.current if u then u.Position=UDim2.new(0,0,0,8)u.
GroupTransparency=1 local v=TweenInfo.new(0.25,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)l:Create(u,v,{Position=UDim2.new(0,0,0,0),GroupTransparency=
0}):Play()end end function s.didUpdate(t,u)if t.props.state.activeTab~=u.state.
activeTab then local v=t.groupRef.current if v then v.Position=UDim2.new(0,0,0,8
)v.GroupTransparency=1 local w=TweenInfo.new(0.25,Enum.EasingStyle.Quad,Enum.
EasingDirection.Out)l:Create(v,w,{Position=UDim2.new(0,0,0,0),GroupTransparency=
0}):Play()end end end function s.render(t)local u=t.props.state local v=t.props.
Store local w=t.props.LayoutOrder local x for y,z in ipairs(u.tabs)do if z.id==u
.activeTab then x=z break end end if not x then return m.createElement(
'CanvasGroup',{ref=t.groupRef,Size=UDim2.new(1,-180,1,0),BackgroundTransparency=
1,LayoutOrder=w},{NoTab=m.createElement('TextLabel',{Size=UDim2.new(1,0,1,0),
BackgroundTransparency=1,Text='No Tab Selected',TextColor3=o.Colors.
TextSecondary,Font=o.Fonts.Medium,TextSize=14})})end local y=x.banner local z=
string.lower(u.searchQuery or'')local A={}local B={}local C=x.pageSections or x.
sections or{}for D,E in ipairs(C)do local F=false local G if z==''then F=true
else local H=E.name and string.find(string.lower(E.name),z)~=nil local I=E.
description and string.find(string.lower(E.description),z)~=nil if H or I then F
=true else local J={}for K,L in ipairs(E.elements)do local M=L.name and string.
find(string.lower(L.name),z)~=nil local N=L.text and string.find(string.lower(L.
text),z)~=nil if M or N then table.insert(J,L)end if L.type=='VStack'or L.type==
'HStack'then local O=false for P,Q in ipairs(L.elements or{})do local R=Q.name
and string.find(string.lower(Q.name),z)~=nil local S=Q.text and string.find(
string.lower(Q.text),z)~=nil if R or S then O=true break end end if O then table
.insert(J,L)end end end if#J>0 then F=true G=J end end end if F then local H=E
if G then H=p(E)H.elements=G end local I=m.createElement(q,{key='Section_'..E.id
,section=H,Store=v,LayoutOrder=D})if E.column==2 or E.column=='Right'then table.
insert(B,I)else table.insert(A,I)end end end local D=#B>0 local E if D then
local F={ColLayout=m.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Vertical,Padding=UDim.new(0,12),SortOrder=Enum.SortOrder.
LayoutOrder})}for G,H in ipairs(A)do F['Sec_'..tostring(G)]=H end local G={
ColLayout=m.createElement('UIListLayout',{FillDirection=Enum.FillDirection.
Vertical,Padding=UDim.new(0,12),SortOrder=Enum.SortOrder.LayoutOrder})}for H,I
in ipairs(B)do G['Sec_'..tostring(H)]=I end E={ColumnsList=m.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,SortOrder=Enum.
SortOrder.LayoutOrder,Padding=UDim.new(0,12)}),LeftCol=m.createElement('Frame',{
Size=UDim2.new(0.5,-6,0,0),BackgroundTransparency=1,AutomaticSize=Enum.
AutomaticSize.Y,LayoutOrder=1},F),RightCol=m.createElement('Frame',{Size=UDim2.
new(0.5,-6,0,0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,
LayoutOrder=2},G)}else local F={SingleColLayout=m.createElement('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,12),SortOrder=Enum.
SortOrder.LayoutOrder})}for G,H in ipairs(A)do F['Sec_'..tostring(G)]=H end E=F
end return m.createElement('CanvasGroup',{ref=t.groupRef,Size=UDim2.new(1,-180,1
,0),BackgroundTransparency=1,LayoutOrder=w},{ScrollFrame=m.createElement(
'ScrollingFrame',{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,
BorderSizePixel=0,ScrollBarThickness=4,ScrollBarImageColor3=o.Colors.Border,
CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y},{Layout=
m.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,
SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,12)}),Padding=m.
createElement('UIPadding',{PaddingTop=UDim.new(0,12),PaddingBottom=UDim.new(0,15
),PaddingLeft=UDim.new(0,12),PaddingRight=UDim.new(0,12)}),Banner=y and m.
createElement('Frame',{Size=UDim2.new(1,0,0,80),BackgroundTransparency=1,
LayoutOrder=1},{Content=m.createElement(r,{username=y.username,subtext=y.subtext
,userId=y.userId})}),GridArea=m.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=2},E)})}
)end return s end d['Components/TextBox']=function()local i=createVirtualModule
'Components/TextBox'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Theme)local o=l.Component:extend'TextBox'function o.init(p)p.state={
hovered=false}end function o.render(p)local q=p.props.element local r=p.props.
Store local s=p.state.hovered local t=q.value or''local u=q.hideLabel return l.
createElement('Frame',{Size=UDim2.new(1,0,0,36),BackgroundTransparency=1},{Label
=not u and l.createElement('TextLabel',{Size=UDim2.new(0,150,1,0),
BackgroundTransparency=1,Text=q.name or'Input',TextColor3=n.Colors.Text,Font=n.
Fonts.Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left})or nil,
InputContainer=l.createElement('Frame',{Size=u and UDim2.new(1,0,0,28)or UDim2.
new(1,-160,0,28),Position=u and UDim2.new(0,0,0.5,-14)or UDim2.new(1,-160,0.5,-
14),BackgroundColor3=n.Colors.Element,BorderSizePixel=0},{Corner=l.
createElement('UICorner',{CornerRadius=n.CornerRadius.Element}),Stroke=l.
createElement('UIStroke',{Color=s and n.Colors.BorderHover or n.Colors.Border,
Thickness=1}),TextInput=l.createElement('TextBox',{Size=UDim2.new(1,-16,1,0),
Position=UDim2.new(0,8,0,0),BackgroundTransparency=1,Text=t,PlaceholderText=q.
placeholder or'Type here...',PlaceholderColor3=n.Colors.TextSecondary,TextColor3
=n.Colors.Text,Font=n.Fonts.Regular,TextSize=12,TextXAlignment=Enum.
TextXAlignment.Left,ClearTextOnFocus=false,[l.Event.MouseEnter]=function()p:
setState{hovered=true}end,[l.Event.MouseLeave]=function()p:setState{hovered=
false}end,[l.Event.FocusLost]=function(v,w)local x=v.Text r:UpdateElement(q.id,
function(y)y.value=x end)if q.callback then task.spawn(q.callback,x,w)end end})}
)})end return o end d['Components/TitleStack']=function()local i=
createVirtualModule'Components/TitleStack'createVirtualFolder''local j=__require
local k=game:GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.
Parent.Parent local n=j(m.Theme)local o=l.Component:extend'TitleStack'local 
function TagComponent(p)local q=p.color or n.Colors.Accent local r=type(q)==
'table'return l.createElement('Frame',{Size=UDim2.new(0,0,0,16),AutomaticSize=
Enum.AutomaticSize.X,BackgroundColor3=not r and q or Color3.fromRGB(255,255,255)
,BorderSizePixel=0,LayoutOrder=p.LayoutOrder},{Corner=l.createElement('UICorner'
,{CornerRadius=UDim.new(0,4)}),Padding=l.createElement('UIPadding',{PaddingLeft=
UDim.new(0,6),PaddingRight=UDim.new(0,6),PaddingTop=UDim.new(0,1),PaddingBottom=
UDim.new(0,1)}),Gradient=r and l.createElement('UIGradient',{Color=ColorSequence
.new(unpack(q)),Rotation=45}),Label=l.createElement('TextLabel',{Size=UDim2.new(
1,0,1,0),BackgroundTransparency=1,Text=string.upper(p.text),TextColor3=Color3.
fromRGB(255,255,255),Font=n.Fonts.Bold,TextSize=8})})end function o.render(p)
local q=p.props.element local r=q.title or q.Title or'Title'local s=q.subtitle
or q.Subtitle local t=q.titleTag or q.TitleTag local u=q.titleTagColor or q.
TitleTagColor local v=q.subtitleTag or q.SubtitleTag local w=q.subtitleTagColor
or q.SubtitleTagColor return l.createElement('Frame',{Size=UDim2.new(1,0,0,0),
BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y},{List=l.
createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical,Padding=
UDim.new(0,2),SortOrder=Enum.SortOrder.LayoutOrder}),TitleRow=l.createElement(
'Frame',{Size=UDim2.new(1,0,0,18),BackgroundTransparency=1,LayoutOrder=1},{List=
l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,
VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,6),SortOrder=
Enum.SortOrder.LayoutOrder}),Label=l.createElement('TextLabel',{Size=UDim2.new(0
,0,1,0),AutomaticSize=Enum.AutomaticSize.X,BackgroundTransparency=1,Text=r,
TextColor3=n.Colors.Text,Font=n.Fonts.Bold,TextSize=13,LayoutOrder=1}),Tag=t and
l.createElement(TagComponent,{text=t,color=u,LayoutOrder=2})or nil}),SubtitleRow
=s and l.createElement('Frame',{Size=UDim2.new(1,0,0,14),BackgroundTransparency=
1,LayoutOrder=2},{List=l.createElement('UIListLayout',{FillDirection=Enum.
FillDirection.Horizontal,VerticalAlignment=Enum.VerticalAlignment.Center,Padding
=UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder}),Label=l.createElement(
'TextLabel',{Size=UDim2.new(0,0,1,0),AutomaticSize=Enum.AutomaticSize.X,
BackgroundTransparency=1,Text=s,TextColor3=n.Colors.TextSecondary,Font=n.Fonts.
Regular,TextSize=10,LayoutOrder=1}),Tag=v and l.createElement(TagComponent,{text
=v,color=w,LayoutOrder=2})or nil})or nil})end return o end d['Components/Toggle'
]=function()local i=createVirtualModule'Components/Toggle'createVirtualFolder''
local j=__require local k=game:GetService'ReplicatedStorage'local l=game:
GetService'TweenService'local m=j(k.Packages.React)local n=i.Parent.Parent local
o=j(n.Theme)local p=m.Component:extend'Toggle'function p.init(q,r)q.state={
hovered=false,value=r.element.value or false}q.switchRef=m.createRef()q.knobRef=
m.createRef()q.strokeRef=m.createRef()end function p.didMount(q)local r=q.
switchRef.current local s=q.knobRef.current local t=q.strokeRef.current local u=
q.state.value local v=q.state.hovered if r then r.BackgroundColor3=u and o.
Colors.Accent or o.Colors.Element end if s then s.Position=u and UDim2.new(1,-19
,0.5,-8)or UDim2.new(0,3,0.5,-8)end if t then t.Color=v and o.Colors.BorderHover
or o.Colors.Border end end function p.didUpdate(q,r,s)local t=q.switchRef.
current local u=q.knobRef.current local v=q.strokeRef.current local w=q.props.
element local x=q.state.value if w.value~=x and w.value~=r.element.value then q:
setState{value=w.value}return end if x~=s.value then local y=x and o.Colors.
Accent or o.Colors.Element local z=x and UDim2.new(1,-19,0.5,-8)or UDim2.new(0,3
,0.5,-8)local A=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out
)if t then l:Create(t,A,{BackgroundColor3=y}):Play()end if u then l:Create(u,A,{
Position=z}):Play()end end local y=q.state.hovered if y~=s.hovered then local z=
y and o.Colors.BorderHover or o.Colors.Border local A=TweenInfo.new(0.2,Enum.
EasingStyle.Quad,Enum.EasingDirection.Out)if v then l:Create(v,A,{Color=z}):
Play()end end end function p.render(q)local r=q.props.element local s=q.props.
Store local t=q.state.hovered local u=q.state.value local v=r.hideLabel return m
.createElement('Frame',{Size=UDim2.new(1,0,0,36),BackgroundTransparency=1},{
Label=not v and m.createElement('TextLabel',{Size=UDim2.new(1,-60,1,0),
BackgroundTransparency=1,Text=r.name or'Toggle',TextColor3=o.Colors.Text,Font=o.
Fonts.Medium,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left})or nil,Switch=
m.createElement('TextButton',{ref=q.switchRef,Size=UDim2.new(0,42,0,22),Position
=UDim2.new(1,-42,0.5,-11),BorderSizePixel=0,Text='',AutoButtonColor=false,[m.
Event.MouseEnter]=function()q:setState{hovered=true}end,[m.Event.MouseLeave]=
function()q:setState{hovered=false}end,[m.Event.MouseButton1Click]=function()
local w=not u q:setState{value=w}s:UpdateElement(r.id,function(x)x.value=w end)
if r.callback then task.spawn(r.callback,w)end end},{Corner=m.createElement(
'UICorner',{CornerRadius=o.CornerRadius.Circle}),Stroke=m.createElement(
'UIStroke',{ref=q.strokeRef,Color=t and o.Colors.BorderHover or o.Colors.Border,
Thickness=1}),Knob=m.createElement('Frame',{ref=q.knobRef,Size=UDim2.new(0,16,0,
16),BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0},{Corner=m.
createElement('UICorner',{CornerRadius=o.CornerRadius.Circle})})})})end return p
end d['Components/VStack']=function()local i=createVirtualModule
'Components/VStack'createVirtualFolder''local j=__require local k=game:
GetService'ReplicatedStorage'local l=j(k.Packages.React)local m=i.Parent.Parent
local n=j(m.Components.ElementRenderer)local o=l.Component:extend'VStack'
function o.render(p)local q=p.props.element local r=p.props.Store local s={
Layout=l.createElement('UIListLayout',{FillDirection=Enum.FillDirection.Vertical
,Padding=q.padding or UDim.new(0,4),HorizontalAlignment=q.horizontalAlignment or
Enum.HorizontalAlignment.Left,VerticalAlignment=q.verticalAlignment or Enum.
VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder})}for t,u in
ipairs(q.elements or{})do local v=n.render(u,r)if v then s['El_'..u.id]=l.
createElement('Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,
AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=t},{Content=v})end end return l.
createElement('Frame',{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,
AutomaticSize=Enum.AutomaticSize.Y},s)end return o end d['Components/Window']=
function()local i=createVirtualModule'Components/Window'createVirtualFolder''
local j=__require local k=game:GetService'ReplicatedStorage'local l=game:
GetService'UserInputService'local m=game:GetService'TweenService'local n=j(k.
Packages.React)local o=i.Parent.Parent local p=j(o.Theme)local q=j(o.Components.
Sidebar)local r=j(o.Components.TabContainer)local s=j(o.Icons).assets local t=n.
Component:extend'Window'local function TagComponent(u)local v=u.color or p.
Colors.Accent local w=type(v)=='table'return n.createElement('Frame',{Size=UDim2
.new(0,0,0,16),AutomaticSize=Enum.AutomaticSize.X,BackgroundColor3=not w and v
or Color3.fromRGB(255,255,255),BorderSizePixel=0,LayoutOrder=u.LayoutOrder},{
Corner=n.createElement('UICorner',{CornerRadius=UDim.new(0,4)}),Padding=n.
createElement('UIPadding',{PaddingLeft=UDim.new(0,6),PaddingRight=UDim.new(0,6),
PaddingTop=UDim.new(0,1),PaddingBottom=UDim.new(0,1)}),Gradient=w and n.
createElement('UIGradient',{Color=ColorSequence.new(unpack(v)),Rotation=45}),
Label=n.createElement('TextLabel',{Size=UDim2.new(1,0,1,0),
BackgroundTransparency=1,Text=string.upper(u.text),TextColor3=Color3.fromRGB(255
,255,255),Font=p.Fonts.Bold,TextSize=8})})end function t.init(u)u.windowRef=n.
createRef()u.containerRef=n.createRef()u.dragConnection=nil u.
dragInputConnection=nil u.state={position=UDim2.new(0.5,-375,0.5,-240),closing=
false}end function t.didMount(u)local v=u.windowRef.current local w=u.
containerRef.current if not v then return end local x=u.props.state.maximized
local y=u.props.state.minimized local z=x and UDim2.new(0,0,0,0)or u.state.
position local A=UDim2.new(z.X.Scale,z.X.Offset,z.Y.Scale,z.Y.Offset+15)v.
Position=A local B=x and UDim2.new(1,0,1,0)or(y and UDim2.new(0,750,0,45)or
UDim2.new(0,750,0,480))v.Size=B v.GroupTransparency=1 local C=TweenInfo.new(0.3,
Enum.EasingStyle.Quad,Enum.EasingDirection.Out)m:Create(v,C,{Position=z,
GroupTransparency=0}):Play()if w then w.GroupTransparency=y and 1 or 0 w.Visible
=not y end local D=false local E local F local G local function update(H)if u.
props.state.maximized then return end local I=H.Position-F u:setState{position=
UDim2.new(G.X.Scale,G.X.Offset+I.X,G.Y.Scale,G.Y.Offset+I.Y)}end local H=v:
WaitForChild('TitleBar',1)if H then u.dragConnection=H.InputBegan:Connect(
function(I)if u.props.state.maximized then return end if not u.props.state.
draggable and u.props.state.draggable~=nil then return end if I.UserInputType==
Enum.UserInputType.MouseButton1 or I.UserInputType==Enum.UserInputType.Touch
then D=true F=I.Position G=v.Position I.Changed:Connect(function()if I.
UserInputState==Enum.UserInputState.End then D=false end end)end end)u.
dragInputConnection=H.InputChanged:Connect(function(I)if I.UserInputType==Enum.
UserInputType.MouseMovement or I.UserInputType==Enum.UserInputType.Touch then E=
I end end)l.InputChanged:Connect(function(I)if I==E and D then update(I)end end)
end end function t.willUnmount(u)if u.dragConnection then u.dragConnection:
Disconnect()end if u.dragInputConnection then u.dragInputConnection:Disconnect()
end end function t.didUpdate(u,v,w)local x=u.windowRef.current local y=u.
containerRef.current if not x then return end local z=u.props.state.minimized
local A=u.props.state.maximized if u.props.state.visible and not v.state.visible
then u:setState{closing=false}x.GroupTransparency=0 end local B=(z~=v.state.
minimized)or(A~=v.state.maximized)local C=(A~=v.state.maximized)if B or C then
local D=A and UDim2.new(1,0,1,0)or(z and UDim2.new(0,750,0,45)or UDim2.new(0,750
,0,480))local E=A and UDim2.new(0,0,0,0)or u.state.position local F=TweenInfo.
new(0.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)m:Create(x,F,{Size=D,
Position=E}):Play()if z~=v.state.minimized then if y then if z then local G=m:
Create(y,F,{GroupTransparency=1})G:Play()G.Completed:Connect(function()if u.
props.state.minimized then y.Visible=false end end)else y.Visible=true y.
GroupTransparency=1 m:Create(y,F,{GroupTransparency=0}):Play()end end end elseif
w.position~=u.state.position and not A then x.Position=u.state.position end end
function t.render(u)local v=u.props.state local w=u.props.Store local x=v.
minimized local y=v.maximized local z=y and UDim2.new(1,0,1,0)or(x and UDim2.
new(0,750,0,45)or UDim2.new(0,750,0,480))local A=y and UDim2.new(0,0,0,0)or u.
state.position local B={Layout=n.createElement('UIListLayout',{FillDirection=
Enum.FillDirection.Horizontal,HorizontalAlignment=Enum.HorizontalAlignment.Right
,VerticalAlignment=Enum.VerticalAlignment.Center,Padding=UDim.new(0,8),SortOrder
=Enum.SortOrder.LayoutOrder})}if v.canMinimize~=false then B.Minimize=n.
createElement('ImageButton',{Size=UDim2.new(0,14,0,14),BackgroundTransparency=1,
Image=s['lucide-minus'],ImageColor3=p.Colors.TextSecondary,LayoutOrder=1,[n.
Event.MouseButton1Click]=function()w:Update(function(C)C.minimized=not C.
minimized if C.minimized then C.maximized=false end end)end})end if v.canZoom~=
false then B.Maximize=n.createElement('ImageButton',{Size=UDim2.new(0,14,0,14),
BackgroundTransparency=1,Image=y and s['lucide-minimize-2']or s[
'lucide-maximize-2'],ImageColor3=p.Colors.TextSecondary,LayoutOrder=2,[n.Event.
MouseButton1Click]=function()w:Update(function(C)C.maximized=not C.maximized if
C.maximized then C.minimized=false end end)end})end if v.canExit~=false then B.
Close=n.createElement('ImageButton',{Size=UDim2.new(0,14,0,14),
BackgroundTransparency=1,Image=s['lucide-x'],ImageColor3=p.Colors.TextSecondary,
LayoutOrder=3,[n.Event.MouseButton1Click]=function()if u.state.closing then
return end u:setState{closing=true}local C=u.windowRef.current if C then local D
=TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)local E=m:
Create(C,D,{Position=UDim2.new(C.Position.X.Scale,C.Position.X.Offset,C.Position
.Y.Scale,C.Position.Y.Offset+15),GroupTransparency=1})E:Play()E.Completed:
Connect(function()w:Update(function(F)F.visible=false end)end)else w:Update(
function(D)D.visible=false end)end end})end return n.createElement('CanvasGroup'
,{ref=u.windowRef,Size=z,Position=A,BackgroundColor3=p.Colors.Background,
BorderSizePixel=0,Active=true,ClipsDescendants=true},{Corner=n.createElement(
'UICorner',{CornerRadius=p.CornerRadius.Window}),Stroke=n.createElement(
'UIStroke',{Color=p.Colors.Border,Thickness=1,ApplyStrokeMode=Enum.
ApplyStrokeMode.Border}),TitleBar=n.createElement('Frame',{Size=UDim2.new(1,0,0,
45),BackgroundTransparency=1,BorderSizePixel=0,ZIndex=2},{Icon=n.createElement(
'ImageLabel',{Size=UDim2.new(0,20,0,20),Position=UDim2.new(0,15,0.5,-10),
BackgroundTransparency=1,Image='rbxassetid://10734950309',ImageColor3=p.Colors.
Accent}),TitleArea=n.createElement('Frame',{Size=v.searching and UDim2.new(1,-
310,1,0)or UDim2.new(1,-180,1,0),Position=UDim2.new(0,45,0,0),
BackgroundTransparency=1},{Layout=n.createElement('UIListLayout',{FillDirection=
Enum.FillDirection.Horizontal,VerticalAlignment=Enum.VerticalAlignment.Center,
Padding=UDim.new(0,8),SortOrder=Enum.SortOrder.LayoutOrder}),Title=n.
createElement('TextLabel',{Size=UDim2.new(0,0,1,0),AutomaticSize=Enum.
AutomaticSize.X,BackgroundTransparency=1,Text=v.title,TextColor3=p.Colors.Text,
Font=p.Fonts.Bold,TextSize=14,LayoutOrder=1}),TitleTag=v.titleTag and n.
createElement(TagComponent,{text=v.titleTag,color=v.titleTagColor,LayoutOrder=2}
),Subtitle=v.subtitle~=''and n.createElement('TextLabel',{Size=UDim2.new(0,0,1,0
),AutomaticSize=Enum.AutomaticSize.X,BackgroundTransparency=1,Text=v.subtitle,
TextColor3=p.Colors.TextSecondary,Font=p.Fonts.Medium,TextSize=11,LayoutOrder=3}
),SubtitleTag=v.subtitleTag and n.createElement(TagComponent,{text=v.subtitleTag
,color=v.subtitleTagColor,LayoutOrder=4})}),SearchBar=v.searching and n.
createElement('Frame',{Size=UDim2.new(0,160,0,24),Position=UDim2.new(1,-290,0.5,
-12),BackgroundColor3=p.Colors.Element,BorderSizePixel=0,ZIndex=3},{Corner=n.
createElement('UICorner',{CornerRadius=p.CornerRadius.Element}),Stroke=n.
createElement('UIStroke',{Color=p.Colors.Border,Thickness=1}),Padding=n.
createElement('UIPadding',{PaddingLeft=UDim.new(0,8),PaddingRight=UDim.new(0,8)}
),Icon=n.createElement('ImageLabel',{Size=UDim2.new(0,12,0,12),Position=UDim2.
new(0,0,0.5,-6),BackgroundTransparency=1,Image=s['lucide-search']or
'rbxassetid://10734943674',ImageColor3=p.Colors.TextSecondary}),Input=n.
createElement('TextBox',{Size=UDim2.new(1,-16,1,0),Position=UDim2.new(0,16,0,0),
BackgroundTransparency=1,Text=v.searchQuery,PlaceholderText='Search...',
PlaceholderColor3=p.Colors.TextSecondary,TextColor3=p.Colors.Text,Font=p.Fonts.
Regular,TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,ClearTextOnFocus=
false,[n.Change.Text]=function(C)w:Update(function(D)D.searchQuery=C.Text end)
end})}),Controls=n.createElement('Frame',{Size=UDim2.new(0,100,1,0),Position=
UDim2.new(1,-115,0,0),BackgroundTransparency=1},B)}),Container=n.createElement(
'CanvasGroup',{ref=u.containerRef,Size=UDim2.new(1,0,1,-45),Position=UDim2.new(0
,0,0,45),BackgroundTransparency=1,Visible=not x},{Layout=n.createElement(
'UIListLayout',{FillDirection=Enum.FillDirection.Horizontal,SortOrder=Enum.
SortOrder.LayoutOrder}),Sidebar=n.createElement(q,{state=v,Store=w,LayoutOrder=1
}),MainContent=n.createElement(r,{state=v,Store=w,LayoutOrder=2})}),
ColorPickerOverlay=v.colorPickerActive and n.createElement(j(o.Components.
ColorPickerDialog),{colorPickerActive=v.colorPickerActive,Store=w})or nil})end
return t end d.Icons=function()createVirtualModule'Icons'createVirtualFolder''
return{assets={['00Circle']='rbxassetid://83028270380365',['00CircleFill']=
...
'rbxassetid://118925074578922',abc='rbxassetid://100440409146443',abs=
'rbxassetid://104303148312310',absBrakesignal='rbxassetid://108288270804521',
absBrakesignalSlash='rbxassetid://139900347181805',absCircle=
'rbxassetid://127912426662073',absCircleFill='rbxassetid://90389741944345',ac=
'rbxassetid://70584542126579',acSlash='rbxassetid://131379895015246',
accessibility='rbxassetid://117778752008722',accessibilityBadgeArrowUpRight=
...
,airplayVideoCircleFill='rbxassetid://85432097318270',airplayaudio=
'rbxassetid://132916942955864',airplayaudioBadgeExclamationmark=
...
airpodGen3Left='rbxassetid://82259989716387',airpodGen3Right=
'rbxassetid://137315291748188',airpodLeft='rbxassetid://111640685320561',
airpodRight='rbxassetid://92417752017903',airpodproLeft=
'rbxassetid://95793086337265',airpodproRight='rbxassetid://74684142613428',
airpods='rbxassetid://102934461761467',airpodsChargingcase=
'rbxassetid://137188300011249',airpodsChargingcaseFill=
...
airpodsProRight='rbxassetid://110293485717436',airpodsmax=
...
airpodsproChargingcaseWirelessRadiowavesLeftAndRight=
'rbxassetid://137171665646568',airportExpress='rbxassetid://84914509690369',
airportExtreme='rbxassetid://91752831773330',airportExtremeTower=
'rbxassetid://84855334762457',airtag='rbxassetid://133510297942223',airtagFill=
'rbxassetid://73459359791943',waveformPath='rbxassetid://72307154936317',
waveformPathBadgeMinus='rbxassetid://136232811740839',waveformPathBadgePlus=
...
'rbxassetid://95978592996094',zzz='rbxassetid://81661835392914'}}end d.init=
function()local i=createVirtualModule'init'createVirtualFolder''local j=
__require local k=game:GetService'ReplicatedStorage'local l=j(k.Packages.React)
local m=j(k.Packages.ReactRoblox)local n=j(i.Theme)local o=j(i.Store)local p=j(i
.AppRecorder)local q={}q.Theme=n q.Themes=n.Themes q.Accents=n.Accents q.
AppRecorder=p q.Icons=j(i.Icons)q._customComponents={}local r=q.Icons.assets
local function toKebabCase(s)local t=s:gsub('(%u)','-%1'):gsub('(%d+)','-%1'):
lower()if t:sub(1,1)=='-'then t=t:sub(2)end return t end local s={checkmark=
'rbxassetid://10747373111',switch2='rbxassetid://10747372910',sliderHorizontal3=
'rbxassetid://10747381986',folder='rbxassetid://10747372910',network=
'rbxassetid://10747381881',pieChart='rbxassetid://10747381986',gear=
'rbxassetid://10747372251',house='rbxassetid://10747379658'}setmetatable(s,{
__index=function(t,u)if type(u)~='string'then return nil end local v=r[u]if v
then return v end v=r['lucide-'..u]if v then return v end local w=toKebabCase(u)
v=r['lucide-'..w]if v then return v end if u=='gear'or u=='settings'then return
r['lucide-settings']elseif u=='house'or u=='home'then return r['lucide-home']
elseif u=='checkmark'or u=='check'then return r['lucide-check']end return nil
end})q.Symbols=s q.Creator={Create=function(t)return function(u)local v=Instance
.new(t)for w,x in pairs(u)do if type(w)=='string'then local y=x if type(x)==
'table'then if x.__instance then y=x.__instance else y=nil end end if y~=nil or
w~='Parent'then v[w]=y end else local y=(type(x)=='table'and x.__instance)and x.
__instance or x if y then y.Parent=v end end end local w={__instance=v}
setmetatable(w,{__index=function(x,y)if y=='__instance'then return v else return
v[y]end end,__newindex=function(x,y,z)v[y]=z end})return w end end}function q.
RegisterComponent(t,u)q._customComponents[t]=u end local t=table.clone or
function(t)local u={}for v,w in pairs(t)do u[v]=w end return u end local u local
v local function getGuiParent()local w,x=pcall(function()return game:GetService
'CoreGui'end)if w and x then return x end local y=game:GetService'Players'local
z=y.LocalPlayer if z then local A=z:FindFirstChild'PlayerGui'or z:WaitForChild(
'PlayerGui',5)if A then return A end end local A,B=pcall(function()return game:
GetService'StarterGui'end)if A and B then return B end return nil end local 
function wrapCallback(w,x)if not x then return nil end return function(...)
return x(w,...)end end local function setupContainerMethods(w,x)function w.Row(y
,z)z=z or{}local A=math.random(1,100000)local B={id=A,type='Row',leftElements={}
,rightElements={}}x(B)local C={}function C.Left(D)local E={}
setupContainerMethods(E,function(F)table.insert(B.leftElements,F)o:Update(
function(G)end)end)return E end function C.Right(D)local E={}
setupContainerMethods(E,function(F)F.hideLabel=true table.insert(B.rightElements
,F)o:Update(function(G)end)end)return E end return C end function w.TitleStack(y
,z)z=z or{}local A={id=math.random(1,100000),type='TitleStack',title=z.Title,
subtitle=z.Subtitle,titleTag=z.TitleTag,titleTagColor=z.TitleTagColor,
subtitleTag=z.SubtitleTag,subtitleTagColor=z.SubtitleTagColor}x(A)local B={}
function B.SetTitle(C,D)A.title=D o:Update(function(E)end)end function B.
SetSubtitle(C,D)A.subtitle=D o:Update(function(E)end)end return B end function w
.Label(y,z)z=z or{}local A={id=math.random(1,100000),type='Label',text=z.Text or
'Label',color=z.TextColor3,font=z.Font,textSize=z.TextSize,alignment=z.
TextXAlignment}x(A)local B={}function B.SetText(C,D)A.text=D o:Update(function(E
)end)end return B end function w.Paragraph(y,z)z=z or{}local A={id=math.random(1
,100000),type='Paragraph',title=z.Title,content=z.Content}x(A)local B={}function
B.SetTitle(C,D)A.title=D o:Update(function(E)end)end function B.SetContent(C,D)A
.content=D o:Update(function(E)end)end return B end function w.Toggle(y,z)z=z or
{}local A={}local B={id=math.random(1,100000),type='Toggle',name=z.Label or
'Toggle',value=z.Value or false,callback=wrapCallback(A,z.ValueChanged)}x(B)
function A.SetValue(C,D)B.value=D o:Update(function(E)end)end function A.SetText
(C,D)B.name=D o:Update(function(E)end)end function A.SetCallback(C,D)B.callback=
wrapCallback(A,D)end return A end function w.TextField(y,z)z=z or{}local A={}
local B={id=math.random(1,100000),type='TextBox',name=z.Label or'Text Field',
placeholder=z.Placeholder,value=z.Value or'',callback=wrapCallback(A,z.
ValueChanged)}x(B)function A.SetValue(C,D)B.value=D o:Update(function(E)end)end
function A.SetText(C,D)B.name=D o:Update(function(E)end)end function A.
SetPlaceholder(C,D)B.placeholder=D o:Update(function(E)end)end function A.
SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end function w.
KeybindField(y,z)z=z or{}local A={}local B={id=math.random(1,100000),type=
'KeybindField',name=z.Label or'Keybind',value=z.Value,callback=wrapCallback(A,z.
ValueChanged)}x(B)function A.SetValue(C,D)B.value=D o:Update(function(E)end)end
function A.SetText(C,D)B.name=D o:Update(function(E)end)end function A.
SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end function w.
ColorPicker(y,z)z=z or{}local A={}local B={id=math.random(1,100000),type=
'ColorPicker',name=z.Title or'Color Picker',value=z.Value or Color3.fromRGB(255,
255,255),transparency=z.Transparency or 0,callback=wrapCallback(A,z.ValueChanged
)}x(B)function A.SetValue(C,D,E)B.value=D if E then B.transparency=E end o:
Update(function(F)end)end function A.SetText(C,D)B.name=D o:Update(function(E)
end)end function A.SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end
function w.ColorPickerWidget(y,z)z=z or{}local A={}local B={id=math.random(1,
100000),type='ColorPickerWidget',name=z.Title or'Color Picker',value=z.Value or
Color3.fromRGB(255,255,255),transparency=z.Transparency or 0,callback=
wrapCallback(A,z.ValueChanged)}x(B)function A.SetValue(C,D,E)B.value=D if E then
B.transparency=E end o:Update(function(F)end)end function A.SetText(C,D)B.name=D
o:Update(function(E)end)end function A.SetCallback(C,D)B.callback=wrapCallback(A
,D)end return A end function w.Button(y,z)z=z or{}local A={}local B={id=math.
random(1,100000),type='Button',name=z.Label or'Button',icon=z.Icon,state=z.State
,callback=wrapCallback(A,z.Pushed)}x(B)function A.SetText(C,D)B.name=D o:Update(
function(E)end)end function A.SetIcon(C,D)B.icon=D o:Update(function(E)end)end
function A.SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end function
w.Slider(y,z)z=z or{}local A={}local B={id=math.random(1,100000),type='Slider',
name=z.Label or'Slider',min=z.Minimum or 0,max=z.Maximum or 100,value=z.Value or
z.Minimum or 0,callback=wrapCallback(A,z.ValueChanged)}x(B)function A.SetValue(C
,D)B.value=D o:Update(function(E)end)end function A.SetMin(C,D)B.min=D o:Update(
function(E)end)end function A.SetMax(C,D)B.max=D o:Update(function(E)end)end
function A.SetText(C,D)B.name=D o:Update(function(E)end)end function A.
SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end function w.Stepper(
y,z)z=z or{}local A={}local B={id=math.random(1,100000),type='Stepper',name=z.
Label or'Stepper',min=z.Minimum or 0,max=z.Maximum or 100,step=z.Step or 1,value
=z.Value or z.Minimum or 0,fielded=z.Fielded~=false,callback=wrapCallback(A,z.
ValueChanged)}x(B)function A.SetValue(C,D)B.value=D o:Update(function(E)end)end
function A.SetText(C,D)B.name=D o:Update(function(E)end)end function A.
SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end function w.
RadioButtonGroup(y,z)z=z or{}local A={}local B={id=math.random(1,100000),type=
'RadioButtonGroup',name=z.Label or'Radio Group',options=z.Options or{},value=z.
Value or 1,callback=wrapCallback(A,z.ValueChanged)}x(B)function A.SetValue(C,D)B
.value=D o:Update(function(E)end)end function A.SetText(C,D)B.name=D o:Update(
function(E)end)end function A.SetCallback(C,D)B.callback=wrapCallback(A,D)end
return A end function w.PopUpButton(y,z)z=z or{}local A={}local B={id=math.
random(1,100000),type='Dropdown',name=z.Label or'Dropdown',options=z.Options or{
},value=z.Value,maximum=z.Maximum or(z.Multi and 9999 or(type(z.Value)=='table'
and 9999 or 1)),callback=wrapCallback(A,z.ValueChanged)}x(B)function A.SetValue(
C,D)B.value=D o:Update(function(E)end)end function A.SetOptions(C,D)B.options=D
o:Update(function(E)end)end function A.SetText(C,D)B.name=D o:Update(function(E)
end)end function A.SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end
function w.PullDownButton(y,z)z=z or{}local A={}local B={id=math.random(1,100000
),type='PullDownButton',name=z.Label or'Menu',options=z.Options or{},callback=
wrapCallback(A,z.ValueChanged)}x(B)function A.SetOptions(C,D)B.options=D o:
Update(function(E)end)end function A.SetText(C,D)B.name=D o:Update(function(E)
end)end function A.SetCallback(C,D)B.callback=wrapCallback(A,D)end return A end
function w.VStack(y,z)z=z or{}local A={id=math.random(1,100000),type='VStack',
padding=z.Padding,horizontalAlignment=z.HorizontalAlignment,verticalAlignment=z.
VerticalAlignment,elements={}}x(A)local B={}setupContainerMethods(B,function(C)
table.insert(A.elements,C)o:Update(function(D)end)end)return B end function w.
HStack(y,z)z=z or{}local A={id=math.random(1,100000),type='HStack',padding=z.
Padding,horizontalAlignment=z.HorizontalAlignment,verticalAlignment=z.
VerticalAlignment,elements={}}x(A)local B={}setupContainerMethods(B,function(C)
table.insert(A.elements,C)o:Update(function(D)end)end)return B end function w.
BarChart(y,z)z=z or{}local A={id=math.random(1,100000),type='BarChart',title=z.
Title,subtitle=z.Subtitle,height=z.Height,data=z.Data or{}}x(A)local B={}
function B.SetData(C,D)A.data=D or{}o:Update(function(E)end)end function B.
PushData(C,D,E)table.insert(A.data,D)E=E or 10 while#A.data>E do table.remove(A.
data,1)end o:Update(function(F)end)end function B.SetTitle(C,D)A.title=D o:
Update(function(E)end)end function B.SetSubtitle(C,D)A.subtitle=D o:Update(
function(E)end)end return B end function w.LineChart(y,z)z=z or{}local A={id=
math.random(1,100000),type='LineChart',title=z.Title,subtitle=z.Subtitle,height=
z.Height,data=z.Data or{},labels=z.Labels or{},color=z.Color}x(A)local B={}
function B.SetData(C,D,E)A.data=D or{}if E then A.labels=E end o:Update(function
(F)end)end function B.PushData(C,D,E,F)table.insert(A.data,D)if E~=nil or#A.
labels>0 then table.insert(A.labels,E or'')end F=F or 10 while#A.data>F do table
.remove(A.data,1)if#A.labels>0 then table.remove(A.labels,1)end end o:Update(
function(G)end)end function B.SetTitle(C,D)A.title=D o:Update(function(E)end)end
function B.SetSubtitle(C,D)A.subtitle=D o:Update(function(E)end)end function B.
SetColor(C,D)A.color=D o:Update(function(E)end)end return B end function w.
PieChart(y,z)z=z or{}local A={id=math.random(1,100000),type='PieChart',title=z.
Title,subtitle=z.Subtitle,size=z.Size,donut=z.Donut,align=z.Align,data=z.Data or
{}}x(A)local B={}function B.SetData(C,D)A.data=D or{}o:Update(function(E)end)end
function B.SetTitle(C,D)A.title=D o:Update(function(E)end)end function B.
SetSubtitle(C,D)A.subtitle=D o:Update(function(E)end)end return B end function w
.InfoGrid(y,z)z=z or{}local A={id=math.random(1,100000),type='InfoGrid',rows=z.
Rows or{}}x(A)local B={}function B.UpdateRow(C,D,E)A.rows[D]=E o:Update(function
(F)end)end return B end function w.PageSection(y,z)z=z or{}local A=math.random(1
,100000)local B={id=A,type='Section',name=z.Title,description=z.Subtitle,column=
z.Column or 1,gradientPreset=z.GradientPreset,gradient=z.Gradient,elements={}}x(
B)local C={}function C.Form(D)local E={}E.type='Form'E.elements={}table.insert(B
.elements,E)setupContainerMethods(E,function(F)table.insert(E.elements,F)o:
Update(function(G)end)end)return E end return C end function w.ImageSurface(y,z)
z=z or{}local A={id=math.random(1,100000),type='Label',text='',icon=z.Image,
color=z.SurfaceColor}x(A)return A end setmetatable(w,{__index=function(y,z)if q.
_customComponents[z]then return function(A,B)B=B or{}local C={id=math.random(1,
100000),type='CustomComponentPlaceholder',instance=nil}x(C)B.Parent=nil local D=
q._customComponents[z](A,B)C.instance=D and D.__instance o:Update(function(E)end
)return D end end return nil end})end function q.New(w)w=w or{}local x='Dark'
local y='Blue'if w.Theme then for z,A in pairs(n.Themes)do if A==w.Theme or z==w
.Theme then x=z break end end end if w.Accent then for z,A in pairs(n.Accents)do
if A==w.Accent or z==w.Accent then y=z break end end end o:Update(function(z)z.
theme=x z.accent=y end)local z=setmetatable({_id='app'},{__index=function(z,A)if
A=='Theme'then return n.Themes[o._state.theme]elseif A=='Accent'then return n.
Accents[o._state.accent]elseif A=='Notification'then return function(B,C)q:
Notification(C)end elseif A=='Page'then return function(B)return q:Page()end
elseif A=='Window'then return function(B,C)return q:CreateWindow(C)end end
return nil end,__newindex=function(z,A,B)if A=='Theme'then local C for D,E in
pairs(n.Themes)do if E==B or D==B then C=D break end end if C then o:Update(
function(D)D.theme=C end)end elseif A=='Accent'then local C for D,E in pairs(n.
Accents)do if E==B or D==B then C=D break end end if C then o:Update(function(D)
D.accent=C end)end else rawset(z,A,B)end end})return z end function q.
CreateWindow(w,x)x=x or{}o:Update(function(y)y.title=x.Title or x.Name or
'Selene UI'y.subtitle=x.Subtitle or''y.titleTag=x.TitleTag y.titleTagColor=x.
TitleTagColor y.subtitleTag=x.SubtitleTag y.subtitleTagColor=x.SubtitleTagColor
y.searching=x.Searching~=false y.draggable=x.Draggable~=false y.canExit=x.
CanExit~=false y.canMinimize=x.CanMinimize~=false y.canZoom=x.CanZoom~=false y.
minimized=x.Minimized==true y.maximized=x.Maximized==true y.sections={}y.
activeTab=nil y.visible=true y.profile=nil end)if not v then local y=
getGuiParent()if not y then error
'Cannot find Gui parent (CoreGui or PlayerGui).'end v=Instance.new'ScreenGui'v.
Name='SeleneUILibrary'v.ResetOnSpawn=false v.ZIndexBehavior=Enum.ZIndexBehavior.
Sibling v.Parent=y end local y=x.ToggleKey or Enum.KeyCode.RightControl local z=
game:GetService'UserInputService'z.InputBegan:Connect(function(A,B)if B then
return end if A.KeyCode==y then o:Update(function(C)C.visible=not C.visible end)
end end)if not u then local function App()local A,B=l.useState(o._state)l.
useEffect(function()local C=o:Subscribe(function(C)local D=n.Themes[C.theme or
'Dark']for E,F in pairs(D)do n.Colors[E]=F end n.Colors.Accent=n.Accents[C.
accent or'Blue']B(t(C))end)return C end,{})local C=i local D=j(C.Components.
Window)local E=j(C.Components.Notification)return l.createElement('Frame',{Size=
UDim2.new(1,0,1,0),BackgroundTransparency=1},{Window=A.visible and l.
createElement(D,{state=A,Store=o})or nil,Notifications=l.createElement(E.List,{
notifications=A.notifications,Store=o})})end u=m.createRoot(v)u:render(l.
createElement(App))end local A={}local B=Instance.new'BindableEvent'A.Destroying
=B.Event setmetatable(A,{__index=function(C,D)if D=='Minimized'then return o.
_state.minimized elseif D=='Maximized'then return o._state.maximized elseif D==
'Visible'then return o._state.visible else return rawget(C,D)end end,__newindex=
function(C,D,E)if D=='Minimized'then o:Update(function(F)F.minimized=E end)
elseif D=='Maximized'then o:Update(function(F)F.maximized=E end)elseif D==
'Visible'then o:Update(function(F)F.visible=E end)else rawset(C,D,E)end end})
function A.SetProfile(C,D)o:Update(function(E)E.profile={username=D.Username or
'User',subtext=D.Subtext or'Developer',userId=D.UserId or 1}end)end function A.
Section(C,D)D=D or{}local E=#o._state.sections+1 local F={id=E,title=D.Title or
'',disclosure=D.Disclosure==true,tabs={}}o:Update(function(G)table.insert(G.
sections,F)end)local G={}function G.Tab(H,I)I=I or{}local J=math.random(1,100000
)local K={id=J,title=I.Title or'Tab',icon=I.Icon,pageSections={}}o:Update(
function(L)for M,N in ipairs(L.sections)do if N.id==E then table.insert(N.tabs,K
)break end end table.insert(L.tabs,K)if not L.activeTab then L.activeTab=J end
end)local function buildTabObj(L,M)local N={}function N.SetBanner(O,P)o:Update(
function(Q)for R,S in ipairs(Q.sections)do for T,U in ipairs(S.tabs)do if U.id==
L then U.banner={username=P.Username or'User',subtext=P.Subtext or'',userId=P.
UserId or 1}break end end end end)end function N.Navigate(O,P)o:Update(function(
Q)for R,S in ipairs(Q.sections)do for T,U in ipairs(S.tabs)do if U.id==L then U.
pageSections=P.elements break end end end end)end function N.Form(O)local P={id=
math.random(1,100000),type='Form',elements={}}o:Update(function(Q)for R,S in
ipairs(Q.sections)do for T,U in ipairs(S.tabs)do if U.id==L then table.insert(U.
pageSections,P)break end end end end)setupContainerMethods(P,function(Q)table.
insert(P.elements,Q)o:Update(function(R)end)end)return P end function N.
PageSection(O,P)P=P or{}local Q=math.random(1,100000)local R={id=Q,type=
'Section',name=P.Title,description=P.Subtitle,column=P.Column or 1,
gradientPreset=P.GradientPreset,gradient=P.Gradient,elements={}}o:Update(
function(S)for T,U in ipairs(S.sections)do for V,W in ipairs(U.tabs)do if W.id==
L then table.insert(W.pageSections,R)break end end end end)local S={}function S.
Form(T)local U={id=math.random(1,100000),type='Form',elements={}}table.insert(R.
elements,U)setupContainerMethods(U,function(V)table.insert(U.elements,V)o:
Update(function(W)end)end)return U end return S end function N.Tab(O,P)P=P or{}
local Q=math.random(1,100000)local R={id=Q,title=P.Title or'Sub Tab',icon=P.Icon
,parentTabId=L,pageSections={}}o:Update(function(S)for T,U in ipairs(S.sections)
do if U.id==M then local V for W,X in ipairs(U.tabs)do if X.id==L then V=W break
end end if V then table.insert(U.tabs,V+1,R)else table.insert(U.tabs,R)end break
end end table.insert(S.tabs,R)end)return buildTabObj(Q,M)end return N end return
buildTabObj(J,E)end return G end function A.Destroy(C)B:Fire()if u then u:
unmount()u=nil end if v then v:Destroy()v=nil end end return A end function q.
Notification(w,x)x=x or{}local y=x.Title or'Notification'local z=x.Subtitle or''
local A=x.AppIcon or x.Icon local B=x.Duration or 5 local C local D=os.clock()o:
Update(function(E)local F for G,H in ipairs(E.notifications)do if H.title==y and
H.subtitle==z then F=H break end end if F then C=F.id F.count=(F.count or 1)+1 F
.spawnTime=D F.duration=B else C=math.random(1,100000)local G={id=C,title=y,
subtitle=z,icon=A,duration=B,count=1,spawnTime=D}table.insert(E.notifications,G)
end E.notifications=t(E.notifications)end)task.spawn(function()task.wait(B)o:
Update(function(E)local F for G,H in ipairs(E.notifications)do if H.id==C then
if os.clock()>=(H.spawnTime or 0)+(H.duration or 5)-0.05 then F=G end break end
end if F then table.remove(E.notifications,F)E.notifications=t(E.notifications)
end end)end)end function q.Page(w)local x={}x.elements={}function x.Form(y)local
z={id=math.random(1,100000),type='Form',elements={}}table.insert(x.elements,z)
setupContainerMethods(z,function(A)table.insert(z.elements,A)o:Update(function(B
)end)end)return z end return x end return q end d.Store=function()
createVirtualModule'Store'createVirtualFolder''local i={_state={title=
'Selene UI',subtitle='v1.0.0',tabs={},activeTab=nil,visible=true,profile=nil,
theme='Dark',accent='Blue',notifications={},searchQuery='',titleTag=nil,
titleTagColor=nil,subtitleTag=nil,subtitleTagColor=nil,searching=true,canExit=
true,canMinimize=true,canZoom=true,minimized=false,maximized=false,
colorPickerActive=nil},_listeners={}}local j=table.find or function(j,k)for l,m
in ipairs(j)do if m==k then return l end end return nil end function i.Subscribe
(k,l)table.insert(k._listeners,l)return function()local m=j(k._listeners,l)if m
then table.remove(k._listeners,m)end end end function i.Update(k,l)l(k._state)
for m,n in ipairs(k._listeners)do n(k._state)end end function i.UpdateElement(k,
l,m)k:Update(function(n)local function searchElements(o,p,q)for r,s in ipairs(o)
do if s.id==p then q(s)return true end if s.elements and searchElements(s.
elements,p,q)then return true end if s.leftElements and searchElements(s.
leftElements,p,q)then return true end if s.rightElements and searchElements(s.
rightElements,p,q)then return true end end return false end for o,p in ipairs(n.
sections or{})do for q,r in ipairs(p.tabs or{})do if r.pageSections and
searchElements(r.pageSections,l,m)then return end end end end)end function i.
SetColorPicker(k,l)k:Update(function(m)m.colorPickerActive=l end)end return i
end d.Theme=function()createVirtualModule'Theme'createVirtualFolder''local i={
Themes={Dark={Background=Color3.fromRGB(13,15,19),Sidebar=Color3.fromRGB(9,10,13
),Card=Color3.fromRGB(20,23,29),Element=Color3.fromRGB(28,32,40),ElementHover=
Color3.fromRGB(36,41,51),Text=Color3.fromRGB(255,255,255),TextSecondary=Color3.
fromRGB(156,163,175),Border=Color3.fromRGB(38,43,54),BorderHover=Color3.fromRGB(
50,56,70)},Light={Background=Color3.fromRGB(243,244,246),Sidebar=Color3.fromRGB(
229,231,235),Card=Color3.fromRGB(255,255,255),Element=Color3.fromRGB(243,244,246
),ElementHover=Color3.fromRGB(209,213,219),Text=Color3.fromRGB(17,24,39),
TextSecondary=Color3.fromRGB(107,114,128),Border=Color3.fromRGB(209,213,219),
BorderHover=Color3.fromRGB(156,163,175)}},Accents={Blue=Color3.fromRGB(59,130,
246),Yellow=Color3.fromRGB(234,179,8),Green=Color3.fromRGB(34,197,94),Graphite=
Color3.fromRGB(107,114,128)},Colors={Background=Color3.fromRGB(13,15,19),Sidebar
=Color3.fromRGB(9,10,13),Card=Color3.fromRGB(20,23,29),Element=Color3.fromRGB(28
,32,40),ElementHover=Color3.fromRGB(36,41,51),Text=Color3.fromRGB(255,255,255),
TextSecondary=Color3.fromRGB(156,163,175),Accent=Color3.fromRGB(59,130,246),
Success=Color3.fromRGB(46,204,113),Border=Color3.fromRGB(38,43,54),BorderHover=
Color3.fromRGB(50,56,70)},Fonts={Bold=Enum.Font.GothamBold,Medium=Enum.Font.
GothamMedium,Regular=Enum.Font.Gotham},CornerRadius={Window=UDim.new(0,10),Card=
UDim.new(0,8),Element=UDim.new(0,6),Circle=UDim.new(1,0)}}return i end return
__require(createVirtualModule'init')