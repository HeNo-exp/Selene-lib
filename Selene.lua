local a = game
local b = a:GetService'ReplicatedStorage'
local c = require
local d = {}
local e = {}
local f = {}
...
local function createVirtualFolder(h)
    if g[h] then
        return g[h]
    end
    ...
    setmetatable(i, {
        __index = function(j, k)
            if k == 'Parent' then
                ...
                for m in string.gmatch(h, '[^/]+')do
                    table.insert(l, m)
                end

                table.remove(l)

                return createVirtualFolder(table.concat(l, '/'))
            ...
            elseif k == 'IsA' then
                return function(l, m)
                    return m == 'Folder' or m == 'ModuleScript' or m == 'Instance'
                end
            end

            local l = h == '' and k or (h .. '/' .. k)

            if d[l] then
                return createVirtualModule(l)
            else
                return createVirtualFolder(l)
            end
        end,
    })
...
local function createVirtualModule(h)
    if f[h] then
        return f[h]
    end
    ...
    setmetatable(i, {
        __index = function(j, k)
            ...
            elseif k == 'IsA' then
                return function(l, m)
                    return m == 'ModuleScript' or m == 'Instance'
                end
            end
...
local function getNativePackage(h)
    if h == 'React' then
        return __require(createVirtualModule'React')
    elseif h == 'ReactRoblox' then
        return __require(createVirtualModule'ReactRoblox')
    end

    error("[Selene Library] Package '" .. tostring(h) .. "' not found.")
end
local function createPkgProxy(h)
    return {
        __isPkgProxy = true,
        __pkgName = h,
    }
end
local function __require(h)
    if type(h) == 'table' and h.__isModule then
        local i = h.__path
        ...
        if not j then
            error('[Selene Library] Module not found: ' .. tostring(i))
        end
        ...
        return k
    elseif type(h) == 'table' and h.__isPkgProxy then
        return getNativePackage(h.__pkgName)
    end
...
local i = setmetatable({}, {
    __index = function(i, j)
        if j == 'Packages' then
            return h
        elseif j == 'FindFirstChild' or j == 'WaitForChild' then
            return function(k, l, m)
                if l == 'Packages' then
                    return h
                end

                return b:FindFirstChild(l)
            end
        end
...
local j = setmetatable({}, {
    __index = function(j, k)
        if k == 'GetService' then
            return function(l, m)
                if m == 'ReplicatedStorage' then
                    return i
                end

                return a:GetService(m)
            end
        end
...
d.AppRecorder = function()
    local k = createVirtualModule'AppRecorder'

    createVirtualFolder''

    local l = __require
    local m = j
    local n = m:GetService'HttpService'
    local o = {}
    ...
    function o.Dump(p)
        local q = l(k.Parent.Store)
        local r = q._state
        local s = {}

        local function traverse(t, u)
            for v, w in ipairs(t)do
                if w.value ~= nil and w.name then
                    s[u] = s[u] or {}
                ...
                if w.elements then
                    traverse(w.elements, u)
                end
                if w.leftElements then
                    traverse(w.leftElements, u)
                end
                if w.rightElements then
                    traverse(w.rightElements, u)
                end
            end
        end

        for t, u in ipairs(r.sections or {})do
            for v, w in ipairs(u.tabs or {})do
                if w.pageSections then
                    traverse(w.pageSections, w.title or 'Tab')
                end
            end
        end

        return n:JSONEncode(s)
    end
    function o.Load(p, q)
        local r, s = pcall(function()
            return n:JSONDecode(q)
        end)

        if not r or type(s) ~= 'table' then
            return
        end

        local t = l(k.Parent.Store)

        local function deserializeValue(u)
            if type(u) == 'table' and u._type == 'Color3' then
                return Color3.new(u.R, u.G, u.B)
            elseif type(u) == 'table' and u._type == 'EnumItem' then
                local v = u.EnumType
                local w = u.Name
                local x = Enum[v]
        ...
        t:Update(function(u)
            local function traverseAndLoad(v, w)
                for x, y in ipairs(v)do
                    local z = s[w]

                    if z and y.name and z[y.name] ~= nil then
                        y.value = deserializeValue(z[y.name])
                    end
                    if y.elements then
                        traverseAndLoad(y.elements, w)
                    end
                    if y.leftElements then
                        traverseAndLoad(y.leftElements, w)
                    end
                    if y.rightElements then
                        traverseAndLoad(y.rightElements, w)
                    end
                end
            end

            for v, w in ipairs(u.sections or {})do
                for x, y in ipairs(w.tabs or {})do
                    if y.pageSections then
                        traverseAndLoad(y.pageSections, y.title or 'Tab')
                    end
                end
            end
        end)
...
d['Components/BarChart'] = function()
    local k = createVirtualModule'Components/BarChart'

    createVirtualFolder''

    local l = __require
    local m = j
    local n = m:GetService'ReplicatedStorage'
    local o = l(n.Packages.React)
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = m:GetService'TweenService'
    local s = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local t = o.Component:extend'AnimatedBar'

    function t.init(u)
        u.ref = o.createRef()
    end
    function t.didMount(u)
        local v = u.ref.current

        if v then
            v.Size = u.props.targetSize
        end
    end
    function t.didUpdate(u, v)
        local w = u.ref.current

        if w and v.targetSize ~= u.props.targetSize then
            w.Size = v.targetSize

            r:Create(w, s, {
                Size = u.props.targetSize,
            }):Play()
        end
    end
    function t.render(u)
    ...
    local u = o.Component:extend'BarChart'

    function u.render(v)
        local w = v.props.element
        local x = w.title or 'Bar Chart'
        local y = w.subtitle
        local z = w.height or 180
        local A = w.data or {}
        local B = #A
        local C = 0
        ...
        local E = 12
        local F = {
            Layout = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Horizontal,
                Padding = UDim.new(0, E),
        ...
        for G, H in ipairs(A)do
            local I = 0
            local J = q.Colors.Accent

            if type(H) == 'table' then
                I = H.value or H.Value or 0
                J = H.color or H.Color or q.Colors.Accent
            elseif type(H) == 'number' then
                I = H
            end

            local K = math.clamp(I / C, 0.05, 1)
            local L = 1 / B
            local M = -E * (1 - L)

            F['BarCol_' .. tostring(G)] = o.createElement('Frame', {
                Size = UDim2.new(L, M, 1, 0),
                BackgroundTransparency = 1,
                LayoutOrder = G,
            }, {
                Bar = o.createElement(t, {
                    targetSize = UDim2.new(0.6, 0, K * 0.85, 0),
                    color = J,
                }, {
                    Corner = o.createElement('UICorner', {
                        CornerRadius = q.CornerRadius.Element,
                    }),
                    ValLabel = o.createElement('TextLabel', {
                        Size = UDim2.new(1.5, 0, 0, 14),
                        Position = UDim2.new(0.5, 0, 0, -16),
                        AnchorPoint = Vector2.new(0.5, 0),
                        BackgroundTransparency = 1,
                        Text = tostring(I),
                        TextColor3 = q.Colors.Text,
                        Font = q.Fonts.Bold,
                        TextSize = 9,
                    }),
                }),
        ...
        for H, I in ipairs(A)do
            local J = ''

            if type(I) == 'table' then
                J = I.label or I.Label or ''
            else
                J = tostring(H)
            end
            ...
            G['Label_' .. tostring(H)] = o.createElement('TextLabel', {
                Size = UDim2.new(K, L, 1, 0),
                BackgroundTransparency = 1,
                Text = J,
                TextColor3 = q.Colors.TextSecondary,
                Font = q.Fonts.Medium,
                TextSize = 9,
                LayoutOrder = H,
            })
        ...
        return o.createElement('Frame', {
            ...
            BackgroundColor3 = q.Colors.Element,
            BorderSizePixel = 0,
        }, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Card,
            }),
            Stroke = o.createElement('UIStroke', {
                Color = q.Colors.Border,
                Thickness = 1,
            }),
            Padding = o.createElement('UIPadding', {
                PaddingTop = UDim.new(0, 10),
                PaddingBottom = UDim.new(0, 10),
                PaddingLeft = UDim.new(0, 12),
                PaddingRight = UDim.new(0, 12),
            }),
            List = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                Padding = UDim.new(0, 10),
                SortOrder = Enum.SortOrder.LayoutOrder,
            }),
            Header = (x or y) and o.createElement('Frame', {
                ...
                TitleLabel = x and o.createElement('TextLabel', {
                ...
                SubLabel = y and o.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 12),
                    BackgroundTransparency = 1,
                    Text = y,
                    TextColor3 = q.Colors.TextSecondary,
                    Font = q.Fonts.Regular,
                    TextSize = 10,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    LayoutOrder = 2,
                }),
            }),
            Canvas = o.createElement('Frame', {
                Size = UDim2.new(1, 0, 1, -D),
                BackgroundTransparency = 1,
                LayoutOrder = 2,
            }, {
                YAxis = o.createElement('Frame', {
                    Name = 'YAxis',
                    Size = UDim2.new(0, 24, 1, -16),
                    BackgroundTransparency = 1,
                }, {
                    Max = o.createElement('TextLabel', {
                    ...
                    Mid = o.createElement('TextLabel', {
                        Size = UDim2.new(1, 0, 0, 10),
                        Position = UDim2.fromScale(0, 0.575),
                        AnchorPoint = Vector2.new(0, 0.5),
                        BackgroundTransparency = 1,
                        Text = tostring(math.round(C / 2)),
                        TextColor3 = q.Colors.TextSecondary,
                        Font = q.Fonts.Medium,
                        TextSize = 8,
                        TextXAlignment = Enum.TextXAlignment.Left,
                    }),
                    Min = o.createElement('TextLabel', {
                        Size = UDim2.new(1, 0, 0, 10),
                ...
                PlotContainer = o.createElement('Frame', {
                    Name = 'PlotContainer',
                    Size = UDim2.new(1, -28, 1, 0),
                    Position = UDim2.new(0, 28, 0, 0),
                    BackgroundTransparency = 1,
                }, {
                    GridLine_Top = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 0, 1),
                        Position = UDim2.fromScale(0, 0.15),
                        BackgroundColor3 = q.Colors.Border,
                        BackgroundTransparency = 0.6,
                        BorderSizePixel = 0,
                    }),
                    GridLine_Mid = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 0, 1),
                        Position = UDim2.fromScale(0, 0.575),
                        BackgroundColor3 = q.Colors.Border,
                        BackgroundTransparency = 0.6,
                    ...
                    GridLine_Bottom = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 0, 1),
                        Position = UDim2.fromScale(0, 1),
                        BackgroundColor3 = q.Colors.Border,
                        BackgroundTransparency = 0.6,
                    ...
                    YAxisLine = o.createElement('Frame', {
                        Size = UDim2.new(0, 1, 1, -16),
                        Position = UDim2.fromScale(0, 0),
                        BackgroundColor3 = q.Colors.Border,
                        BorderSizePixel = 0,
                    }),
                    BarArea = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 1, -16),
                        BackgroundTransparency = 1,
                    }, F),
                    LabelsArea = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 0, 16),
...
d['Components/Button'] = function()
    local k = createVirtualModule'Components/Button'
    ...
    local m = j
    local n = m:GetService'ReplicatedStorage'
    local o = m:GetService'TweenService'
    local p = l(n.Packages.React)
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = p.Component:extend'Button'
    ...
    function s.init(t)
        ...
        t.btnRef = p.createRef()
        t.strokeRef = p.createRef()
    end
    function s.didMount(t)
        local u = t.btnRef.current
        local v = t.strokeRef.current
        local w = t.state.hovered
        local x = t.state.pressed

        if u then
            u.BackgroundColor3 = x and r.Colors.Accent or (w and r.Colors.ElementHover or r.Colors.Element)
        end
        if v then
            v.Color = w and r.Colors.BorderHover or r.Colors.Border
        end
    end
    function s.didUpdate(t, u, v)
        local w = t.btnRef.current
        local x = t.strokeRef.current
        local y = t.state.hovered
        local z = t.state.pressed
        local A = (y ~= v.hovered) or (z ~= v.pressed)

        if A then
            local B = z and r.Colors.Accent or (y and r.Colors.ElementHover or r.Colors.Element)
            local C = y and r.Colors.BorderHover or r.Colors.Border
            local D = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

            if w then
                o:Create(w, D, {BackgroundColor3 = B}):Play()
            end
            if x then
                o:Create(x, D, {Color = C}):Play()
            end
        end
    end
    function s.render(t)
        local u = t.props.element
        local v = t.state.hovered
        local w = t.state.pressed

        return p.createElement('TextButton', {
            ref = t.btnRef,
            Size = UDim2.new(1, 0, 0, 36),
            BackgroundColor3 = w and r.Colors.Accent or (v and r.Colors.ElementHover or r.Colors.Element),
            ...
            [p.Event.MouseButton1Down] = function()
                t:setState{pressed = true}
            end,
            [p.Event.MouseButton1Up] = function()
                if w then
                    t:setState{pressed = false}

                    if u.callback then
                        task.spawn(u.callback)
                    end
                end
            end,
        }, {
            Corner = p.createElement('UICorner', {
                CornerRadius = r.CornerRadius.Element,
            }),
            Stroke = p.createElement('UIStroke', {
                ref = t.strokeRef,
                Color = v and r.Colors.BorderHover or r.Colors.Border,
                Thickness = 1,
            }),
            Content = p.createElement('Frame', {
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
            }, {
                List = p.createElement('UIListLayout', {
                    FillDirection = Enum.FillDirection.Horizontal,
                    HorizontalAlignment = Enum.HorizontalAlignment.Center,
                    VerticalAlignment = Enum.VerticalAlignment.Center,
                    Padding = UDim.new(0, 8),
                }),
                Icon = u.icon and p.createElement('ImageLabel', {
                    Size = UDim2.new(0, 16, 0, 16),
                    BackgroundTransparency = 1,
                    Image = u.icon,
                    ImageColor3 = r.Colors.Text,
                }) or nil,
                Label = p.createElement('TextLabel', {
                    Size = UDim2.new(0, 0, 1, 0),
                    AutomaticSize = Enum.AutomaticSize.X,
                    BackgroundTransparency = 1,
                    Text = u.name or 'Button',
                    TextColor3 = r.Colors.Text,
                    Font = r.Fonts.Bold,
                    TextSize = 13,
                }),
            }),
...
d['Components/ColorPicker'] = function()
    local k = createVirtualModule'Components/ColorPicker'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'ColorPicker'

    function r.init(s)
        s.state = {hovered = false}
    end
    function r.render(s)
        local t = s.props.element
        local u = s.props.Store
        local v = t.value or Color3.fromRGB(255, 255, 255)
        local w = t.transparency or 0
        local x = s.state.hovered
        local y = math.round(v.R * 255)
        local z = math.round(v.G * 255)
        local A = math.round(v.B * 255)
        local B = math.round((1 - w) * 100)
        local C = string.format('#%02X%02X%02X (%d%%)', y, z, A, B)
        ...
        return o.createElement('Frame', {
            ...
            HexLabel = o.createElement('TextLabel', {
            ...
            ColorButton = o.createElement('TextButton', {
                ...
                AutoButtonColor = false,
                LayoutOrder = 2,
                [o.Event.MouseEnter] = function()
                    s:setState{hovered = true}
                end,
                [o.Event.MouseLeave] = function()
                    s:setState{hovered = false}
                end,
                [o.Event.MouseButton1Click] = function()
                    u:SetColorPicker{
                        id = t.id,
                        name = t.name or 'Select Color',
                        value = v,
                        transparency = w,
                        callback = t.callback,
                    }
                ...
                Stroke = o.createElement('UIStroke', {
                    Color = x and q.Colors.BorderHover or q.Colors.Border,
                    Thickness = 1.5,
                }),
            }),
        })
...
d['Components/ColorPickerDialog'] = function()
    local k = createVirtualModule'Components/ColorPickerDialog'
    ...
    local n = m:GetService'ReplicatedStorage'
    local o = l(n.Packages.React)
    local p = m:GetService'UserInputService'
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = o.Component:extend'ColorPickerDialog'

    function s.init(t)
        local u = t.props.colorPickerActive
        local v = u.value or Color3.fromRGB(255, 255, 255)
        local w = u.transparency or 0
        local x, y, z = v:ToHSV()
        ...
        t.wheelRef = o.createRef()
        t.brightnessRef = o.createRef()
        t.opacityRef = o.createRef()
    end
    function s.didMount(t)
        t.inputEndedConnection = p.InputEnded:Connect(function(u)
            if u.UserInputType == Enum.UserInputType.MouseButton1 or u.UserInputType == Enum.UserInputType.Touch then
                t.isDraggingWheel = false
                t.isDraggingBrightness = false
                t.isDraggingOpacity = false
            end
        end)
    end
    function s.willUnmount(t)
        if t.inputEndedConnection then
            t.inputEndedConnection:Disconnect()

            t.inputEndedConnection = nil
        end
    end
    function s.render(t)
        local u = t.props
        local v = u.colorPickerActive
        local w = u.Store
        local x = t.state.h
        local y = t.state.s
        local z = t.state.v
        local A = t.state.a
        local B = v.value or Color3.fromRGB(255, 255, 255)
        local C = v.transparency or 0
        local D = Color3.fromHSV(x, y, z)
        local E = 1 - A
        local F = math.round(D.R * 255)
        local G = math.round(D.G * 255)
        local H = math.round(D.B * 255)
        local I = math.round(A * 100)
        local J = string.format('#%02X%02X%02X', F, G, H)

        local function updateWheel(K)
            local L = t.wheelRef.current

            if not L then
                return
            end

            local M = L.AbsolutePosition
            local N = L.AbsoluteSize
            local O = M + N / 2
            local P = Vector2.new(K.Position.X, K.Position.Y)
            local Q = P - O
            local R = Q.Magnitude
            local S = N.X / 2
            local T = math.clamp(R / S, 0, 1)
            local U = math.atan2(-Q.Y, Q.X)
            local V = (U + math.pi) / (2 * math.pi)
        ...
        local function updateBrightness(K)
            local L = t.brightnessRef.current
            ...
            local M = L.AbsolutePosition
            local N = L.AbsoluteSize
            local O = math.clamp((K.Position.X - M.X) / N.X, 0, 1)

            t:setState{v = O}
        end
        local function updateOpacity(K)
            local L = t.opacityRef.current
        ...
        local function onWheelInput(K, L)
            if L.UserInputType == Enum.UserInputType.MouseButton1 or L.UserInputType == Enum.UserInputType.Touch then
                t.isDraggingWheel = true

                updateWheel(L)
            end
        end
        local function onWheelChange(K, L)
            if t.isDraggingWheel and (L.UserInputType == Enum.UserInputType.MouseMovement or L.UserInputType == Enum.UserInputType.Touch) then
                updateWheel(L)
            end
        end
        local function onBrightnessInput(K, L)
            if L.UserInputType == Enum.UserInputType.MouseButton1 or L.UserInputType == Enum.UserInputType.Touch then
                t.isDraggingBrightness = true

                updateBrightness(L)
            end
        end
        local function onBrightnessChange(K, L)
            if t.isDraggingBrightness and (L.UserInputType == Enum.UserInputType.MouseMovement or L.UserInputType == Enum.UserInputType.Touch) then
                updateBrightness(L)
            end
        end
        local function onOpacityInput(K, L)
            if L.UserInputType == Enum.UserInputType.MouseButton1 or L.UserInputType == Enum.UserInputType.Touch then
                t.isDraggingOpacity = true

                updateOpacity(L)
            end
        end
        local function onOpacityChange(K, L)
            if t.isDraggingOpacity and (L.UserInputType == Enum.UserInputType.MouseMovement or L.UserInputType == Enum.UserInputType.Touch) then
                updateOpacity(L)
            end
        end
        local function setHex(K)
            K = K:gsub('#', '')

            if #K == 6 then
                local L = tonumber(K:sub(1, 2), 16)
                local M = tonumber(K:sub(3, 4), 16)
                local N = tonumber(K:sub(5, 6), 16)

                if L and M and N then
                    local O = Color3.fromRGB(L, M, N)
                    local P, Q, R = O:ToHSV()
        ...
        local function setRGB(K, L)
            local M = tonumber(L)

            if M then
                M = math.clamp(math.round(M), 0, 255)

                local N = K == 'R' and M or F
                local O = K == 'G' and M or G
                local P = K == 'B' and M or H
                local Q = Color3.fromRGB(N, O, P)
                local R, S, T = Q:ToHSV()
        ...
        local K = (x * 2 * math.pi) - math.pi
        local L = y * 50 * math.cos(K)
        local M = -y * 50 * math.sin(K)
        local N = function(N, O, P, Q)
            return o.createElement('Frame', {
                ...
                Box = o.createElement('TextBox', {
                    Size = UDim2.new(0, 60, 1, 0),
                    BackgroundColor3 = r.Colors.Element,
                    BorderSizePixel = 0,
                    Text = O,
                    TextColor3 = r.Colors.Text,
                    Font = r.Fonts.Medium,
                ...
                Label = o.createElement('TextLabel', {
                    Size = UDim2.new(1, -66, 1, 0),
                    Position = UDim2.new(0, 66, 0, 0),
                    BackgroundTransparency = 1,
                    Text = N,
                    TextColor3 = r.Colors.TextSecondary,
                    Font = r.Fonts.Medium,
                    TextSize = 10,
                    TextXAlignment = Enum.TextXAlignment.Left,
                }),
        ...
        return o.createElement('Frame', {
            ...
            Backdrop = o.createElement('TextButton', {
                Size = UDim2.new(1, 0, 1, 0),
                ...
                AutoButtonColor = false,
                ZIndex = 1,
                [o.Event.MouseButton1Down] = function()
                    w:SetColorPicker(nil)
                end,
            }),
            Card = o.createElement('TextButton', {
                Size = UDim2.new(0, 250, 0, 340),
                Position = UDim2.new(0.5, 0, 0.5, 0),
                AnchorPoint = Vector2.new(0.5, 0.5),
                BackgroundColor3 = r.Colors.Card,
                BorderSizePixel = 0,
                Text = '',
                AutoButtonColor = false,
                Active = true,
                ...
                Corner = o.createElement('UICorner', {
                    CornerRadius = r.CornerRadius.Card,
                }),
                Stroke = o.createElement('UIStroke', {
                    Color = r.Colors.Border,
                    Thickness = 1,
                ...
                Title = o.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 18),
                    BackgroundTransparency = 1,
                    Text = v.name or 'Accent Tint Color',
                    TextColor3 = r.Colors.Text,
                    Font = r.Fonts.Bold,
                    TextSize = 13,
                ...
                MainRow = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 100),
                    BackgroundTransparency = 1,
                    LayoutOrder = 2,
                }, {
                    Wheel = o.createElement('ImageLabel', {
                        ref = t.wheelRef,
                        Size = UDim2.new(0, 100, 0, 100),
                        BackgroundTransparency = 1,
                        Image = 'rbxassetid://6020299385',
                        Active = true,
                        [o.Event.InputBegan] = onWheelInput,
                        [o.Event.InputChanged] = onWheelChange,
                    }, {
                        Corner = o.createElement('UICorner', {
                            CornerRadius = UDim.new(0.5, 0),
                        }),
                        Knob = o.createElement('Frame', {
                            Size = UDim2.new(0, 8, 0, 8),
                            Position = UDim2.new(0.5, L, 0.5, M),
                            AnchorPoint = Vector2.new(0.5, 0.5),
                    ...
                    Fields = o.createElement('Frame', {
                        ...
                        HexRow = N('Hex', J, 1, setHex),
                        RedRow = N('Red', tostring(F), 2, function(O)
                            setRGB('R', O)
                        end),
                        GreenRow = N('Green', tostring(G), 3, function(O)
                            setRGB('G', O)
                        end),
                        BlueRow = N('Blue', tostring(H), 4, function(O)
                            setRGB('B', O)
                        end),
                        AlphaRow = N('Alpha', tostring(I), 5, setAlpha),
                    }),
                }),
                Sliders = o.createElement('Frame', {
                    ...
                    Brightness = o.createElement('Frame', {
                        ...
                        Bar = o.createElement('Frame', {
                            ref = t.brightnessRef,
                            Size = UDim2.new(1, -65, 0, 8),
                            Position = UDim2.new(0, 65, 0.5, -4),
                            BorderSizePixel = 0,
                            Active = true,
                            [o.Event.InputBegan] = onBrightnessInput,
                            [o.Event.InputChanged] = onBrightnessChange,
                        }, {
                            Corner = o.createElement('UICorner', {
                                CornerRadius = UDim.new(0.5, 0),
                            }),
                            Gradient = o.createElement('UIGradient', {
                                Color = ColorSequence.new{
                                    ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 0)),
                                    ColorSequenceKeypoint.new(1, Color3.fromHSV(x, y, 1)),
                                },
                            }),
                            Knob = o.createElement('Frame', {
                                Size = UDim2.new(0, 6, 0, 14),
                                Position = UDim2.new(z, 0, 0.5, 0),
                                AnchorPoint = Vector2.new(0.5, 0.5),
                                BackgroundColor3 = Color3.fromRGB(255, 255, 255),
                                BorderSizePixel = 0,
                            }, {
                    ...
                    Opacity = o.createElement('Frame', {
                        ...
                        Bar = o.createElement('Frame', {
                            ...
                            BorderSizePixel = 0,
                            Active = true,
                            [o.Event.InputBegan] = onOpacityInput,
                            [o.Event.InputChanged] = onOpacityChange,
                        }, {
                            Corner = o.createElement('UICorner', {
                                CornerRadius = UDim.new(0.5, 0),
                            }),
                            Gradient = o.createElement('UIGradient', {
                                Color = ColorSequence.new{
                                    ColorSequenceKeypoint.new(0, Color3.fromHSV(x, y, z)),
                                    ColorSequenceKeypoint.new(1, Color3.fromHSV(x, y, z)),
                                },
                ...
                PreviewRow = o.createElement('Frame', {
                    ...
                    NewColor = o.createElement('Frame', {
                        Size = UDim2.new(0.5, 0, 1, 0),
                        Position = UDim2.new(0, 0, 0, 0),
                        BackgroundColor3 = D,
                        BackgroundTransparency = E,
                        BorderSizePixel = 0,
                    }),
                    Initial = o.createElement('Frame', {
                        Size = UDim2.new(0.5, 0, 1, 0),
                        Position = UDim2.new(0.5, 0, 0, 0),
                        BackgroundColor3 = B,
                        BackgroundTransparency = C,
                        BorderSizePixel = 0,
                    }),
                }),
                ActionRow = o.createElement('Frame', {
                    ...
                    Confirm = o.createElement('TextButton', {
                        Size = UDim2.new(1, 0, 0, 26),
                        BackgroundColor3 = r.Colors.Accent,
                        BorderSizePixel = 0,
                        Text = 'Confirm',
                        ...
                        LayoutOrder = 1,
                        [o.Event.MouseButton1Click] = function()
                            w:UpdateElement(v.id, function(O)
                                O.value = D
                                O.transparency = E
                            end)

                            if v.callback then
                                task.spawn(v.callback, D, E)
                            end
                    ...
                    Cancel = o.createElement('TextButton', {
                        Size = UDim2.new(1, 0, 0, 26),
                        BackgroundColor3 = r.Colors.Element,
                        BorderSizePixel = 0,
                        Text = 'Cancel',
...
d['Components/ColorPickerWidget'] = function()
    local k = createVirtualModule'Components/ColorPickerWidget'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'ColorPickerWidget'

    function r.init(s) end
    function r.render(s)
        local t = s.props.element
        local u = s.props.Store
        local v = t.value or Color3.fromRGB(255, 255, 255)
        ...
        local function triggerCallback(x, y)
            if t.callback then
                task.spawn(t.callback, x, y)
            end
        end
        local function updateColor(x, y, z, A)
            local B = Color3.fromRGB(x, y, z)

            u:UpdateElement(t.id, function(C)
                C.value = B
                C.transparency = A
            end)
            triggerCallback(B, A)
        ...
        local x = math.round(v.R * 255)
        local y = math.round(v.G * 255)
        local z = math.round(v.B * 255)
        local A = math.round(w * 100)
        ...
        return o.createElement('Frame', {
            ...
            Header = o.createElement('Frame', {
                ...
                Title = o.createElement('TextLabel', {
                    Size = UDim2.new(0.5, 0, 1, 0),
                    BackgroundTransparency = 1,
                    Text = t.name or 'Color Picker',
                    TextColor3 = q.Colors.TextSecondary,
                    Font = q.Fonts.Medium,
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                }),
                Info = o.createElement('TextLabel', {
                    Size = UDim2.new(0.5, -24, 1, 0),
                    Position = UDim2.new(1, -24, 0, 0),
                    AnchorPoint = Vector2.new(1, 0),
                    BackgroundTransparency = 1,
                    Text = string.format('#%02X%02X%02X (%d%%)', x, y, z, A),
                    TextColor3 = q.Colors.Text,
                    Font = q.Fonts.Bold,
                    TextSize = 11,
                    TextXAlignment = Enum.TextXAlignment.Right,
                }),
                ColorPreview = o.createElement('Frame', {
                    Size = UDim2.new(0, 16, 0, 16),
            ...
            Red = o.createElement(l(p.Components.Slider), {
                Store = u,
                element = {
                    name = 'Red',
                    min = 0,
            ...
            Green = o.createElement(l(p.Components.Slider), {
                Store = u,
                element = {
                    name = 'Green',
                    min = 0,
            ...
            Blue = o.createElement(l(p.Components.Slider), {
                Store = u,
                element = {
                    name = 'Blue',
                    min = 0,
            ...
            Transparency = o.createElement(l(p.Components.Slider), {
                Store = u,
                element = {
                    name = 'Transparency',
                    min = 0,
...
d['Components/Dropdown'] = function()
    local k = createVirtualModule'Components/Dropdown'
    ...
    local q = l(p.Theme)
    local r = m:GetService'TweenService'
    local s = table.find or function(s, t)
    ...
    local t = table.clone or function(t)
        local u = {}
    ...
    local u = o.Component:extend'DropdownOption'

    function u.init(v)
        v.state = {hovered = false}
    end
    function u.render(v)
        local w = v.props
        local x = w.opt
        local y = w.isSelected
        local z = w.isMulti
        local A = w.LayoutOrder
        local B = v.state.hovered
        local C = (not z and y) and q.Colors.Accent or (B and q.Colors.ElementHover or q.Colors.Element)

        return o.createElement('TextButton', {
            ...
            LayoutOrder = A,
            [o.Event.MouseEnter] = function()
                v:setState{hovered = true}
            end,
            [o.Event.MouseLeave] = function()
                v:setState{hovered = false}
            end,
            [o.Event.MouseButton1Click] = w.OnClick,
        }, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Element,
            }),
            Content = o.createElement('Frame', {
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
            }, {
                List = o.createElement('UIListLayout', {
                ...
                Checkbox = z and o.createElement('Frame', {
                    Size = UDim2.fromOffset(14, 14),
                    BackgroundColor3 = y and q.Colors.Accent or q.Colors.Element,
                    BorderSizePixel = 0,
                    LayoutOrder = 1,
                }, {
                    Corner = o.createElement('UICorner', {
                    ...
                    Stroke = o.createElement('UIStroke', {
                        Color = (y or B) and q.Colors.BorderHover or q.Colors.Border,
                        Thickness = 1,
                    }),
                    Inner = y and o.createElement('Frame', {
                ...
                Label = o.createElement('TextLabel', {
                    Size = UDim2.new(1, z and -22 or 0, 1, 0),
                    BackgroundTransparency = 1,
                    Text = tostring(x),
                    TextColor3 = q.Colors.Text,
                    Font = q.Fonts.Medium,
    ...
    local v = o.Component:extend'Dropdown'
    ...
    function v.init(w)
        ...
        w.strokeRef = o.createRef()
        w.arrowRef = o.createRef()
        w.optionsRef = o.createRef()
    end
    function v.didMount(w)
        local x = w.strokeRef.current
        local y = w.arrowRef.current
        local z = w.optionsRef.current
        local A = w.state.hovered
        local B = w.state.isOpen

        if x then
            x.Color = (A or B) and q.Colors.BorderHover or q.Colors.Border
        end
        if y then
            y.Rotation = B and 180 or 0
        end
        if z then
            local C = w.props.element.options or {}
            local D = #C
            local E = D > 0 and (D * 28 + (D - 1) * 4 + 8) or 0

            z.Size = B and UDim2.new(1, 0, 0, E) or UDim2.new(1, 0, 0, 0)
            z.GroupTransparency = B and 0 or 1
            z.Visible = B
        end
    end
    function v.didUpdate(w, x, y)
        local z = w.strokeRef.current
        local A = w.arrowRef.current
        local B = w.optionsRef.current
        local C = w.state.hovered
        local D = w.state.isOpen
        local E = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

        if C ~= y.hovered or D ~= y.isOpen then
            if z then
                local F = (C or D) and q.Colors.BorderHover or q.Colors.Border

                r:Create(z, E, {Color = F}):Play()
            end
        end
        if D ~= y.isOpen then
            if A then
                r:Create(A, E, {
                    Rotation = D and 180 or 0,
                }):Play()
            end
            if B then
                local F = w.props.element.options or {}
                local G = #F
                local H = G > 0 and (G * 28 + (G - 1) * 4 + 8) or 0
    ...
    function v.render(w)
        local x = w.props.element
        local y = w.props.Store
        local z = x.options or {}
        local A = x.maximum ~= nil and x.maximum > 1
        local B = w.state.hovered
        local C = w.state.isOpen
        local D = x.value

        if A then
            if type(D) ~= 'table' then
                D = type(D) == 'number' and {D} or {}
            end
        else
            D = D or 1
        end
        ...
        for G, H in ipairs(z)do
            ...
            F['Opt_' .. tostring(G)] = o.createElement(u, {
                opt = H,
                isSelected = I,
                isMulti = A,
                LayoutOrder = G,
                OnClick = function()
                    if A then
                        ...
                        y:UpdateElement(x.id, function(L)
                            L.value = J
                        end)

                        if x.callback then
                            task.spawn(x.callback, J)
                        end
                    else
                        y:UpdateElement(x.id, function(J)
                            J.value = G
                        end)
                        w:setState{isOpen = false}

                        if x.callback then
                            task.spawn(x.callback, G)
                        end
                    end
                end,
            })
        ...
        return o.createElement('Frame', {
            ...
            Header = o.createElement('TextButton', {
                Size = UDim2.new(1, 0, 0, 36),
                BackgroundColor3 = q.Colors.Element,
                BorderSizePixel = 0,
                Text = '',
                ...
                LayoutOrder = 1,
                [o.Event.MouseEnter] = function()
                    w:setState{hovered = true}
                end,
                [o.Event.MouseLeave] = function()
                    w:setState{hovered = false}
                end,
                [o.Event.MouseButton1Click] = function()
                    w:setState{
                        isOpen = not C,
                ...
                Stroke = o.createElement('UIStroke', {
                    ref = w.strokeRef,
                    Color = (B or C) and q.Colors.BorderHover or q.Colors.Border,
                    Thickness = 1,
                }),
                Title = not G and o.createElement('TextLabel', {
                    Size = UDim2.new(0.5, 0, 1, 0),
                    Position = UDim2.new(0, 10, 0, 0),
                    BackgroundTransparency = 1,
                    Text = x.name or 'Dropdown',
                    TextColor3 = q.Colors.TextSecondary,
                    Font = q.Fonts.Medium,
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                }) or nil,
                Selected = o.createElement('TextLabel', {
                    Size = G and UDim2.new(1, -40, 1, 0) or UDim2.new(0.5, -30, 1, 0),
                    Position = G and UDim2.new(0, 10, 0, 0) or UDim2.new(1, -30, 0, 0),
                    AnchorPoint = G and Vector2.new(0, 0) or Vector2.new(1, 0),
                    BackgroundTransparency = 1,
                    Text = E,
                    TextColor3 = q.Colors.Text,
                    Font = q.Fonts.Bold,
                    TextSize = 12,
                    TextXAlignment = G and Enum.TextXAlignment.Left or Enum.TextXAlignment.Right,
                    TextTruncate = Enum.TextTruncate.AtEnd,
                }),
                Arrow = o.createElement('ImageLabel', {
                    ref = w.arrowRef,
                    Size = UDim2.new(0, 14, 0, 14),
                    Position = UDim2.new(1, -22, 0.5, -7),
                    BackgroundTransparency = 1,
                    Image = 'rbxassetid://10747383844',
                    ImageColor3 = q.Colors.TextSecondary,
                }),
            }),
            Options = o.createElement('CanvasGroup', {
                ref = w.optionsRef,
                BackgroundTransparency = 1,
                LayoutOrder = 2,
...
d['Components/ElementRenderer'] = function()
    local k = createVirtualModule'Components/ElementRenderer'
    ...
    local n = m:GetService'ReplicatedStorage'
    local o = l(n.Packages.React)
    local p = o.Component:extend'CustomComponentPlaceholder'

    function p.init(q)
        q.frameRef = o.createRef()
    end
    function p.didMount(q)
        local r = q.frameRef.current
        local s = q.props.element

        if r and s.instance then
            s.instance.Parent = r
            s.instance.Size = UDim2.new(1, 0, 1, 0)
            s.instance.Position = UDim2.new(0, 0, 0, 0)
        end
    end
    function p.didUpdate(q, r)
        local s = q.frameRef.current
        local t = q.props.element
        local u = r.element

        if u and u.instance and u.instance ~= t.instance then
            u.instance.Parent = nil
        end
        if s and t.instance then
            t.instance.Parent = s
            t.instance.Size = UDim2.new(1, 0, 1, 0)
            t.instance.Position = UDim2.new(0, 0, 0, 0)
        end
    end
    function p.willUnmount(q)
        local r = q.props.element

        if r.instance then
            r.instance.Parent = nil
        end
    end
    function p.render(q)
        local r = q.props.element
        local s = r.instance and r.instance.Size or UDim2.new(1, 0, 0, 70)
    ...
    function q.render(r, s)
        local t = k.Parent.Parent
        local u = r.type

        if u == 'Button' then
            return o.createElement(l(t.Components.Button), {
                element = r,
                Store = s,
            })
        elseif u == 'Toggle' then
            return o.createElement(l(t.Components.Toggle), {
                element = r,
                Store = s,
            })
        elseif u == 'Slider' then
            return o.createElement(l(t.Components.Slider), {
                element = r,
                Store = s,
            })
        elseif u == 'Dropdown' then
            return o.createElement(l(t.Components.Dropdown), {
                element = r,
                Store = s,
            })
        elseif u == 'TextBox' then
            return o.createElement(l(t.Components.TextBox), {
                element = r,
                Store = s,
            })
        elseif u == 'Label' then
            return o.createElement(l(t.Components.Label), {
                element = r,
                Store = s,
            })
        elseif u == 'Paragraph' then
            return o.createElement(l(t.Components.Paragraph), {
                element = r,
                Store = s,
            })
        elseif u == 'InfoGrid' then
            return o.createElement(l(t.Components.InfoGrid), {
                element = r,
                Store = s,
            })
        elseif u == 'VStack' then
            return o.createElement(l(t.Components.VStack), {
                element = r,
                Store = s,
            })
        elseif u == 'HStack' then
            return o.createElement(l(t.Components.HStack), {
                element = r,
                Store = s,
            })
        elseif u == 'ColorPicker' then
            return o.createElement(l(t.Components.ColorPicker), {
                element = r,
                Store = s,
            })
        elseif u == 'ColorPickerWidget' then
            return o.createElement(l(t.Components.ColorPickerWidget), {
                element = r,
                Store = s,
            })
        elseif u == 'KeybindField' then
            return o.createElement(l(t.Components.KeybindField), {
                element = r,
                Store = s,
            })
        elseif u == 'Stepper' then
            return o.createElement(l(t.Components.Stepper), {
                element = r,
                Store = s,
            })
        elseif u == 'RadioButtonGroup' then
            return o.createElement(l(t.Components.RadioButtonGroup), {
                element = r,
                Store = s,
            })
        elseif u == 'BarChart' then
            return o.createElement(l(t.Components.BarChart), {
                element = r,
                Store = s,
            })
        elseif u == 'LineChart' then
            return o.createElement(l(t.Components.LineChart), {
                element = r,
                Store = s,
            })
        elseif u == 'PieChart' then
            return o.createElement(l(t.Components.PieChart), {
                element = r,
                Store = s,
            })
        elseif u == 'PullDownButton' then
            return o.createElement(l(t.Components.PullDownButton), {
                element = r,
                Store = s,
            })
        elseif u == 'TitleStack' then
            return o.createElement(l(t.Components.TitleStack), {
                element = r,
                Store = s,
            })
        elseif u == 'Row' then
            return o.createElement(l(t.Components.Row), {
                element = r,
                Store = s,
            })
        elseif u == 'Form' then
            return o.createElement(l(t.Components.Form), {
                element = r,
                Store = s,
            })
        elseif u == 'Section' then
            return o.createElement(l(t.Components.Section), {
                section = r,
                Store = s,
            })
        elseif u == 'CustomComponentPlaceholder' then
            return o.createElement(p, {element = r})
        end
...
d['Components/Form'] = function()
    local k = createVirtualModule'Components/Form'
    ...
    local o = l(n.Packages.React)
    local p = k.Parent.Parent
    local q = l(p.Components.ElementRenderer)
    local r = o.Component:extend'Form'
    ...
    function r.render(s)
        ...
        for w, x in ipairs(t.elements or {})do
            local y = q.render(x, u)

            if y then
                v['Row_' .. x.id] = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = w,
...
d['Components/HStack'] = function()
    local k = createVirtualModule'Components/HStack'
    ...
    local p = k.Parent.Parent
    local q = l(p.Components.ElementRenderer)
    local r = o.Component:extend'HStack'

    function r.render(s)
        local t = s.props.element
        local u = s.props.Store
        local v = #t.elements
        local w = t.padding and t.padding.Offset or 6
        local x = {
            Layout = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Horizontal,
                Padding = UDim.new(0, w),
                HorizontalAlignment = t.horizontalAlignment or Enum.HorizontalAlignment.Left,
                VerticalAlignment = t.verticalAlignment or Enum.VerticalAlignment.Center,
                SortOrder = Enum.SortOrder.LayoutOrder,
            }),
        }

        for y, z in ipairs(t.elements or {})do
            local A = q.render(z, u)

            if A then
                local B = 1 / v
                local C = -w * (1 - B)

                x['El_' .. z.id] = o.createElement('Frame', {
                    Size = UDim2.new(B, C, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = y,
                }, {Content = A})
...
d['Components/InfoGrid'] = function()
    local k = createVirtualModule'Components/InfoGrid'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'InfoGridBlock'
    ...
    function r.render(s)
        ...
        local u = t.block
        local v = t.scale
        local w = t.widthOffset
        local x = t.LayoutOrder
        local y = s.state.hovered
        local z = u.callback ~= nil
        local A = (z and y) and q.Colors.ElementHover or q.Colors.Element
        local B = (z and y) and q.Colors.BorderHover or q.Colors.Border
        local C = {
            Size = UDim2.new(v, w, 1, 0),
            BackgroundColor3 = A,
            BorderSizePixel = 0,
            LayoutOrder = x,
        }
        ...
        if z then
            ...
            C[o.Event.MouseEnter] = function()
                s:setState{hovered = true}
            end
            C[o.Event.MouseLeave] = function()
                s:setState{hovered = false}
            end
            C[o.Event.MouseButton1Click] = function()
                if u.callback then
                    task.spawn(u.callback)
                end
            end
        ...
        return o.createElement(z and 'TextButton' or 'Frame', C, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Element,
            }),
            Stroke = o.createElement('UIStroke', {
    ...
    local s = o.Component:extend'InfoGrid'
    ...
    function s.render(t)
        ...
        for y, z in ipairs(v)do
            ...
            for C, D in ipairs(z)do
                local E = D.weight or 1
                local F = E / A
                local G = -x * (1 - F)

                B['Block_' .. tostring(C)] = o.createElement(r, {
                    block = D,
                    scale = F,
                    widthOffset = G,
                    LayoutOrder = C,
            ...
            w['Row_' .. tostring(y)] = o.createElement('Frame', {
...
d['Components/KeybindField'] = function()
    local k = createVirtualModule'Components/KeybindField'
    ...
    local m = j
    local n = m:GetService'ReplicatedStorage'
    local o = m:GetService'UserInputService'
    local p = l(n.Packages.React)
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = p.Component:extend'KeybindField'
    ...
    function s.render(t)
        local u = t.props.element
        local v = t.props.Store
        local w = u.value or Enum.KeyCode.Unknown
        local x = w == Enum.KeyCode.Unknown
        local y = x and 'None' or w.Name
        local z = t.state.hovered
        local A = t.state.listening
        local B = A and '...' or y

        return p.createElement('Frame', {
            Size = UDim2.new(1, 0, 0, 36),
            BackgroundTransparency = 1,
        }, {
            Label = p.createElement('TextLabel', {
                ...
                BackgroundTransparency = 1,
                Text = u.name or 'Keybind',
                TextColor3 = r.Colors.Text,
                Font = r.Fonts.Medium,
                TextSize = 13,
                TextXAlignment = Enum.TextXAlignment.Left,
            }),
            BindButton = p.createElement('TextButton', {
                Size = UDim2.new(0, 100, 0, 26),
                Position = UDim2.new(1, -100, 0.5, -13),
                BackgroundColor3 = A and r.Colors.Accent or r.Colors.Element,
                BorderSizePixel = 0,
                Text = B,
                TextColor3 = r.Colors.Text,
                Font = r.Fonts.Bold,
                ...
                [p.Event.MouseButton1Click] = function()
                    if A then
                        return
                    end

                    t:setState{listening = true}

                    if t.connection then
                        t.connection:Disconnect()
                    end

                    t.connection = o.InputBegan:Connect(function(C, D)
                        if C.UserInputType == Enum.UserInputType.Keyboard then
                            local E = C.KeyCode

                            if E == Enum.KeyCode.Escape then
                                E = Enum.KeyCode.Unknown
                            end

                            v:UpdateElement(u.id, function(F)
                                F.value = E
                            end)
                            t:setState{listening = false}
                ...
                Stroke = p.createElement('UIStroke', {
                    Color = (z or A) and r.Colors.BorderHover or r.Colors.Border,
                    Thickness = 1,
                }),
            }),
        })
...
d['Components/Label'] = function()
    local k = createVirtualModule'Components/Label'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'Label'

    function r.render(s)
        local t = s.props.element

        return o.createElement('TextLabel', {
            Size = UDim2.new(1, 0, 0, 18),
            BackgroundTransparency = 1,
            Text = t.text or 'Label',
            TextColor3 = t.color or q.Colors.Text,
            Font = t.font or q.Fonts.Medium,
            TextSize = t.textSize or 13,
            TextXAlignment = t.alignment or Enum.TextXAlignment.Left,
            RichText = true,
        })
    end
...
d['Components/LineChart'] = function()
    local k = createVirtualModule'Components/LineChart'
    ...
    local r = m:GetService'TweenService'
    local s = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local t = o.Component:extend'AnimatedLine'
    ...
    function t.didMount(u)
        ...
        if v then
            ...
            v.Size = w.targetSize
            v.Position = w.targetPosition
            v.Rotation = w.targetRotation
        end
    end
    function t.didUpdate(u, v)
        local w = u.ref.current
        ...
        if w then
            ...
            if v.targetSize ~= x.targetSize or v.targetPosition ~= x.targetPosition or v.targetRotation ~= x.targetRotation then
                w.Size = v.targetSize
                w.Position = v.targetPosition
                w.Rotation = v.targetRotation
    ...
    local u = o.Component:extend'AnimatedDot'

    function u.init(v)
        v.ref = o.createRef()
    end
    function u.didMount(v)
        local w = v.ref.current

        if w then
            w.Position = v.props.targetPosition
        end
    end
    function u.didUpdate(v, w)
        local x = v.ref.current

        if x and w.targetPosition ~= v.props.targetPosition then
            x.Position = w.targetPosition

            r:Create(x, s, {
                Position = v.props.targetPosition,
            }):Play()
        end
    end
    function u.render(v)
        ...
        return o.createElement('Frame', {
            ...
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Circle,
            }),
            Stroke = o.createElement('UIStroke', {
                Color = w.color,
                Thickness = 1.5,
    ...
    local v = o.Component:extend'LineChart'

    function v.init(w)
        w.canvasRef = o.createRef()
        w.state = {
            canvasSize = Vector2.new(300, 100),
        }
    end
    function v.didMount(w)
        local x = w.canvasRef.current

        if x then
            w.sizeConnection = x:GetPropertyChangedSignal'AbsoluteSize':Connect(function(
            )
                w:setState{
                    canvasSize = x.AbsoluteSize,
                }
            end)
    ...
    function v.willUnmount(w)
        if w.sizeConnection then
            w.sizeConnection:Disconnect()
        end
    end
    function v.render(w)
        local x = w.props.element
        local y = x.title or 'Line Chart'
        local z = x.subtitle
        local A = x.height or 180
        local B = x.data or {}
        local C = x.labels or {}
        local D = x.color or q.Colors.Accent
        local E = #B
        local F = w.state.canvasSize
        local G = 0

        for H, I in ipairs(B)do
            local J = type(I) == 'table' and (I.value or I.Value or 0) or (tonumber(I) or 0)
        ...
        for I = 1, E do
            local J = B[I]
            local K = type(J) == 'table' and (J.value or J.Value or 0) or (tonumber(J) or 0)
            local L = K / G
            local M = F.X * (I - 1) / math.max(E - 1, 1)
            local N = F.Y * (1 - (L * 0.7 + 0.15))

            table.insert(H, Vector2.new(M, N))
        end
        ...
        if E > 1 then
            for J = 1, E - 1 do
                ...
                local M = L - K
                local N = M.Magnitude
                local O = math.deg(math.atan2(M.Y, M.X))

                I['Line_' .. tostring(J)] = o.createElement(t, {
                    targetSize = UDim2.new(0, N, 0, 2),
                    targetPosition = UDim2.new(0, K.X + M.X / 2, 0, K.Y + M.Y / 2),
                    targetRotation = O,
                    color = D,
                })
            end
        ...
        for J, K in ipairs(H)do
            I['Dot_' .. tostring(J)] = o.createElement(u, {
                targetPosition = UDim2.new(0, K.X, 0, K.Y),
                color = D,
            })
        end
        ...
        for K = 1, E do
            ...
            J['Label_' .. tostring(K)] = o.createElement('TextLabel', {
        ...
        return o.createElement('Frame', {
            ...
            Header = (y or z) and o.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 0),
                BackgroundTransparency = 1,
                AutomaticSize = Enum.AutomaticSize.Y,
                LayoutOrder = 1,
                ...
                TitleLabel = y and o.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 16),
                    BackgroundTransparency = 1,
                    Text = y,
                    TextColor3 = q.Colors.Text,
                ...
                SubLabel = z and o.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 12),
                    BackgroundTransparency = 1,
                    Text = z,
                    TextColor3 = q.Colors.TextSecondary,
            ...
            ChartContainer = o.createElement('Frame', {
                Size = UDim2.new(1, 0, 1, -K),
                BackgroundTransparency = 1,
                LayoutOrder = 2,
            }, {
                YAxis = o.createElement('Frame', {
                ...
                PlotContainer = o.createElement('Frame', {
                    ...
                    LabelsRow = o.createElement('Frame', {
                        Size = UDim2.new(1, 0, 0, 16),
                        Position = UDim2.new(0, 0, 1, -16),
                        BackgroundTransparency = 1,
                    }, J),
...
d['Components/Notification'] = function()
    local k = createVirtualModule'Components/Notification'
    ...
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = p.Component:extend'NotificationItem'

    function s.init(t)
        t.groupRef = p.createRef()
        t.scaleRef = p.createRef()
    end
    function s.didMount(t)
        local u = t.groupRef.current

        if u then
            u.Position = UDim2.new(1, 50, 0, 0)
            u.GroupTransparency = 1

            local v = TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    ...
    function s.didUpdate(t, u)
        if t.props.item.count ~= u.item.count then
            local v = t.scaleRef.current

            if v then
                v.Scale = 1.08

                local w = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)

                o:Create(v, w, {Scale = 1}):Play()
            end
        end
    end
    function s.render(t)
        local u = t.props.item
        local v = u.title or 'Notification'
        local w = u.subtitle or ''
        local x = u.icon or 'rbxassetid://10747373111'
        local y = u.count or 1
        local z = {}

        if y >= 3 then
            z.StackCard2 = p.createElement('Frame', {
        ...
        if y >= 2 then
            z.StackCard1 = p.createElement('Frame', {
        ...
        z.MainCard = p.createElement('Frame', {
            Size = UDim2.new(1, 0, 1, 0),
            Position = UDim2.new(0, 0, 0, 0),
            BackgroundColor3 = r.Colors.Card,
            BorderSizePixel = 0,
            ...
            Padding = p.createElement('UIPadding', {
                PaddingTop = UDim.new(0, 10),
                PaddingBottom = UDim.new(0, 10),
                PaddingLeft = UDim.new(0, 12),
                PaddingRight = UDim.new(0, 12),
            ...
            AppIcon = p.createElement('ImageLabel', {
                Size = UDim2.new(0, 24, 0, 24),
                BackgroundTransparency = 1,
                Image = x,
                ImageColor3 = r.Colors.Accent,
            }),
            TextFrame = p.createElement('Frame', {
                ...
                TitleLabel = p.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 16),
                    BackgroundTransparency = 1,
                    Text = (y > 1) and (v .. ' <font color="' .. string.format('#%02x%02x%02x', math.floor(r.Colors.Accent.R * 255), math.floor(r.Colors.Accent.G * 255), math.floor(r.Colors.Accent.B * 255)) .. '">x' .. tostring(y) .. '</font>') or v,
                    TextColor3 = r.Colors.Text,
                    Font = r.Fonts.Bold,
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Left,
                ...
                SubLabel = p.createElement('TextLabel', {
        ...
        z.Scale = p.createElement('UIScale', {
            ref = t.scaleRef,
            Scale = 1,
        })

        return p.createElement('CanvasGroup', {
            ref = t.groupRef,
    ...
    local t = p.Component:extend'NotificationList'

    function t.render(u)
        local v = u.props.notifications or {}
        local w = {
            Layout = p.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                Padding = UDim.new(0, 10),
                HorizontalAlignment = Enum.HorizontalAlignment.Right,
                SortOrder = Enum.SortOrder.LayoutOrder,
        ...
        for x, y in ipairs(v)do
            w['Item_' .. tostring(y.id)] = p.createElement(s, {
                item = y,
                LayoutOrder = x,
            })
        end
...
d['Components/Paragraph'] = function()
    local k = createVirtualModule'Components/Paragraph'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'Paragraph'
    ...
    function r.render(s)
        ...
        return o.createElement('Frame', {
            ...
            Title = t.title and o.createElement('TextLabel', {
                Size = UDim2.new(1, 0, 0, 16),
                BackgroundTransparency = 1,
                Text = t.title,
                TextColor3 = q.Colors.Text,
            ...
            Body = o.createElement('TextLabel', {
                Size = UDim2.new(1, 0, 0, 0),
                BackgroundTransparency = 1,
                Text = t.content or '',
                TextColor3 = q.Colors.TextSecondary,
...
d['Components/PieChart'] = function()
    local k = createVirtualModule'Components/PieChart'
    ...
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local t = p.Component:extend'AnimatedWedge'

    function t.init(u)
        u.containerRef = p.createRef()
        u.gradientRef = p.createRef()
    end
    function t.didMount(u)
        local v = u.containerRef.current
        local w = u.gradientRef.current

        if v and w then
            v.Rotation = u.props.startAngle
            w.Rotation = math.clamp(u.props.sweepAngle, 0.1, 180)
        end
    end
    function t.didUpdate(u, v)
        local w = u.containerRef.current
        local x = u.gradientRef.current

        if w and x then
            local y = u.props
            local z = math.clamp(y.sweepAngle, 0.1, 180)

            if v.startAngle ~= y.startAngle then
                w.Rotation = v.startAngle

                o:Create(w, s, {
                    Rotation = y.startAngle,
                }):Play()
            end
            if v.sweepAngle ~= y.sweepAngle then
                local A = math.clamp(v.sweepAngle or 0, 0.1, 180)

                x.Rotation = A

                o:Create(x, s, {Rotation = z}):Play()
            end
        end
    end
    function t.render(u)
        ...
        return p.createElement('Frame', {
            ...
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundTransparency = 1,
            ZIndex = v.zIndex or 1,
        }, {
            ClipBox = p.createElement('Frame', {
                Size = UDim2.new(0.5, 0, 1, 0),
                Position = UDim2.new(0.5, 0, 0, 0),
                ...
                CircleSegment = p.createElement('Frame', {
                    ...
                    Gradient = p.createElement('UIGradient', {
                        ref = u.gradientRef,
                        Rotation = math.clamp(v.sweepAngle, 0.1, 180),
                        Transparency = NumberSequence.new{
    ...
    local u = p.Component:extend'PieChart'

    function u.render(v)
        local w = v.props.element
        local x = w.title
        local y = w.subtitle
        local z = w.size or 140
        local A = w.donut == true
        local B = w.data or {}
        local C = w.align or 'Right'
        local D = 0

        for E, F in ipairs(B)do
            local G = type(F) == 'table' and (F.value or F.Value or 0) or (tonumber(F) or 0)
        ...
        if D <= 0 then
            E.Placeholder = p.createElement('Frame', {
                Size = UDim2.new(1, 0, 1, 0),
                Position = UDim2.fromScale(0.5, 0.5),
                AnchorPoint = Vector2.new(0.5, 0.5),
            ...
            for G, H in ipairs(B)do
                local I = type(H) == 'table' and (H.value or H.Value or 0) or (tonumber(H) or 0)

                if I > 0 then
                    local J = (I / D) * 360
                    local K = type(H) == 'table' and (H.color or H.Color or r.Colors.Accent) or r.Colors.Accent

                    if J <= 180 then
                        E['Slice_' .. tostring(G) .. '_1'] = p.createElement(t, {
                            startAngle = F,
                            sweepAngle = J,
                            color = K,
                            zIndex = G,
                        ...
                        E['Slice_' .. tostring(G) .. '_2'] = p.createElement(t, {
                            startAngle = F + 180,
                            sweepAngle = J - 180,
                            color = K,
                            zIndex = G,
        ...
        if A then
            E.DonutHollow = p.createElement('Frame', {
                Name = 'DonutCenter',
        ...
        for G, H in ipairs(B)do
            local I = type(H) == 'table' and (H.value or H.Value or 0) or (tonumber(H) or 0)
            local J = type(H) == 'table' and (H.label or H.Label or 'Category') or ('Category ' .. tostring(G))
            local K = type(H) == 'table' and (H.color or H.Color or r.Colors.Accent) or r.Colors.Accent
            local L = D > 0 and math.round((I / D) * 100) or 0

            F['Legend_' .. tostring(G)] = p.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 18),
                BackgroundTransparency = 1,
                LayoutOrder = G,
            }, {
                ...
                Indicator = p.createElement('Frame', {
                ...
                Percent = p.createElement('TextLabel', {
        ...
        local G = C == 'Right' and 1 or 3
        local H = C == 'Right' and 3 or 1
        local I = {
            Layout = p.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Horizontal,
                VerticalAlignment = Enum.VerticalAlignment.Center,
            ...
            ChartArea = p.createElement('Frame', {
                Name = 'ChartArea',
                ...
                OuterCircleClip = p.createElement('CanvasGroup', {
                    Size = UDim2.fromScale(1, 1),
                    Position = UDim2.fromScale(0.5, 0.5),
                    AnchorPoint = Vector2.new(0.5, 0.5),
                    BackgroundTransparency = 1,
                    ...
                    WedgeContainer = p.createElement('Frame', {
                        Size = UDim2.fromScale(1, 1),
                        BackgroundTransparency = 1,
                    }, E),
                }),
            }),
            LegendStack = p.createElement('Frame', {
                Size = UDim2.new(1, -z - 12, 1, 0),
                BackgroundTransparency = 1,
                LayoutOrder = 2,
            }, F),
        }
        ...
        if x or y then
            J = p.createElement('Frame', {
                ...
                TitleLabel = x and p.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 18),
                    BackgroundTransparency = 1,
                    Text = x,
                    TextColor3 = r.Colors.Text,
                ...
                SubLabel = y and p.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 36),
                    BackgroundTransparency = 1,
                    Text = y,
                    TextColor3 = r.Colors.TextSecondary,
        ...
        if J then
            K.HeaderArea = J
            K.Divider = p.createElement('Frame', {
                Size = UDim2.new(0, 1, 1, -10),
                BackgroundColor3 = r.Colors.Border,
                BorderSizePixel = 0,
                LayoutOrder = 2,
            })
        end

        K.ChartAndLegend = p.createElement('Frame', {
            Size = UDim2.new(J and 0.65 or 1, -12, 1, 0),
            BackgroundTransparency = 1,
            LayoutOrder = 3,
        }, I)

        return p.createElement('Frame', {
            Size = UDim2.new(1, 0, 0, z + 20),
            BackgroundColor3 = r.Colors.Element,
            BorderSizePixel = 0,
        }, {
            Corner = p.createElement('UICorner', {
...
d['Components/Profile'] = function()
    local k = createVirtualModule'Components/Profile'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'Profile'

    function r.render(s)
        local t = s.props.username or 'Guest'
        local u = s.props.subtext or 'Welcome to Selene'
        local v = s.props.userId or 1

        return o.createElement('Frame', {
            Size = UDim2.new(1, 0, 1, 0),
            BackgroundColor3 = q.Colors.Card,
            BorderSizePixel = 0,
        }, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Card,
            ...
            Content = o.createElement('Frame', {
                ...
                Avatar = o.createElement('ImageLabel', {
                    Size = UDim2.new(0, 48, 0, 48),
                    BackgroundColor3 = q.Colors.Element,
                    BorderSizePixel = 0,
                    Image = 'https://www.roblox.com/headshot-thumbnail/image?userId=' .. tostring(v) .. '&width=150&height=150&format=png',
                }, {
                    Corner = o.createElement('UICorner', {
                        CornerRadius = q.CornerRadius.Circle,
                    }),
                ...
                Info = o.createElement('Frame', {
                    ...
                    Greeting = o.createElement('TextLabel', {
                        Size = UDim2.new(1, 0, 0, 20),
                        BackgroundTransparency = 1,
                        Text = 'Hello, ' .. t,
                        TextColor3 = q.Colors.Text,
...
d['Components/PullDownButton'] = function()
    local k = createVirtualModule'Components/PullDownButton'
    ...
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = p.Component:extend'PullDownOption'
    ...
    function s.render(t)
        local u = t.props
        local v = u.opt
        local w = u.LayoutOrder
        local x = t.state.hovered

        return p.createElement('TextButton', {
            Size = UDim2.new(1, 0, 0, 28),
            BackgroundColor3 = x and r.Colors.ElementHover or r.Colors.Element,
            BorderSizePixel = 0,
            Text = '',
            AutoButtonColor = false,
            LayoutOrder = w,
            ...
            [p.Event.MouseButton1Click] = u.OnClick,
        }, {
            Corner = p.createElement('UICorner', {
                CornerRadius = r.CornerRadius.Element,
            }),
    ...
    local t = p.Component:extend'PullDownButton'
    ...
    function t.init(u)
        ...
        u.strokeRef = p.createRef()
        u.arrowRef = p.createRef()
        u.optionsRef = p.createRef()
    end
    function t.didMount(u)
        local v = u.strokeRef.current
        local w = u.arrowRef.current
        local x = u.optionsRef.current
        local y = u.state.hovered
        local z = u.state.isOpen

        if v then
            v.Color = (y or z) and r.Colors.BorderHover or r.Colors.Border
        end
        if w then
            w.Rotation = z and 180 or 0
        end
        if x then
            local A = u.props.element.options or {}
            local B = #A
            local C = B > 0 and (B * 28 + (B - 1) * 4 + 8) or 0

            x.Size = z and UDim2.new(1, 0, 0, C) or UDim2.new(1, 0, 0, 0)
            x.GroupTransparency = z and 0 or 1
            x.Visible = z
        end
    end
    function t.didUpdate(u, v, w)
        local x = u.strokeRef.current
        local y = u.arrowRef.current
        local z = u.optionsRef.current
        local A = u.state.hovered
        local B = u.state.isOpen
        local C = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

        if A ~= w.hovered or B ~= w.isOpen then
            if x then
                local D = (A or B) and r.Colors.BorderHover or r.Colors.Border

                o:Create(x, C, {Color = D}):Play()
            end
        end
        if B ~= w.isOpen then
            if y then
                o:Create(y, C, {
                    Rotation = B and 180 or 0,
                }):Play()
            end
            if z then
                local D = u.props.element.options or {}
                local E = #D
                local F = E > 0 and (E * 28 + (E - 1) * 4 + 8) or 0

                if B then
                    z.Visible = true
                    z.GroupTransparency = 1
                    z.Size = UDim2.new(1, 0, 0, 0)
    ...
    function t.render(u)
        local v = u.props.element
        local w = v.name or v.label or 'Menu'
        local x = v.options or {}
        local y = u.state.hovered
        local z = u.state.isOpen
        local A = {
            Layout = p.createElement('UIListLayout', {
        ...
        for B, C in ipairs(x)do
            A['Opt_' .. tostring(B)] = p.createElement(s, {
                opt = C,
                LayoutOrder = B,
                OnClick = function()
                    u:setState{isOpen = false}

                    if v.callback then
                        task.spawn(v.callback, B)
                    end
                end,
            })
        end
        ...
        return p.createElement('Frame', {
            ...
            Header = p.createElement('TextButton', {
                Size = UDim2.new(1, 0, 0, 36),
                BackgroundColor3 = r.Colors.Element,
                BorderSizePixel = 0,
                Text = '',
                ...
                LayoutOrder = 1,
                [p.Event.MouseEnter] = function()
                    u:setState{hovered = true}
                end,
                [p.Event.MouseLeave] = function()
                    u:setState{hovered = false}
                end,
                [p.Event.MouseButton1Click] = function()
                    u:setState{
                        isOpen = not z,
                ...
                Stroke = p.createElement('UIStroke', {
                    ref = u.strokeRef,
                    Color = (y or z) and r.Colors.BorderHover or r.Colors.Border,
                    Thickness = 1,
                }),
                Title = p.createElement('TextLabel', {
                ...
                Arrow = p.createElement('ImageLabel', {
                    ref = u.arrowRef,
                    Size = UDim2.new(0, 14, 0, 14),
                    Position = UDim2.new(1, -22, 0.5, -7),
                    BackgroundTransparency = 1,
                    Image = 'rbxassetid://10747383844',
                    ImageColor3 = r.Colors.TextSecondary,
                }),
            }),
            Options = p.createElement('CanvasGroup', {
                ref = u.optionsRef,
                BackgroundTransparency = 1,
                LayoutOrder = 2,
                ClipsDescendants = true,
...
d['Components/RadioButtonGroup'] = function()
    local k = createVirtualModule'Components/RadioButtonGroup'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'RadioOption'
    ...
    function r.render(s)
        local t = s.props
        local u = t.opt
        local v = t.isSelected
        local w = t.LayoutOrder
        local x = s.state.hovered
        ...
        return o.createElement('TextButton', {
            ...
            [o.Event.MouseButton1Click] = t.OnClick,
        }, {
            List = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Horizontal,
                VerticalAlignment = Enum.VerticalAlignment.Center,
            ...
            RadioCircle = o.createElement('Frame', {
                Size = UDim2.new(0, 16, 0, 16),
                BackgroundColor3 = v and q.Colors.Accent or q.Colors.Element,
                BorderSizePixel = 0,
                LayoutOrder = 1,
            }, {
                Corner = o.createElement('UICorner', {
                ...
                Stroke = o.createElement('UIStroke', {
                    Color = (x or v) and q.Colors.BorderHover or q.Colors.Border,
                    Thickness = 1,
                }),
                Fill = v and o.createElement('Frame', {
                    Size = UDim2.new(0, 6, 0, 6),
            ...
            OptionLabel = o.createElement('TextLabel', {
                Size = UDim2.new(1, -24, 1, 0),
                BackgroundTransparency = 1,
                Text = u,
                TextColor3 = v and q.Colors.Text or q.Colors.TextSecondary,
                Font = v and q.Fonts.Bold or q.Fonts.Medium,
                TextSize = 12,
                TextXAlignment = Enum.TextXAlignment.Left,
                LayoutOrder = 2,
            }),
    ...
    local s = o.Component:extend'RadioButtonGroup'

    function s.render(t)
        local u = t.props.element
        local v = t.props.Store
        local w = u.options or {}
        local x = u.value or 1
        local y = {
            Layout = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                Padding = UDim.new(0, 6),
        ...
        for z, A in ipairs(w)do
            ...
            y['Opt_' .. tostring(z)] = o.createElement(r, {
                opt = A,
                isSelected = B,
                LayoutOrder = z,
                OnClick = function()
                    v:UpdateElement(u.id, function(C)
                        C.value = z
                    end)

                    if u.callback then
                        task.spawn(u.callback, z)
                    end
                end,
            })
        end
        ...
        return o.createElement('Frame', {
            ...
            Title = u.name and o.createElement('TextLabel', {
                Size = UDim2.new(1, 0, 0, 16),
                BackgroundTransparency = 1,
                Text = u.name,
                TextColor3 = q.Colors.Text,
            ...
            OptionsFrame = o.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 0),
                BackgroundTransparency = 1,
                AutomaticSize = Enum.AutomaticSize.Y,
                LayoutOrder = 2,
...
d['Components/Row'] = function()
    local k = createVirtualModule'Components/Row'
    ...
    local q = l(p.Components.ElementRenderer)
    local r = o.Component:extend'Row'
    ...
    function r.render(s)
        ...
        for w, x in ipairs(t.leftElements or {})do
            v['Left_' .. x.id] = q.render(x, u)
        end

        local w = {}
        local x = false

        for y, z in ipairs(t.rightElements or {})do
            w['Right_' .. z.id] = q.render(z, u)

            local A = z.type

            if A == 'Slider' or A == 'Stepper' or A == 'BarChart' or A == 'LineChart' or A == 'PieChart' or A == 'VStack' or A == 'HStack' or A == 'ColorPickerWidget' then
                x = true
            end
        end

        if #t.leftElements == 0 then
            x = true
        end
        if x then
            return o.createElement('Frame', {
                ...
                LeftArea = #t.leftElements > 0 and o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 1,
                }, v) or nil,
                RightArea = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 2,
            ...
            return o.createElement('Frame', {
                ...
                LeftArea = o.createElement('Frame', {
...
d['Components/Section'] = function()
    local k = createVirtualModule'Components/Section'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = l(p.Components.ElementRenderer)
    local s = o.Component:extend'Section'
    local t = {
    ...
    function s.render(u)
        local v = u.props.section
        local w = u.props.Store
        local x

        if v.gradientPreset and t[v.gradientPreset] then
            x = t[v.gradientPreset]
        ...
        for z, A in ipairs(v.elements)do
            local B = r.render(A, w)

            if B then
                y['El_' .. A.id] = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = z,
        ...
        return o.createElement('Frame', {
            ...
            BorderSizePixel = 0,
            AutomaticSize = Enum.AutomaticSize.Y,
            LayoutOrder = u.props.LayoutOrder,
        }, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Card,
            }),
            ...
            Gradient = x and o.createElement('UIGradient', {
                Color = x,
                Rotation = 135,
            }),
            Padding = o.createElement('UIPadding', {
            ...
            Content = o.createElement('Frame', {
                ...
                Header = (v.name or v.description) and o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 1,
                    ...
                    Title = v.name and o.createElement('TextLabel', {
                        Size = UDim2.new(1, 0, 0, 18),
                        BackgroundTransparency = 1,
                        Text = v.name,
                        TextColor3 = q.Colors.Text,
                    ...
                    Desc = v.description and o.createElement('TextLabel', {
                        Size = UDim2.new(1, 0, 0, 14),
                        BackgroundTransparency = 1,
                        Text = v.description,
                        TextColor3 = q.Colors.TextSecondary,
                ...
                Divider = (#v.elements > 0 and (v.name or v.description)) and o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 1),
                    BackgroundColor3 = q.Colors.Border,
                    BorderSizePixel = 0,
                    LayoutOrder = 2,
                }),
                ElementsArea = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 3,
...
d['Components/Sidebar'] = function()
    local k = createVirtualModule'Components/Sidebar'
    ...
    local q = l(p.Theme)
    local r = m:GetService'TweenService'
    local s = o.Component:extend'TabButton'

    function s.init(t)
        t.state = {hovered = false}
        t.btnRef = o.createRef()
        t.activeBarRef = o.createRef()
        t.labelRef = o.createRef()
        t.iconRef = o.createRef()
    end
    function s.didMount(t)
        local u = t.btnRef.current
        local v = t.activeBarRef.current
        local w = t.labelRef.current
        local x = t.iconRef.current
        local y = t.props
        local z = y.isActive
        local A = t.state.hovered

        if u then
            u.BackgroundColor3 = z and q.Colors.Element or (A and q.Colors.ElementHover or Color3.fromRGB(0, 0, 0))
            u.BackgroundTransparency = (z or A) and 0 or 1
        end
        if v then
            v.Size = z and UDim2.new(0, 3, 0, 14) or UDim2.new(0, 3, 0, 0)
            v.BackgroundTransparency = z and 0 or 1
        end

        local B = (z or A) and q.Colors.Text or q.Colors.TextSecondary
    ...
    function s.didUpdate(t, u, v)
        local w = t.btnRef.current
        local x = t.activeBarRef.current
        local y = t.labelRef.current
        local z = t.iconRef.current
        local A = t.props
        local B = A.isActive
        local C = t.state.hovered
        local D = (B ~= u.isActive) or (C ~= v.hovered)

        if D then
            local E = B and q.Colors.Element or (C and q.Colors.ElementHover or Color3.fromRGB(0, 0, 0))
            local F = (B or C) and 0 or 1
            local G = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

            if w then
                r:Create(w, G, {
                    BackgroundColor3 = E,
                    BackgroundTransparency = F,
                }):Play()
            end

            local H = (B or C) and q.Colors.Text or q.Colors.TextSecondary

            if y then
                r:Create(y, G, {TextColor3 = H}):Play()
            end
            if z then
                r:Create(z, G, {ImageColor3 = H}):Play()
            end
        end
        if B ~= u.isActive then
            if x then
                local E = B and UDim2.new(0, 3, 0, 14) or UDim2.new(0, 3, 0, 0)
                local F = B and 0 or 1
                local G = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    ...
    function s.render(t)
        ...
        local w = u.isActive
        local x = u.Store
        local y = u.LayoutOrder
        local z = t.state.hovered
        local A = v.parentTabId ~= nil
        local B = A and 16 or 0
        local C = A and UDim2.new(1, -16, 0, 32) or UDim2.new(1, 0, 0, 32)

        return o.createElement('TextButton', {
            ref = t.btnRef,
            Size = C,
            Position = UDim2.new(0, B, 0, 0),
            BackgroundColor3 = w and q.Colors.Element or Color3.fromRGB(0, 0, 0),
            BackgroundTransparency = w and 0 or 1,
            BorderSizePixel = 0,
            Text = '',
            AutoButtonColor = false,
            LayoutOrder = y,
            ...
            ActiveBar = o.createElement('Frame', {
                ref = t.activeBarRef,
                Size = w and UDim2.new(0, 3, 0, 14) or UDim2.new(0, 3, 0, 0),
                Position = UDim2.new(0, 4, 0.5, -7),
                BackgroundColor3 = q.Colors.Accent,
                BackgroundTransparency = w and 0 or 1,
                BorderSizePixel = 0,
            }, {
                Corner = o.createElement('UICorner', {
            ...
            Content = o.createElement('Frame', {
                Size = UDim2.new(1, w and -15 or -10, 1, 0),
                Position = UDim2.new(0, w and 12 or 8, 0, 0),
                BackgroundTransparency = 1,
            }, {
                List = o.createElement('UIListLayout', {
                    FillDirection = Enum.FillDirection.Horizontal,
                ...
                Icon = v.icon and o.createElement('ImageLabel', {
                    ref = t.iconRef,
                    Size = UDim2.new(0, 14, 0, 14),
                    BackgroundTransparency = 1,
                    Image = v.icon,
                    ImageColor3 = (w or z) and q.Colors.Text or q.Colors.TextSecondary,
                }) or nil,
                Label = o.createElement('TextLabel', {
                    ref = t.labelRef,
                    Size = UDim2.new(1, v.icon and -22 or 0, 1, 0),
                    BackgroundTransparency = 1,
                    Text = v.title,
                    TextColor3 = (w or z) and q.Colors.Text or q.Colors.TextSecondary,
                    Font = w and q.Fonts.Bold or q.Fonts.Medium,
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Left,
                }),
            }),
    ...
    local t = o.Component:extend'Sidebar'

    function t.render(u)
        local v = u.props.state
        local w = u.props.Store
        local x = u.props.LayoutOrder
        local y = {
            Layout = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                SortOrder = Enum.SortOrder.LayoutOrder,
        ...
        for A, B in ipairs(v.sections or {})do
            if B.title and B.title ~= '' then
                y['SecHeader_' .. B.id] = o.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 0, 18),
                    BackgroundTransparency = 1,
                    Text = string.upper(B.title),
                    TextColor3 = q.Colors.TextSecondary,
                    Font = q.Fonts.Bold,
                    TextSize = 10,
                    TextXAlignment = Enum.TextXAlignment.Left,
            ...
            for C, D in ipairs(B.tabs or {})do
                local E = v.activeTab == D.id

                y['Tab_' .. D.id] = o.createElement(s, {
                    tab = D,
                    isActive = E,
                    Store = w,
                    LayoutOrder = z,
        ...
        return o.createElement('Frame', {
            ...
            BackgroundColor3 = q.Colors.Sidebar,
            BorderSizePixel = 0,
            LayoutOrder = x,
        }, {
            Border = o.createElement('Frame', {
                Size = UDim2.new(0, 1, 1, 0),
                Position = UDim2.new(1, -1, 0, 0),
                BackgroundColor3 = q.Colors.Border,
                BorderSizePixel = 0,
            }),
            NavList = o.createElement('ScrollingFrame', {
                ...
                ScrollBarImageColor3 = q.Colors.Border,
            }, y),
            ProfileArea = v.profile and o.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 65),
                Position = UDim2.new(0, 0, 1, -65),
                BackgroundTransparency = 1,
            }, {
                Divider = o.createElement('Frame', {
                    Size = UDim2.new(1, -20, 0, 1),
                    Position = UDim2.new(0, 10, 0, 0),
                    BackgroundColor3 = q.Colors.Border,
                    BorderSizePixel = 0,
                }),
                Container = o.createElement('Frame', {
                    ...
                    Avatar = o.createElement('ImageLabel', {
                        ...
                        BackgroundColor3 = q.Colors.Element,
                        BorderSizePixel = 0,
                        Image = 'https://www.roblox.com/headshot-thumbnail/image?userId=' .. tostring(v.profile.userId or 1) .. '&width=150&height=150&format=png',
                    }, {
                        Corner = o.createElement('UICorner', {
                            CornerRadius = q.CornerRadius.Circle,
                        }),
                    ...
                    Info = o.createElement('Frame', {
                        ...
                        Username = o.createElement('TextLabel', {
                            Size = UDim2.new(1, 0, 0, 16),
                            BackgroundTransparency = 1,
                            Text = v.profile.username,
                            TextColor3 = q.Colors.Text,
                            Font = q.Fonts.Bold,
                            TextSize = 12,
                            TextXAlignment = Enum.TextXAlignment.Left,
                        ...
                        Role = o.createElement('TextLabel', {
                            Size = UDim2.new(1, 0, 0, 14),
                            BackgroundTransparency = 1,
                            Text = v.profile.subtext or 'User',
                            TextColor3 = q.Colors.TextSecondary,
                            Font = q.Fonts.Regular,
                            TextSize = 10,
                            TextXAlignment = Enum.TextXAlignment.Left,
...
d['Components/Slider'] = function()
    local k = createVirtualModule'Components/Slider'
    ...
    local n = m:GetService'ReplicatedStorage'
    local o = m:GetService'UserInputService'
    local p = m:GetService'TweenService'
    local q = l(n.Packages.React)
    local r = k.Parent.Parent
    local s = l(r.Theme)
    local t = q.Component:extend'Slider'

    function t.init(u, v)
        u.trackRef = q.createRef()
        u.fillRef = q.createRef()
        u.knobRef = q.createRef()
        u.strokeRef = q.createRef()
        u.dragConnection = nil
        u.releaseConnection = nil
        u.state = {
            hovered = false,
            value = v.element.value or (v.element.min or 0),
        }
    end
    function t.didMount(u)
        local v = u.fillRef.current
        local w = u.knobRef.current
        local x = u.strokeRef.current
        local y = u.props.element
        local z = y.min or 0
        local A = y.max or 100
        local B = u.state.value
        local C = math.clamp((B - z) / (A - z), 0, 1)
        local D = u.state.hovered

        if v then
            v.Size = UDim2.new(C, 0, 1, 0)
        end
        if w then
            w.Position = UDim2.new(C, -6, 0.5, -6)
        end
        if x then
            x.Color = D and s.Colors.BorderHover or s.Colors.Border
        end
    end
    function t.didUpdate(u, v, w)
        local x = u.fillRef.current
        local y = u.knobRef.current
        local z = u.strokeRef.current
        local A = u.props.element
        local B = A.min or 0
        local C = A.max or 100
        local D = u.state.value

        if A.value ~= D and A.value ~= v.element.value then
            u:setState{
                value = A.value,
            }
        ...
        local E = math.clamp((D - B) / (C - B), 0, 1)
        local F = math.clamp((w.value - B) / (C - B), 0, 1)
        local G = u.state.hovered

        if E ~= F then
            local H = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

            if x then
                p:Create(x, H, {
                    Size = UDim2.new(E, 0, 1, 0),
                }):Play()
            end
            if y then
                p:Create(y, H, {
                    Position = UDim2.new(E, -6, 0.5, -6),
                }):Play()
            end
        end
        if G ~= w.hovered then
            local H = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

            if z then
                p:Create(z, H, {
                    Color = G and s.Colors.BorderHover or s.Colors.Border,
                }):Play()
            end
        end
    end
    function t.willUnmount(u)
        if u.dragConnection then
            u.dragConnection:Disconnect()
        end
        if u.releaseConnection then
            u.releaseConnection:Disconnect()
        end
    end
    function t.render(u)
        local v = u.props.element
        local w = u.props.Store
        local x = v.min or 0
        local y = v.max or 100
        local z = u.state.value

        math.clamp((z - x) / (y - x), 0, 1)

        local A = u.state.hovered

        local function updateValue(B)
            local C = u.trackRef.current

            if not C then
                return
            end

            local D = C.AbsoluteSize.X
            local E = C.AbsolutePosition.X
            local F = B.Position.X
            local G = math.clamp((F - E) / D, 0, 1)
            local H = math.round(x + (G * (y - x)))

            u:setState{value = H}
            w:UpdateElement(v.id, function(I)
                I.value = H
            end)

            if v.callback then
                task.spawn(v.callback, H)
            end
        end

        return q.createElement('Frame', {
            Size = UDim2.new(1, 0, 0, 48),
            BackgroundTransparency = 1,
        }, {
            List = q.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                Padding = UDim.new(0, 6),
            }),
            Header = q.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 16),
                BackgroundTransparency = 1,
            }, {
                Title = q.createElement('TextLabel', {
                    Size = UDim2.new(1, -60, 1, 0),
                    BackgroundTransparency = 1,
                    Text = v.name or 'Slider',
                    TextColor3 = s.Colors.Text,
                    Font = s.Fonts.Medium,
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                }),
                Value = q.createElement('TextLabel', {
                    Size = UDim2.new(0, 50, 1, 0),
                    Position = UDim2.new(1, -50, 0, 0),
                    BackgroundTransparency = 1,
                    Text = tostring(z),
                    TextColor3 = s.Colors.TextSecondary,
                    Font = s.Fonts.Bold,
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Right,
                }),
            }),
            BarRow = q.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 24),
                BackgroundTransparency = 1,
            }, {
                LeftIcon = v.leftIcon and q.createElement('ImageLabel', {
                    Size = UDim2.new(0, 16, 0, 16),
                    Position = UDim2.new(0, 0, 0.5, -8),
                    BackgroundTransparency = 1,
                    Image = v.leftIcon,
                    ImageColor3 = s.Colors.TextSecondary,
                }) or nil,
                RightIcon = v.rightIcon and q.createElement('ImageLabel', {
                    Size = UDim2.new(0, 16, 0, 16),
                    Position = UDim2.new(1, -16, 0.5, -8),
                    BackgroundTransparency = 1,
                    Image = v.rightIcon,
                ...
                Track = q.createElement('TextButton', {
                    ref = u.trackRef,
                    Size = UDim2.new(1, (v.leftIcon and -24 or 0) + (v.rightIcon and 
-24 or 0), 0, 8),
                    Position = UDim2.new(0, v.leftIcon and 20 or 0, 0.5, -4),
                    BackgroundColor3 = s.Colors.Element,
                    BorderSizePixel = 0,
                    Text = '',
                    AutoButtonColor = false,
                    [q.Event.MouseEnter] = function()
                        u:setState{hovered = true}
                    end,
                    [q.Event.MouseLeave] = function()
                        u:setState{hovered = false}
                    end,
                    [q.Event.InputBegan] = function(B, C)
                        if C.UserInputType == Enum.UserInputType.MouseButton1 or C.UserInputType == Enum.UserInputType.Touch then
                            ...
                            u.dragConnection = o.InputChanged:Connect(function(
                                D
                            )
                            ...
                            u.releaseConnection = o.InputEnded:Connect(function(
                                D
                            )
                    ...
                    Corner = q.createElement('UICorner', {
                        CornerRadius = s.CornerRadius.Circle,
                    }),
                    Stroke = q.createElement('UIStroke', {
                        ref = u.strokeRef,
                        Color = A and s.Colors.BorderHover or s.Colors.Border,
                        Thickness = 1,
                    }),
                    Fill = q.createElement('Frame', {
                        ref = u.fillRef,
                        BackgroundColor3 = s.Colors.Accent,
                        BorderSizePixel = 0,
                        ZIndex = 1,
                    }, {
                        Corner = q.createElement('UICorner', {
...
d['Components/Stepper'] = function()
    local k = createVirtualModule'Components/Stepper'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'StepperDecButton'
    ...
    function r.render(s)
        local t = s.props
        local u = s.state.hovered

        return o.createElement('TextButton', {
            Size = UDim2.new(0, 26, 0, 26),
            BackgroundColor3 = u and q.Colors.ElementHover or q.Colors.Element,
            BorderSizePixel = 0,
            Text = '-',
            TextColor3 = q.Colors.Text,
            Font = q.Fonts.Bold,
            ...
            Stroke = o.createElement('UIStroke', {
                Color = u and q.Colors.BorderHover or q.Colors.Border,
                Thickness = 1,
            }),
        })
    end

    local s = o.Component:extend'StepperIncButton'
    ...
    function s.render(t)
        ...
        return o.createElement('TextButton', {
            Size = UDim2.new(0, 26, 0, 26),
            BackgroundColor3 = v and q.Colors.ElementHover or q.Colors.Element,
            BorderSizePixel = 0,
            Text = '+',
            TextColor3 = q.Colors.Text,
            Font = q.Fonts.Bold,
            ...
            [o.Event.MouseButton1Click] = u.OnClick,
        }, {
            Corner = o.createElement('UICorner', {
                CornerRadius = q.CornerRadius.Element,
            }),
            Stroke = o.createElement('UIStroke', {
                Color = v and q.Colors.BorderHover or q.Colors.Border,
                Thickness = 1,
            }),
        })
    end

    local t = o.Component:extend'Stepper'
    ...
    function t.render(u)
        ...
        local function updateValue(B)
            ...
            w:UpdateElement(v.id, function(D)
                D.value = B
            end)
        ...
        return o.createElement('Frame', {
            ...
            Container = o.createElement('Frame', {
                ...
                DecButton = o.createElement(r, {
                    OnClick = function()
                        updateValue(A - z)
                    end,
                }),
                Display = o.createElement('Frame', {
                    Size = UDim2.new(0, 68, 0, 26),
                    BackgroundTransparency = 1,
                    LayoutOrder = 2,
                }, {
                    Input = v.fielded and o.createElement('TextBox', {
                        Size = UDim2.new(1, 0, 1, 0),
                        BackgroundTransparency = 1,
                        Text = tostring(A),
                        TextColor3 = q.Colors.Text,
                        ...
                        TextXAlignment = Enum.TextXAlignment.Center,
                        ClearTextOnFocus = false,
                        [o.Event.FocusLost] = function(B)
                            local C = tonumber(B.Text)
...
d['Components/TabContainer'] = function()
    local k = createVirtualModule'Components/TabContainer'
    ...
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = table.clone or function(s)
        local t = {}
    ...
    local t = l(q.Components.Section)
    local u = l(q.Components.Profile)
    local v = p.Component:extend'TabContainer'

    function v.init(w)
        w.groupRef = p.createRef()
    end
    function v.didMount(w)
        local x = w.groupRef.current

        if x then
            x.Position = UDim2.new(0, 0, 0, 8)
            x.GroupTransparency = 1

            local y = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    ...
    function v.didUpdate(w, x)
        if w.props.state.activeTab ~= x.state.activeTab then
            local y = w.groupRef.current

            if y then
                y.Position = UDim2.new(0, 0, 0, 8)
                y.GroupTransparency = 1

                local z = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    ...
    function v.render(w)
        local x = w.props.state
        local y = w.props.Store
        local z = w.props.LayoutOrder
        local A

        for B, C in ipairs(x.tabs)do
            if C.id == x.activeTab then
                A = C
        ...
        if not A then
            return p.createElement('CanvasGroup', {
                ...
                NoTab = p.createElement('TextLabel', {
                    Size = UDim2.new(1, 0, 1, 0),
                    BackgroundTransparency = 1,
        ...
        local B = A.banner
        local C = string.lower(x.searchQuery or '')
        local D = {}
        local E = {}
        local F = A.pageSections or A.sections or {}
        ...
        for G, H in ipairs(F)do
            ...
            if C == '' then
                ...
                local K = H.name and string.find(string.lower(H.name), C) ~= nil
                local L = H.description and string.find(string.lower(H.description), C) ~= nil
                ...
                if K or L then
                    ...
                    for N, O in ipairs(H.elements)do
                        local P = O.name and string.find(string.lower(O.name), C) ~= nil
                        local Q = O.text and string.find(string.lower(O.text), C) ~= nil

                        if P or Q then
                            table.insert(M, O)
                        end
                        if O.type == 'VStack' or O.type == 'HStack' then
                            local R = false

                            for S, T in ipairs(O.elements or {})do
                                local U = T.name and string.find(string.lower(T.name), C) ~= nil
                                local V = T.text and string.find(string.lower(T.text), C) ~= nil
            ...
            if I then
                ...
                if H.column == 2 or H.column == 'Right' then
                    table.insert(E, L)
                else
                    table.insert(D, L)
                end
        ...
        if G then
            local I = {
                ColLayout = p.createElement('UIListLayout', {
                    FillDirection = Enum.FillDirection.Vertical,
                    Padding = UDim.new(0, 12),
                    SortOrder = Enum.SortOrder.LayoutOrder,
                }),
            ...
            H = {
                ColumnsList = p.createElement('UIListLayout', {
                    FillDirection = Enum.FillDirection.Horizontal,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Padding = UDim.new(0, 12),
                }),
                LeftCol = p.createElement('Frame', {
                    Size = UDim2.new(0.5, -6, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 1,
                }, I),
                RightCol = p.createElement('Frame', {
                    Size = UDim2.new(0.5, -6, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 2,
            ...
            local I = {
                SingleColLayout = p.createElement('UIListLayout', {
                    FillDirection = Enum.FillDirection.Vertical,
                    Padding = UDim.new(0, 12),
                    SortOrder = Enum.SortOrder.LayoutOrder,
                }),
        ...
        return p.createElement('CanvasGroup', {
            ...
            ScrollFrame = p.createElement('ScrollingFrame', {
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
                ScrollBarThickness = 4,
                ScrollBarImageColor3 = r.Colors.Border,
                CanvasSize = UDim2.new(0, 0, 0, 0),
                AutomaticCanvasSize = Enum.AutomaticSize.Y,
            }, {
                Layout = p.createElement('UIListLayout', {
                ...
                Banner = B and p.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 80),
                    BackgroundTransparency = 1,
                    LayoutOrder = 1,
                }, {
                    Content = p.createElement(u, {
                        username = B.username,
                        subtext = B.subtext,
                        userId = B.userId,
                    }),
                }),
                GridArea = p.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = 2,
...
d['Components/TextBox'] = function()
    local k = createVirtualModule'Components/TextBox'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'TextBox'
    ...
    function r.render(s)
        ...
        return o.createElement('Frame', {
            ...
            Label = not x and o.createElement('TextLabel', {
                Size = UDim2.new(0, 150, 1, 0),
                BackgroundTransparency = 1,
                Text = t.name or 'Input',
                TextColor3 = q.Colors.Text,
                Font = q.Fonts.Medium,
                TextSize = 13,
                TextXAlignment = Enum.TextXAlignment.Left,
            }) or nil,
            InputContainer = o.createElement('Frame', {
                Size = x and UDim2.new(1, 0, 0, 28) or UDim2.new(1, -160, 0, 28),
                Position = x and UDim2.new(0, 0, 0.5, -14) or UDim2.new(1, -160, 0.5, 
-14),
                BackgroundColor3 = q.Colors.Element,
                BorderSizePixel = 0,
            }, {
                ...
                TextInput = o.createElement('TextBox', {
                    Size = UDim2.new(1, -16, 1, 0),
                    Position = UDim2.new(0, 8, 0, 0),
                    BackgroundTransparency = 1,
                    Text = w,
                    PlaceholderText = t.placeholder or 'Type here...',
                    PlaceholderColor3 = q.Colors.TextSecondary,
                    TextColor3 = q.Colors.Text,
                    Font = q.Fonts.Regular,
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    [o.Event.FocusLost] = function(y, z)
                        local A = y.Text

                        u:UpdateElement(t.id, function(B)
                            B.value = A
                        end)

                        if t.callback then
                            task.spawn(t.callback, A, z)
                        end
                    end,
                }),
            }),
...
d['Components/TitleStack'] = function()
    local k = createVirtualModule'Components/TitleStack'
    ...
    local p = k.Parent.Parent
    local q = l(p.Theme)
    local r = o.Component:extend'TitleStack'

    local function TagComponent(s)
        local t = s.color or q.Colors.Accent
        local u = type(t) == 'table'

        return o.createElement('Frame', {
            Size = UDim2.new(0, 0, 0, 16),
            AutomaticSize = Enum.AutomaticSize.X,
            BackgroundColor3 = not u and t or Color3.fromRGB(255, 255, 255),
            BorderSizePixel = 0,
            ...
            Gradient = u and o.createElement('UIGradient', {
                Color = ColorSequence.new(unpack(t)),
                Rotation = 45,
            }),
            Label = o.createElement('TextLabel', {
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
                Text = string.upper(s.text),
                TextColor3 = Color3.fromRGB(255, 255, 255),
                Font = q.Fonts.Bold,
                TextSize = 8,
            }),
    ...
    function r.render(s)
        local t = s.props.element
        local u = t.title or t.Title or 'Title'
        local v = t.subtitle or t.Subtitle
        local w = t.titleTag or t.TitleTag
        local x = t.titleTagColor or t.TitleTagColor
        local y = t.subtitleTag or t.SubtitleTag
        local z = t.subtitleTagColor or t.SubtitleTagColor
        ...
        return o.createElement('Frame', {
            ...
            TitleRow = o.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 18),
                BackgroundTransparency = 1,
                LayoutOrder = 1,
            }, {
                ...
                Tag = w and o.createElement(TagComponent, {
                    text = w,
                    color = x,
                    LayoutOrder = 2,
                }) or nil,
            }),
            SubtitleRow = v and o.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 14),
                BackgroundTransparency = 1,
                LayoutOrder = 2,
            }, {
                ...
                Tag = y and o.createElement(TagComponent, {
                    text = y,
                    color = z,
                    LayoutOrder = 2,
                }) or nil,
...
d['Components/Toggle'] = function()
    local k = createVirtualModule'Components/Toggle'
    ...
    local q = k.Parent.Parent
    local r = l(q.Theme)
    local s = p.Component:extend'Toggle'

    function s.init(t, u)
        t.state = {
            hovered = false,
            value = u.element.value or false,
        }
        t.switchRef = p.createRef()
        t.knobRef = p.createRef()
        t.strokeRef = p.createRef()
    end
    function s.didMount(t)
        local u = t.switchRef.current
        local v = t.knobRef.current
        local w = t.strokeRef.current
        local x = t.state.value
        local y = t.state.hovered

        if u then
            u.BackgroundColor3 = x and r.Colors.Accent or r.Colors.Element
        end
        if v then
            v.Position = x and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, 
-8)
        end
        if w then
            w.Color = y and r.Colors.BorderHover or r.Colors.Border
        end
    end
    function s.didUpdate(t, u, v)
        local w = t.switchRef.current
        local x = t.knobRef.current
        local y = t.strokeRef.current
        local z = t.props.element
        local A = t.state.value

        if z.value ~= A and z.value ~= u.element.value then
            t:setState{
                value = z.value,
            }
        ...
        if A ~= v.value then
            local B = A and r.Colors.Accent or r.Colors.Element
            local C = A and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
            local D = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            ...
            if x then
                o:Create(x, D, {Position = C}):Play()
            end
        end

        local B = t.state.hovered

        if B ~= v.hovered then
            local C = B and r.Colors.BorderHover or r.Colors.Border
            local D = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

            if y then
                o:Create(y, D, {Color = C}):Play()
            end
        end
    end
    function s.render(t)
        ...
        return p.createElement('Frame', {
            ...
            Label = not y and p.createElement('TextLabel', {
                Size = UDim2.new(1, -60, 1, 0),
                BackgroundTransparency = 1,
                Text = u.name or 'Toggle',
                TextColor3 = r.Colors.Text,
                Font = r.Fonts.Medium,
                TextSize = 13,
                TextXAlignment = Enum.TextXAlignment.Left,
            }) or nil,
            Switch = p.createElement('TextButton', {
                ref = t.switchRef,
                ...
                [p.Event.MouseButton1Click] = function()
                    ...
                    t:setState{value = z}
                    v:UpdateElement(u.id, function(A)
                        A.value = z
                    end)
                ...
                Corner = p.createElement('UICorner', {
                    CornerRadius = r.CornerRadius.Circle,
                }),
                Stroke = p.createElement('UIStroke', {
                    ref = t.strokeRef,
                    Color = w and r.Colors.BorderHover or r.Colors.Border,
                    Thickness = 1,
                }),
                Knob = p.createElement('Frame', {
                    ref = t.knobRef,
                    Size = UDim2.new(0, 16, 0, 16),
                    BackgroundColor3 = Color3.fromRGB(255, 255, 255),
                    BorderSizePixel = 0,
...
d['Components/VStack'] = function()
    local k = createVirtualModule'Components/VStack'
    ...
    local p = k.Parent.Parent
    local q = l(p.Components.ElementRenderer)
    local r = o.Component:extend'VStack'
    ...
    function r.render(s)
        ...
        local v = {
            Layout = o.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Vertical,
                Padding = t.padding or UDim.new(0, 4),
                HorizontalAlignment = t.horizontalAlignment or Enum.HorizontalAlignment.Left,
                VerticalAlignment = t.verticalAlignment or Enum.VerticalAlignment.Center,
                SortOrder = Enum.SortOrder.LayoutOrder,
            }),
        ...
        for w, x in ipairs(t.elements or {})do
            ...
            if y then
                v['El_' .. x.id] = o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = w,
...
d['Components/Window'] = function()
    local k = createVirtualModule'Components/Window'
    ...
    local r = k.Parent.Parent
    local s = l(r.Theme)
    local t = l(r.Components.Sidebar)
    local u = l(r.Components.TabContainer)
    local v = l(r.Icons).assets
    local w = q.Component:extend'Window'

    local function TagComponent(x)
        local y = x.color or s.Colors.Accent
        local z = type(y) == 'table'

        return q.createElement('Frame', {
            Size = UDim2.new(0, 0, 0, 16),
            AutomaticSize = Enum.AutomaticSize.X,
            BackgroundColor3 = not z and y or Color3.fromRGB(255, 255, 255),
            BorderSizePixel = 0,
            LayoutOrder = x.LayoutOrder,
        }, {
            Corner = q.createElement('UICorner', {
                CornerRadius = UDim.new(0, 4),
            }),
            Padding = q.createElement('UIPadding', {
                PaddingLeft = UDim.new(0, 6),
                PaddingRight = UDim.new(0, 6),
                PaddingTop = UDim.new(0, 1),
                PaddingBottom = UDim.new(0, 1),
            }),
            Gradient = z and q.createElement('UIGradient', {
                Color = ColorSequence.new(unpack(y)),
                Rotation = 45,
            }),
            Label = q.createElement('TextLabel', {
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
                Text = string.upper(x.text),
                TextColor3 = Color3.fromRGB(255, 255, 255),
                Font = s.Fonts.Bold,
                TextSize = 8,
            }),
    ...
    function w.init(x)
        x.windowRef = q.createRef()
        x.containerRef = q.createRef()
        x.dragConnection = nil
        x.dragInputConnection = nil
        x.state = {
            position = UDim2.new(0.5, -375, 0.5, -240),
            closing = false,
        }
    end
    function w.didMount(x)
        local y = x.windowRef.current
        local z = x.containerRef.current

        if not y then
            return
        end

        local A = x.props.state.maximized
        local B = x.props.state.minimized
        local C = A and UDim2.new(0, 0, 0, 0) or x.state.position
        local D = UDim2.new(C.X.Scale, C.X.Offset, C.Y.Scale, C.Y.Offset + 15)

        y.Position = D

        local E = A and UDim2.new(1, 0, 1, 0) or (B and UDim2.new(0, 750, 0, 45) or UDim2.new(0, 750, 0, 480))

        y.Size = E
        y.GroupTransparency = 1

        local F = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        ...
        if z then
            z.GroupTransparency = B and 1 or 0
            z.Visible = not B
        end
        ...
        local function update(K)
            if x.props.state.maximized then
                return
            end
            ...
            x:setState{
                position = UDim2.new(J.X.Scale, J.X.Offset + L.X, J.Y.Scale, J.Y.Offset + L.Y),
            }
        end

        local K = y:WaitForChild('TitleBar', 1)

        if K then
            x.dragConnection = K.InputBegan:Connect(function(L)
                if x.props.state.maximized then
                    return
                end
                if not x.props.state.draggable and x.props.state.draggable ~= nil then
                    return
                end
                if L.UserInputType == Enum.UserInputType.MouseButton1 or L.UserInputType == Enum.UserInputType.Touch then
                    G = true
    ...
    function w.willUnmount(x)
        if x.dragConnection then
            x.dragConnection:Disconnect()
        end
        if x.dragInputConnection then
            x.dragInputConnection:Disconnect()
        end
    end
    function w.didUpdate(x, y, z)
        local A = x.windowRef.current
        local B = x.containerRef.current

        if not A then
            return
        end

        local C = x.props.state.minimized
        local D = x.props.state.maximized

        if x.props.state.visible and not y.state.visible then
            x:setState{closing = false}

            A.GroupTransparency = 0
        end

        local E = (C ~= y.state.minimized) or (D ~= y.state.maximized)
        local F = (D ~= y.state.maximized)

        if E or F then
            local G = D and UDim2.new(1, 0, 1, 0) or (C and UDim2.new(0, 750, 0, 45) or UDim2.new(0, 750, 0, 480))
            local H = D and UDim2.new(0, 0, 0, 0) or x.state.position
            local I = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            ...
            if C ~= y.state.minimized then
                if B then
                    if C then
                        local J = p:Create(B, I, {GroupTransparency = 1})

                        J:Play()
                        J.Completed:Connect(function()
                            if x.props.state.minimized then
                                B.Visible = false
                            end
                        end)
                    else
                        ...
                        p:Create(B, I, {GroupTransparency = 0}):Play()
                    end
                end
            end
        elseif z.position ~= x.state.position and not D then
            A.Position = x.state.position
        end
    end
    function w.render(x)
        local y = x.props.state
        local z = x.props.Store
        local A = y.minimized
        local B = y.maximized
        local C = B and UDim2.new(1, 0, 1, 0) or (A and UDim2.new(0, 750, 0, 45) or UDim2.new(0, 750, 0, 480))
        local D = B and UDim2.new(0, 0, 0, 0) or x.state.position
        local E = {
            Layout = q.createElement('UIListLayout', {
                FillDirection = Enum.FillDirection.Horizontal,
                HorizontalAlignment = Enum.HorizontalAlignment.Right,
                VerticalAlignment = Enum.VerticalAlignment.Center,
                Padding = UDim.new(0, 8),
        ...
        if y.canMinimize ~= false then
            E.Minimize = q.createElement('ImageButton', {
                Size = UDim2.new(0, 14, 0, 14),
                BackgroundTransparency = 1,
                Image = v['lucide-minus'],
                ImageColor3 = s.Colors.TextSecondary,
                LayoutOrder = 1,
                [q.Event.MouseButton1Click] = function()
                    z:Update(function(F)
        ...
        if y.canZoom ~= false then
            E.Maximize = q.createElement('ImageButton', {
                Size = UDim2.new(0, 14, 0, 14),
                BackgroundTransparency = 1,
                Image = B and v['lucide-minimize-2'] or v['lucide-maximize-2'],
                ImageColor3 = s.Colors.TextSecondary,
                LayoutOrder = 2,
                [q.Event.MouseButton1Click] = function()
                    z:Update(function(F)
                        F.maximized = not F.maximized
        ...
        if y.canExit ~= false then
            E.Close = q.createElement('ImageButton', {
                Size = UDim2.new(0, 14, 0, 14),
                BackgroundTransparency = 1,
                Image = v['lucide-x'],
                ImageColor3 = s.Colors.TextSecondary,
                LayoutOrder = 3,
                [q.Event.MouseButton1Click] = function()
                    if x.state.closing then
                        return
                    end

                    x:setState{closing = true}

                    local F = x.windowRef.current

                    if F then
                        local G = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                        local H = p:Create(F, G, {
                            Position = UDim2.new(F.Position.X.Scale, F.Position.X.Offset, F.Position.Y.Scale, F.Position.Y.Offset + 15),
                            GroupTransparency = 1,
                        })
        ...
        return q.createElement('CanvasGroup', {
            ref = x.windowRef,
            Size = C,
            Position = D,
            BackgroundColor3 = s.Colors.Background,
            BorderSizePixel = 0,
            Active = true,
            ClipsDescendants = true,
        }, {
            Corner = q.createElement('UICorner', {
                CornerRadius = s.CornerRadius.Window,
            }),
            Stroke = q.createElement('UIStroke', {
                Color = s.Colors.Border,
                Thickness = 1,
            ...
            TitleBar = q.createElement('Frame', {
                Size = UDim2.new(1, 0, 0, 45),
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
                ZIndex = 2,
            }, {
                Icon = q.createElement('ImageLabel', {
                    Size = UDim2.new(0, 20, 0, 20),
                    Position = UDim2.new(0, 15, 0.5, -10),
                    BackgroundTransparency = 1,
                    Image = 'rbxassetid://10734950309',
                    ImageColor3 = s.Colors.Accent,
                }),
                TitleArea = q.createElement('Frame', {
                    Size = y.searching and UDim2.new(1, -310, 1, 0) or UDim2.new(1, 
-180, 1, 0),
                    ...
                    TitleTag = y.titleTag and q.createElement(TagComponent, {
                        text = y.titleTag,
                        color = y.titleTagColor,
                        LayoutOrder = 2,
                    }),
                    Subtitle = y.subtitle ~= '' and q.createElement('TextLabel', {
                        Size = UDim2.new(0, 0, 1, 0),
                        AutomaticSize = Enum.AutomaticSize.X,
                        BackgroundTransparency = 1,
                        Text = y.subtitle,
                    ...
                    SubtitleTag = y.subtitleTag and q.createElement(TagComponent, {
                        text = y.subtitleTag,
                        color = y.subtitleTagColor,
                        LayoutOrder = 4,
                    }),
                }),
                SearchBar = y.searching and q.createElement('Frame', {
                    ...
                    Corner = q.createElement('UICorner', {
                        CornerRadius = s.CornerRadius.Element,
                    }),
                    Stroke = q.createElement('UIStroke', {
                        Color = s.Colors.Border,
                        Thickness = 1,
                    ...
                    Icon = q.createElement('ImageLabel', {
                        ...
                        BackgroundTransparency = 1,
                        Image = v['lucide-search'] or 'rbxassetid://10734943674',
                        ImageColor3 = s.Colors.TextSecondary,
                    }),
                    Input = q.createElement('TextBox', {
                        Size = UDim2.new(1, -16, 1, 0),
                        Position = UDim2.new(0, 16, 0, 0),
                        BackgroundTransparency = 1,
                        Text = y.searchQuery,
                        PlaceholderText = 'Search...',
                        PlaceholderColor3 = s.Colors.TextSecondary,
                        TextColor3 = s.Colors.Text,
                        Font = s.Fonts.Regular,
                        TextSize = 11,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        ClearTextOnFocus = false,
                        [q.Change.Text] = function(F)
                            z:Update(function(G)
                                G.searchQuery = F.Text
                            end)
                        end,
                    }),
                }),
                Controls = q.createElement('Frame', {
                    Size = UDim2.new(0, 100, 1, 0),
                    Position = UDim2.new(1, -115, 0, 0),
                    BackgroundTransparency = 1,
                }, E),
            }),
            Container = q.createElement('CanvasGroup', {
                ref = x.containerRef,
                ...
                Sidebar = q.createElement(t, {
                    state = y,
                    Store = z,
                    LayoutOrder = 1,
                }),
                MainContent = q.createElement(u, {
                    state = y,
                    Store = z,
                    LayoutOrder = 2,
                }),
            }),
            ColorPickerOverlay = y.colorPickerActive and q.createElement(l(r.Components.ColorPickerDialog), {
                colorPickerActive = y.colorPickerActive,
                Store = z,
            }) or nil,
        })
    end
...
d.init = function()
    local k = createVirtualModule'init'
    ...
    local n = m:GetService'ReplicatedStorage'
    local o = l(n.Packages.React)
    local p = l(n.Packages.ReactRoblox)
    local q = l(k.Theme)
    local r = l(k.Store)
    local s = l(k.AppRecorder)
    local t = {}
    ...
    local u = t.Icons.assets

    local function toKebabCase(v)
        local w = v:gsub('(%u)', '-%1'):gsub('(%d+)', '-%1'):lower()
    ...
    setmetatable(v, {
        __index = function(w, x)
            ...
            if x == 'gear' or x == 'settings' then
                return u['lucide-settings']
            elseif x == 'house' or x == 'home' then
                return u['lucide-home']
            elseif x == 'checkmark' or x == 'check' then
                return u['lucide-check']
            end
    ...
    t.Creator = {
        Create = function(w)
            return function(x)
                ...
                for z, A in pairs(x)do
                    if type(z) == 'string' then
                        ...
                        if type(A) == 'table' then
                            if A.__instance then
                                B = A.__instance
                            else
                                B = nil
                            end
                        end
                        if B ~= nil or z ~= 'Parent' then
                            y[z] = B
                        end
                    else
                        local B = (type(A) == 'table' and A.__instance) and A.__instance or A
                ...
                local z = {__instance = y}

                setmetatable(z, {
                    __index = function(A, B)
                        if B == '__instance' then
                            return y
                        else
                            return y[B]
                        end
    ...
    function t.RegisterComponent(w, x)
        t._customComponents[w] = x
    end

    local w = table.clone or function(w)
        local x = {}
    ...
    local function getGuiParent()
        ...
        local B = m:GetService'Players'
        local C = B.LocalPlayer

        if C then
            local D = C:FindFirstChild'PlayerGui' or C:WaitForChild('PlayerGui', 5)
        ...
        local D, E = pcall(function()
            return m:GetService'StarterGui'
        end)
    ...
    local function wrapCallback(z, A)
        if not A then
            return nil
        end
    ...
    local function setupContainerMethods(z, A)
        function z.Row(B, C)
            C = C or {}

            local D = math.random(1, 100000)
            local E = {
                id = D,
                type = 'Row',
                leftElements = {},
            ...
            function F.Left(G)
                ...
                setupContainerMethods(H, function(I)
                    table.insert(E.leftElements, I)
                    r:Update(function(J) end)
                end)
            ...
            function F.Right(G)
                ...
                setupContainerMethods(H, function(I)
                    ...
                    table.insert(E.rightElements, I)
                    r:Update(function(J) end)
                end)
        ...
        function z.TitleStack(B, C)
            ...
            local D = {
                ...
                subtitle = C.Subtitle,
                titleTag = C.TitleTag,
                titleTagColor = C.TitleTagColor,
                subtitleTag = C.SubtitleTag,
                subtitleTagColor = C.SubtitleTagColor,
            }
            ...
            function E.SetTitle(F, G)
                D.title = G

                r:Update(function(H) end)
            end
            function E.SetSubtitle(F, G)
                D.subtitle = G
        ...
        function z.Toggle(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'Toggle',
                name = C.Label or 'Toggle',
                value = C.Value or false,
                callback = wrapCallback(D, C.ValueChanged),
            }

            A(E)

            function D.SetValue(F, G)
                E.value = G

                r:Update(function(H) end)
            end
            function D.SetText(F, G)
                E.name = G

                r:Update(function(H) end)
            end
            function D.SetCallback(F, G)
                E.callback = wrapCallback(D, G)
            end

            return D
        end
        function z.TextField(B, C)
            C = C or {}
        ...
        function z.ColorPicker(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'ColorPicker',
                name = C.Title or 'Color Picker',
                value = C.Value or Color3.fromRGB(255, 255, 255),
                transparency = C.Transparency or 0,
                callback = wrapCallback(D, C.ValueChanged),
            }
        ...
        function z.ColorPickerWidget(B, C)
            C = C or {}
        ...
        function z.Button(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'Button',
                name = C.Label or 'Button',
                icon = C.Icon,
                state = C.State,
                callback = wrapCallback(D, C.Pushed),
            }
        ...
        function z.Slider(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'Slider',
                name = C.Label or 'Slider',
                min = C.Minimum or 0,
                max = C.Maximum or 100,
                value = C.Value or C.Minimum or 0,
                callback = wrapCallback(D, C.ValueChanged),
            }
            ...
            function D.SetMin(F, G)
                E.min = G

                r:Update(function(H) end)
            end
            function D.SetMax(F, G)
                E.max = G
        ...
        function z.Stepper(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'Stepper',
                name = C.Label or 'Stepper',
                min = C.Minimum or 0,
                max = C.Maximum or 100,
                step = C.Step or 1,
                value = C.Value or C.Minimum or 0,
                fielded = C.Fielded ~= false,
                callback = wrapCallback(D, C.ValueChanged),
            }
        ...
        function z.RadioButtonGroup(B, C)
            C = C or {}
        ...
        function z.PopUpButton(B, C)
            ...
            local E = {
                id = math.random(1, 100000),
                type = 'Dropdown',
                name = C.Label or 'Dropdown',
                options = C.Options or {},
                value = C.Value,
                maximum = C.Maximum or (C.Multi and 9999 or (type(C.Value) == 'table' and 9999 or 1)),
                callback = wrapCallback(D, C.ValueChanged),
            }
        ...
        function z.VStack(B, C)
            ...
            setupContainerMethods(E, function(F)
                table.insert(D.elements, F)
                r:Update(function(G) end)
            end)

            return E
        end
        function z.HStack(B, C)
            C = C or {}
        ...
        function z.BarChart(B, C)
            ...
            function E.SetData(F, G)
                D.data = G or {}

                r:Update(function(H) end)
            end
            function E.PushData(F, G, H)
                table.insert(D.data, G)

                H = H or 10

                while#D.data > H do
                    table.remove(D.data, 1)
                end
        ...
        function z.LineChart(B, C)
            ...
            function E.PushData(F, G, H, I)
                table.insert(D.data, G)

                if H ~= nil or #D.labels > 0 then
                    table.insert(D.labels, H or '')
                end
        ...
        function z.PageSection(B, C)
            ...
            local E = {
                ...
                column = C.Column or 1,
                gradientPreset = C.GradientPreset,
                gradient = C.Gradient,
                elements = {},
            }
            ...
            function F.Form(G)
                ...
                table.insert(E.elements, H)
                setupContainerMethods(H, function(I)
                    table.insert(H.elements, I)
                    r:Update(function(J) end)
                end)
            ...
            __index = function(B, C)
                if t._customComponents[C] then
                    return function(D, E)
                        ...
                        local G = t._customComponents[C](D, E)

                        F.instance = G and G.__instance
    ...
    function t.New(z)
        ...
        if z.Theme then
            for C, D in pairs(q.Themes)do
                if D == z.Theme or C == z.Theme then
                    A = C
        ...
        if z.Accent then
            for C, D in pairs(q.Accents)do
                if D == z.Accent or C == z.Accent then
                    B = C
        ...
        local C = setmetatable({
            ...
            __index = function(C, D)
                if D == 'Theme' then
                    return q.Themes[r._state.theme]
                elseif D == 'Accent' then
                    return q.Accents[r._state.accent]
                elseif D == 'Notification' then
                    return function(E, F)
                        t:Notification(F)
                    end
                elseif D == 'Page' then
    ...
    function t.CreateWindow(z, A)
        A = A or {}

        r:Update(function(B)
            B.title = A.Title or A.Name or 'Selene UI'
            B.subtitle = A.Subtitle or ''
            B.titleTag = A.TitleTag
            B.titleTagColor = A.TitleTagColor
            B.subtitleTag = A.SubtitleTag
            B.subtitleTagColor = A.SubtitleTagColor
            B.searching = A.Searching ~= false
            B.draggable = A.Draggable ~= false
            B.canExit = A.CanExit ~= false
            B.canMinimize = A.CanMinimize ~= false
            B.canZoom = A.CanZoom ~= false
            B.minimized = A.Minimized == true
            B.maximized = A.Maximized == true
            B.sections = {}
            B.activeTab = nil
            B.visible = true
            B.profile = nil
        ...
        if not y then
            ...
            y.Name = 'SeleneUILibrary'
            y.ResetOnSpawn = false
            y.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            y.Parent = B
        end

        local B = A.ToggleKey or Enum.KeyCode.RightControl
        local C = m:GetService'UserInputService'

        C.InputBegan:Connect(function(D, E)
            if E then
                return
            end
            if D.KeyCode == B then
        ...
        if not x then
            local function App()
                local D, E = o.useState(r._state)

                o.useEffect(function()
                    local F = r:Subscribe(function(F)
                        local G = q.Themes[F.theme or 'Dark']

                        for H, I in pairs(G)do
                            q.Colors[H] = I
                        end

                        q.Colors.Accent = q.Accents[F.accent or 'Blue']
                ...
                local F = k
                local G = l(F.Components.Window)
                local H = l(F.Components.Notification)

                return o.createElement('Frame', {
                    Size = UDim2.new(1, 0, 1, 0),
                    BackgroundTransparency = 1,
                }, {
                    Window = D.visible and o.createElement(G, {
                        state = D,
                        Store = r,
                    }) or nil,
                    Notifications = o.createElement(H.List, {
            ...
            x:render(o.createElement(App))
        end

        local D = {}
        local E = Instance.new'BindableEvent'

        D.Destroying = E.Event

        setmetatable(D, {
            __index = function(F, G)
                if G == 'Minimized' then
                    return r._state.minimized
                elseif G == 'Maximized' then
                    return r._state.maximized
                elseif G == 'Visible' then
                    return r._state.visible
                else
                    return rawget(F, G)
                end
            end,
        ...
        function D.SetProfile(F, G)
            r:Update(function(H)
                H.profile = {
                    username = G.Username or 'User',
                    subtext = G.Subtext or 'Developer',
                    userId = G.UserId or 1,
                }
            end)
        end
        function D.Section(F, G)
            G = G or {}

            local H = #r._state.sections + 1
            local I = {
                id = H,
                title = G.Title or '',
                disclosure = G.Disclosure == true,
                tabs = {},
            }

            r:Update(function(J)
                table.insert(J.sections, I)
            end)
            ...
            function J.Tab(K, L)
                ...
                local N = {
                    id = M,
                    title = L.Title or 'Tab',
                    icon = L.Icon,
                    pageSections = {},
                }

                r:Update(function(O)
                    for P, Q in ipairs(O.sections)do
                        if Q.id == H then
                            table.insert(Q.tabs, N)

                            break
                        end
                    end

                    table.insert(O.tabs, N)

                    if not O.activeTab then
                        O.activeTab = M
                    end
                end)

                local function buildTabObj(O, P)
                    local Q = {}

                    function Q.SetBanner(R, S)
                        r:Update(function(T)
                            for U, V in ipairs(T.sections)do
                                for W, X in ipairs(V.tabs)do
                                    if X.id == O then
                                        X.banner = {
                                            username = S.Username or 'User',
                                            subtext = S.Subtext or '',
                                            userId = S.UserId or 1,
                                        }
                    ...
                    function Q.Navigate(R, S)
                        r:Update(function(T)
                            for U, V in ipairs(T.sections)do
                                for W, X in ipairs(V.tabs)do
                                    if X.id == O then
                                        X.pageSections = S.elements
                    ...
                    function Q.Form(R)
                        ...
                        setupContainerMethods(S, function(T)
                            table.insert(S.elements, T)
                            r:Update(function(U) end)
                        end)

                        return S
                    end
                    function Q.PageSection(R, S)
                        S = S or {}

                        local T = math.random(1, 100000)
                        local U = {
                            id = T,
                            type = 'Section',
                            name = S.Title,
                            description = S.Subtitle,
                            column = S.Column or 1,
                        ...
                        r:Update(function(V)
                            for W, X in ipairs(V.sections)do
                                for Y, Z in ipairs(X.tabs)do
                                    if Z.id == O then
                                        table.insert(Z.pageSections, U)
                        ...
                        function V.Form(W)
                            ...
                            table.insert(U.elements, X)
                            setupContainerMethods(X, function(Y)
                                table.insert(X.elements, Y)
                                r:Update(function(Z) end)
                            end)
                        ...
                        r:Update(function(V)
                            for W, X in ipairs(V.sections)do
                                if X.id == P then
                                    ...
                                    if Y then
                                        table.insert(X.tabs, Y + 1, U)
                                    else
                                        table.insert(X.tabs, U)
                                    end
                            ...
                            table.insert(V.tabs, U)
                        end)

                        return buildTabObj(T, P)
                    end

                    return Q
                end

                return buildTabObj(M, H)
            end
    ...
    function t.Notification(z, A)
        A = A or {}

        local B = A.Title or 'Notification'
        local C = A.Subtitle or ''
        local D = A.AppIcon or A.Icon
        local E = A.Duration or 5
        local F
        local G = os.clock()
        ...
        r:Update(function(H)
            ...
            for J, K in ipairs(H.notifications)do
                if K.title == B and K.subtitle == C then
                    I = K
            ...
            if I then
                F = I.id
                I.count = (I.count or 1) + 1
                I.spawnTime = G
                I.duration = E
            else
                F = math.random(1, 100000)
                ...
                table.insert(H.notifications, J)
            end

            H.notifications = w(H.notifications)
        end)
        task.spawn(function()
            task.wait(E)
            r:Update(function(H)
                ...
                for J, K in ipairs(H.notifications)do
                    if K.id == F then
                        if os.clock() >= (K.spawnTime or 0) + (K.duration or 5) - 0.05 then
                            I = J
                        end
                ...
                if I then
                    table.remove(H.notifications, I)
    ...
    function t.Page(z)
        ...
        function A.Form(B)
            ...
            table.insert(A.elements, C)
            setupContainerMethods(C, function(D)
                table.insert(C.elements, D)
                r:Update(function(E) end)
            end)
...
d.React = function()
    createVirtualModule'React'
    createVirtualFolder''

    local k = {}

    k.Component = {}
    k.Component.__index = k.Component

    function k.Component.extend(l, m)
        local n = {}

        n.__index = n

        setmetatable(n, {__index = l})

        n.__className = m or 'Component'

        return n
    end
    function k.Component.new(l)
    ...
    function k.Component.setState(l, m, n)
        local o = {}

        for p, q in pairs(l.state or {})do
            o[p] = q
        end
    ...
    function k.createRef()
        return {current = nil}
    end

    k.Event = setmetatable({}, {
        __index = function(l, m)
            return 'Event:' .. m
        end,
    })
    k.Change = setmetatable({}, {
        __index = function(l, m)
            return 'Change:' .. m
        end,
    })

    function k.useState(l)
        local m = l
    ...
    function k.useEffect(l, m)
        task.defer(l)
    end
    function k.createElement(l, m, n)
        m = m or {}

        if type(l) == 'string' then
            local o = Instance.new(l)

            for p, q in pairs(m)do
                if type(p) == 'string' then
                    if p:sub(1, 6) == 'Event:' then
                        local r = p:sub(7)

                        if o[r] then
                            o[r]:Connect(q)
                        end
                    elseif p:sub(1, 7) == 'Change:' then
                        local r = p:sub(8)

                        if o:IsA'Instance' then
                            o:GetPropertyChangedSignal(r):Connect(function()
                                q(o)
                            end)
                        end
                    elseif p == 'ref' then
                    ...
                    elseif p == 'Children' or p == 'Key' or p == 'key' then
                    else
                        pcall(function()
                            o[p] = q
                        end)
            ...
            local function addChildren(p)
                ...
                elseif type(p) == 'table' then
                    if p.__isInstanceWrapper then
                        p.__instance.Parent = o
                    elseif p.Parent ~= nil and typeof(p) == 'Instance' then
                        p.Parent = o
                    else
                        for q, r in pairs(p)do
                            if typeof(r) == 'Instance' then
                                r.Parent = o
                            elseif type(r) == 'table' and r.__nodeInstance then
                                r.__nodeInstance.Parent = o
            ...
            addChildren(n)
            addChildren(m.Children)

            return o
        elseif type(l) == 'table' and l.__index then
            ...
            function o._rerender(s, t)
                local u = o:render()

                if p and p ~= u then
                    if typeof(u) == 'Instance' and typeof(p) == 'Instance' then
                        u.Parent = p.Parent
            ...
            if m.ref then
                if type(m.ref) == 'table' then
                    m.ref.current = o
                elseif type(m.ref) == 'function' then
                    m.ref(o)
                end
            end
            if o.didMount then
...
d.ReactRoblox = function()
    createVirtualModule'ReactRoblox'
    createVirtualFolder''

    local k = {}

    function k.createRoot(l)
        return {
            render = function(m, n)
                l:ClearAllChildren()
                ...
                if typeof(n) == 'Instance' then
                    n.Parent = l
                elseif type(n) == 'table' and n.__nodeInstance then
                    n.__nodeInstance.Parent = l
                end
            end,
            unmount = function(m)
                l:ClearAllChildren()
...
d.Store = function()
    ...
    local l = table.find or function(l, m)
    ...
    function k.Subscribe(m, n)
        table.insert(m._listeners, n)

        return function()
            local o = l(m._listeners, n)

            if o then
                table.remove(m._listeners, o)
            end
        end
    end
    function k.Update(m, n)
        n(m._state)

        for o, p in ipairs(m._listeners)do
            p(m._state)
        end
    end
    function k.UpdateElement(m, n, o)
        m:Update(function(p)
            local function searchElements(q, r, s)
                for t, u in ipairs(q)do
                    ...
                    if u.elements and searchElements(u.elements, r, s) then
                        return true
                    end
                    if u.leftElements and searchElements(u.leftElements, r, s) then
                        return true
                    end
                    if u.rightElements and searchElements(u.rightElements, r, s) then
                        return true
                    end
                end
            ...
            for q, r in ipairs(p.sections or {})do
                for s, t in ipairs(r.tabs or {})do
                    if t.pageSections and searchElements(t.pageSections, n, o) then
                        return
                    end
                end
            end
    ...
    function k.SetColorPicker(m, n)
        m:Update(function(o)
            o.colorPickerActive = n
        end)
    end
...
return __require(createVirtualModule'init')
