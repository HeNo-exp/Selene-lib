...
local function getReactPackage(name)
    ...
    error("[Selene Library] Required package '" .. tostring(name) .. "' not found in ReplicatedStorage.Packages or getgenv()." .. tostring(name))
end
...
local ReplicatedStorage = setmetatable({}, {
    __index = function(t, k)
        if k == "Packages" then
            ...
            return setmetatable({}, {
                __index = function(_, pkgName)
                    return createPkgProxy(pkgName)
                end
            })
        elseif k == "FindFirstChild" or k == "WaitForChild" then
            return function(self, childName, timeOut)
