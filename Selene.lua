--[[
    Selene UI Library - Standalone Bundle (Lightweight)
    Generated automatically.
--]]

local real_game = game
local real_ReplicatedStorage = real_game:GetService("ReplicatedStorage")
...
local function createVirtualFolder(path)
    if __vfolders[path] then return __vfolders[path] end
    local folder = { __path = path, __isFolder = true }
    setmetatable(folder, {
        __index = function(t, k)
            if k == "Parent" then
                if path == "" or path == "src" then
                ...
                table.remove(parts)
                return createVirtualFolder(table.concat(parts, "/"))
            ...
            elseif k == "IsA" then
                return function(self, className)
                    return className == "Folder" or className == "ModuleScript" or className == "Instance"
                end
            end
            local childPath = path == "" and k or (path .. "/" .. k)
            
            if __modules[childPath] then
                return createVirtualModule(childPath)
            else
                return createVirtualFolder(childPath)
            end
        end
    })
    __vfolders[path] = folder
    return folder
end

local function createVirtualModule(path)
    if __vmodules[path] then return __vmodules[path] end
    local mod = { __path = path, __isModule = true }
    setmetatable(mod, {
        __index = function(t, k)
            if k == "Parent" then
                local parts = {}
            ...
            elseif k == "FindFirstChild" or k == "WaitForChild" then
                return function(self, childName, timeOut)
                    local parentFolder = t.Parent
                    if parentFolder then
                        return parentFolder[childName]
                    end
                    return nil
                end
            elseif k == "IsA" then
...
local function getReactPackage(name)
    -- 1. Try real ReplicatedStorage.Packages
    local pkgs = real_ReplicatedStorage:FindFirstChild("Packages")
    if pkgs and pkgs:FindFirstChild(name) then
        local success, res = pcall(real_require, pkgs[name])
        if success then return res end
    end
    -- 2. Try global environment (getgenv, shared)
    local env = (typeof(getgenv) == "function" and getgenv()) or shared
    if env then
        if env[name] then return env[name] end
        if name == "React" and env.React then return env.React end
        if name == "ReactRoblox" and env.ReactRoblox then return env.ReactRoblox end
    end
    -- 3. Try CoreGui.Packages
    local coreGui = real_game:GetService("CoreGui")
    local corePkgs = coreGui:FindFirstChild("Packages")
    if corePkgs and corePkgs:FindFirstChild(name) then
        local success, res = pcall(real_require, corePkgs[name])
        if success then return res end
    end

    error("[Selene Library] Package '" .. tostring(name) .. "' not found in ReplicatedStorage.Packages or getgenv()." .. tostring(name) .. ". Please ensure React is loaded in getgenv() or ReplicatedStorage.Packages.")
end

local function createPkgProxy(pkgName)
    return { __isPkgProxy = true, __pkgName = pkgName }
end

local function __require(target)
    if type(target) == "table" and target.__isModule then
        local name = target.__path
        if __cache[name] then
            return __cache[name]
        end
        local fn = __modules[name]
        if not fn then
            error("[Selene Library] Module not found: " .. tostring(name))
        end
        local res = fn()
        __cache[name] = res
        return res
    elseif type(target) == "table" and target.__isPkgProxy then
        return getReactPackage(target.__pkgName)
    end
    return real_require(target)
end

local VirtualPackages = setmetatable({}, {
    __index = function(t, k)
        local realPkgs = real_ReplicatedStorage:FindFirstChild("Packages")
        if realPkgs and realPkgs:FindFirstChild(k) then
            return realPkgs[k]
        end
        return createPkgProxy(k)
    end,
    __call = function(t, k)
        local realPkgs = real_ReplicatedStorage:FindFirstChild("Packages")
        if realPkgs and realPkgs:FindFirstChild(k) then
...
local ReplicatedStorage = setmetatable({}, {
    __index = function(t, k)
        if k == "Packages" then
            return VirtualPackages
        elseif k == "FindFirstChild" or k == "WaitForChild" then
            return function(self, childName, timeOut)
                if childName == "Packages" then
                    return VirtualPackages
                end
                return real_ReplicatedStorage:FindFirstChild(childName)
            end
        end
        return real_ReplicatedStorage[k]
    end
})

__modules["AppRecorder"] = function()
    local script = createVirtualModule("AppRecorder")
    local Source = createVirtualFolder("")
...
__modules["Components/BarChart"] = function()
    local script = createVirtualModule("Components/BarChart")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function BarChart(props)
    ...
    for _, item in ipairs(data) do
        local val = type(item) == "table" and (item.value or item.Value or 0) or (type(item) == "number" and item or 0)
    ...
    corner.CornerRadius = Theme.CornerRadius.Card
    ...
    if title or subtitle then
        ...
        if title then
            ...
            tLabel.Text = title
            tLabel.TextColor3 = Theme.Colors.Text
        ...
        if subtitle then
            ...
            subLabel.Font = Theme.Fonts.Regular
    ...
    chartArea.Size = UDim2.new(1, 0, 1, (title or subtitle) and -40 or -20)
    chartArea.BackgroundTransparency = 1
    chartArea.LayoutOrder = 2
    chartArea.Parent = container

    local barsFrame = Instance.new("Frame")
    barsFrame.Size = UDim2.new(1, 0, 1, -16)
    barsFrame.BackgroundTransparency = 1
    barsFrame.Parent = chartArea

    local gap = 12
    local barsList = Instance.new("UIListLayout")
    barsList.FillDirection = Enum.FillDirection.Horizontal
    barsList.Padding = UDim.new(0, gap)
    barsList.SortOrder = Enum.SortOrder.LayoutOrder
    barsList.Parent = barsFrame

    for i, item in ipairs(data) do
        local val = type(item) == "table" and (item.value or item.Value or 0) or (type(item) == "number" and item or 0)
        local color = type(item) == "table" and (item.color or item.Color or Theme.Colors.Accent) or Theme.Colors.Accent
        local percentage = math.clamp(val / maxVal, 0.05, 1)

        local scale = N > 0 and (1 / N) or 1
        local offset = -gap * (1 - scale)

        local col = Instance.new("Frame")
        col.Size = UDim2.new(scale, offset, 1, 0)
        col.BackgroundTransparency = 1
        col.LayoutOrder = i
        col.Parent = barsFrame

        local bar = Instance.new("Frame")
        bar.Size = UDim2.new(0.6, 0, 0, 0)
        ...
        barCorner.CornerRadius = Theme.CornerRadius.Element
        barCorner.Parent = bar

        local valLabel = Instance.new("TextLabel")
        valLabel.Size = UDim2.new(1.5, 0, 0, 14)
        valLabel.Position = UDim2.new(0.5, 0, 0, -16)
        ...
        local targetSize = UDim2.new(0.6, 0, percentage * 0.85, 0)
    ...
    xAxisFrame.Size = UDim2.new(1, 0, 0, 14)
    xAxisFrame.Position = UDim2.new(0, 0, 1, -14)
    xAxisFrame.BackgroundTransparency = 1
    xAxisFrame.Parent = chartArea

    local xList = Instance.new("UIListLayout")
    xList.FillDirection = Enum.FillDirection.Horizontal
    xList.Padding = UDim.new(0, gap)
    xList.SortOrder = Enum.SortOrder.LayoutOrder
    xList.Parent = xAxisFrame

    for i, item in ipairs(data) do
        local labelText = type(item) == "table" and (item.label or item.Label or "") or tostring(i)
        local scale = N > 0 and (1 / N) or 1
        local offset = -gap * (1 - scale)

        local xLabel = Instance.new("TextLabel")
        xLabel.Size = UDim2.new(scale, offset, 1, 0)
        xLabel.BackgroundTransparency = 1
        xLabel.Text = labelText
        xLabel.TextColor3 = Theme.Colors.TextSecondary
        xLabel.Font = Theme.Fonts.Medium
        xLabel.TextSize = 9
        xLabel.LayoutOrder = i
        xLabel.Parent = xAxisFrame
    end
...
__modules["Components/Button"] = function()
    local script = createVirtualModule("Components/Button")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Button(props)
    ...
    local corner = Instance.new("UICorner")
    corner.CornerRadius = Theme.CornerRadius.Element
    corner.Parent = btn
...
__modules["Components/ColorPicker"] = function()
    local script = createVirtualModule("Components/ColorPicker")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function ColorPicker(props)
    ...
    hexLabel.Size = UDim2.new(0, 100, 1, 0)
    hexLabel.BackgroundTransparency = 1
    hexLabel.Text = hexStr
    hexLabel.TextColor3 = Theme.Colors.Text
    hexLabel.Font = Theme.Fonts.Bold
    hexLabel.TextSize = 11
    hexLabel.TextXAlignment = Enum.TextXAlignment.Right
    hexLabel.LayoutOrder = 1
    hexLabel.Parent = container

    local colorBtn = Instance.new("TextButton")
    colorBtn.Size = UDim2.fromOffset(24, 24)
    colorBtn.BackgroundColor3 = currentColor
    colorBtn.BackgroundTransparency = currentTrans
    colorBtn.BorderSizePixel = 0
    colorBtn.Text = ""
    colorBtn.AutoButtonColor = false
    colorBtn.LayoutOrder = 2
...
__modules["Components/ColorPickerDialog"] = function()
    local script = createVirtualModule("Components/ColorPickerDialog")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function ColorPickerDialog(props)
    ...
    backdrop.ZIndex = 1
    backdrop.Parent = dialogFrame

    backdrop.MouseButton1Down:Connect(function()
        if Store then Store:SetColorPicker(nil) end
    end)

    -- Card Container
    local card = Instance.new("TextButton")
    card.Size = UDim2.new(0, 250, 0, 340)
    ...
    card.Active = true
    card.ZIndex = 2
    card.Parent = dialogFrame

    local cardCorner = Instance.new("UICorner")
    cardCorner.CornerRadius = Theme.CornerRadius.Card
    cardCorner.Parent = card

    local cardStroke = Instance.new("UIStroke")
    cardStroke.Color = Theme.Colors.Border
    ...
    title.Text = el.name or "Accent Tint Color"
    ...
    -- Main Row (Wheel + Inputs)
    local mainRow = Instance.new("Frame")
    mainRow.Size = UDim2.new(1, 0, 0, 100)
    ...
    wheelKnob.Size = UDim2.new(0, 8, 0, 8)
    ...
    wheelKnob.Parent = wheel

    local knobCorner = Instance.new("UICorner")
    knobCorner.CornerRadius = UDim.new(0.5, 0)
    knobCorner.Parent = wheelKnob

    local knobStroke = Instance.new("UIStroke")
    knobStroke.Color = Color3.fromRGB(0, 0, 0)
    knobStroke.Thickness = 1.5
    knobStroke.Parent = wheelKnob

    -- Fields Container
    local fields = Instance.new("Frame")
    fields.Size = UDim2.new(1, -110, 1, 0)
    fields.Position = UDim2.new(1, 0, 0, 0)
    fields.AnchorPoint = Vector2.new(1, 0)
    fields.BackgroundTransparency = 1
    fields.Parent = mainRow

    local fieldsList = Instance.new("UIListLayout")
    fieldsList.FillDirection = Enum.FillDirection.Vertical
    fieldsList.Padding = UDim.new(0, 2)
    fieldsList.SortOrder = Enum.SortOrder.LayoutOrder
    fieldsList.Parent = fields

    local boxes = {}

    local function createInputField(labelText, layoutOrder)
        local fRow = Instance.new("Frame")
        fRow.Size = UDim2.new(1, 0, 0, 18)
        fRow.BackgroundTransparency = 1
        fRow.LayoutOrder = layoutOrder
        fRow.Parent = fields

        local box = Instance.new("TextBox")
        box.Size = UDim2.new(0, 60, 1, 0)
        box.BackgroundColor3 = Theme.Colors.Element
        box.BorderSizePixel = 0
        box.TextColor3 = Theme.Colors.Text
        box.Font = Theme.Fonts.Medium
        box.TextSize = 10
        ...
        boxStroke.Color = Theme.Colors.Border
        boxStroke.Thickness = 1
        boxStroke.Parent = box

        local lbl = Instance.new("TextLabel")
        lbl.Size = UDim2.new(1, -66, 1, 0)
        lbl.Position = UDim2.new(0, 66, 0, 0)
    ...
    boxes.Hex = createInputField("Hex", 1)
    boxes.R = createInputField("Red", 2)
    boxes.G = createInputField("Green", 3)
    boxes.B = createInputField("Blue", 4)
    boxes.Alpha = createInputField("Alpha", 5)

    -- Sliders Area (Brightness & Opacity)
    local slidersArea = Instance.new("Frame")
    slidersArea.Size = UDim2.new(1, 0, 0, 48)
    slidersArea.BackgroundTransparency = 1
    slidersArea.LayoutOrder = 3
    slidersArea.Parent = card

    local slidersList = Instance.new("UIListLayout")
    slidersList.FillDirection = Enum.FillDirection.Vertical
    slidersList.Padding = UDim.new(0, 6)
    slidersList.SortOrder = Enum.SortOrder.LayoutOrder
    slidersList.Parent = slidersArea

    -- Brightness Bar
    local bFrame = Instance.new("Frame")
    bFrame.Size = UDim2.new(1, 0, 0, 20)
    bFrame.BackgroundTransparency = 1
    bFrame.LayoutOrder = 1
    bFrame.Parent = slidersArea

    local bLabel = Instance.new("TextLabel")
    bLabel.Size = UDim2.new(0, 60, 1, 0)
    bLabel.BackgroundTransparency = 1
    bLabel.Text = "Brightness"
    bLabel.TextColor3 = Theme.Colors.TextSecondary
    bLabel.Font = Theme.Fonts.Medium
    bLabel.TextSize = 10
    bLabel.TextXAlignment = Enum.TextXAlignment.Left
    bLabel.Parent = bFrame

    local bBar = Instance.new("Frame")
    bBar.Size = UDim2.new(1, -65, 0, 8)
    bBar.Position = UDim2.new(0, 65, 0.5, -4)
    ...
    bKnob.Size = UDim2.new(0, 6, 0, 14)
    bKnob.AnchorPoint = Vector2.new(0.5, 0.5)
    bKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    bKnob.BorderSizePixel = 0
    bKnob.Parent = bBar

    local bKnobCorner = Instance.new("UICorner")
    bKnobCorner.CornerRadius = UDim.new(0, 2)
    bKnobCorner.Parent = bKnob

    local bKnobStroke = Instance.new("UIStroke")
    bKnobStroke.Color = Theme.Colors.Border
    bKnobStroke.Thickness = 1
    bKnobStroke.Parent = bKnob

    -- Opacity Bar
    local oFrame = Instance.new("Frame")
    oFrame.Size = UDim2.new(1, 0, 0, 20)
    oFrame.BackgroundTransparency = 1
    oFrame.LayoutOrder = 2
    oFrame.Parent = slidersArea

    local oLabel = Instance.new("TextLabel")
    oLabel.Size = UDim2.new(0, 60, 1, 0)
    oLabel.BackgroundTransparency = 1
    oLabel.Text = "Opacity"
    oLabel.TextColor3 = Theme.Colors.TextSecondary
    oLabel.Font = Theme.Fonts.Medium
    oLabel.TextSize = 10
    oLabel.TextXAlignment = Enum.TextXAlignment.Left
    oLabel.Parent = oFrame

    local oBar = Instance.new("Frame")
    oBar.Size = UDim2.new(1, -65, 0, 8)
    oBar.Position = UDim2.new(0, 65, 0.5, -4)
    oBar.BorderSizePixel = 0
    oBar.Active = true
    oBar.Parent = oFrame

    local oCorner = Instance.new("UICorner")
    oCorner.CornerRadius = UDim.new(0.5, 0)
    oCorner.Parent = oBar

    local oGrad = Instance.new("UIGradient")
    oGrad.Parent = oBar

    local oKnob = Instance.new("Frame")
    oKnob.Size = UDim2.new(0, 6, 0, 14)
    oKnob.AnchorPoint = Vector2.new(0.5, 0.5)
    oKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    oKnob.BorderSizePixel = 0
    oKnob.Parent = oBar

    local oKnobCorner = Instance.new("UICorner")
    oKnobCorner.CornerRadius = UDim.new(0, 2)
    oKnobCorner.Parent = oKnob

    local oKnobStroke = Instance.new("UIStroke")
    oKnobStroke.Color = Theme.Colors.Border
    oKnobStroke.Thickness = 1
    oKnobStroke.Parent = oKnob
    ...
    previewRow.Parent = card

    local previewStroke = Instance.new("UIStroke")
    previewStroke.Color = Theme.Colors.Border
    previewStroke.Thickness = 1
    previewStroke.Parent = previewRow

    local newColorFrame = Instance.new("Frame")
    newColorFrame.Size = UDim2.new(0.5, 0, 1, 0)
    newColorFrame.Position = UDim2.new(0, 0, 0, 0)
    newColorFrame.BorderSizePixel = 0
    newColorFrame.Parent = previewRow

    local initColorFrame = Instance.new("Frame")
    initColorFrame.Size = UDim2.new(0.5, 0, 1, 0)
    initColorFrame.Position = UDim2.new(0.5, 0, 0, 0)
    initColorFrame.BackgroundColor3 = initialColor
    initColorFrame.BackgroundTransparency = initialTrans
    initColorFrame.BorderSizePixel = 0
    initColorFrame.Parent = previewRow
    ...
    confirmBtn.Parent = actionRow

    local confirmCorner = Instance.new("UICorner")
    confirmCorner.CornerRadius = Theme.CornerRadius.Element
    confirmCorner.Parent = confirmBtn

    local cancelBtn = Instance.new("TextButton")
    cancelBtn.Size = UDim2.new(1, 0, 0, 26)
    cancelBtn.BackgroundColor3 = Theme.Colors.Element
    cancelBtn.BorderSizePixel = 0
    cancelBtn.Text = "Cancel"
    cancelBtn.TextColor3 = Theme.Colors.Text
    cancelBtn.Font = Theme.Fonts.Medium
    cancelBtn.TextSize = 11
    cancelBtn.LayoutOrder = 2
    cancelBtn.Parent = actionRow

    local cancelCorner = Instance.new("UICorner")
    cancelCorner.CornerRadius = Theme.CornerRadius.Element
    ...
    local function updateVisuals()
        ...
        boxes.Hex.Text = string.format("#%02X%02X%02X", rInt, gInt, bInt)
        boxes.R.Text = tostring(rInt)
        boxes.G.Text = tostring(gInt)
        boxes.B.Text = tostring(bInt)
        boxes.Alpha.Text = tostring(aPct)

        -- Wheel knob position
        local theta = (h * 2 * math.pi) - math.pi
        local knobX = s * 50 * math.cos(theta)
        local knobY = -s * 50 * math.sin(theta)
        wheelKnob.Position = UDim2.new(0.5, knobX, 0.5, knobY)
        ...
        bKnob.Position = UDim2.new(v, 0, 0.5, 0)
        ...
        oKnob.Position = UDim2.new(a, 0, 0.5, 0)

        newColorFrame.BackgroundColor3 = activeColor
        newColorFrame.BackgroundTransparency = activeTrans
    end

    updateVisuals()

    -- Input handlers for wheel/sliders
    ...
    local function updateBrightness(input)
        local absPos = bBar.AbsolutePosition
        local absSize = bBar.AbsoluteSize
        v = math.clamp((input.Position.X - absPos.X) / absSize.X, 0, 1)
        updateVisuals()
    end

    local function updateOpacity(input)
        local absPos = oBar.AbsolutePosition
        local absSize = oBar.AbsoluteSize
        a = math.clamp((input.Position.X - absPos.X) / absSize.X, 0, 1)
        updateVisuals()
    end
    ...
    local inputChangedConn = UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            if isDraggingWheel then updateWheel(input) end
            if isDraggingBrightness then updateBrightness(input) end
            if isDraggingOpacity then updateOpacity(input) end
        end
    end)
    ...
    boxes.Hex.FocusLost:Connect(function()
        local text = boxes.Hex.Text:gsub("#", "")
        if #text == 6 then
            ...
            if rHex and gHex and bHex then
                ...
                h, s, v = nextColor:ToHSV()
                updateVisuals()
            end
        end
    end)

    local function handleRGBFocus(channel, box)
        local val = tonumber(box.Text)
        if val then
            val = math.clamp(math.round(val), 0, 255)
            local curR = channel == "R" and val or math.round(Color3.fromHSV(h, s, v).R * 255)
            local curG = channel == "G" and val or math.round(Color3.fromHSV(h, s, v).G * 255)
            local curB = channel == "B" and val or math.round(Color3.fromHSV(h, s, v).B * 255)
            local nextColor = Color3.fromRGB(curR, curG, curB)
            h, s, v = nextColor:ToHSV()
            updateVisuals()
        end
    end

    boxes.R.FocusLost:Connect(function() handleRGBFocus("R", boxes.R) end)
    boxes.G.FocusLost:Connect(function() handleRGBFocus("G", boxes.G) end)
    boxes.B.FocusLost:Connect(function() handleRGBFocus("B", boxes.B) end)

    boxes.Alpha.FocusLost:Connect(function()
        local val = tonumber(boxes.Alpha.Text)
        if val then
            a = math.clamp(val, 0, 100) / 100
            updateVisuals()
        end
    end)
    ...
    dialogFrame.Destroying:Connect(function()
...
__modules["Components/ColorPickerWidget"] = function()
    local script = createVirtualModule("Components/ColorPickerWidget")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local Source = script.Parent.Parent
local Theme = require(Source.Theme)
local Slider = require(Source.Components.Slider)

local function ColorPickerWidget(props)
    local el = props.element
    local Store = props.Store
    ...
    title.BackgroundTransparency = 1
    title.Text = el.name or "Color Picker"
    ...
    infoText.Size = UDim2.new(0.5, -24, 1, 0)
    infoText.Position = UDim2.new(1, -24, 0, 0)
    infoText.AnchorPoint = Vector2.new(1, 0)
    infoText.BackgroundTransparency = 1
    infoText.Text = string.format("#%02X%02X%02X (%d%%)", r, g, b, tPct)
    infoText.TextColor3 = Theme.Colors.Text
    infoText.Font = Theme.Fonts.Bold
    ...
    colorPreview.Position = UDim2.new(1, -16, 0.5, -8)
    colorPreview.BackgroundColor3 = currentColor
    colorPreview.BackgroundTransparency = currentTrans
    colorPreview.BorderSizePixel = 0
    colorPreview.Parent = header

    local previewCorner = Instance.new("UICorner")
    previewCorner.CornerRadius = Theme.CornerRadius.Element
    ...
    local function updateColor(newR, newG, newB, newT)
        r, g, b, currentTrans = newR, newG, newB, newT
        tPct = math.round(newT * 100)
        ...
        el.value = newColor
        el.transparency = currentTrans

        if Store then
            Store:UpdateElement(el.id, function(element)
                element.value = newColor
                element.transparency = currentTrans
            end)
        end

        if el.callback then
            task.spawn(el.callback, newColor, currentTrans)
        end
    end

    local redSlider = Slider({
        Store = Store,
        element = {
            name = "Red",
            min = 0,
            max = 255,
            value = r,
            callback = function(newR) updateColor(newR, g, b, currentTrans) end
        }
    })
    redSlider.LayoutOrder = 2
    redSlider.Parent = container

    local greenSlider = Slider({
        Store = Store,
        element = {
            name = "Green",
            min = 0,
            max = 255,
            value = g,
            callback = function(newG) updateColor(r, newG, b, currentTrans) end
        }
    })
    greenSlider.LayoutOrder = 3
    greenSlider.Parent = container

    local blueSlider = Slider({
        Store = Store,
        element = {
            name = "Blue",
            min = 0,
            max = 255,
            value = b,
            callback = function(newB) updateColor(r, g, newB, currentTrans) end
        }
    })
    blueSlider.LayoutOrder = 4
    blueSlider.Parent = container

    local transSlider = Slider({
        Store = Store,
        element = {
            name = "Transparency",
            min = 0,
            max = 100,
            value = tPct,
            callback = function(newT) updateColor(r, g, b, newT / 100) end
        }
    })
    transSlider.LayoutOrder = 5
    transSlider.Parent = container
...
__modules["Components/Dropdown"] = function()
    local script = createVirtualModule("Components/Dropdown")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Dropdown(props)
    ...
    -- Main Selector Box Button
    local selectBtn = Instance.new("TextButton")
    selectBtn.Size = UDim2.new(1, 0, 0, 36)
    selectBtn.BackgroundColor3 = Theme.Colors.Element
    ...
    selectCorner.CornerRadius = Theme.CornerRadius.Element
    ...
    local selectContent = Instance.new("Frame")
    selectContent.Size = UDim2.new(1, -24, 1, 0)
    selectContent.Position = UDim2.new(0, 12, 0, 0)
    selectContent.BackgroundTransparency = 1
    selectContent.Parent = selectBtn

    local selectedLabel = Instance.new("TextLabel")
    selectedLabel.Size = UDim2.new(1, -24, 1, 0)
    selectedLabel.BackgroundTransparency = 1
    selectedLabel.TextColor3 = Theme.Colors.Text
    selectedLabel.Font = Theme.Fonts.Regular
    selectedLabel.TextSize = 13
    selectedLabel.TextXAlignment = Enum.TextXAlignment.Left
    selectedLabel.TextTruncate = Enum.TextTruncate.AtEnd
    selectedLabel.Parent = selectContent

    local arrowIcon = Instance.new("ImageLabel")
    arrowIcon.Size = UDim2.new(0, 16, 0, 16)
    arrowIcon.Position = UDim2.new(1, -16, 0.5, -8)
    arrowIcon.BackgroundTransparency = 1
    arrowIcon.Image = "rbxassetid://10709790948" -- Chevron down
    arrowIcon.ImageColor3 = Theme.Colors.TextSecondary
    arrowIcon.Parent = selectContent

    -- Options Frame (collapsible container)
    local optionsCanvas = Instance.new("CanvasGroup")
    optionsCanvas.Size = UDim2.new(1, 0, 0, 0)
    optionsCanvas.BackgroundColor3 = Theme.Colors.Element
    optionsCanvas.BorderSizePixel = 0
    optionsCanvas.GroupTransparency = 1
    optionsCanvas.Visible = false
    optionsCanvas.Parent = container

    local optionsCorner = Instance.new("UICorner")
    optionsCorner.CornerRadius = Theme.CornerRadius.Element
    optionsCorner.Parent = optionsCanvas

    local optionsStroke = Instance.new("UIStroke")
    optionsStroke.Color = Theme.Colors.Border
    optionsStroke.Thickness = 1
    optionsStroke.Parent = optionsCanvas

    local optionsLayout = Instance.new("UIListLayout")
    optionsLayout.FillDirection = Enum.FillDirection.Vertical
    optionsLayout.Padding = UDim.new(0, 4)
    optionsLayout.SortOrder = Enum.SortOrder.LayoutOrder
    optionsLayout.Parent = optionsCanvas

    local optionsPadding = Instance.new("UIPadding")
    optionsPadding.PaddingTop = UDim.new(0, 4)
    optionsPadding.PaddingBottom = UDim.new(0, 4)
    optionsPadding.Parent = optionsCanvas

    local optionBtnFrames = {}
        ...
        if isMulti then
            if #currentVal > 0 then
                ...
                for _, idx in ipairs(currentVal) do
                    if options[idx] then
                        table.insert(names, tostring(options[idx]))
                    end
                end
            ...
            return tostring(options[currentVal] or "Select...")
        end
    end

    local function updateHeader()
        selectedLabel.Text = getHeaderText()
    end

    updateHeader()

    local function updateOptionsVisuals()
        for idx, btnData in ipairs(optionBtnFrames) do
            local isSelected = isMulti and table_find(currentVal, idx) ~= nil or (currentVal == idx)
            if isMulti then
                btnData.checkbox.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
                btnData.innerCheck.Visible = isSelected
            else
                btnData.btn.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
            end
        end
    end
    ...
    for idx, opt in ipairs(options) do
        ...
        optBtn.BackgroundColor3 = (not isMulti and isSelected) and Theme.Colors.Accent or Theme.Colors.Element
        optBtn.BorderSizePixel = 0
        optBtn.Text = ""
        optBtn.AutoButtonColor = false
        optBtn.LayoutOrder = idx
        optBtn.Parent = optionsCanvas

        local optCorner = Instance.new("UICorner")
        optCorner.CornerRadius = Theme.CornerRadius.Element
        optCorner.Parent = optBtn

        local optContent = Instance.new("Frame")
        optContent.Size = UDim2.new(1, 0, 1, 0)
        ...
        optList.Parent = optContent

        local optPad = Instance.new("UIPadding")
        optPad.PaddingLeft = UDim.new(0, 8)
        optPad.Parent = optContent

        local checkboxFrame, innerCheckFrame
        if isMulti then
            checkboxFrame = Instance.new("Frame")
            checkboxFrame.Size = UDim2.fromOffset(14, 14)
            checkboxFrame.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
            checkboxFrame.BorderSizePixel = 0
            checkboxFrame.Parent = optContent
            ...
            cbStroke.Color = Theme.Colors.Border
            cbStroke.Thickness = 1
            cbStroke.Parent = checkboxFrame

            innerCheckFrame = Instance.new("Frame")
            innerCheckFrame.Size = UDim2.fromOffset(6, 6)
            innerCheckFrame.Position = UDim2.fromScale(0.5, 0.5)
            innerCheckFrame.AnchorPoint = Vector2.new(0.5, 0.5)
            innerCheckFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            innerCheckFrame.BorderSizePixel = 0
            innerCheckFrame.Visible = isSelected
            innerCheckFrame.Parent = checkboxFrame

            local innerCorner = Instance.new("UICorner")
            innerCorner.CornerRadius = UDim.new(0, 1)
            innerCorner.Parent = innerCheckFrame
        end

        local optLabel = Instance.new("TextLabel")
        optLabel.Size = UDim2.new(1, isMulti and -22 or 0, 1, 0)
        optLabel.BackgroundTransparency = 1
        optLabel.Text = tostring(opt)
        optLabel.TextColor3 = Theme.Colors.Text
        optLabel.Font = Theme.Fonts.Medium
        optLabel.TextSize = 12
        optLabel.TextXAlignment = Enum.TextXAlignment.Left
        optLabel.Parent = optContent

        table.insert(optionBtnFrames, { btn = optBtn, checkbox = checkboxFrame, innerCheck = innerCheckFrame })

        optBtn.MouseEnter:Connect(function()
            if not (not isMulti and (currentVal == idx)) then
                optBtn.BackgroundColor3 = Theme.Colors.ElementHover
            end
        end)
        optBtn.MouseLeave:Connect(function()
            local selectedNow = isMulti and table_find(currentVal, idx) ~= nil or (currentVal == idx)
            optBtn.BackgroundColor3 = (not isMulti and selectedNow) and Theme.Colors.Accent or Theme.Colors.Element
        end)

        optBtn.MouseButton1Click:Connect(function()
            if isMulti then
                local pos = table_find(currentVal, idx)
                if pos then
                    table.remove(currentVal, pos)
                else
                    if not el.maximum or #currentVal < el.maximum then
                        table.insert(currentVal, idx)
                    end
                end
            else
                currentVal = idx
            ...
            if el.callback then
                if isMulti then
                    local selectedOpts = {}
                    for _, i in ipairs(currentVal) do
                        table.insert(selectedOpts, options[i])
                    end
                    task.spawn(el.callback, selectedOpts)
                else
                    task.spawn(el.callback, options[currentVal])
                end
            end
            ...
            if not isMulti then
                ...
                TweenService:Create(stroke, tweenInfo, { Color = Theme.Colors.Border }):Play()
                local collapseTween = TweenService:Create(optionsCanvas, tweenInfo, {
                    Size = UDim2.new(1, 0, 0, 0),
                    GroupTransparency = 1,
                })
                collapseTween:Play()
                collapseTween.Completed:Connect(function()
                    if not isOpen then optionsCanvas.Visible = false end
                end)
            end
        end)
    end
    ...
    selectBtn.MouseEnter:Connect(function()
        hovered = true
        local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        TweenService:Create(stroke, tweenInfo, { Color = Theme.Colors.BorderHover }):Play()
    end)
    ...
    selectBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(arrowIcon, tweenInfo, { Rotation = isOpen and 180 or 0 }):Play()
        TweenService:Create(stroke, tweenInfo, { Color = (hovered or isOpen) and Theme.Colors.BorderHover or Theme.Colors.Border }):Play()

        if isOpen then
            optionsCanvas.Visible = true
            optionsCanvas.GroupTransparency = 1
            optionsCanvas.Size = UDim2.new(1, 0, 0, 0)
            TweenService:Create(optionsCanvas, tweenInfo, {
...
__modules["Components/ElementRenderer"] = function()
    local script = createVirtualModule("Components/ElementRenderer")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
function ElementRenderer.render(el, Store)
    ...
    if elType == "CustomComponentPlaceholder" then
        local frame = Instance.new("Frame")
        frame.BackgroundTransparency = 1
        local size = el.instance and el.instance.Size or UDim2.new(1, 0, 0, 70)
        frame.Size = size
        if el.instance then
    ...
    local compName = componentMap[elType]
    if compName then
        local compModule = require(Source.Components[compName])
        return compModule({ element = el, Store = Store })
    elseif elType == "Section" then
        local sectionModule = require(Source.Components.Section)
        return sectionModule({ section = el, Store = Store })
    end
...
__modules["Components/Form"] = function()
    local script = createVirtualModule("Components/Form")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Form(props)
    ...
    for i, subEl in ipairs(el.elements or {}) do
        local childComp = ElementRenderer.render(subEl, Store)
        if childComp then
...
__modules["Components/HStack"] = function()
    local script = createVirtualModule("Components/HStack")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function HStack(props)
    ...
    list.HorizontalAlignment = el.horizontalAlignment or Enum.HorizontalAlignment.Left
    list.VerticalAlignment = el.verticalAlignment or Enum.VerticalAlignment.Center
    list.SortOrder = Enum.SortOrder.LayoutOrder
    list.Parent = container

    for i, subEl in ipairs(el.elements or {}) do
        local compInstance = ElementRenderer.render(subEl, Store)
        if compInstance then
            local scale = N > 0 and (1 / N) or 1
            local offset = -gap * (1 - scale)

            local wrapper = Instance.new("Frame")
            wrapper.Size = UDim2.new(scale, offset, 0, 0)
            wrapper.BackgroundTransparency = 1
            wrapper.AutomaticSize = Enum.AutomaticSize.Y
            wrapper.LayoutOrder = i
            wrapper.Parent = container

            compInstance.Parent = wrapper
        end
    end
...
__modules["Components/InfoGrid"] = function()
    local script = createVirtualModule("Components/InfoGrid")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function InfoGrid(props)
    local el = props.element

    local items = el.items or el.elements or {}
    local cols = el.columns or 2
    local gap = 6
    ...
    for r = 1, numRows do
        ...
        for c = 1, cols do
            ...
            local block = items[idx]

            if block then
                local scale = 1 / cols
                local widthOffset = -gap * (cols - 1) / cols
                local hasCallback = block.callback ~= nil

                local card = Instance.new(hasCallback and "TextButton" or "Frame")
                card.Size = UDim2.new(scale, widthOffset, 1, 0)
                card.BackgroundColor3 = Theme.Colors.Element
                card.BorderSizePixel = 0
                card.LayoutOrder = c
                ...
                titleLabel.Text = block.title or ""
                titleLabel.TextColor3 = Theme.Colors.TextSecondary
                titleLabel.Font = Theme.Fonts.Medium
                ...
                local valLabel = Instance.new("TextLabel")
                valLabel.Size = UDim2.new(1, 0, 1, -14)
                valLabel.BackgroundTransparency = 1
                valLabel.Text = block.content or ""
                valLabel.TextColor3 = Theme.Colors.Text
                valLabel.Font = Theme.Fonts.Bold
                valLabel.TextSize = 12
                valLabel.TextXAlignment = Enum.TextXAlignment.Left
                valLabel.TextWrapped = true
                valLabel.Parent = content

                if hasCallback then
                    card.MouseEnter:Connect(function()
                        card.BackgroundColor3 = Theme.Colors.ElementHover
                        stroke.Color = Theme.Colors.BorderHover
                    end)
...
__modules["Components/KeybindField"] = function()
    local script = createVirtualModule("Components/KeybindField")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function KeybindField(props)
    local el = props.element
    local Store = props.Store
    ...
    bindBtn.BackgroundColor3 = Theme.Colors.Element
    bindBtn.BorderSizePixel = 0
    bindBtn.TextColor3 = Theme.Colors.Text
    bindBtn.Font = Theme.Fonts.Bold
    bindBtn.TextSize = 11
    ...
    local function updateText()
        if listening then
            bindBtn.Text = "..."
            bindBtn.BackgroundColor3 = Theme.Colors.Accent
        else
            local isUnknown = currentBind == Enum.KeyCode.Unknown
            bindBtn.Text = isUnknown and "None" or currentBind.Name
            bindBtn.BackgroundColor3 = Theme.Colors.Element
        end
    end
...
__modules["Components/Label"] = function()
    local script = createVirtualModule("Components/Label")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Label(props)
    ...
    textLabel.Text = el.text or "Label"
    textLabel.TextColor3 = el.color or Theme.Colors.Text
    textLabel.Font = el.font or Theme.Fonts.Medium
    textLabel.TextSize = el.textSize or 13
    textLabel.TextXAlignment = el.alignment or Enum.TextXAlignment.Left
...
__modules["Components/LineChart"] = function()
    local script = createVirtualModule("Components/LineChart")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function LineChart(props)
    local el = props.element
    ...
    chartCanvas.Size = UDim2.new(1, 0, 1, (title or subtitle) and -40 or -20)
    chartCanvas.BackgroundTransparency = 1
    chartCanvas.LayoutOrder = 2
    chartCanvas.Parent = container

    local lineContainer = Instance.new("Frame")
    lineContainer.Size = UDim2.new(1, 0, 1, -16)
    lineContainer.BackgroundTransparency = 1
    lineContainer.ClipsDescendants = true
    lineContainer.Parent = chartCanvas
    ...
    local function drawChart()
        ...
        local canvasSize = lineContainer.AbsoluteSize
        ...
        if N > 1 then
            for i = 1, N - 1 do
                ...
                line.Position = UDim2.new(0, p1.X + diff.X/2, 0, p1.Y + diff.Y/2)
                line.Rotation = angle
        ...
        for _, p in ipairs(points) do
            ...
            dot.ZIndex = 2
            dot.Parent = lineContainer

            local dotCorner = Instance.new("UICorner")
            dotCorner.CornerRadius = Theme.CornerRadius.Circle
            dotCorner.Parent = dot

            local dotStroke = Instance.new("UIStroke")
            dotStroke.Color = lineColor
            dotStroke.Thickness = 1.5
            dotStroke.Parent = dot
        end
    end

    local sizeConn = lineContainer:GetPropertyChangedSignal("AbsoluteSize"):Connect(drawChart)
    ...
    for i = 1, N do
        ...
        local xLabel = Instance.new("TextLabel")
        xLabel.Size = UDim2.new(scale, 0, 1, 0)
        xLabel.BackgroundTransparency = 1
        xLabel.Text = labelText
        xLabel.TextColor3 = Theme.Colors.TextSecondary
        xLabel.Font = Theme.Fonts.Medium
...
__modules["Components/Notification"] = function()
    local script = createVirtualModule("Components/Notification")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function NotificationItem(props)
    local item = props.item
    local title = item.title or "Notification"
    local subtitle = item.subtitle or ""
    local icon = item.icon or "rbxassetid://10747373111"
    ...
    if props.LayoutOrder then group.LayoutOrder = props.LayoutOrder end
    ...
    if count >= 3 then
        local stackCard2 = Instance.new("Frame")
        stackCard2.Size = UDim2.new(0.92, 0, 1, 0)
        stackCard2.Position = UDim2.new(0.04, 0, 0, -8)
        stackCard2.BackgroundColor3 = Theme.Colors.Card
        stackCard2.BorderSizePixel = 0
        stackCard2.ZIndex = 1
        stackCard2.Parent = group

        local sc2Corner = Instance.new("UICorner")
        sc2Corner.CornerRadius = Theme.CornerRadius.Card
        sc2Corner.Parent = stackCard2

        local sc2Stroke = Instance.new("UIStroke")
        sc2Stroke.Color = Theme.Colors.Border
        sc2Stroke.Thickness = 1
        sc2Stroke.Transparency = 0.6
        sc2Stroke.Parent = stackCard2
    end

    if count >= 2 then
        local stackCard1 = Instance.new("Frame")
        stackCard1.Size = UDim2.new(0.96, 0, 1, 0)
        stackCard1.Position = UDim2.new(0.02, 0, 0, -4)
        stackCard1.BackgroundColor3 = Theme.Colors.Card
        stackCard1.BorderSizePixel = 0
        stackCard1.ZIndex = 2
        stackCard1.Parent = group

        local sc1Corner = Instance.new("UICorner")
        sc1Corner.CornerRadius = Theme.CornerRadius.Card
        sc1Corner.Parent = stackCard1

        local sc1Stroke = Instance.new("UIStroke")
        sc1Stroke.Color = Theme.Colors.Border
        sc1Stroke.Thickness = 1
        sc1Stroke.Transparency = 0.3
        sc1Stroke.Parent = stackCard1
    end

    local mainCard = Instance.new("Frame")
    mainCard.Size = UDim2.new(1, 0, 1, 0)
    mainCard.Position = UDim2.new(0, 0, 0, 0)
    mainCard.BackgroundColor3 = Theme.Colors.Card
    mainCard.BorderSizePixel = 0
    mainCard.ZIndex = 3
    mainCard.Parent = group

    local mcCorner = Instance.new("UICorner")
    mcCorner.CornerRadius = Theme.CornerRadius.Card
    mcCorner.Parent = mainCard

    local mcStroke = Instance.new("UIStroke")
    mcStroke.Color = Theme.Colors.Border
    ...
    padding.PaddingLeft = UDim.new(0, 12)
    padding.PaddingRight = UDim.new(0, 12)
    padding.Parent = mainCard

    local list = Instance.new("UIListLayout")
    list.FillDirection = Enum.FillDirection.Horizontal
    list.VerticalAlignment = Enum.VerticalAlignment.Center
    list.Padding = UDim.new(0, 10)
    list.Parent = mainCard

    local appIcon = Instance.new("ImageLabel")
    appIcon.Size = UDim2.new(0, 24, 0, 24)
    appIcon.BackgroundTransparency = 1
    appIcon.Image = icon
    appIcon.ImageColor3 = Theme.Colors.Accent
    appIcon.Parent = mainCard

    local textFrame = Instance.new("Frame")
    textFrame.Size = UDim2.new(1, -34, 1, 0)
    textFrame.BackgroundTransparency = 1
    textFrame.Parent = mainCard

    local tfList = Instance.new("UIListLayout")
    tfList.FillDirection = Enum.FillDirection.Vertical
    tfList.Padding = UDim.new(0, 2)
    tfList.SortOrder = Enum.SortOrder.LayoutOrder
    tfList.Parent = textFrame

    local accentHex = string.format("#%02x%02x%02x", math.floor(Theme.Colors.Accent.R * 255), math.floor(Theme.Colors.Accent.G * 255), math.floor(Theme.Colors.Accent.B * 255))
    local titleText = (count > 1) and (title .. " <font color=\"" .. accentHex .. "\">x" .. tostring(count) .. "</font>") or title
...
local function NotificationList(props)
    local list = props.notifications or {}
    ...
    for i, item in ipairs(list) do
        local nItem = NotificationItem({ item = item, LayoutOrder = i })
        if nItem then nItem.Parent = container end
    end
...
__modules["Components/Paragraph"] = function()
    local script = createVirtualModule("Components/Paragraph")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Paragraph(props)
    ...
    body.Text = el.content or ""
    body.TextColor3 = Theme.Colors.TextSecondary
    body.Font = Theme.Fonts.Regular
    body.TextSize = 11
...
__modules["Components/PieChart"] = function()
    local script = createVirtualModule("Components/PieChart")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function createWedge(startAngle, sweepAngle, color, zIndex)
    local container = Instance.new("Frame")
    ...
    local clipBox = Instance.new("Frame")
    clipBox.Size = UDim2.new(0.5, 0, 1, 0)
    clipBox.Position = UDim2.new(0.5, 0, 0, 0)
    clipBox.AnchorPoint = Vector2.new(0, 0)
    clipBox.ClipsDescendants = true
    clipBox.BackgroundTransparency = 1
    clipBox.Parent = container

    local circleSegment = Instance.new("Frame")
    circleSegment.Size = UDim2.new(2, 0, 1, 0)
    circleSegment.Position = UDim2.new(-1, 0, 0, 0)
    circleSegment.AnchorPoint = Vector2.new(0, 0)
    circleSegment.BackgroundColor3 = color
    circleSegment.BorderSizePixel = 0
    circleSegment.Parent = clipBox

    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0.5, 0)
    corner.Parent = circleSegment

    local grad = Instance.new("UIGradient")
    grad.Rotation = math.clamp(sweepAngle, 0.1, 180)
    ...
    grad.Parent = circleSegment

    return container
end

local function PieChart(props)
    local el = props.element
    ...
    -- Content Frame (Pie circle on left, Legend on right)
    local contentFrame = Instance.new("Frame")
    contentFrame.Size = UDim2.new(1, 0, 0, size)
    contentFrame.BackgroundTransparency = 1
    contentFrame.LayoutOrder = 2
    contentFrame.Parent = card

    local pieWrapper = Instance.new("Frame")
    pieWrapper.Size = UDim2.new(0, size, 0, size)
    pieWrapper.BackgroundTransparency = 1
    pieWrapper.Parent = contentFrame
    ...
    if totalSum <= 0 then
        ...
        for i, item in ipairs(data) do
            ...
            if val > 0 then
                ...
                if sweep <= 180 then
                    local w = createWedge(currentAngle, sweep, color, i)
                    w.Parent = pieWrapper
                else
                    local w1 = createWedge(currentAngle, 180, color, i)
                    w1.Parent = pieWrapper
                    local w2 = createWedge(currentAngle + 180, sweep - 180, color, i)
                    w2.Parent = pieWrapper
                end
    ...
    if isDonut then
        local hollow = Instance.new("Frame")
        hollow.Name = "DonutCenter"
        hollow.Size = UDim2.new(0.6, 0, 0.6, 0)
        hollow.Position = UDim2.fromScale(0.5, 0.5)
        hollow.AnchorPoint = Vector2.new(0.5, 0.5)
        hollow.BackgroundColor3 = Theme.Colors.Element
        hollow.BorderSizePixel = 0
        hollow.ZIndex = 100
        hollow.Parent = pieWrapper

        local hCorner = Instance.new("UICorner")
        hCorner.CornerRadius = UDim.new(0.5, 0)
        hCorner.Parent = hollow
    end

    -- Legend List
    local legendFrame = Instance.new("Frame")
    legendFrame.Size = UDim2.new(1, -(size + 16), 1, 0)
    legendFrame.Position = UDim2.new(0, size + 16, 0, 0)
    legendFrame.BackgroundTransparency = 1
    legendFrame.Parent = contentFrame

    local legendList = Instance.new("UIListLayout")
    legendList.FillDirection = Enum.FillDirection.Vertical
    legendList.Padding = UDim.new(0, 6)
    legendList.VerticalAlignment = Enum.VerticalAlignment.Center
    legendList.SortOrder = Enum.SortOrder.LayoutOrder
    legendList.Parent = legendFrame
    ...
    for i, item in ipairs(data) do
        ...
        lItem.Size = UDim2.new(1, 0, 0, 18)
        lItem.BackgroundTransparency = 1
        lItem.LayoutOrder = i
        lItem.Parent = legendFrame
        ...
        lLabel.Size = UDim2.new(1, -60, 1, 0)
        lLabel.BackgroundTransparency = 1
        lLabel.Text = labelText
        lLabel.TextColor3 = Theme.Colors.Text
        lLabel.Font = Theme.Fonts.Medium
        lLabel.TextSize = 11
        lLabel.TextXAlignment = Enum.TextXAlignment.Left
        lLabel.TextTruncate = Enum.TextTruncate.AtEnd
        lLabel.Parent = lItem
        ...
        pctLabel.TextColor3 = Theme.Colors.TextSecondary
        pctLabel.Font = Theme.Fonts.Bold
        pctLabel.TextSize = 11
        pctLabel.TextXAlignment = Enum.TextXAlignment.Right
        pctLabel.Parent = lItem
    end
...
__modules["Components/Profile"] = function()
    local script = createVirtualModule("Components/Profile")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Profile(props)
    local username = props.username or "Guest"
    local subtext = props.subtext or "Welcome to Selene"
    ...
    avatar.BackgroundColor3 = Theme.Colors.Element
    avatar.BorderSizePixel = 0
    avatar.Image = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. tostring(userId) .. "&width=150&height=150&format=png"
    avatar.Parent = content

    local avatarCorner = Instance.new("UICorner")
    avatarCorner.CornerRadius = Theme.CornerRadius.Circle
    avatarCorner.Parent = avatar

    local avatarStroke = Instance.new("UIStroke")
    avatarStroke.Color = Theme.Colors.Border
    avatarStroke.Thickness = 1
    avatarStroke.Parent = avatar

    local info = Instance.new("Frame")
    info.Size = UDim2.new(1, -63, 0, 40)
...
__modules["Components/PullDownButton"] = function()
    local script = createVirtualModule("Components/PullDownButton")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function PullDownButton(props)
    local el = props.element
    local options = el.options or {}
    ...
    mainBtn.BackgroundColor3 = Theme.Colors.Element
    ...
    label.BackgroundTransparency = 1
    label.Text = el.name or "PullDown"
    label.TextColor3 = Theme.Colors.Text
    label.Font = Theme.Fonts.Bold
    label.TextSize = 13
    label.TextXAlignment = Enum.TextXAlignment.Left
    ...
    for idx, opt in ipairs(options) do
        local optBtn = Instance.new("TextButton")
        optBtn.Size = UDim2.new(1, 0, 0, 28)
        optBtn.BackgroundColor3 = Theme.Colors.Element
        optBtn.BorderSizePixel = 0
        optBtn.Text = ""
        optBtn.AutoButtonColor = false
        optBtn.LayoutOrder = idx
        ...
        optBtn.MouseButton1Click:Connect(function()
            if el.callback then
                task.spawn(el.callback, opt, idx)
            end
            isOpen = false
            local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(arrowIcon, tweenInfo, { Rotation = 0 }):Play()
...
__modules["Components/RadioButtonGroup"] = function()
    local script = createVirtualModule("Components/RadioButtonGroup")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function RadioButtonGroup(props)
    local el = props.element
    local Store = props.Store
    ...
    local function updateVisuals()
        for i, w in ipairs(optionWidgets) do
            local isSelected = (i == selectedIndex)
            w.circle.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
            w.stroke.Color = isSelected and Theme.Colors.BorderHover or Theme.Colors.Border
            w.fill.Visible = isSelected
            w.label.TextColor3 = isSelected and Theme.Colors.Text or Theme.Colors.TextSecondary
            w.label.Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Medium
        end
    end
    ...
    for i, opt in ipairs(options) do
        ...
        optList.VerticalAlignment = Enum.VerticalAlignment.Center
        optList.Padding = UDim.new(0, 8)
        optList.Parent = optBtn

        local circle = Instance.new("Frame")
        circle.Size = UDim2.new(0, 16, 0, 16)
        circle.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
        circle.BorderSizePixel = 0
        circle.Parent = optBtn

        local circleCorner = Instance.new("UICorner")
        circleCorner.CornerRadius = Theme.CornerRadius.Circle
        circleCorner.Parent = circle

        local stroke = Instance.new("UIStroke")
        stroke.Color = isSelected and Theme.Colors.BorderHover or Theme.Colors.Border
        stroke.Thickness = 1
        ...
        fill.Visible = isSelected
        fill.Parent = circle

        local fillCorner = Instance.new("UICorner")
        fillCorner.CornerRadius = Theme.CornerRadius.Circle
        ...
        label.TextColor3 = isSelected and Theme.Colors.Text or Theme.Colors.TextSecondary
        label.Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Medium
        label.TextSize = 12
        label.TextXAlignment = Enum.TextXAlignment.Left
        label.Parent = optBtn

        table.insert(optionWidgets, { btn = optBtn, circle = circle, stroke = stroke, fill = fill, label = label })
        ...
        optBtn.MouseButton1Click:Connect(function()
            ...
            if el.callback then
                task.spawn(el.callback, options[selectedIndex], selectedIndex)
            end
        end)
    end
...
__modules["Components/Row"] = function()
    local script = createVirtualModule("Components/Row")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Row(props)
    ...
    if #(el.leftElements or {}) == 0 then
        isFullWidth = true
    else
        for _, subEl in ipairs(el.rightElements or {}) do
            local t = subEl.type
    ...
    if isFullWidth then
        ...
        if #(el.leftElements or {}) > 0 then
            ...
            leftArea.Size = UDim2.new(1, 0, 0, 0)
            leftArea.BackgroundTransparency = 1
            leftArea.AutomaticSize = Enum.AutomaticSize.Y
            leftArea.LayoutOrder = 1
            leftArea.Parent = container

            for _, subEl in ipairs(el.leftElements) do
                local childComp = ElementRenderer.render(subEl, Store)
                if childComp then childComp.Parent = leftArea end
            end
        end

        local rightArea = Instance.new("Frame")
        rightArea.Size = UDim2.new(1, 0, 0, 0)
        rightArea.BackgroundTransparency = 1
        rightArea.AutomaticSize = Enum.AutomaticSize.Y
        rightArea.LayoutOrder = 2
        rightArea.Parent = container

        for _, subEl in ipairs(el.rightElements or {}) do
            local childComp = ElementRenderer.render(subEl, Store)
            if childComp then childComp.Parent = rightArea end
        end
    else
        local leftArea = Instance.new("Frame")
        leftArea.Size = UDim2.new(0.5, -6, 0, 0)
        leftArea.AutomaticSize = Enum.AutomaticSize.Y
        leftArea.BackgroundTransparency = 1
        leftArea.Parent = container
        ...
        leftList.Parent = leftArea

        for _, subEl in ipairs(el.leftElements or {}) do
            local childComp = ElementRenderer.render(subEl, Store)
            if childComp then childComp.Parent = leftArea end
        end

        local rightArea = Instance.new("Frame")
        rightArea.Size = UDim2.new(0.5, -6, 0, 0)
        rightArea.Position = UDim2.new(0.5, 6, 0, 0)
        rightArea.AutomaticSize = Enum.AutomaticSize.Y
        rightArea.BackgroundTransparency = 1
        rightArea.Parent = container
...
__modules["Components/Section"] = function()
    local script = createVirtualModule("Components/Section")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Section(props)
    ...
    card.BorderSizePixel = 0
    card.AutomaticSize = Enum.AutomaticSize.Y
    if props.LayoutOrder then card.LayoutOrder = props.LayoutOrder end
    ...
    -- Header
    if sec.name or sec.description then
        local header = Instance.new("Frame")
        header.Size = UDim2.new(1, 0, 0, 0)
        header.BackgroundTransparency = 1
        header.AutomaticSize = Enum.AutomaticSize.Y
        ...
        if sec.description then
            local desc = Instance.new("TextLabel")
            desc.Size = UDim2.new(1, 0, 0, 14)
            desc.BackgroundTransparency = 1
            desc.Text = sec.description
    ...
    -- Divider
    if #(sec.elements or {}) > 0 and (sec.name or sec.description) then
    ...
    elementsArea.Size = UDim2.new(1, 0, 0, 0)
    elementsArea.BackgroundTransparency = 1
    elementsArea.AutomaticSize = Enum.AutomaticSize.Y
    elementsArea.LayoutOrder = 3
    elementsArea.Parent = content
    ...
    elementsList.Parent = elementsArea

    for i, el in ipairs(sec.elements or {}) do
        local childComp = ElementRenderer.render(el, Store)
        if childComp then
            local wrapper = Instance.new("Frame")
            wrapper.Size = UDim2.new(1, 0, 0, 0)
            wrapper.BackgroundTransparency = 1
...
__modules["Components/Sidebar"] = function()
    local script = createVirtualModule("Components/Sidebar")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function TabButton(props)
    ...
    btn.BackgroundColor3 = isActive and Theme.Colors.Element or Color3.fromRGB(0, 0, 0)
    ...
    activeBar.Position = UDim2.new(0, 4, 0.5, -7)
    activeBar.BackgroundColor3 = Theme.Colors.Accent
    activeBar.BackgroundTransparency = isActive and 0 or 1
    activeBar.BorderSizePixel = 0
    activeBar.Parent = btn

    local abCorner = Instance.new("UICorner")
    abCorner.CornerRadius = Theme.CornerRadius.Circle
    abCorner.Parent = activeBar

    local content = Instance.new("Frame")
    content.Size = UDim2.new(1, isActive and -15 or -10, 1, 0)
    content.Position = UDim2.new(0, isActive and 12 or 8, 0, 0)
    content.BackgroundTransparency = 1
    content.Parent = btn
    ...
    if tab.icon then
        ...
        icon.BackgroundTransparency = 1
        icon.Image = tab.icon
        icon.ImageColor3 = isActive and Theme.Colors.Text or Theme.Colors.TextSecondary
        icon.Parent = content
    end

    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(1, tab.icon and -22 or 0, 1, 0)
    label.BackgroundTransparency = 1
    label.Text = tab.title or "Tab"
    label.TextColor3 = isActive and Theme.Colors.Text or Theme.Colors.TextSecondary
    label.Font = isActive and Theme.Fonts.Bold or Theme.Fonts.Medium
    label.TextSize = 12
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Parent = content
    ...
    btn.MouseEnter:Connect(function()
        if not isActive then
            local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(btn, tweenInfo, { BackgroundColor3 = Theme.Colors.ElementHover, BackgroundTransparency = 0 }):Play()
            TweenService:Create(label, tweenInfo, { TextColor3 = Theme.Colors.Text }):Play()
            if icon then TweenService:Create(icon, tweenInfo, { ImageColor3 = Theme.Colors.Text }):Play() end
        end
    end)
    ...
    btn.MouseLeave:Connect(function()
        if not isActive then
            ...
            TweenService:Create(label, tweenInfo, { TextColor3 = Theme.Colors.TextSecondary }):Play()
            if icon then TweenService:Create(icon, tweenInfo, { ImageColor3 = Theme.Colors.TextSecondary }):Play() end
        end
    end)
...
local function Sidebar(props)
    ...
    container.BackgroundColor3 = Theme.Colors.Sidebar
    container.BorderSizePixel = 0
    if props.LayoutOrder then container.LayoutOrder = props.LayoutOrder end
    ...
    navList.ScrollBarThickness = 1
    navList.ScrollBarImageColor3 = Theme.Colors.Border
    navList.Parent = container
    ...
    navPadding.PaddingBottom = UDim.new(0, 12)
    ...
    for _, sec in ipairs(state.sections or {}) do
        if sec.title and sec.title ~= "" then
            ...
            secHeader.Text = string.upper(sec.title)
            secHeader.TextColor3 = Theme.Colors.TextSecondary
            secHeader.Font = Theme.Fonts.Bold
            secHeader.TextSize = 10
            secHeader.TextXAlignment = Enum.TextXAlignment.Left
            secHeader.LayoutOrder = layoutOrder
            secHeader.Parent = navList
            layoutOrder = layoutOrder + 1
        end

        for _, tab in ipairs(sec.tabs or {}) do
            local isActive = state.activeTab == tab.id
            local tBtn = TabButton({
                tab = tab,
                isActive = isActive,
                Store = Store,
    ...
    if state.profile then
        ...
        profileArea.Size = UDim2.new(1, 0, 0, 65)
        profileArea.Position = UDim2.new(0, 0, 1, -65)
        profileArea.BackgroundTransparency = 1
        profileArea.Parent = container
        ...
        div.Parent = profileArea

        local pContainer = Instance.new("Frame")
        pContainer.Size = UDim2.new(1, -20, 1, -1)
        pContainer.Position = UDim2.new(0, 10, 0, 1)
        pContainer.BackgroundTransparency = 1
        pContainer.Parent = profileArea
        ...
        pList.Parent = pContainer

        local avatar = Instance.new("ImageLabel")
        avatar.Size = UDim2.new(0, 36, 0, 36)
        avatar.BackgroundColor3 = Theme.Colors.Element
        avatar.BorderSizePixel = 0
        avatar.Image = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. tostring(state.profile.userId or 1) .. "&width=150&height=150&format=png"
        avatar.Parent = pContainer

        local aCorner = Instance.new("UICorner")
        aCorner.CornerRadius = Theme.CornerRadius.Circle
        aCorner.Parent = avatar

        local aStroke = Instance.new("UIStroke")
        aStroke.Color = Theme.Colors.Border
        aStroke.Thickness = 1
        aStroke.Parent = avatar

        local info = Instance.new("Frame")
        info.Size = UDim2.new(1, -46, 0, 36)
        info.BackgroundTransparency = 1
        info.Parent = pContainer
        ...
        username.Text = state.profile.username
        ...
        role.Size = UDim2.new(1, 0, 0, 14)
        role.BackgroundTransparency = 1
        role.Text = state.profile.subtext or "User"
        role.TextColor3 = Theme.Colors.TextSecondary
        role.Font = Theme.Fonts.Regular
        role.TextSize = 10
        role.TextXAlignment = Enum.TextXAlignment.Left
        role.TextTruncate = Enum.TextTruncate.AtEnd
        role.Parent = info
    end
...
__modules["Components/Slider"] = function()
    local script = createVirtualModule("Components/Slider")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Slider(props)
    ...
    barRow.Size = UDim2.new(1, 0, 0, 24)
    ...
    if el.leftIcon then
        ...
        leftIcon.Image = el.leftIcon
        leftIcon.ImageColor3 = Theme.Colors.TextSecondary
        leftIcon.Parent = barRow
    end

    if el.rightIcon then
        ...
        rightIcon.Image = el.rightIcon
        rightIcon.ImageColor3 = Theme.Colors.TextSecondary
        rightIcon.Parent = barRow
    end

    local track = Instance.new("TextButton")
    track.Size = UDim2.new(1, (el.leftIcon and -24 or 0) + (el.rightIcon and -24 or 0), 0, 8)
    track.Position = UDim2.new(0, el.leftIcon and 20 or 0, 0.5, -4)
    track.BackgroundColor3 = Theme.Colors.Element
    ...
    trackCorner.CornerRadius = Theme.CornerRadius.Circle
    ...
    local knobCorner = Instance.new("UICorner")
    knobCorner.CornerRadius = Theme.CornerRadius.Circle
    knobCorner.Parent = knob

    local knobStroke = Instance.new("UIStroke")
    knobStroke.Color = Theme.Colors.Border
...
__modules["Components/Stepper"] = function()
    local script = createVirtualModule("Components/Stepper")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Stepper(props)
    ...
    controls.Size = UDim2.new(0, 110, 0, 26)
    controls.Position = UDim2.new(1, -110, 0.5, -13)
    ...
    decBtn.BackgroundColor3 = Theme.Colors.Element
    decBtn.BorderSizePixel = 0
    decBtn.Text = "-"
    decBtn.TextColor3 = Theme.Colors.Text
    decBtn.Font = Theme.Fonts.Bold
    decBtn.TextSize = 14
    decBtn.LayoutOrder = 1
    decBtn.AutoButtonColor = false
    decBtn.Parent = controls

    local decCorner = Instance.new("UICorner")
    decCorner.CornerRadius = Theme.CornerRadius.Element
    decCorner.Parent = decBtn

    local decStroke = Instance.new("UIStroke")
    decStroke.Color = Theme.Colors.Border
    decStroke.Thickness = 1
    decStroke.Parent = decBtn

    -- Value Display
    local valDisplay = Instance.new("Frame")
    valDisplay.Size = UDim2.new(0, 50, 0, 26)
    valDisplay.BackgroundColor3 = Theme.Colors.Element
    valDisplay.BorderSizePixel = 0
    valDisplay.LayoutOrder = 2
    valDisplay.Parent = controls

    local valCorner = Instance.new("UICorner")
    valCorner.CornerRadius = Theme.CornerRadius.Element
    valCorner.Parent = valDisplay

    local valStroke = Instance.new("UIStroke")
    valStroke.Color = Theme.Colors.Border
    valStroke.Thickness = 1
    valStroke.Parent = valDisplay

    local valText = Instance.new("TextLabel")
    valText.Size = UDim2.new(1, 0, 1, 0)
    valText.BackgroundTransparency = 1
    valText.Text = tostring(currentVal)
    valText.TextColor3 = Theme.Colors.Text
    valText.Font = Theme.Fonts.Bold
    valText.TextSize = 12
    valText.Parent = valDisplay

    -- Increment button (+)
    local incBtn = Instance.new("TextButton")
    incBtn.Size = UDim2.new(0, 26, 0, 26)
    incBtn.BackgroundColor3 = Theme.Colors.Element
    incBtn.BorderSizePixel = 0
    incBtn.Text = "+"
    incBtn.TextColor3 = Theme.Colors.Text
    incBtn.Font = Theme.Fonts.Bold
    incBtn.TextSize = 14
    incBtn.LayoutOrder = 3
    incBtn.AutoButtonColor = false
    incBtn.Parent = controls

    local incCorner = Instance.new("UICorner")
    incCorner.CornerRadius = Theme.CornerRadius.Element
    incCorner.Parent = incBtn

    local incStroke = Instance.new("UIStroke")
    incStroke.Color = Theme.Colors.Border
    incStroke.Thickness = 1
    incStroke.Parent = incBtn
    ...
    decBtn.MouseEnter:Connect(function()
        decBtn.BackgroundColor3 = Theme.Colors.ElementHover
        decStroke.Color = Theme.Colors.BorderHover
    end)
    decBtn.MouseLeave:Connect(function()
        decBtn.BackgroundColor3 = Theme.Colors.Element
        decStroke.Color = Theme.Colors.Border
    end)
    ...
    incBtn.MouseEnter:Connect(function()
        incBtn.BackgroundColor3 = Theme.Colors.ElementHover
        incStroke.Color = Theme.Colors.BorderHover
    end)
    incBtn.MouseLeave:Connect(function()
        incBtn.BackgroundColor3 = Theme.Colors.Element
        incStroke.Color = Theme.Colors.Border
    end)
...
__modules["Components/TabContainer"] = function()
    local script = createVirtualModule("Components/TabContainer")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function TabContainer(props)
    ...
    local group = Instance.new("CanvasGroup")
    group.Size = UDim2.new(1, -180, 1, 0)
    group.BackgroundTransparency = 1
    if props.LayoutOrder then group.LayoutOrder = props.LayoutOrder end

    if not activeTab then
        local noTab = Instance.new("TextLabel")
        noTab.Size = UDim2.new(1, 0, 1, 0)
        noTab.BackgroundTransparency = 1
        noTab.Text = "No Tab Selected"
        noTab.TextColor3 = Theme.Colors.TextSecondary
        noTab.Font = Theme.Fonts.Medium
        noTab.TextSize = 14
        noTab.Parent = group
        return group
    end
    ...
    scrollPad.PaddingBottom = UDim.new(0, 15)
    scrollPad.PaddingLeft = UDim.new(0, 12)
    scrollPad.PaddingRight = UDim.new(0, 12)
    scrollPad.Parent = scrollFrame

    if activeTab.banner then
        local bannerFrame = Instance.new("Frame")
        bannerFrame.Size = UDim2.new(1, 0, 0, 80)
        bannerFrame.BackgroundTransparency = 1
        bannerFrame.LayoutOrder = 1
        bannerFrame.Parent = scrollFrame

        local prof = ProfileComponent({
            username = activeTab.banner.username,
            subtext = activeTab.banner.subtext,
            userId = activeTab.banner.userId,
        })
        if prof then prof.Parent = bannerFrame end
    end
    ...
    for i, sec in ipairs(pageSections) do
        ...
        if showSection then
            ...
            if sec.column == 2 or sec.column == "Right" then
                table.insert(rightSections, sectionComp)
            else
                table.insert(leftSections, sectionComp)
            end
        end
    end

    local gridArea = Instance.new("Frame")
    gridArea.Size = UDim2.new(1, 0, 0, 0)
    gridArea.BackgroundTransparency = 1
    gridArea.AutomaticSize = Enum.AutomaticSize.Y
    gridArea.LayoutOrder = 2
    gridArea.Parent = scrollFrame
    ...
    if #rightSections > 0 then
        ...
        gridList.Padding = UDim.new(0, 12)
        gridList.Parent = gridArea
        ...
        for _, secEl in ipairs(leftSections) do
            secEl.Parent = leftCol
        end
        ...
        for _, secEl in ipairs(rightSections) do
            secEl.Parent = rightCol
        end
    else
        local singleList = Instance.new("UIListLayout")
        singleList.FillDirection = Enum.FillDirection.Vertical
        singleList.Padding = UDim.new(0, 12)
        singleList.SortOrder = Enum.SortOrder.LayoutOrder
        singleList.Parent = gridArea

        for _, secEl in ipairs(leftSections) do
            secEl.Parent = gridArea
        end
    end
...
__modules["Components/TextBox"] = function()
    local script = createVirtualModule("Components/TextBox")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function TextBoxComp(props)
    local el = props.element
    local Store = props.Store
    ...
    inputContainer.Size = hideLabel and UDim2.new(1, 0, 0, 28) or UDim2.new(1, -160, 0, 28)
    inputContainer.Position = hideLabel and UDim2.new(0, 0, 0.5, -14) or UDim2.new(1, -160, 0.5, -14)
    inputContainer.BackgroundColor3 = Theme.Colors.Element
    ...
    textInput.Size = UDim2.new(1, -16, 1, 0)
    textInput.Position = UDim2.new(0, 8, 0, 0)
    textInput.BackgroundTransparency = 1
    textInput.Text = currentVal
    textInput.PlaceholderText = el.placeholder or "Type here..."
    textInput.PlaceholderColor3 = Theme.Colors.TextSecondary
    textInput.TextColor3 = Theme.Colors.Text
    textInput.Font = Theme.Fonts.Regular
...
__modules["Components/TitleStack"] = function()
    local script = createVirtualModule("Components/TitleStack")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function createTag(text, color, layoutOrder)
    ...
    tagFrame.BackgroundColor3 = not isGradient and color or Color3.fromRGB(255, 255, 255)
    tagFrame.BorderSizePixel = 0
    if layoutOrder then tagFrame.LayoutOrder = layoutOrder end
    ...
    if isGradient then
        local grad = Instance.new("UIGradient")
        grad.Color = ColorSequence.new(unpack(color))
        grad.Rotation = 45
        grad.Parent = tagFrame
    end
...
local function TitleStack(props)
    local el = props.element
    ...
    if titleTag then
        local tag = createTag(titleTag, titleTagColor, 2)
        tag.Parent = titleRow
    end
    ...
    if subtitle then
        ...
        subRow.Size = UDim2.new(1, 0, 0, 14)
        subRow.BackgroundTransparency = 1
        subRow.LayoutOrder = 2
        subRow.Parent = container
        ...
        if subtitleTag then
            local tag = createTag(subtitleTag, subtitleTagColor, 2)
            tag.Parent = subRow
        end
    end
...
__modules["Components/Toggle"] = function()
    local script = createVirtualModule("Components/Toggle")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function Toggle(props)
    ...
    local hovered = false
    local currentVal = el.value or false
    local hideLabel = el.hideLabel
    ...
    switchBtn.BackgroundColor3 = currentVal and Theme.Colors.Accent or Theme.Colors.Element
    ...
    local corner = Instance.new("UICorner")
    corner.CornerRadius = Theme.CornerRadius.Circle
    corner.Parent = switchBtn

    local stroke = Instance.new("UIStroke")
    stroke.Color = Theme.Colors.Border
    stroke.Thickness = 1
    stroke.Parent = switchBtn
...
__modules["Components/VStack"] = function()
    local script = createVirtualModule("Components/VStack")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local function VStack(props)
    ...
    local list = Instance.new("UIListLayout")
    list.FillDirection = Enum.FillDirection.Vertical
    list.Padding = el.padding or UDim.new(0, 4)
    list.HorizontalAlignment = el.horizontalAlignment or Enum.HorizontalAlignment.Left
    list.VerticalAlignment = el.verticalAlignment or Enum.VerticalAlignment.Center
    list.SortOrder = Enum.SortOrder.LayoutOrder
    list.Parent = container
...
__modules["Components/Window"] = function()
    local script = createVirtualModule("Components/Window")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local Source = script.Parent.Parent
local Theme = require(Source.Theme)
local SidebarComponent = require(Source.Components.Sidebar)
local TabContainerComponent = require(Source.Components.TabContainer)
local ColorPickerDialog = require(Source.Components.ColorPickerDialog)
...
local function TagComponent(props)
    ...
    tagFrame.BackgroundColor3 = not isGradient and color or Color3.fromRGB(255, 255, 255)
    tagFrame.BorderSizePixel = 0
    if props.LayoutOrder then tagFrame.LayoutOrder = props.LayoutOrder end
    ...
    label.Size = UDim2.new(1, 0, 1, 0)
    label.BackgroundTransparency = 1
    label.Text = string.upper(props.text or "")
    label.TextColor3 = Color3.fromRGB(255, 255, 255)
    label.Font = Theme.Fonts.Bold
    label.TextSize = 8
    label.Parent = tagFrame
...
function WindowModule.render(state, Store)
    local isMinimized = state.minimized
    local isMaximized = state.maximized

    local defaultPos = UDim2.new(0.5, -375, 0.5, -240)
    local size = isMaximized and UDim2.new(1, 0, 1, 0) or (isMinimized and UDim2.new(0, 750, 0, 45) or UDim2.new(0, 750, 0, 480))
    local pos = isMaximized and UDim2.new(0, 0, 0, 0) or (state._lastPosition or defaultPos)

    local dragFrame = Instance.new("CanvasGroup")
    dragFrame.Name = "WindowFrame"
    dragFrame.Size = size
    dragFrame.Position = pos
    dragFrame.BackgroundColor3 = Theme.Colors.Background
    dragFrame.BorderSizePixel = 0
    dragFrame.Active = true
    dragFrame.ClipsDescendants = true

    local windowCorner = Instance.new("UICorner")
    windowCorner.CornerRadius = Theme.CornerRadius.Window
    windowCorner.Parent = dragFrame

    local windowStroke = Instance.new("UIStroke")
    windowStroke.Color = Theme.Colors.Border
    windowStroke.Thickness = 1
    windowStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    windowStroke.Parent = dragFrame
    ...
    appIcon.Position = UDim2.new(0, 15, 0.5, -10)
    appIcon.BackgroundTransparency = 1
    appIcon.Image = "rbxassetid://10734950309"
    appIcon.ImageColor3 = Theme.Colors.Accent
    appIcon.Parent = titleBar

    local titleArea = Instance.new("Frame")
    titleArea.Size = state.searching and UDim2.new(1, -310, 1, 0) or UDim2.new(1, -180, 1, 0)
    titleArea.Position = UDim2.new(0, 45, 0, 0)
    titleArea.BackgroundTransparency = 1
    titleArea.Parent = titleBar
    ...
    titleList.SortOrder = Enum.SortOrder.LayoutOrder
    titleList.Parent = titleArea

    local titleLabel = Instance.new("TextLabel")
    titleLabel.Size = UDim2.new(0, 0, 1, 0)
    titleLabel.AutomaticSize = Enum.AutomaticSize.X
    titleLabel.BackgroundTransparency = 1
    titleLabel.Text = state.title or "Selene UI"
    titleLabel.TextColor3 = Theme.Colors.Text
    titleLabel.Font = Theme.Fonts.Bold
    ...
    if state.titleTag then
        local tTag = TagComponent({ text = state.titleTag, color = state.titleTagColor, LayoutOrder = 2 })
        tTag.Parent = titleArea
    end
    ...
    if state.subtitle and state.subtitle ~= "" then
        ...
        subLabel.AutomaticSize = Enum.AutomaticSize.X
        subLabel.BackgroundTransparency = 1
        subLabel.Text = state.subtitle
        subLabel.TextColor3 = Theme.Colors.TextSecondary
        subLabel.Font = Theme.Fonts.Medium
        subLabel.TextSize = 11
        subLabel.LayoutOrder = 3
        subLabel.Parent = titleArea
    end

    if state.subtitleTag then
        local sTag = TagComponent({ text = state.subtitleTag, color = state.subtitleTagColor, LayoutOrder = 4 })
        sTag.Parent = titleArea
    end

    -- Top SearchBar
    if state.searching then
        local searchBar = Instance.new("Frame")
        searchBar.Size = UDim2.new(0, 160, 0, 24)
        searchBar.Position = UDim2.new(1, -290, 0.5, -12)
        searchBar.BackgroundColor3 = Theme.Colors.Element
        searchBar.BorderSizePixel = 0
        searchBar.ZIndex = 3
        searchBar.Parent = titleBar

        local sbCorner = Instance.new("UICorner")
        sbCorner.CornerRadius = Theme.CornerRadius.Element
        sbCorner.Parent = searchBar

        local sbStroke = Instance.new("UIStroke")
        sbStroke.Color = Theme.Colors.Border
        sbStroke.Thickness = 1
        sbStroke.Parent = searchBar

        local sbPadding = Instance.new("UIPadding")
        sbPadding.PaddingLeft = UDim.new(0, 8)
        sbPadding.PaddingRight = UDim.new(0, 8)
        sbPadding.Parent = searchBar

        local sbIcon = Instance.new("ImageLabel")
        sbIcon.Size = UDim2.new(0, 12, 0, 12)
        sbIcon.Position = UDim2.new(0, 0, 0.5, -6)
        sbIcon.BackgroundTransparency = 1
        sbIcon.Image = Icons["lucide-search"] or "rbxassetid://10734943674"
        sbIcon.ImageColor3 = Theme.Colors.TextSecondary
        sbIcon.Parent = searchBar

        local sbInput = Instance.new("TextBox")
        sbInput.Size = UDim2.new(1, -16, 1, 0)
        sbInput.Position = UDim2.new(0, 16, 0, 0)
        sbInput.BackgroundTransparency = 1
        sbInput.Text = state.searchQuery or ""
        sbInput.PlaceholderText = "Search..."
        sbInput.PlaceholderColor3 = Theme.Colors.TextSecondary
        sbInput.TextColor3 = Theme.Colors.Text
        sbInput.Font = Theme.Fonts.Regular
        sbInput.TextSize = 11
        sbInput.TextXAlignment = Enum.TextXAlignment.Left
        sbInput.ClearTextOnFocus = false
        sbInput.Parent = searchBar

        sbInput:GetPropertyChangedSignal("Text"):Connect(function()
            Store:Update(function(s)
                s.searchQuery = sbInput.Text
            end)
        end)
    end
    ...
    if state.canMinimize ~= false then
        ...
        minBtn.Image = Icons["lucide-minus"] or "rbxassetid://10747372910"
        minBtn.ImageColor3 = Theme.Colors.TextSecondary
        ...
        minBtn.MouseButton1Click:Connect(function()
            Store:Update(function(s)
                ...
                if s.minimized then s.maximized = false end
            end)
        end)
    end
    ...
    if state.canZoom ~= false then
        ...
        maxBtn.Image = isMaximized and (Icons["lucide-minimize-2"] or "rbxassetid://10747381986") or (Icons["lucide-maximize-2"] or "rbxassetid://10747381986")
        maxBtn.ImageColor3 = Theme.Colors.TextSecondary
        ...
        maxBtn.MouseButton1Click:Connect(function()
            Store:Update(function(s)
                ...
                if s.maximized then s.minimized = false end
            end)
        end)
    end
    ...
    if state.canExit ~= false then
        ...
        closeBtn.Image = Icons["lucide-x"] or "rbxassetid://10747384394"
        closeBtn.ImageColor3 = Theme.Colors.TextSecondary
    ...
    -- Container Body (Sidebar + TabContainer)
    local containerBody = Instance.new("CanvasGroup")
    containerBody.Name = "ContainerBody"
    containerBody.Size = UDim2.new(1, 0, 1, -45)
    containerBody.Position = UDim2.new(0, 0, 0, 45)
    containerBody.BackgroundTransparency = 1
    containerBody.Visible = not isMinimized
    containerBody.Parent = dragFrame

    local bodyLayout = Instance.new("UIListLayout")
    bodyLayout.FillDirection = Enum.FillDirection.Horizontal
    bodyLayout.SortOrder = Enum.SortOrder.LayoutOrder
    bodyLayout.Parent = containerBody

    local sidebarObj = SidebarComponent({ state = state, Store = Store, LayoutOrder = 1 })
    if sidebarObj then sidebarObj.Parent = containerBody end

    local mainContentObj = TabContainerComponent({ state = state, Store = Store, LayoutOrder = 2 })
    if mainContentObj then mainContentObj.Parent = containerBody end

    -- Overlay for ColorPicker if active
    if state.colorPickerActive then
        local dialog = ColorPickerDialog({ colorPickerActive = state.colorPickerActive, Store = Store })
        if dialog then dialog.Parent = dragFrame end
    end
    ...
    titleBar.InputBegan:Connect(function(input)
        ...
        if not state.draggable and state.draggable ~= nil then return end
    ...
    local inputConn = UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging and not state.maximized then
            ...
            state._lastPosition = newPos
        end
    end)

    dragFrame.Destroying:Connect(function()
...
__modules["Icons"] = function()
    local script = createVirtualModule("Icons")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
__modules["init"] = function()
    local script = createVirtualModule("init")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
local AppRecorderObj = require(script.AppRecorder)
...
Library.Icons = require(script.Icons)
...
local activeNotificationInstance = nil
...
local function setupContainerMethods(container, insertCallback)
    ...
    function container:TitleStack(config)
        ...
        local elData = {
            ...
            subtitle = config.Subtitle or config.subtitle,
            titleTag = config.TitleTag or config.titleTag,
            titleTagColor = config.TitleTagColor or config.titleTagColor,
            subtitleTag = config.SubtitleTag or config.subtitleTag,
            subtitleTagColor = config.SubtitleTagColor or config.subtitleTagColor,
        }
    ...
    function container:Form()
        local formId = math.random(1, 100000)
        local formData = {
            id = formId,
            type = "Form",
            elements = {},
        }
        insertCallback(formData)
        
        local FormObj = {}
        setupContainerMethods(FormObj, function(el)
            table.insert(formData.elements, el)
            Store:Update(function(state) end)
        end)
        return FormObj
    end
    ...
    function container:VStack(config)
        config = config or {}
        local vstackId = math.random(1, 100000)
        local vstackData = {
            id = vstackId,
            type = "VStack",
        ...
        insertCallback(vstackData)
        
        local VStackObj = {}
        setupContainerMethods(VStackObj, function(el)
            table.insert(vstackData.elements, el)
            Store:Update(function(state) end)
        end)
        return VStackObj
    end
    ...
    function container:HStack(config)
        config = config or {}
        local hstackId = math.random(1, 100000)
        local hstackData = {
            id = hstackId,
            type = "HStack",
            padding = config.Padding,
        ...
        insertCallback(hstackData)
        
        local HStackObj = {}
        setupContainerMethods(HStackObj, function(el)
            table.insert(hstackData.elements, el)
            Store:Update(function(state) end)
        end)
        return HStackObj
    end
    ...
    function container:Button(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Button",
            name = config.Text or config.Title or config.Name or "Button",
            icon = config.Icon or config.Image,
            callback = nil,
        }
        elData.callback = wrapCallback(elData, config.Callback)
        insertCallback(elData)
        
        local btnObj = {}
        function btnObj:SetText(text)
            elData.name = text
            Store:UpdateElement(elData.id, function(e) e.name = text end)
        end
        return btnObj
    end
    ...
    function container:Toggle(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Toggle",
            name = config.Text or config.Title or config.Name or "Toggle",
            value = config.Value == true or config.Default == true,
            callback = nil,
        }
        elData.callback = wrapCallback(elData, config.Callback)
        insertCallback(elData)
        ...
        function toggleObj:SetValue(val)
            elData.value = val
            Store:UpdateElement(elData.id, function(e) e.value = val end)
            if elData.SetValue then elData.SetValue(val) end
        end
        return toggleObj
    end
    ...
    function container:Slider(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Slider",
            name = config.Text or config.Title or config.Name or "Slider",
            min = config.Min or 0,
            max = config.Max or 100,
            value = config.Value or config.Default or (config.Min or 0),
    ...
    function container:Dropdown(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Dropdown",
            name = config.Text or config.Title or config.Name,
            options = config.Options or {},
            value = config.Value or config.Default or 1,
        ...
        function dropdownObj:SetOptions(opts)
            elData.options = opts
            Store:UpdateElement(elData.id, function(e) e.options = opts end)
        end
    ...
    function container:PullDownButton(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "PullDownButton",
            name = config.Text or config.Title or config.Name or "PullDown",
            options = config.Options or {},
            callback = nil,
        }
        elData.callback = wrapCallback(elData, config.Callback)
    ...
    function container:Input(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "TextBox",
            name = config.Text or config.Title or config.Name or "Input",
            placeholder = config.Placeholder or "Type here...",
            value = config.Value or config.Default or "",
            callback = nil,
        }
        elData.callback = wrapCallback(elData, config.Callback)
        insertCallback(elData)
    ...
    container.TextBox = container.Input
    ...
    function container:Keybind(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "KeybindField",
            name = config.Text or config.Title or config.Name or "Keybind",
            value = config.Value or config.Default or Enum.KeyCode.Unknown,
            callback = nil,
        }
        elData.callback = wrapCallback(elData, config.Callback)
        insertCallback(elData)
    ...
    container.KeybindField = container.Keybind
    ...
    function container:Stepper(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Stepper",
            name = config.Text or config.Title or config.Name or "Stepper",
            min = config.Min or 0,
            max = config.Max or 100,
    ...
    container.RadioGroup = container.RadioButtonGroup
    ...
    function container:Label(config)
        ...
        local elData = {
            id = math.random(1, 100000),
            type = "Label",
            text = config.Text or config.Title or "Label",
            color = config.Color,
            font = config.Font,
    ...
    function container:InfoGrid(config)
        ...
        local elData = {
            ...
            items = config.Items or config.Elements or {},
        }
        insertCallback(elData)
        return elData
    end
    ...
    -- ColorPicker (Inline and Popup)
    function container:ColorPicker(config)
        config = config or {}
        local elData = {
            id = math.random(1, 100000),
            type = config.Inline and "ColorPickerWidget" or "ColorPicker",
            name = config.Text or config.Title or config.Name or "Color Picker",
            value = config.Value or config.Default or Color3.fromRGB(255, 255, 255),
    ...
    local function renderApp()
        ...
        if not state.visible then
            if activeWindowInstance then
                activeWindowInstance:Destroy()
                activeWindowInstance = nil
            end
            if activeNotificationInstance then
                activeNotificationInstance:Destroy()
                activeNotificationInstance = nil
            end
            return
        end
        ...
        activeWindowInstance = WindowComponent.render(state, Store)
        if activeWindowInstance then
            activeWindowInstance.Parent = screenGui
        end

        activeNotificationInstance = NotificationComponent.List({
            notifications = state.notifications,
            Store = Store,
        })
        if activeNotificationInstance then
            activeNotificationInstance.Parent = screenGui
        end
    end
        ...
        function SectionObj:Tab(tabConfig)
            ...
            local function buildTabObj(currentTabId, currentSectionId)
                ...
                function TabObj:PageSection(config)
                    ...
                    setupContainerMethods(PageSecObj, function(el)
                        table.insert(pageSecData.elements, el)
                        Store:Update(function(state) end)
                    end)
                ...
                setupContainerMethods(TabObj, function(el)
                    local pageSecId = math.random(1, 100000)
                    local pageSecData = {
                        id = pageSecId,
                        type = "Section",
...
__modules["Store"] = function()
    local script = createVirtualModule("Store")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
...
__modules["Theme"] = function()
    local script = createVirtualModule("Theme")
    local Source = createVirtualFolder("")
    local require = __require
    local ReplicatedStorage = ReplicatedStorage
