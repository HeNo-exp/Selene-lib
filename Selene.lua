...
-- Shallow clone helper (available everywhere)
...
function React.Component:setState(partialState, callback)
    -- Snapshot the PREVIOUS state before mutating
...
function React.createElement(elementType, props, children)
    ...
    elseif type(elementType) == "table" and elementType.__index then
        ...
        -- For didUpdate: we need to supply prevProps.
        -- Since props.state may be a mutable reference (e.g. Store._state),
        -- we take a snapshot of the props at mount time so didUpdate can
        -- compare old vs new.  We deep-snapshot props.state if it exists.
        ...
        function componentInstance:_rerender(oldState)
            if componentInstance.didUpdate then
                ...
                -- Take new snapshot for NEXT didUpdate call
