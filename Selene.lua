...
function Sidebar:render()
    ...
    -- Fallback: render top-level tabs in state.tabs if not already rendered inside a section
    for _, tab in ipairs(state.tabs or {}) do
        if not renderedTabIds[tab.id] then
            renderedTabIds[tab.id] = true
            local isActive = state.activeTab == tab.id
...
function Window:didMount()
    ...
    -- Setup initial entry animation (slide-up/pop) ONCE at startup
    ...
    local lastActiveTab = self.props.state.activeTab
    local lastSearchQuery = self.props.state.searchQuery
    local lastTabsCount = #(self.props.state.tabs or {})
    
    -- Subscribe to Store updates to refresh UI content in-place WITHOUT destroying window frame or losing Search Box focus
    self.unsubscribeStore = self.props.Store:Subscribe(function(newState)
        local dragFrameObj = self.windowRef and self.windowRef.current
        local containerObj = self.containerRef and self.containerRef.current
        if not dragFrameObj then return end
        
        dragFrameObj.Visible = newState.visible
        
        local isMin = newState.minimized
        local isMax = newState.maximized
        local targetSize = isMax and UDim2.new(1, 0, 1, 0) or (isMin and UDim2.new(0, 750, 0, 45) or UDim2.new(0, 750, 0, 480))
        local targetPos = isMax and UDim2.new(0, 0, 0, 0) or self.state.position
        
        local tInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        TweenService:Create(dragFrameObj, tInfo, {
            Size = targetSize,
            Position = targetPos,
        }):Play()
        
        if containerObj then
            containerObj.Visible = not isMin
            
            local tabChanged = newState.activeTab ~= lastActiveTab
            local searchChanged = newState.searchQuery ~= lastSearchQuery
            local tabsCountChanged = #(newState.tabs or {}) ~= lastTabsCount
            
            if not isMin and (tabChanged or searchChanged or tabsCountChanged) then
                lastActiveTab = newState.activeTab
                lastSearchQuery = newState.searchQuery
                lastTabsCount = #(newState.tabs or {})
                
                containerObj:ClearAllChildren()
                
                local layoutEl = React.createElement("UIListLayout", {
                    FillDirection = Enum.FillDirection.Horizontal,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                })
                local sidebarEl = React.createElement(Sidebar, { state = newState, Store = self.props.Store, LayoutOrder = 1 })
                local tabContainerEl = React.createElement(TabContainer, { state = newState, Store = self.props.Store, LayoutOrder = 2 })
                
                if layoutEl then layoutEl.Parent = containerObj end
                if sidebarEl then sidebarEl.Parent = containerObj end
                if tabContainerEl then tabContainerEl.Parent = containerObj end
            end
        end
    end)
end
...
function Window:willUnmount()
    if self.unsubscribeStore then
        self.unsubscribeStore()
        self.unsubscribeStore = nil
    end
    if self.dragConnection then self.dragConnection:Disconnect() end
...
function Window:didUpdate(prevProps, prevState)
    ...
    if prevState.position ~= self.state.position and not self.props.state.maximized then
...
function Window:render()
    ...
    return React.createElement("CanvasGroup", {
        ...
        Container = React.createElement("CanvasGroup", {
            ...
            TabContainer = React.createElement(TabContainer, {
                state = state,
                Store = Store,
                LayoutOrder = 2,
            }),
...
function React.createElement(elementType, props, children)
            ...
            elseif type(childList) == "table" then
                ...
                elseif childList.Parent ~= nil and typeof(childList) == "Instance" then
                    ...
                    for key, child in pairs(childList) do
                        local targetChild = (type(child) == "table" and child.__nodeInstance) and child.__nodeInstance or child
                        if typeof(targetChild) == "Instance" then
                            if type(key) == "string" and (targetChild.Name == targetChild.ClassName or targetChild.Name == "Frame") then
                                targetChild.Name = key
                            end
                            targetChild.Parent = instance
                        elseif type(child) == "table" and child.__isInstanceWrapper then
                            if type(key) == "string" and (child.__instance.Name == child.__instance.ClassName or child.__instance.Name == "Frame") then
                                child.__instance.Name = key
                            end
                            child.__instance.Parent = instance
    ...
    elseif type(elementType) == "table" and elementType.__index then
        ...
        local prevProps = props
        local prevState = table.clone and table.clone(componentInstance.state) or componentInstance.state
...
local ReactRoblox = require(script.ReactRoblox)
...
function Library:CreateWindow(config)
    ...
    if not root then
        ...
        local function renderApp()
            ...
            local activeTheme = Theme.Themes[state.theme or "Dark"]
            if activeTheme then
            ...
            return React.createElement("Frame", {
                ...
                Window = React.createElement(WindowComponent, {
                    state = state,
                    Store = Store,
                }),
