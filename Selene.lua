...
function ColorPickerDialog:render()
    ...
    local function updateWheel(input)
        local wheelFrame = self.wheelRef.current
    ...
    local function updateBrightness(input)
        local bar = self.brightnessRef.current
    ...
    local function updateOpacity(input)
        local bar = self.opacityRef.current
        if not bar then return end
        local absPos = bar.AbsolutePosition
        local absSize = bar.AbsoluteSize
...
function Dropdown:didMount()
    local stroke = self.strokeRef.current
    local arrow = self.arrowRef.current
    local optionsFrame = self.optionsRef.current
...
function CustomComponentPlaceholder:didMount()
    local container = self.frameRef.current
    local el = self.props.element
...
function NotificationItem:didMount()
    local group = self.groupRef.current
    if group then
...
function NotificationItem:didUpdate(prevProps)
    if self.props.item.count ~= prevProps.item.count then
        local scale = self.scaleRef.current
        if scale then
...
function TabButton:didMount()
    local btn = self.btnRef.current
    local activeBar = self.activeBarRef.current
    local label = self.labelRef.current
    local icon = self.iconRef.current
...
function Slider:didMount()
    local fill = self.fillRef.current
    local knob = self.knobRef.current
    local stroke = self.strokeRef.current
...
function Slider:render()
    ...
    local function updateValue(input)
        local track = self.trackRef.current
...
function Toggle:didMount()
    local switch = self.switchRef.current
    local knob = self.knobRef.current
    local stroke = self.strokeRef.current
    local currentVal = self.state.value
    local hovered = self.state.hovered
...
function Window:didMount()
    local dragFrame = self.windowRef.current
    local container = self.containerRef.current
...
function Library:CreateWindow(config)
    ...
    -- Mount React root if needed
    if not root then
        local function App()
            local state, setState = React.useState(Store._state)
