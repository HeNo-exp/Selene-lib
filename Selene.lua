--[[
    Selene UI Library - Standalone Bundled Module (Pure Luau)
    Zero External Dependencies / Pure Native Roblox Luau UI Engine
--]]
...
local function __require(path)
    if __cache[path] then
        return __cache[path]
    end
    local mod = __modules[path]
    if not mod then
        error("Module not found in bundle: " .. tostring(path), 2)
    end
    local res = mod()
    __cache[path] = res
    return res
...
local function customRequire(target)
    if typeof(target) == "table" and target.__path then
        return __require(target.__path)
    elseif typeof(target) == "string" then
        return __require(target)
    end
    return __require(tostring(target))
end
...
local Icons = require(script.Icons).assets
...
Library.elements = {} -- Flat registry for AppRecorder

local IconsList = Icons
...
setmetatable(Symbols, {
    __index = function(self, key)
        ...
        local asset = IconsList[key]
        if asset then return asset end
        
        asset = IconsList["lucide-" .. key]
        if asset then return asset end
        
        local kebab = toKebabCase(key)
        asset = IconsList["lucide-" .. kebab]
        if asset then return asset end
        
        if key == "gear" or key == "settings" then
            return IconsList["lucide-settings"]
        elseif key == "house" or key == "home" then
            return IconsList["lucide-home"]
        elseif key == "checkmark" or key == "check" then
            return IconsList["lucide-check"]
        end
        ...
        return function(props)
            ...
            for k, v in pairs(props) do
                ...
                elseif typeof(v) == "Instance" then
                    v.Parent = instance
                elseif type(v) == "table" and v.__instance then
                    v.__instance.Parent = instance
                end
            end
            
            if props.Parent then
                local p = props.Parent
                if type(p) == "table" and p.__instance then
                    p = p.__instance
                end
                pcall(function()
                    instance.Parent = p
...
-- AppRecorder Implementation (pure Luau state serialization)
local AppRecorder = {}
...
function AppRecorder:Dump()
    ...
    for _, el in ipairs(Library.elements) do
        if el.value ~= nil and el.name and el.tabTitle then
            dumpData[el.tabTitle] = dumpData[el.tabTitle] or {}
            local val = el.value
            ...
            dumpData[el.tabTitle][el.name] = val
        end
    end
...
function AppRecorder:Load(jsonData)
    if not HttpService or not jsonData or jsonData == "" then return end
    local ok, dumpData = pcall(function()
        return HttpService:JSONDecode(jsonData)
    end)
    if not ok or type(dumpData) ~= "table" then return end
    ...
    local function deserializeValue(val)
        ...
        elseif type(val) == "table" and val._type == "EnumItem" then
            local enumObj = Enum[val.EnumType]
            if enumObj and enumObj[val.Name] then
    ...
    for _, el in ipairs(Library.elements) do
        local tabDump = dumpData[el.tabTitle]
        if tabDump and el.name and tabDump[el.name] ~= nil then
            local val = deserializeValue(tabDump[el.name])
            pcall(function()
                el:SetValue(val)
            end)
        end
...
Library.AppRecorder = AppRecorder

-- Global UI instance references
local screenGui = nil
...
local function getGuiParent()
    ...
    local ok2, StarterGui = pcall(function() return game:GetService("StarterGui") end)
    if ok2 and StarterGui then return StarterGui end
    return nil
end

-- Setup Container Layout Methods (Row, VStack, HStack, PageSection, Form, Elements)
local function setupContainerMethods(container, containerFrame, tabTitle)
        ...
        __index = function(t, k)
            if Library._customComponents[k] then
                return function(self, properties)
                    ...
                    properties.Parent = containerFrame
                    local customObj = Library._customComponents[k](self, properties)
                    local raw = (type(customObj) == "table" and customObj.__instance) and customObj.__instance or customObj
                    if raw and raw.Parent ~= containerFrame then
                        raw.Parent = containerFrame
                    end
                    return customObj
                end
            end
    ...
    function container:Row(rowConfig)
        ...
        leftFrame.Size = UDim2.new(0.5, -6, 0, 0)
        leftFrame.AutomaticSize = Enum.AutomaticSize.Y
        leftFrame.BackgroundTransparency = 1
        leftFrame.Parent = rowFrame
        ...
        rightFrame.Size = UDim2.new(0.5, -6, 0, 0)
        rightFrame.AutomaticSize = Enum.AutomaticSize.Y
        rightFrame.BackgroundTransparency = 1
        rightFrame.Parent = rowFrame
        ...
        function RowObj:Left()
            local LeftObj = {}
            setupContainerMethods(LeftObj, leftFrame, tabTitle)
            return LeftObj
        end
        function RowObj:Right()
            local RightObj = {}
            setupContainerMethods(RightObj, rightFrame, tabTitle)
            return RightObj
        end
        return RowObj
    end
    ...
    function container:TitleStack(config)
        ...
        subtitle.Font = Theme.Fonts.Regular
        subtitle.TextSize = 11
        subtitle.TextXAlignment = Enum.TextXAlignment.Left
        subtitle.Parent = frame
        
        local Obj = {}
        function Obj:SetTitle(txt)
            title.Text = txt
        end
        function Obj:SetSubtitle(txt)
            subtitle.Text = txt
        end
        return Obj
    end
    ...
    function container:Label(config)
        ...
        label.TextColor3 = config.TextColor3 or Theme.Colors.Text
        label.Font = config.Font or Theme.Fonts.Regular
    ...
    function container:Paragraph(config)
        ...
        content.TextColor3 = Theme.Colors.TextSecondary
        content.Font = Theme.Fonts.Regular
        content.TextSize = 11
    ...
    function container:Button(config)
        ...
        label.BackgroundTransparency = 1
        label.Text = config.Label or "Button"
        ...
        btn.MouseEnter:Connect(function()
            TweenService:Create(btn, TweenInfo.new(0.2), { BackgroundColor3 = Theme.Colors.ElementHover }):Play()
            TweenService:Create(stroke, TweenInfo.new(0.2), { Color = Theme.Colors.BorderHover }):Play()
        end)
        btn.MouseLeave:Connect(function()
            TweenService:Create(btn, TweenInfo.new(0.2), { BackgroundColor3 = Theme.Colors.Element }):Play()
            TweenService:Create(stroke, TweenInfo.new(0.2), { Color = Theme.Colors.Border }):Play()
        end)
        
        btn.MouseButton1Click:Connect(function()
            if config.Pushed then
                task.spawn(function()
                    pcall(config.Pushed)
                end)
            end
    ...
    function container:Toggle(config)
        ...
        switch.BackgroundColor3 = config.Value and Theme.Colors.Accent or Theme.Colors.Element
        switch.Text = ""
        switch.AutoButtonColor = false
        switch.Parent = frame
        
        local switchCorner = Instance.new("UICorner")
        switchCorner.CornerRadius = Theme.CornerRadius.Circle
        switchCorner.Parent = switch
        
        local switchStroke = Instance.new("UIStroke")
        switchStroke.Color = Theme.Colors.Border
        switchStroke.Thickness = 1
        switchStroke.Parent = switch
        
        local knob = Instance.new("Frame")
        knob.Size = UDim2.new(0, 16, 0, 16)
        knob.Position = config.Value and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
        ...
        local function updateVisuals()
            local targetColor = value and Theme.Colors.Accent or Theme.Colors.Element
        ...
        switch.MouseEnter:Connect(function()
            TweenService:Create(switchStroke, TweenInfo.new(0.2), { Color = Theme.Colors.BorderHover }):Play()
        end)
        switch.MouseLeave:Connect(function()
            TweenService:Create(switchStroke, TweenInfo.new(0.2), { Color = Theme.Colors.Border }):Play()
        end)
        ...
        switch.MouseButton1Click:Connect(function()
            ...
            if config.ValueChanged then
                task.spawn(function()
                    pcall(config.ValueChanged, value)
                end)
            end
        end)
        ...
        function Obj:SetCallback(func)
            config.ValueChanged = func
        end
        
        -- Handle reactive getter for AppRecorder
        setmetatable(Obj, {
            __index = function(t, k)
    ...
    function container:Slider(config)
        ...
        barRow.Position = UDim2.new(0, 0, 0, 20)
        ...
        track.MouseEnter:Connect(function()
            TweenService:Create(trackStroke, TweenInfo.new(0.2), { Color = Theme.Colors.BorderHover }):Play()
        end)
        track.MouseLeave:Connect(function()
            TweenService:Create(trackStroke, TweenInfo.new(0.2), { Color = Theme.Colors.Border }):Play()
        end)
    ...
    function container:TextField(config)
        ...
        inputContainer.Size = UDim2.new(1, -160, 0, 28)
        inputContainer.Position = UDim2.new(1, -160, 0.5, -14)
        ...
        textbox.Font = Theme.Fonts.Regular
        textbox.TextSize = 12
        ...
        textbox.FocusLost:Connect(function(enterPressed)
            ...
            if config.ValueChanged then
                task.spawn(function()
                    pcall(config.ValueChanged, text, enterPressed)
                end)
            end
        end)
        ...
        function Obj:SetPlaceholder(txt)
    ...
    function container:KeybindField(config)
        ...
        keyButton.Size = UDim2.new(0, 100, 0, 26)
        keyButton.Position = UDim2.new(1, -100, 0.5, -13)
        keyButton.BackgroundColor3 = Theme.Colors.Element
        keyButton.TextColor3 = Theme.Colors.Text
        keyButton.Font = Theme.Fonts.Bold
        ...
        local currentKey = config.Value or Enum.KeyCode.Unknown
        local isListening = false
        local keybindConn = nil
        
        local function updateText()
            keyButton.Text = isListening and "..." or (currentKey == Enum.KeyCode.Unknown and "None" or currentKey.Name)
            keyButton.BackgroundColor3 = isListening and Theme.Colors.Accent or Theme.Colors.Element
        end
        ...
        keyButton.MouseButton1Click:Connect(function()
            ...
            keybindConn = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    ...
                    if config.ValueChanged then
                        task.spawn(function()
                            pcall(config.ValueChanged, currentKey)
                        end)
                    end
                end
            end)
        ...
        function Obj:SetValue(val)
            currentKey = val or Enum.KeyCode.Unknown
            updateText()
        end
        function Obj:SetText(txt)
            label.Text = txt
        ...
        setmetatable(Obj, {
            __index = function(t, k)
                if k == "value" then return currentKey end
                return rawget(t, k)
            end,
            __newindex = function(t, k, v)
                if k == "value" then
                    currentKey = v or Enum.KeyCode.Unknown
                    updateText()
                else
                    rawset(t, k, v)
                end
    ...
    -- ColorPicker (Inline Hex + Button modal trigger)
    function container:ColorPicker(config)
        ...
        controls.Position = UDim2.new(1, -150, 0, 0)
        ...
        local currentColor = config.Value or Color3.fromRGB(255, 255, 255)
        local currentTrans = config.Transparency or 0
        
        local function updateVisuals()
            local r = math.round(currentColor.R * 255)
            local g = math.round(currentColor.G * 255)
            local b = math.round(currentColor.B * 255)
            local pct = math.round((1 - currentTrans) * 100)
            hexLabel.Text = string.format("#%02X%02X%02X (%d%%)", r, g, b, pct)
        ...
        colorBtn.MouseButton1Click:Connect(function()
            -- Spawn modular colorpicker dialog overlay on Window
            -- (Will update values direct via callback)
            pcall(function()
                local overlay = screenGui.SeleneRoot.ColorPickerOverlay
                if overlay then
                    overlay:Destroy()
                end
                
                -- Simple direct overlay implementation
                local overlayFrame = Instance.new("Frame")
                overlayFrame.Name = "ColorPickerOverlay"
                overlayFrame.Size = UDim2.new(1, 0, 1, 0)
                overlayFrame.BackgroundTransparency = 1
                overlayFrame.ZIndex = 1000
                overlayFrame.Parent = screenGui.SeleneRoot
                ...
                backdrop.Text = ""
                backdrop.Parent = overlayFrame
                backdrop.MouseButton1Click:Connect(function()
                    overlayFrame:Destroy()
                end)
                
                local dialog = Instance.new("Frame")
                dialog.Size = UDim2.fromOffset(320, 260)
                dialog.Position = UDim2.new(0.5, -160, 0.5, -130)
                dialog.BackgroundColor3 = Theme.Colors.Card
                dialog.Parent = overlayFrame
                
                local dCorner = Instance.new("UICorner")
                dCorner.CornerRadius = Theme.CornerRadius.Card
                dCorner.Parent = dialog
                
                local dStroke = Instance.new("UIStroke")
                dStroke.Color = Theme.Colors.Border
                dStroke.Thickness = 1
                ...
                title.BackgroundTransparency = 1
                title.Text = config.Title or "Select Color"
                title.TextColor3 = Theme.Colors.Text
                title.Font = Theme.Fonts.Bold
                title.TextSize = 13
                title.TextXAlignment = Enum.TextXAlignment.Left
                ...
                -- Quick rgb preview box
                local preview = Instance.new("Frame")
                preview.Size = UDim2.new(1, -24, 0, 40)
                preview.BackgroundColor3 = currentColor
                preview.Parent = dialog
                
                local prevCorner = Instance.new("UICorner")
                prevCorner.CornerRadius = Theme.CornerRadius.Element
                prevCorner.Parent = preview
                
                -- Red, Green, Blue inputs
                local sliders = Instance.new("Frame")
                sliders.Size = UDim2.new(1, -24, 0, 80)
                sliders.BackgroundTransparency = 1
                sliders.Parent = dialog
                
                local slideLayout = Instance.new("UIListLayout")
                slideLayout.Padding = UDim.new(0, 8)
                slideLayout.Parent = sliders
                
                local function makeColorSlider(name, val, max, setter)
                    local sFrame = Instance.new("Frame")
                    sFrame.Size = UDim2.new(1, 0, 0, 20)
                    sFrame.BackgroundTransparency = 1
                    sFrame.Parent = sliders
                    
                    local sLabel = Instance.new("TextLabel")
                    sLabel.Size = UDim2.new(0, 40, 1, 0)
                    sLabel.BackgroundTransparency = 1
                    sLabel.Text = name
                    sLabel.TextColor3 = Theme.Colors.TextSecondary
                    sLabel.Font = Theme.Fonts.Medium
                    sLabel.TextSize = 11
                    sLabel.TextXAlignment = Enum.TextXAlignment.Left
                    sLabel.Parent = sFrame
                    
                    local sTrack = Instance.new("TextButton")
                    sTrack.Size = UDim2.new(1, -50, 0, 6)
                    sTrack.Position = UDim2.new(0, 50, 0.5, -3)
                    sTrack.BackgroundColor3 = Theme.Colors.Element
                    sTrack.Text = ""
                    sTrack.AutoButtonColor = false
                    sTrack.Parent = sFrame
                    
                    local sFill = Instance.new("Frame")
                    sFill.BackgroundColor3 = Theme.Colors.Accent
                    ...
                    local function update()
                        sFill.Size = UDim2.new(currentVal / max, 0, 1, 0)
                        setter(currentVal)
                        preview.BackgroundColor3 = currentColor
                        preview.BackgroundTransparency = currentTrans
                    end
                ...
                makeColorSlider("Red", currentColor.R, 1, function(v)
                    currentColor = Color3.new(v, currentColor.G, currentColor.B)
                end)
                makeColorSlider("Green", currentColor.G, 1, function(v)
                    currentColor = Color3.new(currentColor.R, v, currentColor.B)
                end)
                makeColorSlider("Blue", currentColor.B, 1, function(v)
                ...
                apply.Position = UDim2.new(1, -80, 0, 0)
                apply.BackgroundColor3 = Theme.Colors.Accent
                apply.Text = "Apply"
                apply.TextColor3 = Color3.fromRGB(255, 255, 255)
                apply.Font = Theme.Fonts.Bold
                apply.TextSize = 12
                apply.Parent = footer
                
                local aCorner = Instance.new("UICorner")
                aCorner.CornerRadius = Theme.CornerRadius.Element
                aCorner.Parent = apply
                
                apply.MouseButton1Click:Connect(function()
                    updateVisuals()
                    if config.ValueChanged then
                        task.spawn(function()
                            pcall(config.ValueChanged, currentColor, currentTrans)
                        end)
                    end
                    overlayFrame:Destroy()
                end)
        ...
        function Obj:SetValue(color, trans)
            currentColor = color or Color3.fromRGB(255, 255, 255)
            currentTrans = trans or 0
            updateVisuals()
        end
        function Obj:SetText(txt)
            label.Text = txt
        ...
        setmetatable(Obj, {
            __index = function(t, k)
                if k == "value" then return currentColor end
                if k == "transparency" then return currentTrans end
                return rawget(t, k)
            end,
            __newindex = function(t, k, v)
                if k == "value" then
                    currentColor = v or Color3.fromRGB(255, 255, 255)
                    updateVisuals()
                else
                    rawset(t, k, v)
                end
    ...
    function container:ColorPickerWidget(config)
        return container:ColorPicker(config)
    end
    ...
    function container:Stepper(config)
        ...
        label.BackgroundTransparency = 1
        label.Text = config.Label or "Stepper"
        label.TextColor3 = Theme.Colors.Text
        label.Font = Theme.Fonts.Medium
        label.TextSize = 13
        label.TextXAlignment = Enum.TextXAlignment.Left
        ...
        control.Size = UDim2.fromOffset(120, 26)
        control.Position = UDim2.new(1, -120, 0.5, -13)
        ...
        local function update(newVal)
            ...
            local prec = 1 / step
            if prec == math.floor(prec) then
                value = math.round(value * prec) / prec
            end
            control.Display.Input.Text = tostring(value)
            if config.ValueChanged then
                task.spawn(function()
                    pcall(config.ValueChanged, value)
                end)
        ...
        dec.Size = UDim2.fromOffset(26, 26)
        dec.BackgroundColor3 = Theme.Colors.Element
        dec.Text = "-"
        dec.TextColor3 = Theme.Colors.Text
        dec.Font = Theme.Fonts.Bold
        dec.TextSize = 14
        dec.AutoButtonColor = false
        dec.Parent = control
        
        local decCorner = Instance.new("UICorner")
        decCorner.CornerRadius = Theme.CornerRadius.Element
        decCorner.Parent = dec
        ...
        display.Size = UDim2.fromOffset(68, 26)
        display.BackgroundTransparency = 1
        display.Parent = control
        ...
        inc.Size = UDim2.fromOffset(26, 26)
        inc.BackgroundColor3 = Theme.Colors.Element
        inc.Text = "+"
        inc.TextColor3 = Theme.Colors.Text
        inc.Font = Theme.Fonts.Bold
        inc.TextSize = 14
        inc.AutoButtonColor = false
        inc.Parent = control
        
        local incCorner = Instance.new("UICorner")
        incCorner.CornerRadius = Theme.CornerRadius.Element
        incCorner.Parent = inc
    ...
    function container:RadioButtonGroup(config)
        ...
        local selectedIndex = config.Value or 1
        ...
        opLayout.FillDirection = Enum.FillDirection.Vertical
        opLayout.Padding = UDim.new(0, 6)
        opLayout.Parent = optionsFrame
        
        local optButtons = {}
        
        local function updateVisuals()
            for idx, optBtn in ipairs(optButtons) do
                local isSelected = (idx == selectedIndex)
                local rc = optBtn:FindFirstChild("RadioCircle")
                if rc then
                    rc.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
                    local fill = rc:FindFirstChild("Fill")
                    if fill then
                        fill.Visible = isSelected
                    end
                end
                local ol = optBtn:FindFirstChild("OptionLabel")
                if ol then
                    ol.TextColor3 = isSelected and Theme.Colors.Text or Theme.Colors.TextSecondary
                    ol.Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Medium
                end
            end
        end
        ...
        for i, opt in ipairs(options) do
            ...
            btnLayout.Parent = optBtn
            
            local rc = Instance.new("Frame")
            rc.Name = "RadioCircle"
            rc.Size = UDim2.fromOffset(16, 16)
            rc.Parent = optBtn
            
            local rcCorner = Instance.new("UICorner")
            rcCorner.CornerRadius = Theme.CornerRadius.Circle
            rcCorner.Parent = rc
            
            local rcStroke = Instance.new("UIStroke")
            rcStroke.Color = Theme.Colors.Border
            rcStroke.Thickness = 1
            rcStroke.Parent = rc
            
            local fill = Instance.new("Frame")
            fill.Name = "Fill"
            fill.Size = UDim2.fromOffset(6, 6)
        ...
        local Obj = {
            name = config.Label or "RadioGroup",
            tabTitle = tabTitle,
        }
        function Obj:SetValue(idx)
            selectedIndex = math.clamp(idx, 1, #options)
            updateVisuals()
        end
        function Obj:SetText(txt)
            if title then
        ...
        setmetatable(Obj, {
            __index = function(t, k)
                if k == "value" then return selectedIndex end
                return rawget(t, k)
            end,
            __newindex = function(t, k, v)
                if k == "value" then
                    selectedIndex = math.clamp(v, 1, #options)
                    updateVisuals()
                else
                    rawset(t, k, v)
                end
    ...
    -- PopUpButton (Dropdown, supports single & multi selection)
    function container:PopUpButton(config)
        config = config or {}
        local options = config.Options or {}
        local isMulti = config.Maximum and config.Maximum > 1 or (type(config.Value) == "table")
        local maximum = config.Maximum or (isMulti and 9999 or 1)
        local value = config.Value or (isMulti and {} or 1)
        ...
        header.BackgroundColor3 = Theme.Colors.Element
        header.Text = ""
        header.AutoButtonColor = false
        header.Parent = frame
        
        local headerCorner = Instance.new("UICorner")
        headerCorner.CornerRadius = Theme.CornerRadius.Element
        headerCorner.Parent = header
        
        local headerStroke = Instance.new("UIStroke")
        headerStroke.Color = Theme.Colors.Border
        ...
        title.BackgroundTransparency = 1
        title.Text = config.Label or "Dropdown"
        ...
        selectedText.TextColor3 = Theme.Colors.Text
        selectedText.Font = Theme.Fonts.Bold
        ...
        arrow.Image = "rbxassetid://10747383844"
        arrow.ImageColor3 = Theme.Colors.TextSecondary
        arrow.Parent = header
        ...
        opLayout.Parent = optionsContainer
        
        local opPadding = Instance.new("UIPadding")
        opPadding.PaddingTop = UDim.new(0, 4)
        opPadding.PaddingBottom = UDim.new(0, 4)
        opPadding.Parent = optionsContainer
        ...
        local function getHeaderText()
            if isMulti then
                local currentVal = type(value) == "table" and value or {}
                ...
                return tostring(options[value] or "Select...")
            end
        end
        
        local function updateVisuals()
            selectedText.Text = getHeaderText()
            for idx, optBtn in ipairs(optionButtons) do
                local isSelected = false
                if isMulti then
                    isSelected = table.find(value, idx) ~= nil
                else
                    isSelected = idx == value
                end
                optBtn.BackgroundColor3 = (not isMulti and isSelected) and Theme.Colors.Accent or Theme.Colors.Element
                local ol = optBtn:FindFirstChildOfClass("TextLabel")
                if ol then
                    ol.TextColor3 = (not isMulti and isSelected) and Color3.fromRGB(255, 255, 255) or Theme.Colors.Text
                    ol.Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Regular
                end
                
                -- Checkbox visual for multi-select
                local cb = optBtn:FindFirstChild("Checkbox")
                if cb then
                    cb.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
                    local inner = cb:FindFirstChild("Inner")
                    if inner then
        ...
        local function rebuildOptions()
            ...
            for i, opt in ipairs(options) do
                ...
                content.BackgroundTransparency = 1
                content.Parent = optBtn
                
                local btnLayout = Instance.new("UIListLayout")
                btnLayout.FillDirection = Enum.FillDirection.Horizontal
                btnLayout.VerticalAlignment = Enum.VerticalAlignment.Center
                btnLayout.Padding = UDim.new(0, 8)
                btnLayout.Parent = content
                
                local btnPadding = Instance.new("UIPadding")
                btnPadding.PaddingLeft = UDim.new(0, 8)
                btnPadding.Parent = content
                ...
                local ol = Instance.new("TextLabel")
                ol.Size = UDim2.new(1, isMulti and -26 or -16, 1, 0)
                ol.BackgroundTransparency = 1
                ol.Text = tostring(opt)
                ol.TextXAlignment = Enum.TextXAlignment.Left
                ol.TextSize = 12
    ...
    function container:PullDownButton(config)
        ...
        title.Position = UDim2.new(0, 10, 0, 0)
        title.BackgroundTransparency = 1
        title.Text = config.Label or "Menu"
        title.TextColor3 = Theme.Colors.Text
        title.Font = Theme.Fonts.Bold
        title.TextSize = 12
        title.TextXAlignment = Enum.TextXAlignment.Left
        ...
        local function rebuildOptions()
            ...
            for i, opt in ipairs(options) do
                ...
                ol.BackgroundTransparency = 1
                ol.Text = tostring(opt)
                ol.TextColor3 = Theme.Colors.Text
                ol.Font = Theme.Fonts.Regular
                ol.TextXAlignment = Enum.TextXAlignment.Left
                ol.TextSize = 12
                ol.Parent = optBtn
    ...
    function container:VStack(config)
        ...
        stackFrame.Size = UDim2.new(1, 0, 0, 0)
        stackFrame.AutomaticSize = Enum.AutomaticSize.Y
        stackFrame.BackgroundTransparency = 1
        stackFrame.Parent = containerFrame
        
        local layout = Instance.new("UIListLayout")
        layout.FillDirection = Enum.FillDirection.Vertical
        layout.Padding = UDim.new(0, config.Padding or 6)
        if config.HorizontalAlignment then layout.HorizontalAlignment = config.HorizontalAlignment end
        if config.VerticalAlignment then layout.VerticalAlignment = config.VerticalAlignment end
        layout.Parent = stackFrame
        
        local VStackObj = {}
        setupContainerMethods(VStackObj, stackFrame, tabTitle)
        return VStackObj
    end
    ...
    function container:HStack(config)
        ...
        local HStackObj = {}
        setupContainerMethods(HStackObj, stackFrame, tabTitle)
        return HStackObj
    end
    ...
    function container:BarChart(config)
        ...
        local function rebuildChart()
            ...
            for i, val in ipairs(data) do
                local colWidth = count > 0 and (1 / count) or 0.1
                local bar = Instance.new("Frame")
                bar.Size = UDim2.new(colWidth, -8, val / maxVal, 0)
        ...
        function Obj:PushData(val, maxPoints)
            table.insert(data, val)
            maxPoints = maxPoints or 10
            while #data > maxPoints do
                table.remove(data, 1)
            end
            rebuildChart()
        end
        function Obj:SetTitle(txt) end
        function Obj:SetSubtitle(txt) end
        return Obj
    end

    -- LineChart
    function container:LineChart(config)
        return container:BarChart(config)
    end
    ...
    function container:InfoGrid(config)
        ...
        local function rebuildGrid()
            ...
            for rIdx, rData in ipairs(rowsData) do
                local rFrame = Instance.new("Frame")
                rFrame.Size = UDim2.new(1, 0, 0, 0)
                ...
                rLayout.Parent = rFrame
                
                local totalWeight = 0
                for _, cell in ipairs(rData) do
                    totalWeight = totalWeight + (cell.weight or 1)
                end
                
                for _, cell in ipairs(rData) do
                    local cellWeight = cell.weight or 1
                    local cellWidth = cellWeight / totalWeight
                    
                    local cellFrame = Instance.new("Frame")
                    cellFrame.Size = UDim2.new(cellWidth, -8, 0, 48)
                    cellFrame.BackgroundColor3 = Theme.Colors.Element
                    cellFrame.Parent = rFrame
                    
                    local cellCorner = Instance.new("UICorner")
                    cellCorner.CornerRadius = Theme.CornerRadius.Element
                    cellCorner.Parent = cellFrame
                    
                    local cellStroke = Instance.new("UIStroke")
                    cellStroke.Color = Theme.Colors.Border
                    cellStroke.Thickness = 1
                    cellStroke.Parent = cellFrame
                    
                    local cellLayout = Instance.new("UIListLayout")
                    cellLayout.FillDirection = Enum.FillDirection.Vertical
                    cellLayout.Padding = UDim.new(0, 2)
                    cellLayout.VerticalAlignment = Enum.VerticalAlignment.Center
                    cellLayout.Parent = cellFrame
                    
                    local cellPadding = Instance.new("UIPadding")
                    cellPadding.PaddingLeft = UDim.new(0, 10)
                    cellPadding.Parent = cellFrame
                    
                    local cellTitle = Instance.new("TextLabel")
                    cellTitle.Size = UDim2.new(1, 0, 0, 14)
                    cellTitle.BackgroundTransparency = 1
                    cellTitle.Text = cell.title or ""
                    cellTitle.TextColor3 = Theme.Colors.TextSecondary
                    cellTitle.Font = Theme.Fonts.Medium
                    cellTitle.TextSize = 10
                    cellTitle.TextXAlignment = Enum.TextXAlignment.Left
                    cellTitle.Parent = cellFrame
                    
                    local cellContent = Instance.new("TextLabel")
                    cellContent.Size = UDim2.new(1, 0, 0, 16)
                    cellContent.BackgroundTransparency = 1
                    cellContent.Text = cell.content or ""
                    cellContent.TextColor3 = Theme.Colors.Text
                    cellContent.Font = Theme.Fonts.Bold
                    cellContent.TextSize = 12
                    cellContent.TextXAlignment = Enum.TextXAlignment.Left
                    cellContent.Parent = cellFrame
                end
        ...
        local Obj = {}
        function Obj:UpdateRow(rowIdx, rowData)
            rowsData[rowIdx] = rowData
            rebuildGrid()
        end
        return Obj
    end
    ...
    function container:ImageSurface(config)
        ...
        image.ImageColor3 = config.SurfaceColor or Color3.fromRGB(255, 255, 255)
...
function Library.New(properties)
    ...
    local activeTheme = Theme.Themes[initialTheme]
    if activeTheme then
        for k, v in pairs(activeTheme) do
            Theme.Colors[k] = v
        end
    end
    Theme.Colors.Accent = Theme.Accents[initialAccent] or Theme.Accents.Blue
    ...
    local app = setmetatable({
        ...
        __index = function(t, k)
            if k == "Theme" then
                return Theme.Themes[initialTheme]
        ...
        __newindex = function(t, k, v)
            if k == "Theme" then
                ...
                if themeName then
                    ...
                    local colors = Theme.Themes[themeName]
...
-- Create Window (Pure Luau OOP UI Layout)
function Library:CreateWindow(config)
    ...
    windowFrame.Size = UDim2.fromOffset(750, 480)
    windowFrame.Position = UDim2.new(0.5, -375, 0.5, -240)
    windowFrame.BackgroundColor3 = Theme.Colors.Background
    windowFrame.Parent = rootFrame
    
    local wCorner = Instance.new("UICorner")
    wCorner.CornerRadius = Theme.CornerRadius.Window
    wCorner.Parent = windowFrame
    
    local wStroke = Instance.new("UIStroke")
    wStroke.Color = Theme.Colors.Border
    wStroke.Thickness = 1
    ...
    appIcon.Size = UDim2.fromOffset(20, 20)
    ...
    titleArea.Size = UDim2.new(1, -310, 1, 0)
    ...
    tLayout.Parent = titleArea
    
    local title = Instance.new("TextLabel")
    title.Size = UDim2.new(0, 0, 1, 0)
    title.AutomaticSize = Enum.AutomaticSize.X
    title.BackgroundTransparency = 1
    title.Text = config.Title or config.Name or "Selene UI"
    title.TextColor3 = Theme.Colors.Text
    title.Font = Theme.Fonts.Bold
    title.TextSize = 14
    title.Parent = titleArea
    
    if config.TitleTag then
        local tagFrame = Instance.new("Frame")
        tagFrame.Size = UDim2.new(0, 0, 0, 16)
        tagFrame.AutomaticSize = Enum.AutomaticSize.X
        tagFrame.BackgroundColor3 = Theme.Colors.Accent
        tagFrame.BorderSizePixel = 0
        tagFrame.Parent = titleArea
        ...
        tagLabel.Text = string.upper(config.TitleTag)
        tagLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
        tagLabel.Font = Theme.Fonts.Bold
        tagLabel.TextSize = 8
        tagLabel.Parent = tagFrame
        
        if type(config.TitleTagColor) == "table" then
            local grad = Instance.new("UIGradient")
            grad.Color = ColorSequence.new(unpack(config.TitleTagColor))
            grad.Rotation = 45
    ...
    local controls = Instance.new("Frame")
    controls.Size = UDim2.fromOffset(100, 45)
    ...
    -- Minimize Button
    if config.CanMinimize ~= false then
        local minBtn = Instance.new("ImageButton")
        minBtn.Size = UDim2.fromOffset(14, 14)
        minBtn.BackgroundTransparency = 1
        minBtn.Image = "rbxassetid://10747372251" -- fallback icon
        minBtn.ImageColor3 = Theme.Colors.TextSecondary
        minBtn.Parent = controls
        
        minBtn.MouseButton1Click:Connect(function()
            isMinimized = not isMinimized
            local targetSize = isMinimized and UDim2.fromOffset(750, 45) or UDim2.fromOffset(750, 480)
            TweenService:Create(windowFrame, TweenInfo.new(0.25), { Size = targetSize }):Play()
            container.Visible = not isMinimized
        end)
    end
    
    -- Close Button
    if config.CanExit ~= false then
    ...
    sbBorder.BackgroundColor3 = Theme.Colors.Border
    ...
    sbLayout.Parent = navList
    
    local sbPadding = Instance.new("UIPadding")
    sbPadding.PaddingTop = UDim.new(0, 12)
    sbPadding.PaddingLeft = UDim.new(0, 12)
    sbPadding.PaddingRight = UDim.new(0, 12)
    sbPadding.Parent = navList
    ...
    notificationContainer.Parent = rootFrame
    ...
    -- Profile Card Area
    local profileArea = nil
    
    -- Window OOP Object
    local Window = {}
    
    function Window:SetProfile(profileConfig)
        if profileArea then profileArea:Destroy() end
        
        profileArea = Instance.new("Frame")
        ...
        pLayout.Parent = pContainer
        
        local avatar = Instance.new("ImageLabel")
        avatar.Size = UDim2.fromOffset(36, 36)
        avatar.BackgroundColor3 = Theme.Colors.Element
        avatar.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(profileConfig.UserId or 1) .. "&w=150&h=150"
        ...
        username.Text = profileConfig.Username or "Guest"
        ...
        role.Text = profileConfig.Subtext or "User"
    ...
    function Window:Section(secConfig)
        ...
        if secConfig.Title and secConfig.Title ~= "" then
            local header = Instance.new("TextLabel")
            header.Name = "SecHeader_" .. secConfig.Title
            header.Size = UDim2.new(1, 0, 0, 18)
            header.BackgroundTransparency = 1
            header.Text = string.upper(secConfig.Title)
            header.TextColor3 = Theme.Colors.TextSecondary
            header.Font = Theme.Fonts.Bold
            header.TextSize = 10
            header.TextXAlignment = Enum.TextXAlignment.Left
            header.Parent = navList
        end
        ...
        function SectionObj:Tab(tabConfig)
            ...
            btnCorner.CornerRadius = Theme.CornerRadius.Element
            ...
            local barCorner = Instance.new("UICorner")
            barCorner.CornerRadius = Theme.CornerRadius.Circle
            ...
            local label = Instance.new("TextLabel")
            label.Size = UDim2.new(1, tabConfig.Icon and -22 or 0, 1, 0)
            label.BackgroundTransparency = 1
            ...
            tabScroll.ScrollBarImageColor3 = Theme.Colors.Border
            ...
            local function selectTab()
                ...
                if activeTabButton then
                    ...
                    activeTabButton.ActiveBar.Size = UDim2.new(0, 3, 0, 0)
                    activeTabButton.ActiveBar.BackgroundTransparency = 1
                    activeTabButton.Content.Label.TextColor3 = Theme.Colors.TextSecondary
                    activeTabButton.Content.Label.Font = Theme.Fonts.Medium
                    if activeTabButton.Content:FindFirstChild("Icon") then
                        activeTabButton.Content.Icon.ImageColor3 = Theme.Colors.TextSecondary
                    end
                end
                
                activeTabFrame = tabScroll
                activeTabButton = tabBtn
                
                tabScroll.Visible = true
                tabBtn.BackgroundColor3 = Theme.Colors.Element
                ...
                if content:FindFirstChild("Icon") then
                    content.Icon.ImageColor3 = Theme.Colors.Text
                end
            end
            ...
            function TabObj:SetBanner(bannerConfig)
                ...
                banner.BackgroundColor3 = Theme.Colors.Card
                banner.Parent = tabScroll
                
                local bCorner = Instance.new("UICorner")
                bCorner.CornerRadius = Theme.CornerRadius.Card
                ...
                bPadding.PaddingLeft = UDim.new(0, 15)
                bPadding.PaddingRight = UDim.new(0, 15)
                bPadding.Parent = banner
                
                local avatar = Instance.new("ImageLabel")
                avatar.Size = UDim2.fromOffset(48, 48)
                avatar.BackgroundColor3 = Theme.Colors.Element
                avatar.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(bannerConfig.UserId or 1) .. "&w=150&h=150"
                avatar.Parent = banner
                
                local avCorner = Instance.new("UICorner")
                avCorner.CornerRadius = Theme.CornerRadius.Circle
                avCorner.Parent = avatar
                
                local avStroke = Instance.new("UIStroke")
                avStroke.Color = Theme.Colors.Border
                avStroke.Thickness = 1
                avStroke.Parent = avatar
                
                local bInfo = Instance.new("Frame")
                bInfo.Size = UDim2.new(1, -63, 0, 40)
                bInfo.BackgroundTransparency = 1
                bInfo.Parent = banner
                
                local bInfoLayout = Instance.new("UIListLayout")
                bInfoLayout.FillDirection = Enum.FillDirection.Vertical
                bInfoLayout.VerticalAlignment = Enum.VerticalAlignment.Center
                bInfoLayout.Padding = UDim.new(0, 2)
                bInfoLayout.Parent = bInfo
                
                local greet = Instance.new("TextLabel")
                greet.Size = UDim2.new(1, 0, 0, 20)
                greet.BackgroundTransparency = 1
                greet.Text = "Hello, " .. (bannerConfig.Username or "Guest")
                greet.TextColor3 = Theme.Colors.Text
                greet.Font = Theme.Fonts.Bold
                greet.TextSize = 16
                ...
                sub.BackgroundTransparency = 1
                sub.Text = bannerConfig.Subtext or ""
                sub.TextColor3 = Theme.Colors.TextSecondary
            ...
            function TabObj:PageSection(pSecConfig)
                pSecConfig = pSecConfig or {}
                local secFrame = Instance.new("Frame")
                secFrame.Size = UDim2.new(1, 0, 0, 0)
                secFrame.AutomaticSize = Enum.AutomaticSize.Y
                secFrame.BackgroundColor3 = Theme.Colors.Card
                secFrame.Parent = tabScroll
                
                local secCorner = Instance.new("UICorner")
                secCorner.CornerRadius = Theme.CornerRadius.Card
                secCorner.Parent = secFrame
                
                local secStroke = Instance.new("UIStroke")
                secStroke.Color = Theme.Colors.Border
                secStroke.Thickness = 1
                secStroke.Parent = secFrame
                
                local secPadding = Instance.new("UIPadding")
                secPadding.PaddingTop = UDim.new(0, 14)
                ...
                secContent.Size = UDim2.new(1, 0, 1, 0)
                ...
                if pSecConfig.Title or pSecConfig.Subtitle then
                    local sHeader = Instance.new("Frame")
                    sHeader.Size = UDim2.new(1, 0, 0, 0)
                    sHeader.AutomaticSize = Enum.AutomaticSize.Y
                    sHeader.BackgroundTransparency = 1
                    sHeader.Parent = secContent
                    
                    local shLayout = Instance.new("UIListLayout")
                    shLayout.Padding = UDim.new(0, 2)
                    shLayout.Parent = sHeader
                    
                    if pSecConfig.Title then
                        local shTitle = Instance.new("TextLabel")
                        shTitle.Size = UDim2.new(1, 0, 0, 18)
                        shTitle.BackgroundTransparency = 1
                        shTitle.Text = pSecConfig.Title
                        shTitle.TextColor3 = Theme.Colors.Text
                        shTitle.Font = Theme.Fonts.Bold
                        shTitle.TextSize = 15
                        shTitle.TextXAlignment = Enum.TextXAlignment.Left
                        shTitle.Parent = sHeader
                    end
                    if pSecConfig.Subtitle then
                        local shSub = Instance.new("TextLabel")
                        shSub.Size = UDim2.new(1, 0, 0, 14)
                        shSub.BackgroundTransparency = 1
                        shSub.Text = pSecConfig.Subtitle
                        shSub.TextColor3 = Theme.Colors.TextSecondary
                        shSub.Font = Theme.Fonts.Regular
                        shSub.TextSize = 11
                        shSub.TextXAlignment = Enum.TextXAlignment.Left
                        shSub.Parent = sHeader
                    end
                ...
                function PageSecObj:Form()
                    ...
                    formFrame.Parent = secContent
                    
                    local fLayout = Instance.new("UIListLayout")
                    fLayout.Padding = UDim.new(0, 8)
                    fLayout.Parent = formFrame
                    
                    local FormObj = {}
                    setupContainerMethods(FormObj, formFrame, tabConfig.Title)
                    return FormObj
                end
                return PageSecObj
            end
            ...
            -- Nesting support: Tab inside Tab
            function TabObj:Tab(subTabConfig)
                subTabConfig = subTabConfig or {}
                subTabConfig.parentTabId = tabConfig.Title
                return SectionObj:Tab(subTabConfig)
            end
...
-- Spawn Stack Notification
function Library:Notification(config)
    config = config or {}
    local title = config.Title or "Notification"
    local subtitle = config.Subtitle or ""
    local icon = config.AppIcon or config.Icon or "rbxassetid://10747373111"
    ...
    item.Parent = notificationContainer
    ...
    local appIcon = Instance.new("ImageLabel")
    appIcon.Size = UDim2.fromOffset(24, 24)
    appIcon.BackgroundTransparency = 1
    appIcon.Image = icon
    appIcon.ImageColor3 = Theme.Colors.Accent
    appIcon.Parent = item
    ...
    item.Position = UDim2.new(1, 50, 0, 0)
    item.BackgroundTransparency = 1
    appIcon.ImageTransparency = 1
    ...
    TweenService:Create(appIcon, TweenInfo.new(0.4), { ImageTransparency = 0 }):Play()
    ...
    task.spawn(function()
        task.wait(duration)
        local outTween = TweenService:Create(item, TweenInfo.new(0.3), {
            Position = UDim2.new(1, 50, 0, 0),
            BackgroundTransparency = 1
        })
        TweenService:Create(appIcon, TweenInfo.new(0.3), { ImageTransparency = 1 }):Play()
