...
__modules["Components/BarChart"] = function()
    local require = customRequire
local Source = (script and (typeof or type)(script) == "Instance" and script.Parent and script.Parent.Parent) or Source or {}
...
function ColorPicker:init(props)
    self.hexLabelRef = React.createRef()
    self.colorBtnRef = React.createRef()
    self.strokeRef = React.createRef()
    
    local el = props.element or {}
    self.state = {
        hovered = false,
        value = el.value or Color3.fromRGB(255, 255, 255),
        transparency = el.transparency or 0,
    }
end

function ColorPicker:didMount()
    local Store = self.props.Store
    if Store and Store.Subscribe then
        self.unsubscribe = Store:Subscribe(function()
            local el = self.props.element
            if el and (el.value ~= self.state.value or el.transparency ~= self.state.transparency) then
                self:setState({
                    value = el.value or Color3.fromRGB(255, 255, 255),
                    transparency = el.transparency or 0,
                })
...
function ColorPicker:didUpdate(prevProps, prevState)
    local hexLabel = self.hexLabelRef and self.hexLabelRef.current
    local colorBtn = self.colorBtnRef and self.colorBtnRef.current
    local stroke = self.strokeRef and self.strokeRef.current
    
    local currentColor = self.state.value or Color3.fromRGB(255, 255, 255)
    local currentTrans = self.state.transparency or 0
    local hovered = self.state.hovered
...
function ColorPicker:render()
    ...
    return React.createElement("Frame", {
        ...
        -- Right Controls Area (Hex Label + Square Color Button)
        Controls = React.createElement("Frame", {
            Size = hideLabel and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 150, 1, 0),
            Position = hideLabel and UDim2.new(0, 0, 0, 0) or UDim2.new(1, -150, 0, 0),
            BackgroundTransparency = 1,
        }, {
            List = React.createElement("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
...
function ColorPickerDialog:init()
    ...
    local currentColor = el and el.value or Color3.fromRGB(255, 255, 255)
    local currentTrans = el and el.transparency or 0
...
function ColorPickerDialog:render()
    ...
    local initialColor = el and el.value or Color3.fromRGB(255, 255, 255)
    local initialTrans = el and el.transparency or 0
    ...
    local function onWheelChange(input)
        if self.isDraggingWheel and input and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            updateWheel(input)
        end
    end
    ...
    local function onBrightnessChange(input)
        if self.isDraggingBrightness and input and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            updateBrightness(input)
        end
    end
    
    local function onOpacityInput(input)
        if input and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
    ...
    local function onOpacityChange(input)
        if self.isDraggingOpacity and input and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            updateOpacity(input)
        end
    end
    ...
    return React.createElement("Frame", {
        ...
        Modal = React.createElement("Frame", {
            ...
            -- Main Workspace: Left Wheel, Right Inputs
            Workspace = React.createElement("Frame", {
            ...
            Footer = React.createElement("Frame", {
                ...
                ApplyBtn = React.createElement("TextButton", {
                    Size = UDim2.new(0, 60, 1, 0),
                    ...
                    AutoButtonColor = false,
                    [React.Event.MouseButton1Click] = function()
                        local finalColor = activeColor
                        local finalTrans = activeTrans
                        
                        Store:UpdateElement(el.id, function(element)
                            element.value = finalColor
                            element.transparency = finalTrans
                        end)
                        
                        if el.callback then
                            task.spawn(el.callback, finalColor, finalTrans)
                        end
...
function DropdownOption:init(props)
    self.state = {
        hovered = false,
    }
end
...
function DropdownOption:render()
    ...
    return React.createElement("TextButton", {
        ...
        Content = React.createElement("Frame", {
            ...
            -- Option text
            Label = React.createElement("TextLabel", {
                Size = UDim2.new(1, isMulti and -26 or -16, 1, 0),
                BackgroundTransparency = 1,
                Text = tostring(opt),
                TextColor3 = (not isMulti and isSelected) and Color3.fromRGB(255, 255, 255) or Theme.Colors.Text,
                Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Regular,
                TextSize = 12,
                TextXAlignment = Enum.TextXAlignment.Left,
                LayoutOrder = 2,
            }),
...
function Dropdown:init(props)
    ...
    self.selectedTextRef = React.createRef()
    
    local el = props.element
    local isMulti = el and el.maximum ~= nil and el.maximum > 1
    local initialVal = el and el.value
    if isMulti then
        if type(initialVal) ~= "table" then
            initialVal = type(initialVal) == "number" and { initialVal } or {}
        end
    else
        initialVal = initialVal or 1
    end
...
function Dropdown:getHeaderText(val)
    local el = self.props.element
    local options = el.options or {}
    local isMulti = el.maximum ~= nil and el.maximum > 1
    
    if isMulti then
        local currentVal = type(val) == "table" and val or {}
        ...
        return tostring(options[val] or "Select...")
    end
end
...
function Dropdown:didMount()
    ...
    if Store and Store.Subscribe then
        self.unsubscribe = Store:Subscribe(function()
            local el = self.props.element
            if el and el.value ~= self.state.value then
...
function Dropdown:didUpdate(prevProps, prevState)
    ...
    local selectedText = self.selectedTextRef and self.selectedTextRef.current
    ...
    -- 1. Update Selected Header Text
    if selectedText then
        selectedText.Text = self:getHeaderText(currentVal)
    end
    
    -- 2. Update Visual Styles of all Option Buttons directly in DOM
    if optionsFrame then
        for i, _ in ipairs(options) do
            local optBtn = optionsFrame:FindFirstChild("Opt_" .. tostring(i))
            if optBtn then
                local isSelected = false
                if isMulti then
                    isSelected = type(currentVal) == "table" and table_find(currentVal, i) ~= nil
                else
                ...
                local content = optBtn:FindFirstChild("Content")
                if content then
                    local label = content:FindFirstChild("Label")
                    if label then
                        label.TextColor3 = (not isMulti and isSelected) and Color3.fromRGB(255, 255, 255) or Theme.Colors.Text
                        label.Font = isSelected and Theme.Fonts.Bold or Theme.Fonts.Regular
                    end
                    
                    if isMulti then
                        local checkbox = content:FindFirstChild("Checkbox")
                        if checkbox then
                            checkbox.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
                            local inner = checkbox:FindFirstChild("Inner")
                            if inner then
...
function Dropdown:render()
    ...
    local headerDisplay = self:getHeaderText(currentVal)
    ...
    for i, opt in ipairs(options) do
        ...
        optionButtons["Opt_" .. tostring(i)] = React.createElement(DropdownOption, {
            ...
            OnClick = function()
                if isMulti then
                    local newVal = type(self.state.value) == "table" and table_clone(self.state.value) or {}
                    local existingIdx = table_find(newVal, i)
                    if existingIdx then
                        table.remove(newVal, existingIdx)
                    else
                        if #newVal < (el.maximum or 999) then
                    ...
                    self:setState({ value = i, isOpen = false })
...
function CustomComponentPlaceholder:didMount()
    ...
    local el = self.props.element
    if container and el and el.instance then
        pcall(function()
            local inst = (type(el.instance) == "table" and el.instance.__instance) or el.instance
...
function CustomComponentPlaceholder:didUpdate(prevProps)
    local container = self.frameRef and self.frameRef.current
    local el = self.props.element
    local prevEl = prevProps and prevProps.element
    
    if prevEl and prevEl.instance and prevEl.instance ~= el.instance then
        pcall(function()
            local prevInst = (type(prevEl.instance) == "table" and prevEl.instance.__instance) or prevEl.instance
            if typeof(prevInst) == "Instance" then
                prevInst.Parent = nil
            end
        end)
    end
...
function CustomComponentPlaceholder:render()
    local el = self.props.element
    local inst = el and ((type(el.instance) == "table" and el.instance.__instance) or el.instance)
    local size = (typeof(inst) == "Instance" and inst.Size) or UDim2.new(1, 0, 0, 70)
    return React.createElement("Frame", {
        ref = self.frameRef,
        Size = size,
        BackgroundTransparency = 1,
...
function KeybindField:init(props)
    self.connection = nil
    ...
    self.state = {
        hovered = false,
        listening = false,
        bind = el.value or Enum.KeyCode.Unknown,
    }
end

function KeybindField:didUpdate(prevProps, prevState)
    local btn = self.buttonRef and self.buttonRef.current
    local el = self.props.element
    if btn then
        local currentBind = self.state.bind or Enum.KeyCode.Unknown
        local isUnknown = currentBind == Enum.KeyCode.Unknown
        local label = isUnknown and "None" or currentBind.Name
        local buttonText = self.state.listening and "..." or label
        btn.Text = buttonText
    end
end
            ...
            [React.Event.MouseButton1Click] = function()
                ...
                self.connection = UserInputService.InputBegan:Connect(function(input, processed)
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        self:setState({ bind = key, listening = false })
...
function RadioOption:init(props)
    self.state = {
        hovered = false,
    }
end
...
function RadioButtonGroup:init(props)
    self.optionsFrameRef = React.createRef()
    local el = props.element or {}
    self.state = {
        value = el.value or 1
    }
...
function RadioButtonGroup:didMount()
    local Store = self.props.Store
    if Store and Store.Subscribe then
        self.unsubscribe = Store:Subscribe(function()
            local el = self.props.element
...
function RadioButtonGroup:willUnmount()
    if self.unsubscribe then
        self.unsubscribe()
    end
end

function RadioButtonGroup:didUpdate(prevProps, prevState)
    local optionsFrame = self.optionsFrameRef and self.optionsFrameRef.current
    local el = self.props.element
    local options = el.options or {}
    local selectedIndex = self.state.value
    ...
    if optionsFrame then
        for i, _ in ipairs(options) do
            ...
            if optBtn then
                ...
                local radioCircle = optBtn:FindFirstChild("RadioCircle")
                if radioCircle then
                    radioCircle.BackgroundColor3 = isSelected and Theme.Colors.Accent or Theme.Colors.Element
                    local stroke = radioCircle:FindFirstChildOfClass("UIStroke")
                    if stroke then
                        stroke.Color = isSelected and Theme.Colors.BorderHover or Theme.Colors.Border
                    end
                    local fill = radioCircle:FindFirstChild("Fill")
                    if fill then
                        fill.Visible = isSelected
                    end
                end
                
                local label = optBtn:FindFirstChild("OptionLabel")
                if label then
...
function TabButton:didMount()
    ...
    -- Subscribe to Store for activeTab changes
    local Store = self.props.Store
    if Store and Store.Subscribe then
        local lastActiveTab = Store._state.activeTab
        Store:Subscribe(function(state)
            if state.activeTab ~= lastActiveTab then
                local wasActive = self.props.isActive
                self.props.isActive = (state.activeTab == self.props.tab.id)
                lastActiveTab = state.activeTab
                if self.props.isActive ~= wasActive then
                    self:setState({}) -- trigger didUpdate
                end
            end
        end)
    end
...
-- Sidebar Component: renders shell, imperatively manages tab buttons
local Sidebar = React.Component:extend("Sidebar")

function Sidebar:init()
    self.navListRef = React.createRef()
end
...
function Sidebar:didMount()
    ...
    -- Subscribe to Store for section/tab changes
    local Store = self.props.Store
    if Store and Store.Subscribe then
        local lastSectionCount = #(Store._state.sections or {})
        local lastTabTotal = 0
        for _, sec in ipairs(Store._state.sections or {}) do
            lastTabTotal = lastTabTotal + #(sec.tabs or {})
        end
        
        Store:Subscribe(function(state)
            local newSectionCount = #(state.sections or {})
            local newTabTotal = 0
            for _, sec in ipairs(state.sections or {}) do
                newTabTotal = newTabTotal + #(sec.tabs or {})
            end
            
            if newSectionCount ~= lastSectionCount or newTabTotal ~= lastTabTotal then
                lastSectionCount = newSectionCount
                lastTabTotal = newTabTotal
                self:rebuildTabs()
            end
        end)
    end
...
function Sidebar:rebuildTabs()
    local navList = self.navListRef and self.navListRef.current
    if not navList then return end
    
    -- Clear existing tab children (keep UIListLayout and UIPadding)
    ...
    for _, sec in ipairs(state.sections or {}) do
        ...
        if sec.title and sec.title ~= "" then
            ...
            header.Name = "SecHeader_" .. sec.id
            header.Size = UDim2.new(1, 0, 0, 18)
            header.BackgroundTransparency = 1
            header.Text = string.upper(sec.title)
        ...
        for _, tab in ipairs(sec.tabs or {}) do
            ...
            local tabBtn = React.createElement(TabButton, {
                tab = tab,
                isActive = isActive,
                Store = Store,
                LayoutOrder = layoutOrder,
            })
            if typeof(tabBtn) == "Instance" then
...
function Sidebar:render()
    ...
    return React.createElement("Frame", {
        ...
        -- Navigation List (shell ??tabs added imperatively via rebuildTabs)
        ...
        ProfileArea = state.profile and React.createElement("Frame", {
            ...
            Container = React.createElement("Frame", {
                ...
                Avatar = React.createElement("ImageLabel", {
                    ...
                    BackgroundColor3 = Theme.Colors.Element,
                    BorderSizePixel = 0,
                    Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(state.profile.userId or 1) .. "&w=150&h=150",
                }, {
                    Corner = React.createElement("UICorner", {
                        CornerRadius = Theme.CornerRadius.Circle,
                    }),
...
function Slider:init(props)
    ...
    self.strokeRef = React.createRef()
    self.valueLabelRef = React.createRef()
...
function Slider:didMount()
    ...
    local stroke = self.strokeRef and self.strokeRef.current
    local valLabel = self.valueLabelRef and self.valueLabelRef.current
...
function Stepper:init(props)
    self.displayRef = React.createRef()
    local el = props.element or {}
    self.state = {
        value = el.value or (el.min or 0)
    }
end

function Stepper:didUpdate(prevProps, prevState)
    local display = self.displayRef and self.displayRef.current
    local el = self.props.element
    if display and el then
        if el.value ~= self.state.value and el.value ~= prevProps.element.value then
            self:setState({ value = el.value })
        end
    end
    if display and self.state.value ~= prevState.value then
        display.Text = tostring(self.state.value)
    end
end
    ...
    return React.createElement("Frame", {
        ...
        Container = React.createElement("Frame", {
            ...
            DecButton = React.createElement(StepperDecButton, {
                OnClick = function()
                    updateValue(self.state.value - step)
                end,
            }),
            ...
            Display = React.createElement("Frame", {
                ...
                Input = el.fielded and React.createElement("TextBox", {
                    ...
                    [React.Event.FocusLost] = function(enterPressed)
                        ...
                        local num = tonumber(display.Text)
                        if num then
                            updateValue(num)
                        else
                            display.Text = tostring(self.state.value)
...
function TabContainer:init()
    self.groupRef = React.createRef()
    self.scrollRef = React.createRef()
end
...
function TabContainer:didMount()
    ...
    -- Subscribe to Store for activeTab and searchQuery changes
    local Store = self.props.Store
    if Store and Store.Subscribe then
        local lastActiveTab = Store._state.activeTab
        local lastSearchQuery = Store._state.searchQuery
        local lastTabCount = #(Store._state.tabs or {})
        
        Store:Subscribe(function(state)
            local tabChanged = state.activeTab ~= lastActiveTab
            local searchChanged = state.searchQuery ~= lastSearchQuery
            local tabCountChanged = #(state.tabs or {}) ~= lastTabCount
            
            if tabChanged or searchChanged or tabCountChanged then
                lastActiveTab = state.activeTab
                lastSearchQuery = state.searchQuery
                lastTabCount = #(state.tabs or {})
                
                self:rebuildContent()
                
                -- Tab switch animation
                if tabChanged and group then
                    group.Position = UDim2.new(0, 0, 0, 8)
                    group.GroupTransparency = 1
...
function TabContainer:rebuildContent()
    local scrollFrame = self.scrollRef and self.scrollRef.current
    if not scrollFrame then return end
    
    -- Clear existing content (except layout/padding)
    ...
    if tabBanner then
        local bannerFrame = React.createElement("Frame", {
            Name = "Banner",
        ...
        if typeof(bannerFrame) == "Instance" then
    ...
    if useDualColumns then
        local gridArea = React.createElement("Frame", {
            Name = "GridArea",
            Size = UDim2.new(1, 0, 0, 0),
            BackgroundTransparency = 1,
            AutomaticSize = Enum.AutomaticSize.Y,
        ...
        if typeof(gridArea) == "Instance" then
            local leftCol = React.createElement("Frame", {
                Name = "LeftCol",
                Size = UDim2.new(0.5, -6, 0, 0),
                BackgroundTransparency = 1,
                AutomaticSize = Enum.AutomaticSize.Y,
            ...
            if typeof(leftCol) == "Instance" then
                for _, secEl in ipairs(leftSections) do
                    if typeof(secEl) == "Instance" then
            ...
            local rightCol = React.createElement("Frame", {
                Name = "RightCol",
                Size = UDim2.new(0.5, -6, 0, 0),
                BackgroundTransparency = 1,
                AutomaticSize = Enum.AutomaticSize.Y,
            ...
            if typeof(rightCol) == "Instance" then
...
function TextBox:init()
    ...
    self.state = {
        hovered = false,
        value = self.props.element and self.props.element.value or ""
    }
end

function TextBox:didUpdate(prevProps, prevState)
    local box = self.boxRef and self.boxRef.current
    local el = self.props.element
    if box and el then
        if el.value ~= self.state.value and el.value ~= prevProps.element.value then
            self:setState({ value = el.value })
            box.Text = tostring(el.value or "")
        end
    end
end
...
function Toggle:render()
    ...
    return React.createElement("Frame", {
        ...
        Switch = React.createElement("TextButton", {
            ...
            [React.Event.MouseLeave] = function() self:setState({ hovered = false }) end,
            [React.Event.MouseButton1Click] = function()
                -- Read current state at click time, NOT the stale render closure
...
function Window:init()
    ...
    self.state = {
        position = UDim2.new(0.5, -375, 0.5, -240), -- Centered 750x480 by default
        closing = false,
    }
end
...
function Window:didMount()
    ...
    -- Subscribe to Store so external state changes (minimize, maximize, visible)
    -- trigger didUpdate via setState with proper prevState snapshots
    local Store = self.props.Store
    if Store and Store.Subscribe then
        local lastMinimized = Store._state.minimized
        local lastMaximized = Store._state.maximized
        local lastVisible = Store._state.visible
        
        Store:Subscribe(function(state)
            local changed = false
            
            if state.minimized ~= lastMinimized then
                changed = true
                lastMinimized = state.minimized
            end
            if state.maximized ~= lastMaximized then
                changed = true
                lastMaximized = state.maximized
            end
            if state.visible ~= lastVisible then
                changed = true
                lastVisible = state.visible
            end
            
            if changed then
                -- Trigger setState to fire didUpdate with proper prevState/prevProps snapshots
                self:setState({})
            end
        end)
    end
...
function Window:didUpdate(prevProps, prevState)
    ...
    if sizeChanged or posChanged then
        ...
        -- Handle container visibility during minimize
...
--[[
    ...
    Mount-once architecture – components update themselves via
    setState → didUpdate → refs/TweenService.
]]

-- typeof polyfill for environments that may not have it
local _typeof = typeof or type

local React = {}

-- Shallow clone helper
local shallowClone = table.clone or function(tbl)
    local copy = {}
    for k, v in pairs(tbl) do
        copy[k] = v
    end
...
function React.Component:setState(partialState, callback)
    local prevState = shallowClone(self.state or {})
...
function React.createElement(elementType, props, children)
    ...
    elseif type(elementType) == "table" and elementType.__index then
        ...
        -- Snapshot props for didUpdate comparison
        local function snapshotProps(p)
            local snap = shallowClone(p)
            if type(p.state) == "table" then
                snap.state = shallowClone(p.state)
            end
            if type(p.element) == "table" then
                snap.element = shallowClone(p.element)
            end
            return snap
        end
        
        local prevPropsSnap = snapshotProps(props)
        
        function componentInstance:_rerender(oldState)
            if componentInstance.didUpdate then
                local frozenPrevProps = prevPropsSnap
                prevPropsSnap = snapshotProps(componentInstance.props)
                
                task.spawn(function()
                    local ok, err = pcall(function()
                        componentInstance:didUpdate(frozenPrevProps, oldState)
                    end)
                    if not ok then
                        warn("[React Engine] Error in didUpdate:", err)
                    end
                end)
            end
        end
        
        local renderOk, renderRes = pcall(function()
            return componentInstance:render()
        end)
        renderedInstance = renderOk and renderRes or nil
        ...
        if props.ref then
            ...
            elseif type(props.ref) == "function" then
                pcall(props.ref, componentInstance)
            end
        end
        ...
        if componentInstance.didMount then
            task.defer(function()
                ...
                if not ok then
                    warn("[React Engine] Error in didMount:", err)
                end
            end)
        end
...
--[[
    ...
    Mount-once architecture: renders the tree once, then components
    handle their own updates via setState / didUpdate / refs.
]]
...
function ReactRoblox.createRoot(container)
    ...
    return {
        render = function(self, element)
            if mounted then
                -- Already mounted – do NOT destroy and recreate.
                -- Components update themselves via setState/didUpdate.
                return
            end
            mounted = true
...
function Library:CreateWindow(config)
    ...
    -- Mount UI root (mount once, never re-render the whole tree)
    if not root then
        -- Apply initial theme
        local activeTheme = Theme.Themes[Store._state.theme or "Dark"]
        if activeTheme then
            for k, v in pairs(activeTheme) do
                Theme.Colors[k] = v
            end
        end
        if Store._state.accent and Theme.Accents[Store._state.accent] then
            Theme.Colors.Accent = Theme.Accents[Store._state.accent]
        end
        ...
        -- Create root frame (always present)
        ...
        -- Create Window (always present, toggle via Visible)
        local windowInstance = React.createElement(WindowComponent, {
            state = Store._state,
            Store = Store,
        })
        if typeof(windowInstance) == "Instance" then
            windowInstance.Name = "Window"
            windowInstance.Visible = Store._state.visible
            windowInstance.Parent = rootFrame
        end
        
        -- Notification container
        local notifContainer = React.createElement("Frame", {
        ...
        notifContainer.Parent = rootFrame
        ...
        -- Track existing notification instances by id
        local notifInstances = {}
        
        local function syncNotifications()
            local currentNotifs = Store._state.notifications or {}
            local currentIds = {}
            
            for i, item in ipairs(currentNotifs) do
                currentIds[item.id] = true
                
                if not notifInstances[item.id] then
                    -- Create new notification
                    local notifElement = React.createElement(NotificationComponent.Item, {
                        item = item,
                        LayoutOrder = i,
                    })
                    if typeof(notifElement) == "Instance" then
                        notifElement.Name = "Notif_" .. tostring(item.id)
                        notifElement.Parent = notifContainer
                        notifInstances[item.id] = notifElement
                    end
                end
            end
            
            -- Remove notifications that no longer exist
            for id, inst in pairs(notifInstances) do
                if not currentIds[id] then
                    if typeof(inst) == "Instance" then
                        inst:Destroy()
                    end
                    notifInstances[id] = nil
                end
            end
        end
        ...
        -- Subscribe for ongoing changes (theme, visibility, notifications)
        Store:Subscribe(function(state)
            -- Update theme
            local newTheme = Theme.Themes[state.theme or "Dark"]
            if newTheme then
                for k, v in pairs(newTheme) do
                    Theme.Colors[k] = v
                end
            end
            if state.accent and Theme.Accents[state.accent] then
                Theme.Colors.Accent = Theme.Accents[state.accent]
            end
            
            -- Toggle window visibility
            if typeof(windowInstance) == "Instance" then
                windowInstance.Visible = state.visible
            end
