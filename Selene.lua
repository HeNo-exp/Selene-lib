...
function RadioOption:didUpdate(prevProps)
    if self.props.isSelected ~= prevProps.isSelected then
        self:setState({ isSelected = self.props.isSelected })
    end
end
...
function RadioOption:render()
    local props = self.props
    local opt = props.opt
    local isSelected = self.state.isSelected
    local layoutOrder = props.LayoutOrder
    local hovered = self.state.hovered
...
function RadioButtonGroup:didUpdate(prevProps)
    local el = self.props.element
    if el and el.value ~= self.state.value and el.value ~= prevProps.element.value then
        self:setState({ value = el.value })
    end
end
